...
To expose SUSE Observability Server using Traefik Ingress Controller with self-signed certificates, you need to:
...
Create a values file (for example, `+ingress-with-tls-values.yaml+`) with Ingress configuration:
...
SUSE Observability Server has several points of interaction with external systems. For example, event handlers can call out to webhooks in other systems. With the default configuration, SUSE Observability Server will not be able to communicate with these systems if they are secured with TLS using a self-signed certificate, or a certificate that is not by default trusted by the JVM.

To mitigate this, SUSE Observability Server allows configuration of a custom trust store.

=== Create a custom trust store

You need to have the custom TLS certificate available. If you do not have that, you will need to xref:/use/security/self-signed-certificates.adoc#_retrieve_certificate_via_the_browser[retrieve it via the browser].
...
If you do not have the JVM installed on your computer, you can also xref:/use/security/self-signed-certificates.adoc#_using_a_docker_jvm[use a JVM Docker image] instead.
...
. The `+custom_cacerts+` store file now includes the `+site.cert+` certificate. You can verify that by searching for the alias in the output of
+
[,bash]
----
