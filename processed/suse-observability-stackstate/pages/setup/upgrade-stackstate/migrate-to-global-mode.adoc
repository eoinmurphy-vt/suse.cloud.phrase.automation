...
[NOTE]
====
The `+global.suseObservability+` configuration method is available starting from version `+2.8.0+` and is now the recommended installation approach. The legacy `+suse-observability-values+` chart method is deprecated but will continue to work for existing installations.
====
...
[NOTE]
====
Before running helm commands, ensure you have the latest chart version:
...
[NOTE]
====
* The `+adminPassword+` must be a bcrypt hash in the new method. See <<bcrypt-hash>> for how to generate one.
* For air-gapped environments or custom registries, see xref:/k8s-suse-rancher-prime-air-gapped.adoc[Installing {stackstate-product-name} in Air-Gapped Mode] for additional configuration including `+imageRegistry+` and `+pullSecret+` settings.
====

=== Using a values file instead

If you prefer using a values file, create `+values.yaml+` with the configuration shown in <<_step_3_create_new_values_file>>, ensure you have the latest chart with `+helm repo update+`, then run:
...
* *License key*: Your {stackstate-product-name} license
* *Base URL*: The URL where {stackstate-product-name} is accessible
* *Admin password*: The admin password (you can also use the bcrypt hash of the password, the value for that is `+adminPasswordBcrypt+`)
...
| `+imageRegistry+` (air-gapped only)
| `+global.imageRegistry+`

| `+pullSecret.username+` (air-gapped only)
| `+global.suseObservability.pullSecret.username+`

| `+pullSecret.password+` (air-gapped only)
| `+global.suseObservability.pullSecret.password+`
|===

[NOTE]
====
For air-gapped environments or custom registries requiring `+imageRegistry+` and `+pullSecret+` configuration, see xref:/k8s-suse-rancher-prime-air-gapped.adoc[Installing {stackstate-product-name} in Air-Gapped Mode].
====
...
First, ensure you have the latest chart version:
...
For the legacy method, the `+adminPassword+` was provided as plain text to the suse-observability-values chart, which hashed it to a bcrypt hash. In the new method, you can provide the plain text `+adminPassword+` to the suse-observability chart, or provide the bcrypt hash directly to the suse-observability chart using `+adminPasswordBcrypt+`.
...
* xref:/setup/upgrade-stackstate/steps-to-upgrade.adoc[Steps to upgrade {stackstate-product-name}]
