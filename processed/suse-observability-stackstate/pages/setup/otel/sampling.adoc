...
Tail sampling postpones the sampling decision until a trace is (almost) complete. This allows the tail sampler to make sampling decisions based on the entire trace, for example to always sample failed traces and/or slow traces. There are many more possibilities, such as sampling based on specific attributes or service participation.

The OpenTelemetry Collector provides a https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/tailsamplingprocessor[tail sampling processor] that can be used to apply tail sampling policies.

The main advantage of tail sampling is the additional flexibility it provides in making sampling decisions based on complete trace data, rather than on partial information available when individual spans are created.


[WARNING]
====
Although advantageous, tail sampling has the following downsides:
