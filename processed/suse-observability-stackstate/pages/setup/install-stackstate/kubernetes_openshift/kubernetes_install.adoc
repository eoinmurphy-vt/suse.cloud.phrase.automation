...
. xref:/setup/install-stackstate/kubernetes_openshift/kubernetes_install.adoc#_deploy_suse_observability_with_helm[Deploy {stackstate-product-name} with Helm]
. xref:/setup/install-stackstate/kubernetes_openshift/kubernetes_install.adoc#_access_the_suse_observability_ui[Access the {stackstate-product-name} UI]


=== Create namespace

Start by creating the namespace where you want to install {stackstate-product-name} and deploy the secret in that namespace. In our walkthrough we will use the namespace `+suse-observability+`:
...
You can generate the {stackstate-product-name} configuration files by running a separate Helm Chart. This is the `+suse-observability/suse-observability-values+` chart:
...
| `+-u+` `+-p+`
| The username and password used by {stackstate-product-name} to pull images. For air-gapped environments these need to be the username and password for the local contaier registry.
...
| Image Registry
| `+imageRegistry+`
| The registry where the {stackstate-product-name} images are hosted. If not provided, the default value will be 'registry.rancher.com'. Only needed for air-gapped environments or custom registries.

| Pull Secret Username
| `+pullSecret.username+`
| The username used to pull images from the Docker registry where the {stackstate-product-name} images are hosted. Only needed for custom registries.

| Pull Secret Password
| `+pullSecret.password+`
| The password used to pull images from the Docker registry where the {stackstate-product-name} images are hosted. Only needed for custom registries.
...
| Sizing profile
| `+sizing.profile+`
| OneOf trial, 10-nonha, 20-nonha, 50-nonha, 100-nonha, 150-ha, 250-ha, 500-ha. Based on these profiles, you can generate the `+sizing_values.yaml+` file containing default sizes for the {stackstate-product-name} resources and configuration to deploy in HA or Non-HA mode. For example, 10-nonha produces a `+sizing_values.yaml+` to deploy a NonHa {stackstate-product-name} instance to observe a 10 node cluster in a Non High availability mode. Currently, moving from a Non-HA to an HA environment is not possible. If you expect that your environment will require observing around 150 nodes, then it's better to go with HA immediately.
...
{stackstate-product-name} will now be available in your browser at `+https://localhost:8080+`. Log in with the username `+admin+` and the default password provided in the `+baseConfig_values.yaml+` file.
