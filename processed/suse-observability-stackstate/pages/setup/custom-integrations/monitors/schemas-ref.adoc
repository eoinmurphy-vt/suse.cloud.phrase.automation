...
* `+remediationHint+`: A description of what the user can do when the monitor fails. The format is markdown, with optionally use of handlebars variables to customize the hint based on time series or other data (xref:use/alerting/k8s-add-monitors-cli.adoc#_write_the_remediation_hint[more explanation below]).
...
* `+urnTemplate+`: A template to construct the urn of the component a result of the monitor will be xref:use/alerting/k8s-add-monitors-cli.adoc#_bind_the_results_of_the_monitor_to_the_correct_components[bound to].
