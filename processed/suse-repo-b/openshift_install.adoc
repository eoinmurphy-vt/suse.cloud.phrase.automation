...
Before you start the installation of {stackstate-product-name}:
...
== Install {stackstate-product-name}
...
. xref:/setup/install-stackstate/kubernetes_openshift/openshift_install.adoc#_create_project[Create the project where {stackstate-product-name} will be installed]
. xref:/setup/install-stackstate/kubernetes_openshift/openshift_install.adoc#create-values-yaml[Create `+values.yaml+` configuration file]
...
. xref:/setup/install-stackstate/kubernetes_openshift/openshift_install.adoc#_deploy_suse_observability_with_helm[Deploy {stackstate-product-name} with Helm]
. xref:/setup/install-stackstate/kubernetes_openshift/openshift_install.adoc#_access_the_suse_observability_ui[Access the {stackstate-product-name} UI]
...
Start by creating the project where you want to install {stackstate-product-name}. In our walkthrough we will use the namespace `+suse-observability+`:
...
You can generate the {stackstate-product-name} `+baseConfig_values.yaml+`, `+sizing_values.yaml+`, and `+affinity_values.yaml+` files by running a separate Helm Chart. The `+suse-observability/suse-observability-values+` chart. A sample command line is:
...
| Receiver API Key
| `+receiverApiKey+`
| The API key used by {stackstate-product-name} to receive data from agents. This is a secret key that should be kept private. If you omit this, a random key will be generated for you.

| Base URL
| `+baseUrl+`
| The `+<STACKSTATE_BASE_URL>+`. The external URL for {stackstate-product-name} that users and agents will use to connect. For example `+https://suse-observability.internal+`. If you haven't decided on an Ingress configuration yet, use `+http://localhost:8080+`. This can be updated later in the generated file.

| Username and password**
| `+-u+` `+-p+`
| The username and password used by {stackstate-product-name} to pull images. For air-gapped environments these need to be the username and password for the local docker registry.

| License key
| `+license+`
| The {stackstate-product-name} license key.

| Default password
| `+adminPassword+`
| The password for the default user (`+admin+`) to access {stackstate-product-name}'s UI. Alternatively, you can provide a bcrypt hashed password using `+adminPasswordBcrypt+`. Only one of the two options can be used at the same time.

| Image Registry
| `+imageRegistry+`
| The registry where the {stackstate-product-name} images are hosted. If not provided, the default value will be 'quay.io'

| Pull Secret Username
| `+pullSecret.username+`
| The username used to pull images from the Docker registry where the {stackstate-product-name} images are hosted.

| Pull Secret Password
| `+pullSecret.password+`
| The password used to pull images from the Docker registry where the {stackstate-product-name} images are hosted.
...
| Sizing profile
| `+sizing.profile+`
| OneOf trial, 10-nonha, 20-nonha, 50-nonha, 100-nonha, 150-ha, 250-ha, 500-ha. Based on these profiles, you can generate the `+sizing_values.yaml+` file containing default sizes for the {stackstate-product-name} resources and configuration to be deployed in HA or Non-HA mode. For example, 10-nonha produces a `+sizing_values.yaml+` to deploy a Non-HA {stackstate-product-name} instance to observe a 10 node cluster in a Non High availability mode. Currently, moving from a Non-HA to an HA environment is not possible. If you expect that your environment will require observing around 150 nodes, then it's better to go with HA immediately.
...
[NOTE]
=====
Store the generated `+basicConfig.yaml+` and `+sizing_values.yaml+` files safely. You can reuse these files for upgrades, which saves time and ensures that {stackstate-product-name} continues to use the same API key. This is desirable as it means Agents and other data providers for {stackstate-product-name} won't need updating.
...
The recommended deployment of {stackstate-product-name} is a production ready, high availability setup with many services running redundantly. If required, it's also possible to run {stackstate-product-name} in a non-redundant setup, where each service has only a single replica. This setup is only recommended for a test environment.
...
To deploy {stackstate-product-name} in a high availability setup on OpenShift:
...
To deploy {stackstate-product-name} in an air-gapped setup on OpenShift:
...
After the install, the {stackstate-product-name} release should be listed in the {stackstate-product-name} namespace and all pods should be running:
...
=== Access the {stackstate-product-name} UI

After {stackstate-product-name} has been deployed, you can check if all pods are up and running:
...
{stackstate-product-name} will now be available in your browser at `+https://localhost:8080+`. Log in with the username `+admin+` and the default password provided in the `+values.yaml+` file.

Next steps are

* xref:/setup/install-stackstate/kubernetes_openshift/ingress.adoc[Expose {stackstate-product-name} outside of the cluster]
...
If you can't use an administrator account to install {stackstate-product-name} on OpenShift, ask your administrator to apply the below `+SecurityContextConfiguration+` objects.
...
* For other configuration and management options, refer to the Kubernetes documentation - xref:/setup/install-stackstate/kubernetes_openshift/kubernetes_install.adoc[manage a {stackstate-product-name} Kubernetes installation]
* xref:/setup/upgrade-stackstate/migrate-to-global-mode.adoc[Migration guide for existing installations]
