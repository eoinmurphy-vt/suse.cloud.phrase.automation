...
Use `+curl -sfL https://get.k3s.io | INSTALL_K3S_ARTIFACT_URL=<PRIME-ARTIFACTS-URL>/k3s sh -s - server --secrets-encryption+` if installing from script, or other methods described in xref:installation/configuration.adoc#_configuration_with_install_script[Configuration Options].
====
