...
If any such entries are present, the configuration file is not used, as such entries would not function properly within pods that https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy[inherit name resolution configuration] from their node.
If no usable resolv.conf is found, K3s prints a warning message to the logs, and generate a stub resolv.conf that uses `+8.8.8.8+` and `+2001:4860:4860::8888+` as the nameservers.
...
Keys matching `+*.override+` are imported into the `+:.53+` Server Block.
Additional Server Blocks may be placed in keys matching `+*.server+`.
Additional content (zone files, etc) may also be present, and are mounted under `+/etc/coredns/custom+` in the coredns pods.

Here is an example ConfigMap that forwards lookups to `+example.com+` to a nameserver at `+10.0.0.1+`, and serves `+example.net+` from an https://datatracker.ietf.org/doc/html/rfc1035#section-5[RFC 1035] compliant text file:
...
See `+ContainerdConfigTemplateV3+` and `+ContainerdConfigTemplate+` in https://github.com/k3s-io/k3s/blob/main/pkg/agent/templates/templates.go[`+templates.go+`] for the default template content.
The template is executed with a https://github.com/k3s-io/k3s/blob/main/pkg/agent/templates/templates.go#L22-L33[`+ContainerdConfig+`] struct as its dot value (data argument).
...
* Download `+k3s-rootless.service+` from https://github.com/k3s-io/k3s/blob/main/k3s-rootless.service[`+https://github.com/k3s-io/k3s/blob/<VERSION>/k3s-rootless.service+`].
