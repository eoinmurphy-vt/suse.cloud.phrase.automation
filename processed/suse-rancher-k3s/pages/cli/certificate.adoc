...
If you want to generate new certificates and keys, instead of extending the validity of the existing certificates, use the xref:#_rotating_client_and_server_certificates[`+rotate+`] subcommand documented below.
...
The authoritative CA certificates and keys are stored within the datastore's bootstrap key, encrypted using the xref:cli/token.adoc#_server[server token] as the PBKDF2 passphrase with AES256-GCM and HMAC-SHA1.
...
This means that any user with a valid client certificate (or kubeconfig) for one cluster is also able to authenticate to other clusters, and any RBAC that matches a client's user or group is also applied in those other clusters.
Similarly, server certificates issued by one cluster are also trusted in all other clusters, and by all other infrastructure or clients that trust that root CA.
...
An example script to pre-create the appropriate certificates and keys is available https://github.com/k3s-io/k3s/blob/main/contrib/util/generate-custom-ca-certs.sh[in the K3s repo at `+contrib/util/generate-custom-ca-certs.sh+`].
...
Custom CA Certificates generated by the example script have the following topology:
...
If you used the `+--force+` option or changed the root CA, ensure that any nodes that were joined with a xref:cli/token.adoc#_secure[secure token] are reconfigured to use the new token value, prior to being restarted.
...
If the cluster has been started with default self-signed CA certificates, rotation will be disruptive. All nodes that were joined with a xref:cli/token.adoc#_secure[secure token] will need to be reconfigured to trust the new CA hash.
...
An example script to create updated CA certificates and keys cross-signed by the existing CAs is available https://github.com/k3s-io/k3s/blob/main/contrib/util/rotate-default-ca-certs.sh[in the K3s repo at `+contrib/util/rotate-default-ca-certs.sh+`].
...
Ensure that any nodes that were joined with a xref:cli/token.adoc#_secure[secure token], including other server nodes, are reconfigured to use the new token value prior to being restarted.
The token may be stored in a `+.env+` file, systemd unit, or config.yaml, depending on how the node was configured during initial installation.
