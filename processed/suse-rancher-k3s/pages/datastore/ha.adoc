...
This section describes how to install a high-availability {k3s-product-name} cluster with an external database.
...
{k3s-product-name} requires two or more server nodes for this HA configuration. See the xref:installation/requirements.adoc[Requirements] guide for minimum machine requirements.
...
Where `+INSTALL_K3S_ARTIFACT_URL+` is the https://scc.suse.com/rancher-docs/rancherprime/latest/en/reference-guide.html#prime-artifacts-url[Prime Artifacts URL]
