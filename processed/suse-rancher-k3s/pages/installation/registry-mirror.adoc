...
or by loading image archives created by `+docker save+` via the `+ctr -n k8s.io image import+` command or the xref:add-ons/import-images.adoc#_pre_import_images[pre-import feature].
