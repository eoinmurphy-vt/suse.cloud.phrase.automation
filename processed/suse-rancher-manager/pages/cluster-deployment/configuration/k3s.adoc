...
* <<_cluster_config_file_reference,Cluster Config File>>: Instead of using the Rancher UI to choose Kubernetes options for the cluster, advanced users can create a K3s config file. Using a config file lets you set any of the https://rancher.com/docs/k3s/latest/en/installation/install-options/[options] available during a K3s installation.
...
For a complete reference of configurable options for K3s clusters in YAML, see the https://documentation.suse.com/cloudnative/k3s/latest/en/installation/configuration.html[K3s documentation].
...
For details on upgrading or rolling back Kubernetes, refer to {xref-ds-backup-restore}[Upgrading Kubernetes].

[#_pod_security_admission_configuration_template]
===== Pod Security Admission Configuration Template

The default {xref-psact}[pod security admission configuration template] for the cluster.

[#_encrypt_secrets]
===== Encrypt Secrets

Option to enable or disable secrets encryption. When enabled, secrets are encrypted using a AES-CBC key. If disabled, previously encrypted secrets are not readable until encryption is enabled again. Refer to the https://documentation.suse.com/cloudnative/k3s/latest/en/security/secrets-encryption.html[K3s documentation] for details.
...
Option to enable or disable https://documentation.suse.com/cloudnative/k3s/latest/en/advanced.html#_selinux_support[SELinux] support.

[#_coredns]
===== CoreDNS

By default, https://coredns.io/[CoreDNS] is installed as the default DNS provider. If CoreDNS is not installed, an alternate DNS provider must be installed yourself. Refer to the https://documentation.suse.com/cloudnative/k3s/latest/en/networking/networking-services.html#_coredns[K3s documentation] for details.

[#_klipper_service_lb]
===== Klipper Service LB

Option to enable or disable the https://github.com/rancher/klipper-lb[Klipper] service load balancer. Refer to the https://documentation.suse.com/cloudnative/k3s/latest/en/networking/networking-services.html#_service_load_balancer[K3s documentation] for details.

[#_traefik_ingress]
===== Traefik Ingress

Option to enable or disable the https://traefik.io/[Traefik] HTTP reverse proxy and load balancer. For more details and configuration options, see the https://documentation.suse.com/cloudnative/k3s/latest/en/networking/networking-services.html#_traefik_ingress_controller[K3s documentation].

[#_local_storage]
===== Local Storage

Option to enable or disable https://documentation.suse.com/cloudnative/k3s/latest/en/storage.html[local storage] on the node(s).
...
Additional Kubernetes manifests, managed as a https://kubernetes.io/docs/concepts/cluster-administration/addons/[Add-on], to apply to the cluster on startup. Refer to the https://documentation.suse.com/cloudnative/k3s/latest/en/installation/packaged-components.html#_auto_deploying_manifests_addons[K3s documentation] for details.

[#_agent_environment_vars]
==== Agent Environment Vars

Option to set environment variables for https://documentation.suse.com/cloudnative/k3s/latest/en/architecture.html[K3s agents]. The environment variables can be set using key value pairs. Refer to the https://documentation.suse.com/cloudnative/k3s/latest/en/installation/configuration.html#_configuration_file[K3 documentation] for more details.
...
* https://documentation.suse.com/cloudnative/k3s/latest/en/networking/basic-network-options.html#_dual_stack_ipv4_ipv6_networking[K3s documentation: Dual-stack (IPv4 + IPv6) Networking]
* https://documentation.suse.com/cloudnative/k3s/latest/en/networking/basic-network-options.html#_single_stack_ipv6_networking[K3s documentation: Single-stack IPv6 Networking]
...
For more information, refer to the {xref-node-reqs}[Node Requirements] page.
====
...
For more detail on how an authorized cluster endpoint works and why it is used, refer to the {xref-arch-com-ds-cluster}#_4_authorized_cluster_endpoint[architecture section.]

We recommend using a load balancer with the authorized cluster endpoint. For details, refer to the {xref-arch-rec}#_architecture_for_an_authorized_cluster_endpoint_ace[recommended architecture section.]

[#_registries]
==== Registries

Select the image repository to pull Rancher images from. For more details and configuration options, see the https://documentation.suse.com/cloudnative/k3s/latest/en/installation/private-registry.html[K3s documentation].
...
Editing clusters in YAML allows you to set configurations that are already listed in <<_configuration_options_in_the_rancher_ui,Configuration Options in the Rancher UI>>, as well as set Rancher-specific parameters.

.Click to Expand *Example Cluster Config File Snippet*
[%collapsible]
======
