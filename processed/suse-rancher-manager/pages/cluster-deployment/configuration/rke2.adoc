...
For a complete reference of configurable options for RKE2 clusters in YAML, see the https://documentation.suse.com/cloudnative/rke2/latest/en/install/configuration.html[RKE2 documentation].
...
ifeval::["{build-type}" == "srfa"]
This subsection covers generic machine pool configurations. For specific EC2 infrastructure provider configurations, refer to the following:
...
You can configure a {xref-set-up-cloud-providers}[Kubernetes cloud provider]. If you want to use dynamically provisioned {xref-persistent-storage}[volumes and storage] in Kubernetes, typically you must select the specific cloud provider in order to use it. For example, if you want to use Amazon EBS, you would need to select the `+aws+` cloud provider.
...
Select a {xref-compliance-scans-howto}[compliance benchmark] to validate the system configuration against.
...
Option to set environment variables for {xref-about-rancher-agents}[Rancher agents]. The environment variables can be set using key value pairs. Refer to the https://documentation.suse.com/cloudnative/rke2/latest/en/reference/linux_agent_config.html[RKE2 documentation] for more details.
...
* https://documentation.suse.com/cloudnative/rke2/latest/en/networking/basic_network_options.html#_dual_stack_configuration[RKE2 documentation: Dual-stack configuration]
* https://documentation.suse.com/cloudnative/rke2/latest/en/networking/basic_network_options.html#_ipv6_setup[RKE2 documentation: IPv6-only setup]
