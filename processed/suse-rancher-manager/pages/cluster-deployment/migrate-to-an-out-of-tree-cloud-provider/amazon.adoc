...
----
+
. To install the AWS cloud controller manager with leader migration enabled, follow Steps 1-3 for {xref-cloud-provider-amazon}#_using_the_out_of_tree_aws_cloud_provider[deploying the cloud controller manager chart]. From Kubernetes 1.22 onwards, the kube-controller-manager will utilize a default configuration which will satisfy the controller-to-manager migration. Update container args of the `+aws-cloud-controller-manager+` under `+spec.rkeConfig.additionalManifest+` to enable leader migration:
+
[,shell]
----
