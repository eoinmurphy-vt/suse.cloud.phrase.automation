...
You can also {xref-migrate-out-of-tree-amazon}[migrate from an in-tree to an out-of-tree AWS cloud provider] on Kubernetes 1.26 and earlier. All existing clusters must migrate prior to upgrading to v1.27 in order to stay functional.
...
While creating an {xref-infra-aws}[Amazon EC2 cluster], you must fill in the *IAM Instance Profile Name* (not ARN) of the created IAM role when creating the *Node Template*.

While creating a {xref-custom-clusters}[Custom cluster], you must manually attach the IAM role to the instance(s).
...
When you create an {xref-infra-aws}[Amazon EC2 Cluster], the `+ClusterID+` is automatically configured for the created nodes. Other resources still need to be manually tagged.
...
----
+
You must not enable `+useInstanceMetadataHostname+` when setting custom values for `+hostname-override+` for custom clusters. When you create a {xref-custom-clusters}[custom cluster], add `+--node-name+` to the `+docker run+` node registration command to set `+hostname-override+` -- for example, `+"$(hostname -f)"+`. This can be done manually or by using *Show Advanced Options* in the Rancher UI to add *Node Name*.
