...
. In GKE, enable Network Policy at the cluster level. Refer to the https://cloud.google.com/kubernetes-engine/docs/how-to/network-policy[official GKE guide] for instructions.
. After enabling Network Policy, import the cluster into Rancher and enable PNI for project-level isolation.
...
Private clusters require additional planning and configuration outside of Rancher. Refer to the {xref-gke-private-clusters}[private cluster guide].
====
...
The default option is "Container-Optimized OS with Docker". The read-only filesystem on GCP's Container-Optimized OS is not compatible with the xref:[legacy logging] implementation in Rancher. If you need to use the legacy logging feature, select "Ubuntu with Docker" or "Ubuntu with Containerd". The {xref-logging}[current logging feature] is compatible with the Container-Optimized OS image.
====
