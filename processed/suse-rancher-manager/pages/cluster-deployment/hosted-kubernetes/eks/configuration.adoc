...
| Cloud Credentials
| Select the cloud credentials that you created for your IAM policy. For more information on creating cloud credentials in Rancher, refer to {xref-cloud-credentials}[this page.]
|===
...
. During cluster creation, expand the *Networking* section.
. Under *IP Family*, select the *IPv6* radio button.
+
[NOTE]
====
Changing the IP Family clears any current subnet selections you have made in the form.
====

. Under *VPCs and Subnets*, choose either to *Create a new vpc and subnet automatically* or *Select from existing subnets*.
+
[WARNING]
.Warning for Custom VPCs 
====
If you choose to select existing subnets, you *must* select Dual-Stack subnets. The selected subnets must have both an IPv4 CIDR block and an IPv6 CIDR block. IPv6-only subnets are not supported.
====

. Complete the remainder of the cluster configuration (Node Groups, etc.).
. Click *Create*.

[IMPORTANT]
====
The IP Family setting is *immutable*. After an EKS cluster is created, you cannot convert an IPv4 cluster to IPv6, nor can you convert an IPv6 cluster to IPv4.
====
...
. Follow the standard cluster registration process (*Cluster Management > Add Cluster > Generic*).
. Rancher queries the AWS EKS API to read the cluster’s upstream state.
. Rancher automatically detects if the cluster was provisioned with IPv6.
