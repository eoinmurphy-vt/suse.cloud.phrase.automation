...
For the full list of EKS cluster configuration options, see {xref-deploy-eks-config}[this page.]
...
[NOTE]
====
These permissions are only necessary when Rancher is *creating* a new IPv6 cluster. During new cluster creation, Rancher needs these permissions to generate the dual-stack VPC, create the IAM OIDC provider, and assign an inline policy (`+RancherManaged++_++AllowIPv6ForCNI+`) to the Node Instance Role.
...
The EKS provisioner can synchronize the state of an EKS cluster between Rancher and the provider. For an in-depth technical explanation of how this works, see {xref-sync-hosted-clusters}[Syncing.]

For information on configuring the refresh interval, refer to {xref-deploy-eks-config}#_configuring_the_refresh_interval[this section.]
