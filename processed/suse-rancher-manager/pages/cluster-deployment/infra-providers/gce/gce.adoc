...
In this section, you'll learn how to use Rancher to provision an {url-rke2-landing-page}[RKE2] or {url-k3s-landing-page}[K3s] Kubernetes cluster on the Google Cloud Platform (GCP) using Google Compute Engine (GCE) through Rancher.
...
.. For each machine pool, define the machine configuration. Refer to the {xref-infra-gce-machine-config}[Google GCE machine configuration reference] for information on configuration options.
. Use the **Cluster Configuration** to choose the version of Kubernetes that will be installed, what network provider will be used and if you want to enable project network isolation. For help configuring the cluster, refer to the {xref-rke2-config}[RKE2] and {xref-k3s-config}[K3s] cluster configuration references.
...
If the cluster being provisioned will utilize the {xref-access-cluster-kubectl}#_authenticating_directly_with_a_downstream_cluster[Authorized Cluster Endpoint (ACE) feature], controlplane nodes must expose port `+6443+`. This port is not exposed in the default machine pool configuration to prevent it from being exposed across all cluster nodes, and to reduce the number of firewall rules created by Rancher.
