...
If Rancher is installed on a high-availability Kubernetes cluster, the Rancher server three-node cluster and downstream clusters have different requirements. For Rancher installation requirements, refer to the node requirements in the {xref-install-reqs}[installation section.]
====
...
If you plan to use ARM64, see {xref-rancher-on-arm64}[Running on ARM64 (Experimental).]
...
SUSE Linux may have a firewall that blocks all ports by default. In that situation, follow {xref-port-reqs}#_opening_suse_linux_ports[these steps] to open the ports needed for adding a host to a custom cluster.

[#_flatcar_container_linux_nodes]
=== Flatcar Container Linux Nodes

When {xref-launch-k8s-with-rancher}[Launching Kubernetes with Rancher] using Flatcar Container Linux nodes, it is required to use the following configuration in the Cluster Config File.
...
The Docker service is enabled automatically when using {xref-provisioning-drivers}#_node_drivers[Node Drivers].
...
Windows nodes can be used for worker nodes only. See {xref-use-windows-clusters}[Configuring Custom Clusters for Windows]
...
The ports required to be open are different depending on how the user cluster is launched. Each of the sections below list the ports that need to be opened for different {xref-cluster-deployment}[cluster creation options].
...
Details on which ports are used in each situation are found under {xref-port-reqs}#_downstream_kubernetes_cluster_nodes[Downstream Cluster Port Requirements].
...
For more information on the hardening guide and details on which version of the guide corresponds to your Rancher and Kubernetes versions, refer to the {xref-security-overview}#_rancher_hardening_guide[security section.]
