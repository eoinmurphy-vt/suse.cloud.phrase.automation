...
Before running the DNS checks, make sure that xref:./networking.adoc#_check_if_overlay_network_is_functioning_correctly[the overlay network is functioning correctly] for your DNS provider as this can also be the reason why DNS resolution (partly) fails.
