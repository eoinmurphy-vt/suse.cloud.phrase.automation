...
Double check if all the {xref-node-reqs}#_networking_requirements[required ports] are opened in your (host) firewall. The overlay network uses UDP in comparison to all other required ports which are TCP.
...
----
+
If you see error in the output, there is some issue with the route between the pods on the two hosts.  In the above output the node `+wk2+` has no connectivity over the overlay network. This could be because the {xref-node-reqs}#_networking_requirements[required ports] for overlay networking are not opened for `+wk2+`.
