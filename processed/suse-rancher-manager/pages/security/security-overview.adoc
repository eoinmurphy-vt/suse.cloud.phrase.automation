...
Security is at the heart of all Rancher features. From integrating with all the popular authentication tools and services, to an enterprise grade {xref-manage-rbac}[RBAC capability], Rancher makes your Kubernetes clusters even more secure.
...
NeuVector is an open-source, container-focused security application that is now integrated into Rancher. NeuVector provides production security, DevOps vulnerability protection, and a container firewall, et al. Please see the {xref-neuvector}[Rancher docs] and the https://documentation.suse.com/cloudnative/security/{neuvector-docs-version}/en/overview.html[{neuvector-product-name} docs] for more information.
...
For details, refer to the section on {xref-compliance-scans-howto}[security scans].

[#_selinux_rpm]
== SELinux RPM

We provide three RPMs (RPM Package Manager) that enable Rancher products to function properly on SELinux-enforcing hosts: `+rancher-selinux+`, `+rke2-selinux+` and `+k3s-selinux+`.  For details, see {xref-selinux-rpm}[this page].
...
Rancher is committed to informing the community of security issues in our products. For the list of CVEs (Common Vulnerabilities and Exposures) for issues we have resolved, refer to {xref-cves}[this page.]
...
For recommendations on securing your Rancher Manager deployments, refer to the {xref-security-best-practices}[Rancher Security Best Practices] guide.
