...
If you require such features, combine Layer 7 firewalls with {xref-authn-authz}#_external_vs_local_authentication[external authentication providers].

[#_use_external_load_balancers_to_protect_vulnerable_ports]
== Use External Load Balancers to Protect Vulnerable Ports

You should protect the following ports behind an {xref-l4-l7-lb}#_layer_4_load_balancer[external load balancer] that has SSL offload enabled:
...
In Rancher, `+admin+` is the default username for the Administrator user, as highlighted {xref-bootstrap-password}[here]
...
[IMPORTANT]
====
Rancher currently enforces only a {xref-users-and-groups}#_minimum_password_length[minimum password length].
====
