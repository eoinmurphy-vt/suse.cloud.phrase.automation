...
Some tests are designated as "Not Applicable." These tests will not be run on any CIS scan because of the way that Rancher provisions RKE2/K3s clusters. For information on how test results can be audited, and why some tests are designated to be not applicable, refer to Rancher's {xref-security-overview}#_the_cis_benchmark_and_self_assessment[self-assessment guide] for the corresponding Kubernetes version.
...
Refer to {xref-security-overview}[the table in the cluster hardening guide] for information on which versions of Kubernetes, the Benchmark, Rancher, and our cluster hardening guide correspond to each other. Also refer to the hardening guide for configuration files of CIS-compliant clusters and information on remediating failed tests.
...
In order to pass the "Hardened" profile, you will need to follow the steps on the {xref-security-overview}#_rancher_hardening_guide[hardening guide] and use the `+cluster.yml+` defined in the hardening guide to provision a hardened cluster.
...
For information about permissions, refer to {xref-compliance-scans-rbac}[this page]

[#_configuration]
== Configuration

For more information about configuring the custom resources for the scans, profiles, and benchmark versions, refer to {xref-compliance-scans-config}[this page]

[#_how_to_guides]
== How-to Guides

Please refer to the {xref-compliance-scans-howto}[Compliance Scan Guides] to learn how to run compliance scans.
