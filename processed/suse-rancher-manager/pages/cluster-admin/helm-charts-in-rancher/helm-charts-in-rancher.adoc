...
If you want to upgrade an installed legacy app, the {xref-experimental-features}[legacy feature flag] must be turned on. This flag is automatically turned on if you had a legacy app already running before you upgraded Rancher.

. Enable the {xref-experimental-features}[legacy feature flag], if it isn't enabled already.
. Click *☰ > Cluster Management*.
