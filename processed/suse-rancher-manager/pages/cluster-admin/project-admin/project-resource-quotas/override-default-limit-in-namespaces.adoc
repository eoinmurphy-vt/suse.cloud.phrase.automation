...
In the diagram below, the Rancher administrator has a resource quota in effect for their project. However, the administrator wants to override the cpu limits for `+Namespace 3+` so that it has more resources available. Therefore, the administrator {xref-projects-and-namespaces}[raises the cpu limits] for `+Namespace 3+` so that the namespace can access more resources.

^Namespace Default Limit Override^

image::rancher-resource-quota-override.svg[Namespace Default Limit Override]

How to: {xref-projects-and-namespaces}[Editing Namespace Resource Quotas]
...
+
For more information about each *Resource Type*, see {xref-resource-quota-types}[the type reference].
+
