 ...
 ** Security group: `+K8sMasterSg+`. More info at {xref-port-reqs}#_downstream_kubernetes_cluster_nodes[RKE2 ports (custom nodes tab)]
 ** Tags:
...
* Security group: `+K8sWorkerSg+` More info at {xref-port-reqs}#_downstream_kubernetes_cluster_nodes[RKE2 ports (custom nodes tab)].
* Tags:
 ** `+kubernetes.io/cluster/<clusterID>: owned+`
...
More info is at {xref-cloud-provider-amazon}[RKE2 clusters on AWS] and https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/cloudprovider/aws/README.md[Cluster Autoscaler on AWS].
...
* master (etcd+controlplane): Depending your needs, deploy three master instances with proper size. More info is at {xref-production-checklist}[the recommendations for production-ready clusters.]
