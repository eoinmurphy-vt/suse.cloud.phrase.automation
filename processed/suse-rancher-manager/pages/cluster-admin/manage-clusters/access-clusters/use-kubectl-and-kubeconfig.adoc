...
This section intended to help you set up an alternative method to access a {xref-launch-k8s-with-rancher}[Rancher-launched cluster].

This method is only available RKE2 and K3s clusters that have the {xref-arch-com-ds-cluster}#_4_authorized_cluster_endpoint[authorized cluster endpoint] enabled. When Rancher creates the cluster, it generates a kubeconfig file that includes additional kubectl context(s) for accessing your cluster. This additional context allows you to use kubectl to authenticate with the downstream cluster without authenticating through Rancher. For a longer explanation of how the authorized cluster endpoint works, refer to {xref-kubeconfig-file}[this page].

On RKE2 and K3s clusters, you need to {xref-register-clusters}#_authorized_cluster_endpoint_support_for_rke2_and_k3s_clusters[manually enable] authorized cluster endpoints.
