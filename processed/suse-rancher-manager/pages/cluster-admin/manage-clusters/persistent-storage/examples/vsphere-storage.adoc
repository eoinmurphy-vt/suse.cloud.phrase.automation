...
In order to dynamically provision storage in vSphere, the vSphere provider must be enabled. See the following pages for more: {xref-cloud-provider-vsphere-out-of-tree}[Out-of-tree vSphere] and {xref-cloud-provider-vsphere}[in-tree vSphere].

[#_prerequisites]
== Prerequisites

In order to provision vSphere volumes in a cluster created with the {xref-launch-k8s-with-rancher}[RKE2], the https://rancher.com/docs/rke/latest/en/config-options/cloud-providers/vsphere[vSphere cloud provider] must be explicitly enabled in the {xref-rke2-config}#_cloud_provider[cluster options].
...
. Choose the cluster you want to provide vSphere storage to and click *Explore*.
