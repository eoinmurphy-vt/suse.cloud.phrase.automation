...
This section assumes that you understand the Kubernetes concepts of persistent volumes and persistent volume claims. For more information, refer to the section on {xref-about-persistent-storage}[how storage works.]
====
...
* To create a persistent volume as a Kubernetes resource, you must have the `+Manage Volumes+` {xref-cluster-project-roles}#_project_role_reference[role.]
...
ifeval::["{build-type}" != "srfa"]
The steps to set up a persistent storage device will differ based on your infrastructure. We provide examples of how to set up storage using {xref-vsphere-storage-example}[vSphere,] {xref-nfs-storage}[NFS,] or Amazon's {xref-amazon-ebs-storage}[EBS.]
endif::[]
ifeval::["{build-type}" == "srfa"]
The steps to set up a persistent storage device will differ based on your infrastructure. We provide examples of how to set up storage using {xref-nfs-storage}[NFS] or Amazon's {xref-amazon-ebs-storage}[EBS.]
endif::[]

If you have a pool of block storage, and you don't want to use a cloud provider, Longhorn could help you provide persistent storage to your Kubernetes cluster. For more information, see {xref-longhorn}[Cloud Native Storage with Longhorn].
