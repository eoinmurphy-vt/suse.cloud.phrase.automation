...
This section assumes that you understand the Kubernetes concepts of storage classes and persistent volume claims. For more information, refer to the section on {xref-about-persistent-storage}[how storage works.]
...
* The cloud provider must be enabled. For details on enabling cloud providers, refer to {xref-set-up-cloud-providers}[this page.]
...
To use a storage provisioner that is not on the above list, you will need to use a {xref-unsupported-storage-drivers}[feature flag to enable unsupported storage drivers.]
