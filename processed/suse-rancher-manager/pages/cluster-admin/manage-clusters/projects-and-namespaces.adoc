...
[NOTE]
====
Projects are de-emphasized on the UI because it is not required to create Kubernetes resources within a project scope. However, resources such as {xref-secrets}#_creating_secrets_in_projects[Secrets] can still be created in a project scope.
====
...
For more information on creating and moving namespaces, see {xref-namespaces}[Namespaces].
...
If your permissions are restricted to the project level, it is better to {xref-namespaces}[create a namespace through Rancher] to ensure that you will have permission to access the namespace.
...
* Assign users to a group of namespaces (i.e., {xref-cluster-project-roles}[project membership]).
* Assign users specific roles in a project. A role can be owner, member, read-only, or {xref-custom-roles}[custom].
...
* Choose `+Custom+` to create a custom role on the fly: {xref-cluster-project-roles}#_custom_project_roles[Custom Project Roles].
====
...
. In the *Project Permissions* section, choose a role. For more information, refer to the {xref-cluster-project-roles}[documentation on project roles.]

[#_optional_add_resource_quotas]
=== Optional: Add Resource Quotas

Resource quotas limit the resources that a project (and its namespaces) can consume. For more information, see {xref-project-resource-quotas}[Resource Quotas].

To add a resource quota,

. In the *Resource Quotas* tab, click *Add Resource*.
. Select a *Resource Type*. For more information, see {xref-project-resource-quotas}[Resource Quotas.].
. Enter values for the *Project Limit* and the *Namespace Default Limit*.
. *Optional:* Specify *Container Default Resource Limit*, which will be applied to every container started in the project. The parameter is recommended if you have CPU or Memory limits set by the Resource Quota. It can be overridden on per an individual namespace or a container level. For more information, see {xref-project-resource-quotas}[Container Default Resource Limit]
. Click *Create*.
...
When you delete a project, any namespaces that were formerly associated with the project will remain on the cluster. You can find these namespaces in the Rancher UI, in the *Not in a Project* tab of the *Projects/Namespaces* page. You can reassign these namespaces to a project by {xref-namespaces}#_moving_namespaces_to_another_project[moving] them.

[#_further_reading]
== Further Reading

You can create projects and project members more programmatically via the xref:api/quickstart.adoc[Public API]. See the xref:api/workflows/projects.adoc[project workflow doc] for more information.
