...
|===
| Removed Component | {xref-infra-providers}[Nodes Hosted by Infrastructure Provider] | {xref-custom-clusters}[Custom Nodes] | {xref-hosted-k8s-clusters}[Hosted Cluster] | {xref-register-clusters}[Registered Nodes]
...
. Open a web browser, navigate to https://github.com/rancher/rancher/blob/main/cleanup/user-cluster.sh[GitHub], and download `+user-cluster.sh+`.
...
[NOTE]
====
For instructions on cleaning nodes in RKE2 clusters that weren't deployed by Rancher, see the {url-rke2-uninstall}[official RKE2 documentation] on uninstalling clusters.
====
