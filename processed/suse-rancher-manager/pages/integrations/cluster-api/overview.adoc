...
[NOTE]
====
Starting with Rancher v2.14, the built-in `+embedded-cluster-api+` functionality (also known as the `+rancher-provisioning-capi+` chart) has been removed. {xref-CAPI-docs}[{turtles-product-name}] is now the only supported method for Cluster API integration with Rancher.
