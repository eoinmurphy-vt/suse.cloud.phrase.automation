...
When a user from an {xref-authn-authz}[external authentication source] signs into Rancher for the first time, they're automatically assigned a set of global permissions (hereafter, permissions). By default, after a user logs in for the first time, they are created as a user and assigned the default `+user+` permission. The standard `+user+` permission allows users to login and create clusters.
...
See the list xref:rancher-admin/users/authn-and-authz/manage-role-based-access-control-rbac/global-resources.adoc[Global Resources] for relevant resources.
...
* You have set up an {xref-authn-authz}#_external_vs_local_authentication[external authentication provider]
* The external authentication provider supports {xref-users-and-groups}[user groups]
