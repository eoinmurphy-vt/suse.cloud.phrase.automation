...
[WARNING]
====
When a Cluster Member creates a project, the user is automatically assigned <<Project Roles, Project Owner privileges>>. This grants them comprehensive control over the project and its associated resources, including permissions to deploy workloads. Without enforced {xref-psa-pss}[Pod Security Standards (PSS) and Pod Security Admission (PSA)], a Cluster Member is able to execute privileged containers in the cluster.
====
...
After an administrator {xref-custom-roles}[sets up a custom cluster role,] cluster owners and admins can then assign those roles to cluster members.
...
* *Assign Custom Roles*: Create a {xref-custom-roles}[custom role] for either your <<_custom_cluster_roles,cluster>> or <<_custom_project_roles,project>>, and then set the custom role as default.
...
* Although you can {xref-locked-roles}[lock] a default role, the system still assigns the role to users who create a cluster/project.
