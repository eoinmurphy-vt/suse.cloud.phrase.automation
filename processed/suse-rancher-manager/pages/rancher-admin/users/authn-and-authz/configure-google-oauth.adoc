...
Within Rancher, only administrators or users with the *Manage Authentication* {xref-global-permissions}[global role] can configure authentication.
...
. Sign into Rancher using a local user assigned the {xref-global-permissions}[administrator] role. This user is also called the local principal.
