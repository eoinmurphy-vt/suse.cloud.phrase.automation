...
Admins should create a {xref-backup-cluster}[Rancher backup] before they commit to the endpoint migration described below.
====
...
. Admins are encouraged to use the proper restore process if they want to go back. Please see {xref-backup-cluster}[backup docs], {xref-rancher-restore}[restore docs], and {xref-back-restore-example-configs}[examples] for reference.
 ...
 ** Use another third-party auth system and set that up in Rancher. Please see the {xref-authn-authz}[authentication docs] to learn how to configure other open authentication providers.
====
