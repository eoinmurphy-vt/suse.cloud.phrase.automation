...
If admins have xref:api/api-tokens.adoc#_disable_tokens_in_generated_kubeconfigs[kubeconfig token generation turned off], the kubeconfig file requires the {xref-rancher-cli}[Rancher CLI] to be present in your PATH when you run `+kubectl+`. Otherwise, you'll see an error like:
