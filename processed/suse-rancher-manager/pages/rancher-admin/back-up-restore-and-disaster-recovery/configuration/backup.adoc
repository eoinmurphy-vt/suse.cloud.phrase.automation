...
You must first {xref-backup-cluster}[install] the `+rancher-backup+` operator. After you do so, you can access the *Backup: Create* page:
...
| `+resourceSetName+`
|  Provide the name of the ResourceSet to define which resources will be included in this backup.
|===
...
For example, when {xref-migrate-cluster}[migrating Rancher to a new cluster] the file is used to re-create the secret in the new cluster.
====
...
. *Credential Secret* (optional): If you need an AWS access key or secret key to access an S3 bucket, {xref-secrets}[create a secret] using your credentials, with keys and directives named `+accessKey+` and `+secretKey+`. The secret can be in any namespace. An example secret is <<_example_credentialsecret,here>>. This directive is unnecessary if the nodes running your operator are in EC2 and assigned <<_iam_permissions_for_ec2_nodes_to_access_s3,IAM permissions to access S3>>. The *Credential Secret* dropdown lists secrets in all namespaces.
...
| If you need an AWS access key or secret key to access an S3 bucket, {xref-secrets}[create a secret] using your credentials, with keys and directives named `+accessKey+` and `+secretKey+`. The secret can be in any namespace. An example secret is <<_example_credentialsecret,here>>. This directive is unnecessary if the nodes running your operator are in EC2 and assigned <<_iam_permissions_for_ec2_nodes_to_access_s3,IAM permissions to access S3>>.
|
