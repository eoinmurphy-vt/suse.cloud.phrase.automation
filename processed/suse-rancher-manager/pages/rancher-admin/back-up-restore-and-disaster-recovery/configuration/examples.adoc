...
NOTE: Refer to the {xref-rancher-backup-config}[backup config reference page] for more information on configuring the options below.
...
This example shows that the AWS credential secret does not have to be provided to create a backup if the nodes running `+rancher-backup+` have {xref-rancher-backup-config}#_iam_permissions_for_ec2_nodes_to_access_s3[these permissions for access to S3.]
...
NOTE: Refer to the {xref-rancher-restore-config}[restore config reference page] for more information on configuring the options below.
...
This example shows that the AWS credential secret does not have to be provided to restore from backup if the nodes running `+rancher-backup+` have {xref-rancher-backup-config}#_iam_permissions_for_ec2_nodes_to_access_s3[these permissions for access to S3.]
