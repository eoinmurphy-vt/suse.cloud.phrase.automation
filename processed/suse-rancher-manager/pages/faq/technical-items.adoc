...
Kubernetes install (Helm - *Applicable to Rancher Prime v2.13.1*):
...
See {xref-logging-troubleshooting}[Troubleshooting: Logging]
...
When the IP address of the node changed, Rancher lost connection to the node, so it will be unable to clean the node properly. See {xref-clean-cluster-nodes}[Cleaning cluster nodes] to clean the node.
...
You can add more arguments/binds/environment variables via the respective {xref-rke2-config}#_cluster_configuration[RKE2 Config File] or {xref-k3s-config}#_cluster_configuration[K3s Config File].
