...
You can add your own `+nodeSelector+` settings and add `+tolerations+` for additional taints by editing the logging Helm chart values. For details, see {xref-logging-taints-and-tolerations}[this page.]
...
To use Logging v2 with SELinux, we recommend installing the `+rancher-selinux+` RPM according to these {xref-selinux-rpm}[instructions].
