...
For information on enabling experimental features, refer to {xref-experimental-features}[Enabling Experimental Features].
...
| `+auditLog.enabled+`
| false
| `+bool+` - Enables / disables audit logging.

| `+auditLog.level+`
| 0
| `+int+` - set the {xref-enable-api-audit-log}[API Audit Log] level. [0-3]
...
| `+ingress.enabled+`
| true
| `+string+` - When set to false, Helm will not install a Rancher ingress. Set the option to false to deploy your own ingress.
...
| `+image.registry+`
| ""
| `+string+` - Override rancher image-specific registry, e.g, `+\http://registry.example.com/+`
...
| `+image.tag+`
| ""
| `+string+` - Defaults to .Chart.appVersion for rancher/rancher image tag
...
You can {xref-bootstrap-password}[set a specific bootstrap password] during Rancher installation. If you don't set a specific bootstrap password, Rancher randomly generates a password for the first admin account.

When you log in for the first time, use the bootstrap password you set to log in. If you did not set a bootstrap password, the Rancher UI shows commands that can be used to {xref-bootstrap-password}#_retrieving_the_bootstrap_password[retrieve the bootstrap password]. Run those commands and log in to the account. After you log in for the first time, you are asked to reset the admin password.

[#_api_audit_log]
=== API Audit Log

Enabling the {xref-enable-api-audit-log}[API Audit Log].

You can collect this log as you would any container log. Enable {xref-logging}[logging] for the `+System+` Project on the Rancher server cluster.
...
See {xref-tls-settings}[TLS settings] for more information and options.
...
For details on installing Rancher with a private registry, see the {xref-air-gap}[air gap installation docs.]
...
If you are using a Private CA signed certificate (or if `+agent-tls-mode+` is set to `+strict+`), add `+--set privateCA=true+` and see {xref-tls-secrets}[Adding TLS Secrets - Using a Private CA Signed Certificate] to add the CA cert for Rancher.
====
