...
To learn more about feature values and how to enable them, see {xref-experimental-features}[Enabling Experimental Features].
...
To control whether Rancher should manage the Kubernetes version of imported RKE2/K3s clusters, it is recommended to use the {xref-register-clusters}#_configuring_version_management_for_rke2_and_k3s_clusters[imported-cluster-version-management] feature that is available in Rancher v2.11.0 or newer.
====

[WARNING]
====
If the `+managed-system-upgrade-controller+` flag was **disabled** in Rancher v2.10.x, and any imported RKE2/K3s clusters were upgraded **outside of Rancher**, follow the steps below to prevent the unexpected installation of the system-upgrade-controller app and to ensure the {xref-register-clusters}#_configuring_version_management_for_rke2_and_k3s_clusters[imported-cluster-version-management] feature works correctly:
...
You can enable this feature on a per-cluster basis. For more information, please refer to the {xref-register-clusters}#_configuring_version_management_for_rke2_and_k3s_clusters[documentation].
====
...
| Removed
| v2.7.14, v2.8.5
| This flag affected {xref-cluster-project-roles}#_external_roletemplate_behavior[external `+RoleTemplate+` behavior]. It is removed in Rancher v2.9.0 and later as the behavior is enabled by default.
