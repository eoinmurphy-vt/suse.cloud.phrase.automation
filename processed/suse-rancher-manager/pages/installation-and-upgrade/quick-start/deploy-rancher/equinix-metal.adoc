...
The intent of these guides is to quickly launch a sandbox that you can use to evaluate Rancher. The Docker install is not recommended for production environments. For comprehensive setup instructions, see {xref-install-upgrade}[Installation].
====
...
* For a full list of port requirements, refer to {xref-node-reqs}[Docker Installation].
* Provision the host according to our {xref-install-reqs}[Requirements].
====
...
Use Rancher to create a deployment. For more information, see {xref-qs-deploy-workloads}[Creating Deployments].
