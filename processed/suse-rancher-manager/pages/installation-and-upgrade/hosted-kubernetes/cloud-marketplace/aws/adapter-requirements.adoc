...
Follow the {xref-rancher-on-eks}[Rancher docs] to create an EKS cluster. When you get to the {xref-rancher-on-eks}#_8_install_the_rancher_helm_chart[final step to install Rancher], *stop and return to this page*. This cluster will need to meet the following requirements:
...
In addition to the options specified to install Rancher in the {xref-rancher-on-eks}#_8_install_the_rancher_helm_chart[Rancher docs], you will also need to enable extra metrics.
