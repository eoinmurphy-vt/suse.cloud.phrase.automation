...
The port requirements differ depending on how the downstream cluster was launched. Each of the tabs below list the ports that need to be opened for different {xref-cluster-deployment}[cluster types].

The following diagram depicts the ports that are opened for each {xref-cluster-deployment}[cluster type].
...
Refer to the {xref-harvester-overview}#_port_requirements[{harvester-product-name} Integration Overview] for more information on Harvester port requirements.
...
[%collapsible]
======
The following table depicts the port requirements for {xref-launch-k8s-with-rancher}[Rancher Launched Kubernetes] with nodes created in an {xref-infra-providers}[Infrastructure Provider].
...
[%collapsible]
======
The following table depicts the port requirements for {xref-launch-k8s-with-rancher}[Rancher Launched Kubernetes] with {xref-custom-clusters}[Custom Nodes].
...
[%collapsible]
======
The following table depicts the port requirements for {xref-hosted-k8s-clusters}[hosted clusters].
...
[%collapsible]
======
The following table depicts the port requirements for {xref-register-clusters}[registered clusters].
...
When using the {xref-infra-aws}[AWS EC2 node driver] to provision cluster nodes in Rancher, you can choose to let Rancher create a security group called `+rancher-nodes+`. The following rules are automatically added to this security group.
