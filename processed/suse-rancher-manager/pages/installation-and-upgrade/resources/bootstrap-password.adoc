...
You can {xref-users-and-groups}#_minimum_password_length[customize the minimum password length] for user accounts, within limitations.
...
During {xref-install-rancher}[Rancher installation], set `+bootstrapPassword+` alongside any other flags for the Rancher Helm chart. For example:
