...
Follow these steps to rotate an SSL certificate and private CA used by Rancher {xref-install-rancher}[installed on a Kubernetes cluster], or migrate to an SSL certificate signed by a private CA.
...
.Important:
====
   As the certificate is signed by a private CA, it is important to ensure {xref-helm-chart-options}#_common_options[`+privateCA: true+`] is set in the `+values.yaml+` file.
====
...
If the clusters have lost connection to Rancher but {xref-kubeconfig-file}[Authorized Cluster Endpoint] (ACE) is enabled on all clusters, then go with method 2.
...
Using a Kubeconfig for each downstream cluster update the environment variable for the two agent deployments. If the {xref-kubeconfig-file}[ACE] is enabled for the cluster, {xref-access-cluster-kubectl}#_authenticating_directly_with_a_downstream_cluster[the kubectl context can be adjusted] to connect directly to the downstream cluster.
...
. Run the kubectl command from the previous step using a kubeconfig for the downstream cluster with one of the following methods:
 .. If the {xref-kubeconfig-file}[ACE] is enabled for the cluster, {xref-access-cluster-kubectl}#_authenticating_directly_with_a_downstream_cluster[the context can be adjusted] to connect directly to the downstream cluster
...
Select 'Force Update' for the clusters within the {xref-fleet-overview}#_accessing_fleet_in_the_rancher_ui[Continuous Delivery] view of the Rancher UI to allow the fleet-agent in downstream clusters to successfully connect to Rancher.
...
. Update the current Helm values in the `+values.yaml+` file:
 ...
 .. Adjust the `+ingress.tls.source+` field as necessary. Please {xref-helm-chart-options}#_common_options[refer to the chart options] for more details. Here are some examples:
