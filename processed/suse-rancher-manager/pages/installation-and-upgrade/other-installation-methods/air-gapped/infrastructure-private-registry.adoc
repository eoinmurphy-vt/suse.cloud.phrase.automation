...
The infrastructure depends on whether you are installing Rancher on a K3s Kubernetes cluster, an RKE Kubernetes cluster, or a single Docker container. For more information on each installation option, refer to {xref-install-upgrade}[this page.]
...
For an example of one way to set up the database, refer to this {xref-mysql-amazon-rds}[tutorial] for setting up a MySQL database on Amazon's RDS service.
