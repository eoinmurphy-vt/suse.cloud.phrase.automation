...
If the registry has certs, reference the {url-k3s-private-registry}[K3s documentation] about adding a private registry. The certs and registry configuration files need to be mounted into the Rancher container.
====
...
. Go to the https://scc.suse.com/rancher-docs/rancherprime/latest/en/reference-guide.html#prime-artifacts-url[Prime Artifacts URL] and find the Rancher v2.x.x release that you want to install, and click *show*.
. To install Rancher in an air-gapped environment, you must download the following files:
+
|===
...
[NOTE]
====
Recent changes to cert-manager require an upgrade. If you are upgrading Rancher and using a version of cert-manager older than v0.12.0, please see our {xref-upgrade-certmanager}[upgrade documentation].
====
+
[,console]
----
...
----
+
*Result:* Docker begins pulling the images used for an air-gap install. Be patient. This process takes a few minutes.
