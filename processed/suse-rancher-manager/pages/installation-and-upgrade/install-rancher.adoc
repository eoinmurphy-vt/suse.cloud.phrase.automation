...
* *RKE2:* For the tutorial to install an RKE2 Kubernetes cluster, refer to {xref-rke2-for-rancher}[this page.] For help setting up the infrastructure for a high-availability RKE2 cluster, refer to {xref-ha-rke2}[this page.]
* *Amazon EKS:* For details on how to install Rancher on Amazon EKS, including how to install an Ingress controller so that the Rancher server can be accessed, refer to {xref-rancher-on-eks}[this page.]
* *AKS:* For details on how to install Rancher with Azure Kubernetes Service, including how to install an Ingress controller so that the Rancher server can be accessed, refer to {xref-rancher-on-aks}[this page.]
* *GKE:* For details on how to install Rancher with Google Kubernetes Engine, including how to install an Ingress controller so that the Rancher server can be accessed, refer to {xref-rancher-on-gke}[this page.] GKE has two modes of operation when creating a Kubernetes cluster, Autopilot and Standard mode. The cluster configuration for Autopilot mode has restrictions on editing the kube-system namespace. However, Rancher needs to create resources in the kube-system namespace during installation. As a result, you will not be able to install Rancher on a GKE cluster created in Autopilot mode.
...
* https://docs.helm.sh/using_helm/#installing-helm[helm] - Package management for Kubernetes. Refer to the {xref-helm-reqs}[Helm version requirements] to choose a version of Helm to install Rancher. Refer to the https://helm.sh/docs/intro/install/[instructions provided by the Helm project] for your specific platform.
...
For systems without direct internet access, see {xref-air-gap-rancher-ha}[Air Gap: Kubernetes install].

To choose a Rancher version to install, refer to xref:[Choosing a Rancher Version.]

To choose a version of Helm to install Rancher with, refer to the {xref-helm-reqs}[Helm version requirements]
...
Recent changes to cert-manager require an upgrade. If you are upgrading Rancher and using a version of cert-manager older than v0.11.0, please see our {xref-upgrade-certmanager}[upgrade documentation].
====
...
Now that Rancher is deployed, see {xref-tls-secrets}[Adding TLS Secrets] to publish the certificate files so Rancher and the Ingress controller can use them.
--
======
...
See the {xref-helm-chart-options}[Chart Options] for the full list of options.
...
Doesn't work? Take a look at the {xref-install-upgrade-troubleshooting}[Troubleshooting] Page
