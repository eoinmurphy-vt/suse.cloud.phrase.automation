...
Make sure that your node fulfills the general {xref-install-reqs}[installation requirements.]
...
Provision a single Linux host according to our {xref-install-reqs}[Requirements] to launch your Rancher Server.
...
* Create a Kubernetes cluster: {xref-cluster-deployment}[Provisioning Kubernetes Clusters].
...
For help troubleshooting certificates, see {xref-certificate-troubleshooting}[this section.]
...
If you want to record all transactions with the Rancher API, enable the {xref-enable-api-audit-log}[API Auditing] feature by adding the flags below into your install command.
...
If you are visiting this page to complete an {xref-air-gap}[Air Gap Installation], you must pre-pend your private registry URL to the server tag when running the installation command in the option that you choose. Add `+<REGISTRY.DOMAIN.COM:PORT>+` with your private registry URL in front of `+rancher/rancher:latest+`.
