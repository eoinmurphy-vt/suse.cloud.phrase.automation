...
[NOTE]
====
For details on Traefik Ingress health checks, refer to <<_health_check_paths_for_traefik_ingresses,this section.>>
====
...
The health check path is `+/ping+`. By default `+/ping+` is always matched (regardless of Host), and a response from https://docs.traefik.io/operations/ping/[Traefik itself] is always served.
