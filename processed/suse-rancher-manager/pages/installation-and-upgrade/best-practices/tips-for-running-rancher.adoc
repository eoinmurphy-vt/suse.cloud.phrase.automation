...
If you are installing Rancher in a vSphere environment, refer to the best practices documented {xref-rancher-on-vsphere}[here.]
...
The Rancher server's Kubernetes cluster should run within the {xref-install-reqs}[system and hardware requirements] as closely as possible. The more you deviate from the system and hardware requirements, the more risk you take.
...
After you {xref-monitoring-and-dashboards}[enable monitoring] in the cluster, you can set up alerts to let you know if your cluster is approaching its capacity. You can also use the Prometheus and Grafana monitoring framework to establish a baseline for key metrics as you scale.
