...
By default, Rancher generates a {xref-access-cluster-kubectl}[kubeconfig file] that contains credentials for proxying through the Rancher server to connect to the Kubernetes API server on a downstream user cluster. The kubeconfig file (`+kube_config_rancher-cluster.yml+`) contains full access to the cluster.
...
ifeval::["{build-type}" == "srfa"]
* Provisions clusters by calling the required Docker machine drivers and Kubernetes engines
endif::[]

By default, to enable Rancher to communicate with a downstream cluster, the cluster controller connects to the cluster agent. If the cluster agent is not available, the cluster controller can connect to a <<_3_rancher_system_agent,Rancher system agent>> instead.
...
For more information, refer to the section on accessing your cluster with {xref-access-cluster-kubectl}[kubectl and the kubeconfig file].
...
For more information on connecting to a cluster without the Rancher authentication proxy and other configuration options, refer to the {xref-access-cluster-kubectl}[kubeconfig file] documentation.
...
ifeval::["{build-type}" == "srfa"]
Rancher can dynamically provision nodes in Amazon EC2, and then install Kubernetes on them.
endif::[]
...
ifeval::["{build-type}" == "srfa"]
When setting up this type of cluster, Kubernetes is installed by Amazon Elastic Container Service for Kubernetes.
endif::[]
