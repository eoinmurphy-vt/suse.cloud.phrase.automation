...
A container management and deployment engine designed to offer users more control on the local cluster and constant monitoring through GitOps. Take a look at https://documentation.suse.com/cloudnative/continuous-delivery/latest/en/index.html/[Fleet documentation] to know more about Fleet.
endif::[]
...
ifeval::["{build-type}" == "product"]
Rancher's next-generation, fully conformant Kubernetes distribution that focuses on security and compliance within the U.S. Federal Government sector. See https://documentation.suse.com/cloudnative/rke2/latest/en/introduction.html[documentation] for more details.
endif::[]
ifeval::["{build-type}" == "community"]
