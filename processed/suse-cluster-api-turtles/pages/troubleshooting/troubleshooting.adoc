...
One way to monitor logs from the Cluster API control plane pods is by using `+kubectl logs+` with the label selector.
...
== How to enable debug mode for {product_name} and CAPI providers

* {product_name} - To enable debug mode for {product_name}, you need to patch `+cattle-system/rancher-config+` configmap as follows:
+
[source,bash]
----
...
* Cluster API Providers - Edit the CAPIProvider resources for providers where increasing the log level is needed. Change to your desidered level:
+
[source,bash]
----
