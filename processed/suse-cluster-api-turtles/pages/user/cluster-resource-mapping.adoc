...
A `+provisioning.cattle.io.Cluster+` may be created to wrap the CAPI Cluster and expose Rancher-specific fields (e.g., display name, RKE config). However, in most {product_name} workflows it is being phased out and the user is not required to interact with this resource.
