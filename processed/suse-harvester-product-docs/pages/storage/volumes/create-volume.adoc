...
[CAUTION]
====
* If the replica count of the selected StorageClass is `+1+` and the volume is attached to a virtual machine, then the virtual machine is considered xref:virtual-machines/live-migration.adoc#_non_migratable_virtual_machines[non-migratable].
...
[CAUTION]
====
Do not use the `+vmstate-persistence+` and `+longhorn-static+` StorageClasses when creating new volumes. `+vmstate-persistence+` is used for TPM and UEFI persistence, while `+longhorn-static+` is used for for management of existing {longhorn-product-name} volumes.
====
