= Upgrade from v1.4.2 or v1.4.3 to v1.5.2
:revdate: 2025-11-11
...
You can directly upgrade from v1.4.2 to v1.5.2 because {harvester-product-name} allows a maximum of one minor version upgrade for underlying components. v1.4.2 and v1.4.3 use the same minor version of {rke2-short-name} (v1.31), while v1.5.0, v1.5.1 and v1.5.2 use the next minor version (v1.32).
...
You must use **v1.5.2** of the Harvester UI Extension to import {harvester-product-name} v1.5.2 clusters on {rancher-short-name} v2.11.0.
...
. Select version *1.5.2*, and then click *Update*.

. Allow some time for the extension to be updated, and then refresh the screen.

== Known issues

=== Air-gapped upgrade stuck with `+ImagePullBackOff+` error in Fluentd and Fluent Bit pods
