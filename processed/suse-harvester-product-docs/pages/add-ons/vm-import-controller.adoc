...
You can import virtual machines from VMware, OpenStack, and Open Virtual Appliance (OVA) packages into {harvester-product-name} using the vm-import-controller add-on. The add-on must be enabled before you start importing virtual machines.
...
As part of the reconciliation process, the controller attempts to list virtual machines in the project and marks the source as ready.
...
The optional `+httpTimeoutSeconds+` field allows you to specify the maximum time (in seconds) {harvester-product-name} waits for an HTTP request to be completed. This period covers the entire transaction, including establishing the connection, handling redirects, and reading the response body. When the value is `+0+`, the timeout feature is disabled. The default value is `+600+` (10 minutes).
...
* Open Virtual Appliance (OVA) sources: The value is used only if {harvester-product-name} is unable to automatically detect the bus type.
...
When you attempt to import a VMware-based virtual machine in {harvester-product-name} v1.6.0, the following issues occur if https://knowledge.broadcom.com/external/article/315382/overview-of-vmware-tools.html[VMware Tools] is not installed on the virtual machine:
...
* Shut down the virtual machine before migrating it to {harvester-product-name}. 
...
You must reboot the virtual machine to apply the changes.