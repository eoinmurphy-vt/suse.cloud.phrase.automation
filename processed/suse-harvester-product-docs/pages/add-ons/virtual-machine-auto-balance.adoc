...
[NOTE]
====
*virtual-machine-auto-balance* is an *experimental* add-on. For more information about experimental features, see xref:introduction/document-conventions.adoc#_feature_labels[Feature Labels].
====

The *virtual-machine-auto-balance* add-on allows {harvester-product-name} to use the https://github.com/kubernetes-sigs/descheduler[Kubernetes Descheduler] for rebalancing of virtual machine workloads. The Kubernetes Descheduler optimizes workload scheduling by evicting pods that are not optimally placed according to administrator-defined policies. This crucial function enhances resource utilization, balances workloads across nodes, and improves overall cluster performance.
