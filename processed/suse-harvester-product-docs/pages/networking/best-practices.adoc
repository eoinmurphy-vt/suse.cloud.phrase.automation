...
* Network configuration: By default, {harvester-product-name} creates a bond interface named `+mgmt-bo+` for the management network. On top of that is a bridge interface named `+mgmt-br+`, which may optionally use a VLAN. Each cluster network also has one new bond interface. You can view the current connection details using the `+nmcli+` tool.
+
Example:
+
----
