...
[NOTE]
====
In {harvester-product-name} v1.5.x and earlier versions, the entire VLAN ID range (2 to 4094) was assigned to the `+mgmt+` interfaces. However, this exceeded the upper limit of supported VLANs on certain network cards, so hardware VLAN offloading stopped working correctly.

For more information, see https://github.com/harvester/harvester/issues/7650[issue #7650].
====

Starting with v1.6.0, only the xref:installation-setup/config/configuration-file.adoc#_install_management_interface[primary VLAN ID] provided during installation is automatically added to the `+mgmt-br+` bridge and the `+mgmt-bo+` interface. You can <<Add secondary VLAN interfaces,add secondary VLAN interfaces>> after installation is completed.
...
For more information, see <<Change the MTU of a network configuration with an attached storage network>>.
...
. <<General changes,Change the MTU>> of the remaining network configuration.
+
[IMPORTANT]
====
