...
xref:virtual-machines/live-migration.adoc#_live_migratable_virtual_machines[Upgrades] and xref:hosts/hosts.adoc#_node_maintenance[node maintenance] both benefit from live migration. The underlying process, which is called *batch migration*, is slightly different from the one described in xref:#_starting_a_migration[Starting a Migration]. This process involves the following steps:
