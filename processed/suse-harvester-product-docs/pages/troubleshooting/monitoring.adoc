...
== {harvester-product-name} UI stops reporting virtual machine metrics after upgrade

=== Issue description

After an upgrade, the {harvester-product-name} UI stops reporting virtual machine metrics while the cluster metrics remain available. Disabling and re-enabling the `+rancher-monitoring+` add-on does not resolve the issue.
...
. Verify that the `+monitorAccount+` and `+monitorNamespace+` fields exist.
+
[,console]
----
...
. Delete all ConfigMap objects (`+kubevirt-install-strategy-*+`).

. Roll out the `+virt-operator+` deployment.
+
