...
. Configure a password to access the node. The default SSH user is `+rancher+`.
+
...
The default MTU value of the bonded NIC is `+1500+`. To use a different MTU value, configure the xref:/installation-setup/config/configuration-file.adoc#_install_management_interface[[`+install.management_interface+`] setting in a Harvester configuration file per following step.
