...
To perform PXE boot over tagged VLAN network, the {harvester-product-name} hosts must be configured with the following BIOS/UEFI settings:
...
Enables predictable network interface names in the installer, which will match the system default after {harvester-product-name} is installed. This allows the system to utilize the correct interfaces.