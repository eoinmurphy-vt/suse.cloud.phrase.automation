...
`+fleet apply+` reads `+known_hosts+``+ data from an environment variable (+`FLEET_KNOWN_HOSTS`), similarly to what the git
cloner does.

This means that, should you want to run `+fleet apply+` directly on your host machine, providing it a custom
value of `+KNOWN_HOSTS+`, you need to export `+FLEET_KNOWN_HOSTS+` beforehand or run it as
