...
{product_name} evaluates the `+Ready+` status of each `+BundleDeployment+` to determine when to proceed to the next partition. For more information, refer to xref:reference/ref-status-fields.adoc[Status fields].
...
[NOTE]
====
If `+rolloutStrategy+` is not specified in `+fleet.yaml+`, {product_name} uses the default values.
====
...
[NOTE]
====
{product_name} rolls out deployments in batches of up to `+maxNew+` clusters per partition (default: 50), regardless of partitions having more clusters assigned. After each batch, {product_name} checks the `+maxUnavailable+` threshold before continuing. After all deployments for a partition have been created, `+maxUnavailable+` is also evaluated. For example:

* If a partition has 25 clusters and `+maxUnavailable+` is 5, {product_name} deploys to all 25 before checking `+maxUnavailable+`.
* If a partition has 100 clusters, {product_name} deploys to the first 50 (default `+maxNew+`), checks `+maxUnavailable+`, and proceeds with the remaining 50 only if the threshold is not exceeded.
====

The following diagram displays how {product_name} handles rollout:

image::flow-rollout-fleet.png[A visual asset displaying flow of rollout in {product_name}.]

Various limits that can be configured in {product_name}:
...
| maxNew
| Maximum number of `+BundleDeployments+` that can be staged per reconciliation.
| 50
...
{product_name} supports automatic and manual partitioning. For more information about configuration options, refer to the xref:reference/ref-fleet-yaml.adoc[`+rolloutStrategy+` option in the fleet.yaml reference.]

**Automatic Partitioning**: {product_name} automatically creates partitions using `+autoPartitionSize+`.

For example, you have 200 clusters and set `+autoPartitionSize+` to 25%, {product_name} creates four partitions of 50 clusters each. Rollout proceeds in 50-cluster batches, checking `+maxUnavailable+` before continuing.
...
* **At or above the threshold**: {product_name} creates multiple partitions based on `+autoPartitionSize+`.
...
The following diagram illustrates how {product_name} handles rollout across multiple partitions, including readiness checks and deployment flow:

image::deploy-targets-partition.png[A visual asset displaying the flow of partition rollout.]

[NOTE]
====
`+maxNew+` controls how many `+BundleDeployments+` can be staged per reconciliation. The default is 50. You can configure this via `+rolloutStrategy.maxNew+` in `+fleet.yaml+`.
====

Within each partition, {product_name} rolls out up to `+maxNew+` `+BundleDeployments+` at a time (default: 50). The diagram below shows how {product_name} determines whether to proceed or wait during this process:
...
* For fewer than `+autoPartitionThreshold+` clusters (default 200), {product_name} uses a single partition.
* For `+autoPartitionThreshold+` or more clusters, {product_name} uses the `+autoPartitionSize+` value (default 25%) to create partitions.

For example, with 200 clusters (meeting the default `+autoPartitionThreshold+`), {product_name} uses the default `+autoPartitionSize+` of 25%. This means, {product_name} creates 4 partitions (25% of 200 = 50 clusters per partition). {product_name} processes up to 50 clusters at a time, which means it:

1. Rolls out to the first 50 clusters.
2. Evaluate readiness based on `+maxUnavailable+`.
...
To avoid this, {product_name} can control how many clusters are updated at a time. You can use the following rollout configuration options to slow down and stage the rollout:

* `+autoPartitionSize+`
* `+partitions+`
* `+maxUnavailable+`

{product_name} does not add artificial delays during rollout. Instead, it proceeds based on the `+readiness+` status of workloads in each cluster. Factors that affect readiness include image pull time, startup time, and readiness probes. Although using readiness probes is recommended, they are not strictly required to control rollout speed.
...
1. {product_name} begins with the first partition (40 clusters).
2. It deploys up to 50 `+BundleDeployments+` at once. So it deploys to all 40 clusters in the partition in one batch.
3. {product_name} checks the readiness of clusters in the partition.
...
4. When the entire partition is mostly ready (90%), {product_name} moves to the next partition.

If you want or need to process fewer than 40 deployments at once, you can put fewer clusters into each partition.

== Use Cases and Behavior

If the number of clusters doesn’t divide evenly, {product_name} rounds down partition sizes. For example, 230 clusters with `+autoPartitionSize: 25%+` results in:
...
* {product_name} creates one partition containing all 50 clusters, since no partitions are defined.
  * No requirement to specify `+maxUnavailablePartitions+`, as only one partition is created.
* Although there is no specified manual partition and `+maxUnavailable+` is set to 10%, {product_name} deploys to all 50 clusters at once (batch behavior overrides `+maxUnavailable+` initially).
* Evaluation occurs after all deployments are created.

The following diagram illustrates how {product_name} handles 50 clusters in a single partition:
...
* {product_name} creates one partition containing all 100 clusters, since no partitions are defined.
  * No requirement to specify `+maxUnavailablePartitions+`, as you have only one.
* Although there is no specified manual partition and `+maxUnavailable+` is set to 10%, {product_name} deploys to 50 clusters at once (batch behavior overrides `+maxUnavailable+` initially).
...
* {product_name} creates 10 partitions, each with 20 clusters.
...
* {product_name} creates `+BundleDeployments+` for clusters in the first partition (for example, `+demoRollout+`).
* The rollout proceeds strictly in order, {product_name} only moves to the next partition when the current one is considered ready.
...
The following diagram describes how {product_name} handles whether to continue or pause rollout.
