...
* a https://semver.org/semantic[https://semver.org/semantic] versioning constraint is specified, such as ++0.1.x++, ++< 2.0.0++. More information on supported constraints https://github.com/Masterminds/semver?tab=readme-ov-file#checking-version-constraints[checking version constraints].
...
In this case, polling can make sense, because referencing the chart using a repository allows {product_name} to check the repository's ++index.yaml++ for available versions matching the ++version++ field.
...
For more information, refer to xref:how-tos-for-users/gitrepo-add.adoc#_using_private_helm_repositories[Private Helm repo].
