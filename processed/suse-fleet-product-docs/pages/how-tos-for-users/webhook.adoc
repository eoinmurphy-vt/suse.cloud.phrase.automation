...
For larger deployments, configure webhooks to trigger reconciliation when new commits arrive. This reduces the delay between a Git push and {product_name} processing the change.

When multiple commits arrive in quick succession, the {product_name} controllers process them through the normal reconciliation loop when updating the corresponding `+GitRepo+` resources. This helps avoid update conflicts and improves reliability in large-scale deployments.
