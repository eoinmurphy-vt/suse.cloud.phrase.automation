...
image::fleet-ociStorage-flow.png[A visual asset displaying the flow of {product_name} with OCI Storage.]

[NOTE]
====
{product_name} checks for the integrity of OCI artifacts and tags OCI artifacts as `+latest+`.
====
...
* A {product_name} installation (v2.12.0 or later).
...
| `+username+`        | Username with write access to the registry. | Base64-encoded string       | If not specified, {product_name} accesses the registry without authentication.
| `+password+`        | Password for the write-access user.         | Base64-encoded string       | If not specified, {product_name} accesses the registry without authentication.
...
image::ociStorage-secret-ss.png[A screenshot of OCI secrets enabled for {product_name}]
