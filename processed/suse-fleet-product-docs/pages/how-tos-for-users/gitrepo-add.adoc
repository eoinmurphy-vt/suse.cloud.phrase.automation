...
[NOTE]
====
{product_name} does not support SSH proxy server authentication when cloning <<_adding_a_private_git_repository,adding-a-private-git-repository>>or <<_using_private_helm_repositories,Using Private Helm Repositories>>
...
`+known_hosts+` entries are sourced in priority from secrets referenced in `+GitRepo+` resources, e.g. `+helmSecretName+` for
accessing Helm charts or `+clientSecretName+` for cloning git repositories.
...
More details xref:how-tos-for-operators/installation.adoc#additional-ssh-known-hosts[here] on where the default entries
come from and how to populate additional ones at installation time.
...
Fleet only uses a single_source of `+known_hosts+` entries at a time. This means that, even if a secret contains invalid
...
A {product_name} agent in a downstream cluster monitors a cluster-specific namespace on the upstream cluster.
...
Pausing a GitRepo only prevents bundles and bundle deployments from being created or updated. It only affects _controller_ operations, not {product_name} _agent_ operations.
