...
Because the chart is downloaded at this stage, `+targetCustomizations+` cannot be used to dynamically fetch a different chart from:

* a local directory
* a different Helm repository
* a separate OCI registry (for example, one behind a firewall)

If you attempt to override these fields, the override may be ignored during chart retrieval. In some cases, Fleet may attempt to include multiple chart versions in the same bundle, which can significantly increase bundle size and may exceed etcd size limits.

If different environments require different charts or registries, use one of the following approaches:

* Define separate `+fleet.yaml+` files for each chart configuration.
* Create separate `+GitRepo+` resources for each environment.
