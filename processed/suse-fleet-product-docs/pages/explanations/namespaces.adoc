...
If you are using {product_name} in a xref:explanations/concepts.adoc[single cluster] style, the namespace will always be *fleet-local*.  Refer to the <<_cluster_registration_namespace_fleet_local,`+fleet-local+` namespaces section>> for more information.
...
When {product_name} is installed the `+fleet-local+` namespace is created along with one
