...
For how to select a `+kubernetesVersion+` please check our xref:references/kubernetesversions.adoc[Kubernetes Versions] page.
...
In {elemental-product-name}, we set this to a xref:references/machineinventoryselectortemplate-reference.adoc[MachineInventorySelectorTemplate].
...
.warning
====
The labels for the example are manual set labels, they are not set by {elemental-product-name} automatically..
====
