...
[IMPORTANT]
====
This guide will not cover the Rancher installation behind a proxy. It's a different use case and you can find the detailed documentation at the https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/other-installation-methods/http-proxy/http-proxy.html[Installing SUSE Rancher Prime behind an HTTP Proxy] page.
====
...
You can do it either with xref:quickstarts/quickstart-ui.adoc#_add_a_machine_registration_endpoint[UI] or xref:quickstarts/quickstart-cli.adoc#_prepare_your_kubernetes_resources[CLI].
...
xref:rancher-os-management/architecture/architecture-components.adoc#_elemental_register_command_line_tool[{elemental-product-name}-register] is the first communication endpoint between the new host and Rancher Manager, this is the first place where proxy settings need to be set.
...
To do that, you have to create a xref:references/seedimage-reference.adoc[SeedImage] definition.
...
Boot the ISO and you should see your new system appears in xref:references/machineinventory-reference.adoc[Machine inventory].
