...
This guide provides instructions on creating and managing virtual clusters in K3k and covers common use cases using both the *Custom Resource Definitions (CRDs)* and the *K3K CLI* so you can choose the method that fits your workflow.
...
This creates a virtual cluster in `+shared+` mode and expose it via an ingress with the specified host name.
