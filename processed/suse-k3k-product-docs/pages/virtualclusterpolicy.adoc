...
WARNING: Using a combination of `+RKE2+` and `+Cilium+` in the host cluster could result in issues with network policies. The recommended solution is to use `+VirtualClusterPolicy+` to disable network policy setup with K3k.
...
You can enforce Pod Security Standards (PSS) by specifying a Pod Security Admission (PSA) level. K3k applies the corresponding PSA labels to each bound Namespace. The allowed values are `+privileged+`, `+baseline+`, `+restricted+`, and this adds labels like `+pod-security.kubernetes.io/enforce: <level>+` to the bound Namespace.
...
* For a complete reference of all `+VirtualClusterPolicy+` spec fields, see xref:./references/crds.adoc#VirtualClusterPolicy[API Reference for VirtualClusterPolicy].
