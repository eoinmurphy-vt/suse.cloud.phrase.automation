...
Following invocations are fast because {project-name} uses Wasmtime's cache feature.
The first execution can take about 20 seconds with `+kwctl+`, later executions, 1 to 2 seconds.
So, {short-project-name} Policy Server has a slower start-up time but then policy evaluation times aren't usually effected by the usage of Kubernetes objects.
====
...
. Create a `+payload+` object. Do this using a helper function of the {short-project-name} SDK: `+kubewarden_testing.BuildValidationRequest+`.
...
This doesn't happen when the policy evaluates in {short-project-name},
