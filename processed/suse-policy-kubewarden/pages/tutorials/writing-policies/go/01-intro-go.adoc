...
There's another Go compiler that can build WebAssembly binaries usable by {project-name}.
...
The {short-project-name} project currently suggests using TinyGo for two reasons:
...
{short-project-name} policies need to process JSON data such as policy settings and the request received by Kubernetes.

Despite TinyGo's current limitations, it's still easy to write {short-project-name} validation policies with it.

== Tooling

Writing {short-project-name} policies requires a version of TinyGo greater than `+v0.28.1+`.
...
These Go libraries are useful when writing a {short-project-name} policy:
...
Set `+union+`, `+intersection+`, `+difference+` are common operation in {short-project-name} policies.

Lastly, the {short-project-name} project provides a
https://github.com/kubewarden/go-policy-template[template Go policy project]
you can use to create {short-project-name} Go policies.
...
CLI tool provided by {short-project-name} to run its policies outside of Kubernetes, among other actions.
