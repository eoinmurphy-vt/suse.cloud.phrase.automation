...
A {short-project-name} policy exposes two different functions that receive the policy settings as input:
...
object.
* `+validate_settings+`: Call this function when the policy is first loaded by {short-project-name}.
...
All {short-project-name} policies have to implement settings validation.
...
https://github.com/kubewarden/policy-sdk-go[{short-project-name}'s SDK].
