...
Rego policies support context aware data from the {project-name} 1.9 release.
====
...
If you're more interested in the {short-project-name} implementation,
...
starting from the {short-project-name} 1.9 release.
...
but {short-project-name} hasn't yet implemented mutating policies with Gatekeeper compatibility.
You can use policies that use the {short-project-name} SDK to write mutating policies,
but currently, you can't run Gatekeeper mutating policies in {short-project-name}.
