...
{short-project-name} provides https://github.com/kubewarden/cel-policy[cel-policy].
...
In addition, `+cel-policy+` bundles a *{short-project-name} CEL extension library* that exposes
{short-project-name}'s https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[host capabilities as native
CEL]:
...
make use of the features added by {short-project-name}.
...
=== Benefits of {short-project-name}'s `+cel-policy+` in comparison with ValidatingAdmissionPolicies

The {short-project-name} `+cel-policy+`:
...
ValidatingAdmissionPolicyBinding, as this is included in {short-project-name}'s
(Cluster)Admissionpolicies definitions.
* Can be deployed to clusters that lack ValidatingAdmissionPolicies support.
* Is context-aware, and makes use of {short-project-name}'s fine-grained permissions
for xref:../../../reference/spec/05-context-aware-policies.adoc[context awareness].
* Is deployed as (Cluster)AdmissionPolicy.
* Benefits from {short-project-name}'s tracing and telemetry on policies.
