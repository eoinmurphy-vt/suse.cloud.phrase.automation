...
:description: Configure context-aware policies to ensure unique Ingress rules across hosts, preventing conflicts with {project-name}'s CEL extension libraries.
...
{project-name}'s `+cel-policy+` supports the xref:reference/spec/05-context-aware-policies.adoc[context
...
To achieve this, we can use the https://github.com/kubewarden/cel-policy?tab=readme-ov-file#host-capabilities[{short-project-name}'s CEL extension
