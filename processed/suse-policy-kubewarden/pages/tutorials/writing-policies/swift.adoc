...
:description: Learn how to use the Swift compiler with WebAssembly support through SwiftWasm to access tools for building {project-name} policies.
...
In the meantime, you can use the tool chain provided by the SwiftWasm project to build {project-name} policies.
...
* https://github.com/kubewarden/policy-sdk-swift[{short-project-name} Swift SDK]: this provides a set of ``+struct+``s and functions that simplify the process of writing policies.
* https://github.com/kubewarden/swift-policy-template[{short-project-name} Swift template project]: use this template to scaffold a Swift-based policy.
...
has a {short-project-name} Policy written in Swift.
