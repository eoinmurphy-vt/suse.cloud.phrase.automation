...
You also need to update the `+ValidationResponse+` struct and the `+MutateRequest+` function in `+kw_sdk.go+` to remove the Kubernetes-specific types and use {short-project-name} types instead:
