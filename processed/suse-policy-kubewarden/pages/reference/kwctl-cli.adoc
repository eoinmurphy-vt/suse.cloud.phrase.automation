...
Add {short-project-name} metadata to a WebAssembly module
...
Benchmarks a {short-project-name} policy.
...
- Local YAML file: e.g., `+file://home/tux/cluster-admission-policy.yaml+` (contains declarations of {short-project-name} Custom Resources like `+ClusterAdmissionPolicy+`, `+AdmissionPolicy+`, etc.)

Default Behavior:
If the schema is omitted, `+file://+` is assumed, rooted in the current directory.

Notes on {short-project-name} Custom Resources:
...
- The `+--raw+` flag cannot be used, as {short-project-name}'s Custom Resources do not support `+raw+` policies.
...
* `+<URI_OR_SHA_PREFIX_OR_YAML_FILE>+` — Policy URI, SHA prefix or YAML file containing {short-project-name} policy resources. Supported schemes: registry://, https://, file://. If schema is omitted, file:// is assumed, rooted on the current directory.
...
Inspect {short-project-name} policy
...
Pulls a {short-project-name} policy from a given URI
...
* `+--github-owner <VALUE>+` — GitHub owner expected in the certificates generated in CD pipelines
* `+--github-repo <VALUE>+` — GitHub repository expected in the certificates generated in CD pipelines
* `+-o+`, `+--output-path <PATH>+` — Output file. If not provided will be downloaded to the {short-project-name} store
* `+--sigstore-trust-config <PATH>+` — JSON-formatted file conforming to the ClientTrustConfig message in the Sigstore protobuf specs. This file configures the entire Sigstore instance state, including the URIs used to access the CA and artifact transparency services as well as the cryptographic root of trust itself
* `+--sources-path <PATH>+` — YAML file holding source information (https, registry insecure hosts, custom CA's...)
* `+-a+`, `+--verification-annotation <KEY=VALUE>+` — Annotation in key=value format. Can be repeated multiple times
* `+--verification-config-path <PATH>+` — YAML file holding verification config information (signatures, public keys...)
...
Pushes a {short-project-name} policy to an OCI registry
...
Removes a {short-project-name} policy from the store
...
Run one or more {short-project-name} policies locally.
...
* `+vap+` — Convert a Kubernetes `+ValidatingAdmissionPolicy+` into a {short-project-name} `+ClusterAdmissionPolicy+`
...
* `+-o+`, `+--operation <TYPE>+` — {short-project-name} Custom Resource type +
...
* `+-t+`, `+--type <VALUE>+` — {short-project-name} Custom Resource type +
...
Convert a Kubernetes `+ValidatingAdmissionPolicy+` into a {short-project-name} `+ClusterAdmissionPolicy+`
...
Verify a {short-project-name} policy from a given URI using Sigstore
