...
OCI Artifacts are used to store objects such as {project-name} policies,
Helm charts, and more.

These are the capabilities exposed by the {short-project-name} host,
...
Computing the digest involves a series of network requests between the {short-project-name} policy host and the remote registry.
These operations can be time expensive,
so the results are cached for 1 minute.

=== Authentication

Interactions with private registries require the {short-project-name} policy host to authenticate against the remote registry.
...
Fetching this information involves network requests between the {short-project-name}
policy host and the remote registry. These operations can be time expensive so
the results are cached for 1 minute.

=== Authentication

Interactions with private registries require the {short-project-name} policy host to
