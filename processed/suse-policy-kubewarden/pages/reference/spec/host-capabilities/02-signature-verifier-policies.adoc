...
{project-name} implements support for the
...
like a {short-project-name} policy.
...
{short-project-name}'s `+policy-server+` or `+kwctl+`.

The different language SDKs for {short-project-name} policies take care of this.
They provide functions for verification of container image,
{short-project-name} policies, Helm charts and other kinds of OCI artifact.
