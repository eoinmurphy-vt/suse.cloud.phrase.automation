...
:description: Explore Kubernetes capabilities using {project-name}'s context-aware policies and the waPC protocol to access cluster resources.
...
{project-name} context aware policies require access to resources from the Kubernetes cluster where they are running.
For that, the {short-project-name} SDKs expose functions that use the waPC communication protocol to talk with the host system asking for data about the cluster.
