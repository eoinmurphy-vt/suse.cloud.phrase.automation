...
The {project-name} policy server receives:
...
* A JSON `+request+` attribute containing the free-form request document, in case
  of a raw policy. Check the xref:../../howtos/raw-policies.adoc[Raw policies]
...
You can specify these `+message+` and `+code+` attributes when the request isn't
accepted. The `+message+` is a free-form textual error and `+code+` represents an
HTTP error code.

The Kubernetes API server ignores the `+message+` and `+code+` values on request
acceptance.
