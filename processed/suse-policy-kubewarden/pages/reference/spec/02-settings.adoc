...
:description: Configure policy settings using {project-name}'s for validation and serialization of policy data.
...
{project-name} takes care of serializing the policy settings into JSON and provides
...
{short-project-name}'s https://github.com/chimera-kube/policy-server[policy-server]
