...
The `+default+` Policy Server deployed by {project-name} Helm charts has access to
...
| Rego
| ✅
| Since {short-project-name} 1.9 release

| WASI
| ✅
| Since {short-project-name} 1.10.0 release, only for Go SDK
|===

== Constraints

{short-project-name}'s priority is to reduce the number of queries against the Kubernetes
API server. {short-project-name} considers two restraints:
...
{short-project-name} policies can fetch multiple resources at the same time. For example,
...
query. This limitation is to be removed in future releases of {short-project-name}.

=== Fetching a specific resource

{short-project-name} policies can get a specific resource defined inside of the cluster.
...
policy author using the {short-project-name} SDKs.
