...
:description: Configure verification configuration formats for secure container image signing using a policy-server or policy to verify provenance and signatures.
...
* `+githubAction+`: For signatures performed with Sigstore's keyless workflow
  inside GitHub Actions. {short-project-name} checks this information against the x509
...
With {short-project-name}, you can verify policy signature are from trusted users *and*
...
This last example tests if a given policy came from the {short-project-name}
organization:
