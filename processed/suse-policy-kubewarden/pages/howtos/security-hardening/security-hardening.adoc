...
:sidebar_position: 50
:description: Secure {project-name}'s with hardening tips and best practices to mitigate threats.
:keywords: ["{project-name}", kubewarden, kubernetes, security]
:doc-persona: kubewarden-operator, kubewarden-integrator
:doc-type: howto
:doc-topic: operator-manual, security

{project-name} strives to be secure with little configuration. In this
section and its subpages, you can find hardening tips (with their trade-offs)
to secure {short-project-name} itself.
...
Operators can obtain a secure deployment by installing all the {short-project-name} Helm
...
ensure the {short-project-name} stack is safe from other workloads.

== Verifying {short-project-name} artifacts

Refer to the xref:/tutorials/verifying-kubewarden.adoc[Verifying
{short-project-name}] tutorial.

== RBAC

{short-project-name} describes RBAC configurations in different _Explanations_
...
By default, {short-project-name} excludes specific Namespaces from
{short-project-name} coverage. This is done to simplify first-time use and
...
From 1.23, {short-project-name}'s stack is able to run in a Namespace where the
...
restricted` label to the {short-project-name} deployment Namespace.
