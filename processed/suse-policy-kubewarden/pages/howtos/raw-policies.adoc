...
:description: Configure raw policies as a generic policy evaluation engine using {project-name}.
...
From `+v1.9.0+` onwards, {project-name} supports the ability to write and execute policies
...
arbitrary JSON documents against a {short-project-name} policy.
...
== Running a Policy Server inside Kubernetes without the {short-project-name} controller

It's not possible to use a Policy Server instance managed by the {short-project-name} controller to host raw policies.
...
[IMPORTANT]
====
The Policy Server instance deployed will have access to Kubernetes resources that could be used by context aware policies.
...
Similarly to policies that validate Kubernetes resources, raw policies are written in WebAssembly using {short-project-name} SDKs.
