...
(PSP) is removed. Now you can use {project-name} for admission control on your
Kubernetes clusters.

{short-project-name} has separate policies to achieve the same goal as a monolithic PSP
configuration. Each {short-project-name} policy definition functions as a different
configuration section in the specification of a PSP. The mapping of PSP
configuration fields to their respective {short-project-name} policies is in the
<<_mapping_kubewarden_policies_to_psp_fields,mapping table>> below.

With {short-project-name}, operators have granular control of policy configuration in
their clusters.

With a {short-project-name} instance, you can deploy policies to replace the
...
== {short-project-name} replacements for PSP

Now we will create {short-project-name} policies to achieve the same goal.
You enforce each rule with a separate {short-project-name} policy.
...
== Mapping {short-project-name} policies to PSP fields

This table maps PSP configuration fields to corresponding {short-project-name} policies.

|===
| PSP field | {short-project-name} equivalent policy
...
The {short-project-name} team has developed a script for PSP migration. It uses the
...
policies. It does this for {short-project-name} and other policy engines.

[CAUTION]
====

The AppVia migration tool is out of control of the Kuberwarden maintainers.
This means that it's possible it generates out-of-date {short-project-name} policies. Use
with caution. We need a pull request for AppVia for which work is ongoing.
Contact us for more information if you need to.
====


The script is available in the {short-project-name}
...
{short-project-name} policies can be applied directly to a cluster using the following command:
