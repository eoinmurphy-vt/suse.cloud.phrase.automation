...
:description: Configure metrics reporting on the Policy Server using Prometheus and {project-name} enabled through Helm charts for data collection.
...
section of this book. You need it for this section to work correctly.

====

You use https://prometheus.io/[Prometheus] to collect metrics exposed by the
Policy Server.

== Install Prometheus

You use https://github.com/prometheus-operator/prometheus-operator[Prometheus
Operator], that lets us define Prometheus' targets.
...
You can create the two ServiceMonitors named `+kubewarden-controller+` and
...
You can now install {short-project-name}, in the recommended way with Helm
charts.
...
First, you should add the Helm repository that contains {short-project-name}:
...
Helm chart creates a PolicyServer named `+default+`.

You should configure the Helm chart so that you have metrics enabled
in {short-project-name}. The `+kubewarden-values.yaml+` file should have the following contents:
...
`+kubewarden_policy_evaluations_total+`. You can see that the number of
evaluations grows over time as more requests go through the policy.
...
You can now login with the default username `+admin+` and password
`+prom-operator+`.

=== Using the {short-project-name} Grafana dashboard

{short-project-name} has a Grafana dashboard with basic metrics giving an
overview about how {short-project-name} behaves in the cluster. This dashboard
is available in the GitHub releases of the {short-project-name} policy-server
repository as a
https://github.com/kubewarden/policy-server/releases/latest/download/kubewarden-dashboard.json[JSON
file] or at the https://grafana.com/grafana/dashboards/15314[Grafana website].
...
https://grafana.com/docs/grafana/latest/dashboards/export-import/#import-dashboard[import
it]. Visit `+/dashboard/import+` in the Grafana dashboard and follow these steps:

. Copy and paste the JSON file contents into the `+Import via panel json+` box in
  the Grafana UI.
...
. Copy the dashboard ID from the
  https://grafana.com/grafana/dashboards/15314[dashboard page],
. Paste it into the `+Import via grafana.com+` field
. Click the `+load+` button.
. After importing the dashboard, define the Prometheus data source to use and
  finish the import process.

The Grafana dashboard has panes showing the state of all policies managed by
{short-project-name}. It also has policy-specific panels.
