...
:description: Configure the {project-name} to securely send telemetry data to a custom OpenTelemetry Collector deployed within the cluster.
...
This guide explains how to configure {short-project-name} to send telemetry
data to an OpenTelemetry collector already deployed on the cluster.

You should deploy only one instance of the
https://opentelemetry.io/docs/collector/[OpenTelemetry Collector] in the
cluster.
...
You need encrypted communication between the {project-name} components
and the collector. You can use https://cert-manager.io/[cert-manager] to manage
all the certificates required for secure communication.

OpenTelemetry Collector traces get sent to a
https://www.jaegertracing.io/[Jaeger] instance.

The {short-project-name} stack sends metrics to the OpenTelemetry Collector.
This one exposes the metrics as a Prometheus endpoint. The metrics are then
collected by a Prometheus instance and stored in its database. The same
Prometheus instance also exposes a UI to view and use the metrics.
...
You setup communication between {short-project-name} components and the
OpenTelemetry Collector using mTLS.
...
The Prometheus service monitor obtains the {short-project-name} metrics by
scraping the OpenTelemetry collector running in the `+kubewarden+` Namespace.

====

== Install OpenTelemetry Collector

Now you can deploy a custom OpenTelemetry Collector in the `+kubewarden+`
Namespace.
...
You secure communication between the {short-project-name} stack and the
OpenTelemetry Collector using mTLS. However the communication between the
OpenTelemetry Collector and Jaeger isn't secured, to reduce the complexity of
the example.

== Install {short-project-name} stack

When the OpenTelemetry Collector is running, you can deploy
{short-project-name} in the usual way.

You need to configure the {short-project-name} components so they send events
and metrics to the OpenTelemetry Collector.
...
certificate and its key used by the {short-project-name} stack to authenticate
...
You can see the trace events generated by {short-project-name} by using the
Jaeger web UI. They're grouped under the `+kubewarden-policy-server+` service:
...
The web UI is reachable at `+\http://localhost:8080+`.
