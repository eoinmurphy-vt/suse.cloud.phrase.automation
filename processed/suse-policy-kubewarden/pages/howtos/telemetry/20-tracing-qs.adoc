...
exposed by the `+my-open-telemetry-query+` Service in the `+jaeger+` Namespace.
...
If you installed Traefik in the previous guide, expose the Jaeger Query UI with
this `+Ingress+`:
...
To access the Jaeger UI with Traefik, run:
...
If you prefer to bypass Traefik, access Jaeger directly:
