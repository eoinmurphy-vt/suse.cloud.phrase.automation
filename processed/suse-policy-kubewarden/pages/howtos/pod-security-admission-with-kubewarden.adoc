...
= Using Pod Security Admission with {project-name}
:revdate: 2026-03-18
:page-revdate: {revdate}
:description: Configure Pod Security Admission in a Kubernetes cluster with {project-name}'s to secure pods.
...
{short-project-name} can be used to *integrate* a PSA profile to avoid these limitations.

[NOTE]
====

You could use {short-project-name} to replace the old PSP configuration as shown in xref:howtos/psp-migration.adoc[PSP migration].
However, the goal of this article is to show how {short-project-name} can complement the new PSA.
====
...
This is where {short-project-name} can help.
...
You need the {short-project-name} stack in the Kubernetes cluster for this example.
...
Now the deployment is mutated by {short-project-name}'s policy which allows the Pod to be started.
...
The {short-project-name} integration can do more in this scenario.
It can check the value of the `+runAsUser+` provided.

This resource is rejected by the {short-project-name} policy from earlier:
...
Using {short-project-name} together with PSA helps fill this gap.
