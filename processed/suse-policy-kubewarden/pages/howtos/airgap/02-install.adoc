...
:description: Configure a private registry with {project-name} images and policies to install {project-name} in an air-gapped environment.
...
----
+
A file with a list of default policies is in the {short-project-name} defaults
...
. Download `+kubewarden-save-policies.sh+` and `+kubewarden-load-policies.sh+` from
  the https://github.com/kubewarden/kubewarden-controller/blob/main/scripts[`+kubewarden-controller+` repository].
