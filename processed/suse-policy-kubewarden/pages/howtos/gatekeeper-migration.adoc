...
= Migrating Gatekeeper Policies to {project-name}
:revdate: 2026-03-18
:page-revdate: {revdate}
:description: Configure a {project-name} policy by migrating an existing Gatekeeper policy through two main steps: compiling Rego code into WebAssembly.
...
{project-name} policy. This process involves two main steps:
. Compile the Rego program into a WebAssembly (Wasm) module.
. Distribute the WebAssembly module as a {short-project-name} policy.
...
into a functional {short-project-name} policy. It uses a basic Gatekeeper demo policy.
...
* link:https://github.com/kubewarden/kwctl/releases[`+kwctl+`]: tool you use to
  prepare and run {short-project-name} web assembly module
...
already available policies in the {short-project-name}
...
available OPA and Gatekeeper policies migrated to {short-project-name}.

Also, take a look at our
xref:/explanations/comparisons/opa-comparison.adoc[comparison]
documentation between {short-project-name} and Gatekeeper.

== Step 1: Initialize Your {short-project-name} policy project

First, use the {short-project-name} Gatekeeper
...
`+ConstraintTemplate+` and its associated `+Constraint+` resources into a {short-project-name}
policy. In a {short-project-name} context, consider the `+ConstraintTemplate+` as the core
policy code, while the `+Constraint+` instances translate into policy instances
running within {short-project-name}.

First, copy the Rego code from your `+ConstraintTemplate+` into the `+policy.rego+`
file the {short-project-name} template generated. For this example, you should use the
following basic
...
=== Adapt Rego code for {short-project-name}

You need to make sure the `+package+` name used inside of the Rego code is `+policy+`.
This is the value expected in many places by the {short-project-name} Gatekeeper template.
...
The {short-project-name} Gatekeeper template includes both Rego unit tests and end-to-end
...
`+Constraint+`. You need to update the `+settings+` section in the generated {short-project-name}
