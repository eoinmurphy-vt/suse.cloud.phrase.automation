...
:description: Configure custom certificate authorities for use with {project-name} installations to ensure secure communication and authentication.
...
In a standard {project-name} installation, the `+policy-server+` uses the
CA store shipped with its Linux container.
On the client side, `+kwctl+` uses your operating system CA store.

If you are using the
https://github.com/kubewarden/kubewarden-controller[Controller],
