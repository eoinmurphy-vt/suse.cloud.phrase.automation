...
[NOTE]
====
Context aware policies are only available in {project-name} versions ≥ v1.6.0.
====
...
By default, {short-project-name} blocks all cluster resources. A {project-name} administrator
...
policy. Policy authors do this by annotating the policy. {project-name}
...
To prevent system abuse, {project-name} administrators must review the resources
the policy accesses.
