...
. Pour la mise à jour, utilisez la même commande helm que pour la xref:/setup/install-stackstate/kubernetes_openshift/kubernetes_install.adoc#_deploy_suse_observability_with_helm[première installation de Kubernetes]. Le nouveau diagramme helm tirera les nouvelles versions des images Docker et s'occupera de la mise à jour.
--
...
. Pour mettre à jour, utilisez la même commande helm que pour la xref:/setup/install-stackstate/kubernetes_openshift/openshift_install.adoc#_deploy_suse_observability_with_helm[première installation d'OpenShift]. Le nouveau diagramme helm tirera les nouvelles versions des images Docker et s'occupera de la mise à jour.
--
...
Une fois SUSE Observability mis à niveau et démarré, vérifiez que la nouvelle installation de SUSE Observability est accessible et que l'application fonctionne.
