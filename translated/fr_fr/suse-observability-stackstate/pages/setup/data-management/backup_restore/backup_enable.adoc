...
Remplacez `+CONTAINER+` par le nom de votre conteneur Azure Blob Storage. Les sauvegardes des différents datastores seront organisées en utilisant les valeurs configurées de `+s3Prefix+`. Les valeurs `+AZURE_STORAGE_ACCOUNT_NAME+` et `+AZURE_STORAGE_ACCOUNT_KEY+` de la section précédente s'appliquent ici.
