...
Suivez le xref:/setup/otel/getting-started/getting-started-lambda.adoc[guide de démarrage] pour configurer un collecteur qui enverra les données à SUSE Observability. Pour personnaliser la configuration du collecteur afin de mettre en place l'échantillonnage, le filtrage, etc., consultez notre xref:/setup/otel/collector.adoc[documentation sur le collecteur].
