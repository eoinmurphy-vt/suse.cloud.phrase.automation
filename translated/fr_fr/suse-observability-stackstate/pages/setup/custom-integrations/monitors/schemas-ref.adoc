...
* `+remediationHint+`: Description de ce que l'utilisateur peut faire en cas d'échec du moniteur. Le format est markdown, avec éventuellement l'utilisation de variables handlebars pour personnaliser l'indice sur la base de séries temporelles ou d'autres donnéesxref:use/alerting/k8s-add-monitors-cli.adoc#_write_the_remediation_hint[(plus d'explications ci-dessous).]
...
* `+urnTemplate+`: Un modèle pour construire l'urne du composant xref:use/alerting/k8s-add-monitors-cli.adoc#_bind_the_results_of_the_monitor_to_the_correct_components[auquel] le résultat du moniteur sera xref:use/alerting/k8s-add-monitors-cli.adoc#_bind_the_results_of_the_monitor_to_the_correct_components[lié].
