...
L'API SUSE Observability Receiver recevra et traitera automatiquement toutes les données de santé entrantes. SUSE Observability ne nécessite pas de configuration supplémentaire pour activer la synchronisation de l'état de santé, mais les données de santé reçues doivent correspondre au format JSON attendu.

Les pages suivantes fournissent des informations détaillées sur la manière d'intégrer les données relatives à la santé :
...
Voici un exemple de création d'un site `+External Monitor+` à l'aide du xref:/setup/cli/cli-sts.adoc[CLI de SUSE Observability]
