...
Le jeton secret spécifié dans la configuration du canal est inclus dans les demandes de webhook dans l'en-tête `+X-StackState-Webhook-Token+`. Votre point de terminaison webhook peut vérifier la valeur pour s'assurer que les demandes sont légitimes.
