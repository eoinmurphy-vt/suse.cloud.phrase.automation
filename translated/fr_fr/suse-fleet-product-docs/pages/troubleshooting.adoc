...
Les premières choses à vérifier lorsque {product_name} se comporte de manière inattendue seraient :

* Les journaux de `+fleet-controller+` sur le cluster de gestion : {product_name} a-t-il échoué à réconcilier l'état actuel de toute ressource (bundle, déploiement de bundle) avec son état attendu ?
* Les journaux des pods `+gitjob+` sur le cluster de gestion : {product_name} a-t-il rencontré un problème lors de la création de tâches pour générer de nouveaux bundles pour de nouveaux commits trouvés dans les dépôts git surveillés ?
...
Si le problème est spécifique à un cluster cible, les utilisateurs doivent vérifier les journaux de l'agent Fleet sur ce cluster : {product_name} a-t-il échoué à déployer un déploiement de bundle sur ce cluster ?
...
Lors du test de {product_name} avec des dépôts git privés, vous remarquerez que les secrets HTTP ne sont plus pris en charge dans Github. Pour contourner ce problème, suivez ces étapes :
...
=== {product_name} échoue avec un code de réponse incorrect : 403

Si votre GitJob renvoie l'erreur ci-dessous, le problème peut être que {product_name} ne peut pas accéder au dépôt Helm que vous avez spécifié dans votre xref:reference/ref-fleet-yaml.adoc[`+fleet.yaml+`] :
...
=== Migrer la grappe locale vers le {product_name} espace de travail par défaut du cluster ?
