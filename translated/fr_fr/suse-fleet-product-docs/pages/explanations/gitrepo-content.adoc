...
* Dans le deuxième cas, le répertoire de base est `+driven/simple+`, qui ne contient que des manifestes de ressources Kubernetes (`+configmap.yaml+` et `+service.yaml+`). Puisqu'aucun fichier `+fleet.yaml+` ou options n'est spécifié, {product_name} générera un Bundle en utilisant le comportement par défaut : simplement empaqueter toutes les ressources trouvées dans le répertoire.
...
Lors de l'utilisation de ce mode de numérisation de bundle, {product_name} n'exclut pas les fichiers de configuration de bundle qui ne sont pas explicitement référencés dans le GitRepo. Par exemple, dans la structure de répertoire ci-dessus :
...
{product_name} prend en charge l'exclusion de fichiers et de répertoires par le biais de fichiers `+.fleetignore+`, de manière similaire à la façon dont les fichiers `+.gitignore+` se comportent dans les dépôts Git :
...
- Ordre de fusion : `+helm.values+` → `+helm.valuesFiles+` → `+helm.valuesFrom+`

image::FleetValuesStages.svg[flux des étapes de valeurs de Fleet]

L'étape de ciblage peut traiter les valeurs comme des modèles en utilisant les informations du cluster. Plus d'infos : xref:/reference/ref-fleet-yaml.adoc#_values_templating[modèles dans fleet.yaml].
