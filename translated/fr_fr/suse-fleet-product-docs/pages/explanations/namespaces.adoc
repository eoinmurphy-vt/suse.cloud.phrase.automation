...
* Si l'espace de noms n'existe pas sur le cluster en aval, le déploiement échoue avec une erreur. La pré-création de l'espace de noms et son étiquetage correct sont requis ; l'option `+createNamespace+` de Helm ne s'applique pas lorsque ce sélecteur est défini.
...
Rancher créera deux espaces de noms {product_name} : *fleet-default* et *fleet-local*.
