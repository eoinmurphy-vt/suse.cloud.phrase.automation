...
* *Hors de synchronisation* : Les Bundles sont synchronisés depuis le contrôleur Fleet, mais les BundleDeployments mis à jour n'ont pas encore été créés, donc les agents en aval ne peuvent pas synchroniser les changements.
...
* `+state+` : Représente l'état du GitRepo (par exemple, `+GitUpdating+`) ou le plus haut BundleState selon le <<state-ranking,Rang d'État>>. Si `+Ready+`, il est défini sur une valeur vide.
