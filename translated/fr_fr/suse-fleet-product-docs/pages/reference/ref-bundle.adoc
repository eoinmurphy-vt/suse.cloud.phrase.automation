...
Les paquets sont automatiquement créés par {product_name} lorsqu'un `+GitRepo+` est créé.

Le contenu de la ressource correspond au xref:reference/ref-crds.adoc#_bundlespec[BundleSpec].
Pour plus d'informations sur la procédure d'utilisation de la ressource de paquet xref:how-tos-for-users/bundle-add.adoc[Créer une ressource de paquet].
