...
Le CLI {product_name} est une interface de ligne de commande (CLI) qui vous permet d'interagir directement avec {product_name} depuis votre machine locale. Il vous permet de créer, appliquer et inspecter `+bundles+` sans nécessiter un `+GitRepo+`. Les cas d'utilisation typiques incluent :
...
+
Vous pouvez l'utiliser dans ces scénarios :
...
Vous pouvez déployer des charges de travail sans utiliser de GitRepos en les appliquant localement avec la CLI. Par exemple, utilisez le link:https://github.com/rancher/fleet-examples[dépôt d'exemples Fleet] :
...
Pour plus d'informations, consultez link:https://www.suse.com/c/rancher_blog/fleet-multi-cluster-deployment-with-the-help-of-external-secrets/[Fleet : Déploiement Multi-Cluster avec l'aide des Secrets Externes].
