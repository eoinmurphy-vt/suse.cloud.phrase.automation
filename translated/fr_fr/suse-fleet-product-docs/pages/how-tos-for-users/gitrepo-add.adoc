...
Les dépôts Git sont ajoutés au gestionnaire {product_name} en utilisant le type de ressource personnalisée `+GitRepo+`. Le type `+GitRepo+` est dans un espace de noms. Par défaut, Rancher créera deux espaces de travail {product_name} : 
...
{product_name} prend en charge les mécanismes d'authentification suivants pour les dépôts privés : +
...
{product_name} prend en charge l'ajout de `+known_hosts+` dans le secret ssh. Voici un exemple de la façon de l'ajouter :
...
Les champs suivants sont nécessaires pour permettre à {product_name} de s'authentifier auprès de GitHub en utilisant une appli GitHub :
...
_Exemple_ : Pour le chemin du dépôt `+world-domination/ui_charts+` et un secret contenant les clés suivantes, les identifiants sous le glob _deuxième_ seront utilisés :
...
[NOTE]
====
Le fichier doit être nommé `+secrets-path.yaml+` ; sinon {product_name} ne pourra pas l'utiliser.
====
...
Lors de l'utilisation de Kubernetes link:https://kubernetes.io/docs/tasks/administer-cluster/encrypt-data/[chiffrement au repos] et du stockage des identifiants dans Git, configurez le cluster en amont pour inclure plusieurs {product_name} CRDs dans la liste des ressources de chiffrement :
...
Lors de la sauvegarde et de la restauration de {product_name} avec des charges de travail existantes, qu'il s'agisse de GitRepos ou de HelmOps, considérez :
...
Un GitRepo xref:reference/ref-gitrepo.adoc[mis en pause] mettra en pause les bundles et les déploiements de bundles. Cela signifie :

* La suppression d'un déploiement de bundle d'un GitRepo mis en pause : {product_name} ne recréera pas le déploiement de bundle tant que le GitRepo n'est pas réactivé.
* La suppression d'un bundle d'un GitRepo mis en pause : {product_name} supprimera les déploiements de bundle provenant de ce bundle, et ne recréera pas le bundle (ni les déploiements de bundle) tant que le GitRepo n'est pas réactivé.
