...
Procédez à la mise à niveau vers {longhorn-product-name} en utilisant le chart {longhorn-product-name} :
...
. Vérifiez la version actuelle de votre `+longhorn-crd+` release Helm :
+
[,shell]
----
...
. Clonez le dépôt `+rancher/charts+` contenant la version spécifique du chart `+longhorn-crd+` qui correspond à votre release actuelle (par exemple, `+108.2.0+up1.10.1+` pour Longhorn v1.10.1) :
+
[,shell]
----
...
Alternativement, vous pouvez utiliser le link:{attachmentsdir}/patch-resource-policy-annotation.sh[script] pour appliquer automatiquement un correctif à l'annotation :
+
[,shell]
----
...
. Mettez à niveau la release Helm `+longhorn-crd+` en utilisant le chart patché localement :
+
[,shell]
----
...
Après que les CRDs ont été préparés, vous pouvez procéder à la mise à niveau de votre installation Longhorn vers {longhorn-product-name} :
