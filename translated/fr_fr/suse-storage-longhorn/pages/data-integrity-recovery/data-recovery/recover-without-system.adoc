...
. Copier le https://github.com/longhorn/longhorn/blob/v{patch-version}/examples/restore_to_file.yaml.template[modèle yaml] : Faire une copie de `+examples/restore_to_file.yaml.template+` par exemple `+restore.yaml+`. 
...
Nous fournissons également un script, https://raw.githubusercontent.com/longhorn/longhorn/v{patch-version}/scripts/restore-backup-to-file.sh[restore-backup-to-file.sh], pour restaurer une sauvegarde. Les paramètres suivants doivent être spécifiés :
