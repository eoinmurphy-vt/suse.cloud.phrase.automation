...
Il y a quelques composants dans {longhorn-product-name} : Gestionnaire, Moteur, Pilote et UI. Par défaut, tous ces composants s'exécutent en tant que pods dans l'espace de noms `+longhorn-system+` dans le cluster Kubernetes.
...
----
. Activez le serveur `+pprof+` pour le composant d'exécution souhaité (par exemple, sync-agent) :
+
...
. Désactivez le profileur après avoir terminé votre analyse :
+
[,bash]
----
