 ...
 ** Un volume est inchangé : Les utilisateurs n'ont besoin de rien faire.
 ** Les données sont mises à jour : En général, les utilisateurs n'ont rien à faire. Longhorn marquera automatiquement comme défaillantes les répliques qui ne contiennent pas les données les plus récentes.
 ** Un nouveau volume est créé : Ce volume disparaîtra après la restauration. Les utilisateurs doivent recréer un nouveau volume, lancer xref:data-integrity-recovery/data-recovery/retrieve-volume-data-from-replica.adoc[a single replica volume] basé sur la réplique du volume disparu, puis transférer les données vers le nouveau volume.
 ...
 ** Pour les volumes DR : Les utilisateurs n'ont besoin de rien faire. Longhorn effectuera une restauration complète.
 ** Certaines opérations sont appliquées à un volume :
  ...
  *** Instantané : Les informations d'instantané du volume doivent être resynchronisées une fois le volume rattaché.
