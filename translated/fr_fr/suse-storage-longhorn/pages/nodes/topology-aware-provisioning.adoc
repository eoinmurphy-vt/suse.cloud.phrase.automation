...
{longhorn-product-name} prend en charge cela grâce à deux fonctionnalités complémentaires :
...
+
* *{longhorn-product-name} UI* : Allez à *Paramètre > Général > Clés de topologie autorisées CSI* et entrez, par exemple, `+topology.kubernetes.io/zone+`.
...
| 3 | Immédiatement | zone : [a, b] | `+zone+` | false | zone dans [a, b]
| 4 | WFFC | aucune | `+""+` (vide) | false | aucune
| 5 | WFFC | aucune | `+zone+` | false | zone dans [a, b, c]
| 6 | WFFC | aucune | `+zone+` | true | zone dans [sélectionné]
| 7 | WFFC | zone : [a] | `+zone+` | false | zone dans [a]
| 8 | WFFC | zone : [a, b, c] | `+zone+` | true | zone dans [sélectionné]
|===
