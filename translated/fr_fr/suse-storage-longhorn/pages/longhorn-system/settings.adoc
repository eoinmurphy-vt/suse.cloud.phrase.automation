...
* `+block-if-contains-last-replica+` : {longhorn-product-name} bloque la vidange lorsque le nœud contient la dernière réplique saine d'un volume.
* `+allow-if-replica-is-stopped+` : {longhorn-product-name} permet la vidange lorsque le nœud contient la dernière réplique saine d'un volume mais que la réplique est arrêtée.
+
[WARNING]
====
...
====
+
* `+always-allow+` : {longhorn-product-name} permet automatiquement la vidange même si le nœud contient la dernière réplique saine d'un volume.
+
[WARNING]
====
...
====
+
* `+block-for-eviction+` : {longhorn-product-name} expulse automatiquement toutes les répliques et bloque la vidange jusqu'à ce que l'expulsion soit terminée.
+
[WARNING]
====
...
====
+
* `+block-for-eviction-if-contains-last-replica+` : {longhorn-product-name} expulse automatiquement toutes les répliques qui n'ont pas de contrepartie saine et bloque la vidange jusqu'à ce que l'expulsion soit terminée.
+
[WARNING]
====
Cela peut entraîner des vidanges lentes et un mouvement de données supplémentaire associé à la reconstruction de répliques.
...
* `+best-effort+` : {longhorn-product-name} tente de garder une réplique sur le même nœud que le volume ou la charge de travail attaché. Le volume continue de fonctionner même si des contraintes environnementales empêchent le placement local, comme un espace disque insuffisant ou des étiquettes de disque incompatibles.
* `+strict-local+` : {longhorn-product-name} impose le placement d'une seule réplique sur le même nœud que le volume attaché. Ce mode offre un nombre d'IOPS plus élevé et une latence plus faible.
...
*Exemple* : `+pass:[https://longhorn.example.com]+` ou `+pass:[https://longhorn.example.com:8443]+`
...
* *activé* : Active le hachage périodique des fichiers de disque instantané et les vérifications complètes de l'intégrité des données. Le système {longhorn-product-name} hache périodiquement les fichiers de disque instantané pour détecter une corruption non détectée par le système de fichiers, telle que la dégradation des bits. Ces vérifications peuvent affecter les performances du système lors de chaque analyse.
...
Les options de spécification de volume disponibles sont :
...
À partir de {longhorn-product-name} v1.6.0, {longhorn-product-name} vous permet de modifier les paramètres de la zone de danger sans avoir à attendre que tous les volumes soient détachés. Vos paramètres préférés sont immédiatement appliqués dans les scénarios suivants :
