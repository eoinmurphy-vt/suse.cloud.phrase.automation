...
Lors de l'installation de {longhorn-product-name} via l'interface utilisateur de Rancher, sélectionnez *Modifier en YAML* et ajoutez les paramètres suivants au YAML :
...
Lors de l'installation de {longhorn-product-name} avec Helm, définissez la valeur `+csi.podAntiAffinityPreset+` dans votre fichier `+values.yaml+` :
...
Si vous déployez {longhorn-product-name} en utilisant `+kubectl+`, modifiez manuellement le déploiement `+longhorn-driver-deployer+` et ajoutez la variable d'environnement suivante à la spécification du conteneur :
