...
+
Si le système de fichiers est `+ext4+`, le volume pourrait avoir besoin d'être https://linux.die.net/man/8/mount[monté] et https://linux.die.net/man/8/umount[démonté] une fois avant de redimensionner manuellement le système de fichiers. Sinon, l'exécution de `+resize2fs+` pourrait entraîner une erreur :
+
----
