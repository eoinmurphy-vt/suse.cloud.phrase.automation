...
Obtenez le {longhorn-product-name} Chart et décompressez l'archive tar téléchargée :
+
[subs="+attributes",bash]
----
...
Spécifiez le `+repository+` et le `+tag+` pour chaque image de composant {longhorn-product-name} :
...
Installez {longhorn-product-name} en exécutant la commande suivante dans le répertoire cloné :
+
[,bash]
----
