...
Chaque nœud du cluster Kubernetes où {longhorn-product-name} est installé doit remplir les exigences suivantes :
...
* Google Kubernetes Engine (GKE) : Voir xref:installation-setup/os-distro/gke.adoc[GKE].
* Clusters K3s : Voir xref:installation-setup/os-distro/k3s.adoc[K3s].
...
Voici les répertoires nécessitant un accès avec des permissions root et privilégiées pour les composants {longhorn-product-name} :
...
* RHEL, CentOS et EKS _(AMI Kubernetes Worker EKS avec l'image AmazonLinux2)_ : Exécutez les commandes suivantes :
+
----
...
* Pour RHEL, CentOS et EKS avec `+EKS Kubernetes Worker AMI with AmazonLinux2 image+`, utilisez cette commande :
+
----
yum install nfs-utils
----

* Pour SUSE/OpenSUSE, vous pouvez installer un client NFSv4 via :
+
----
