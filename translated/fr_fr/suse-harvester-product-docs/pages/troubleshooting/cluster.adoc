 ...
 ** xref:/installation-setup/config/settings.adoc#_support_bundle_expiration[support-bundle-expiration] : Définit le temps autorisé pour conserver un fichier de bundle de support
 ** xref:/installation-setup/config/settings.adoc#_support_bundle_timeout[support-bundle-timeout] : Définit le temps autorisé pour générer un fichier de bundle de support
...
+
* Téléchargez le script : `+curl -o collector-harvester https://raw.githubusercontent.com/rancher/support-bundle-kit/refs/heads/master/hack/collector-harvester+`
* Ajoutez des permissions d'exécution : `+chmod +x collector-harvester+`
* Exécutez le script : `+./collector-harvester / /tmp/support-bundle+`
+
...
Suivez ces étapes pour réinitialiser le paramètre :

. Consultez xref:./faq.adoc[la FAQ] pour vous connecter en SSH au nœud et passer à l'utilisateur `+root+`.

 $ sudo -s

. Modifier le paramètre `+ssl-parameters+` manuellement en utilisant `+kubectl+` :
+
----
...
. Créez le fichier `+/etc/modprobe.d/ixgbe.conf+` manuellement avec le contenu :
+
----
