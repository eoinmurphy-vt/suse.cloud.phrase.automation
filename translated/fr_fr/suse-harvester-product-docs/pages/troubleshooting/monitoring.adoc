...
== La surveillance est inutilisable

Lorsque le {harvester-product-name} tableau de bord n'affiche aucune métrique de surveillance, cela peut être causé par les raisons suivantes.

=== La surveillance est inutilisable en raison d'un Pod bloqué dans l'état `+Terminating+`

{harvester-product-name} Les pods de surveillance sont déployés aléatoirement sur les nœuds du cluster. Lorsque le nœud hébergeant les pods tombe accidentellement en panne, les pods concernés peuvent rester bloqués dans l'état `+Terminating+`, rendant la surveillance inutilisable depuis l'interface Web.
...
La surveillance peut être récupérée en utilisant des commandes CLI pour forcer la suppression des pods concernés. Le cluster redéploiera de nouveaux pods pour les remplacer.
...
Attendez quelques minutes pour que les nouveaux pods soient créés et prêts à rendre le tableau de bord de surveillance à nouveau utilisable.
...
== Étendre la taille du PV/Volume

{harvester-product-name} intègre {longhorn-product-name} comme fournisseur de stockage par défaut.

{harvester-product-name} La surveillance utilise un Volume Persistant (PV) pour stocker les données en cours d'exécution. Lorsqu'un cluster fonctionne depuis un certain temps, le Volume Persistant peut nécessiter d'étendre sa taille.

Pour des informations sur la façon d'augmenter la taille du volume, voir https://documentation.suse.com/cloudnative/storage/1.8/en/volumes/volume-expansion.html[Expansion de Volume] dans {longhorn-product-name} la documentation.

=== Voir le Volume

==== Depuis l'interface utilisateur intégrée {longhorn-product-name}

Accédez à l'interface utilisateur intégrée {longhorn-product-name} selon xref:./cluster.adoc#_access_embedded_rancher_and_longhorn_dashboards[ce document].

{longhorn-product-name} La vue par défaut du tableau de bord.

image::troubleshooting/2-longhorn-dashboard.png[]

Cliquez sur `+Volume+` pour lister tous les volumes existants.

image::troubleshooting/3-view-all-volume.png[]

==== Depuis la CLI

Vous pouvez également utiliser `+kubectl+` pour obtenir tous les volumes.
...
=== Réduire un déploiement

Pour détacher le `+Volume+`, vous devez réduire le `+deployment+` qui utilise le `+Volume+`.

L'exemple ci-dessous concerne le PVC revendiqué par `+rancher-monitoring-grafana+`.

Trouvez le `+deployment+` dans l'espace de noms `+cattle-monitoring-system+`.
...
Réduisez le déploiement `+rancher-monitoring-grafana+` à 0.

----
# kubectl scale --replicas=0 deployment/rancher-monitoring-grafana -n cattle-monitoring-system
----

Vérifiez le déploiement et le volume.
...
Dans l'interface utilisateur {longhorn-product-name}, le volume associé devient `+Detached+`. Cliquez sur l'icône dans la colonne `+Operation+`, et sélectionnez `+Expand Volume+`.

image::troubleshooting/4-select-volume-to-expand.png[]

Entrez une nouvelle taille, et {longhorn-product-name} étendra le volume à cette taille.

image::troubleshooting/5-expand-volue-to-new-size.png[]

=== Augmenter un déploiement

Après que le `+Volume+` soit étendu à la taille cible, vous devez augmenter le déploiement mentionné ci-dessus à ses répliques d'origine. Pour l'exemple ci-dessus de `+rancher-monitoring-grafana+`, les répliques d'origine sont 1.

----
# kubectl scale --replicas=1 deployment/rancher-monitoring-grafana -n cattle-monitoring-system
----

Vérifiez à nouveau le déploiement.
...
Le `+Volume+` est attaché au nouveau POD.

image::troubleshooting/6-after-scale-up.png[]

À présent, le `+Volume+` est étendu à la nouvelle taille et le POD l'utilise sans problème.

== Échec de l'activation du `+rancher-monitoring+` produit complémentaire

Vous pouvez rencontrer cela lorsque vous installez {harvester-product-name} v1.3.0 ou une version ultérieure sur un cluster avec la xref:../installation-setup/requirements.adoc#_hardware_requirements[taille de disque minimale requise].

=== Reproduire les étapes

. Installez le {harvester-product-name} cluster.
. Activez le produit complémentaire `+rancher-monitoring+` xref:../add-ons/add-ons.adoc[add-on], vous observerez :

* Le POD `+prometheus-rancher-monitoring-prometheus-0+` dans l'espace de noms `+cattle-monitoring-system+` ne parvient pas à démarrer en raison d'un échec d'attachement du PVC.
+
----
  $ kubectl get pods -n cattle-monitoring-system
  NAME                                                     READY   STATUS      RESTARTS   AGE
...
* Le Longhorn Manager ne peut pas planifier la réplique.
+
----
...
. Désactivez le produit complémentaire `+rancher-monitoring+` si vous l'avez déjà activé.
+
Tous les pods dans `+cattle-monitoring-system+` sont supprimés mais les PVC sont conservés. Pour plus d'informations, consultez [Addons].
+
----
 $ kubectl get pods -n cattle-monitoring-system
...
. Supprimez le PVC nommé `+prometheus+`, mais conservez le PVC nommé `+alertmanager+`.
+
----
...
. Sur l'écran *Addons* de l'interface utilisateur {harvester-product-name}, sélectionnez *⋮* (icône de menu) puis sélectionnez *Edit YAML*.
+
image::troubleshooting/edit-rancher-monitoring.png[]

. Comme indiqué ci-dessous, changez les deux occurrences du nombre `+50+` en `+30+` sous prometheusSpec, puis enregistrez. La fonctionnalité `+prometheus+` utilisera un disque de 30 GiB pour stocker des données.
+
image::troubleshooting/edit-rancher-monitoring-yaml.png[]
+
Alternativement, vous pouvez utiliser `+kubectl+` pour modifier l'objet.
+
...
. Activez le produit complémentaire `+rancher-monitoring+` et attendez quelques minutes.
. Tous les pods sont déployés avec succès, et la fonctionnalité `+rancher-monitoring+` est disponible.
+
----
 $ kubectl get pods -n cattle-monitoring-system
 NAME                                                     READY   STATUS      RESTARTS   AGE
...
== `+rancher-monitoring-crd+` L'état du ManagedChart est `+Modified+`

=== Description du problème

Dans certaines situations, l'état de l'objet `+rancher-monitoring-crd+` ManagedChart change en `+Modified+` (avec le message `+...rancher-monitoring-crd-manager missing...+`).
...
L'objet `+ManagedChart+` a un objet en aval nommé `+Bundle+`, qui contient des informations similaires.
...
Lorsque le problème existe et que vous xref:../upgrades/upgrades.adoc#_start_an_upgrade[démarrez une mise à niveau], {harvester-product-name} peut renvoyer le message d'erreur suivant : `+admission webhook "validator.harvesterhci.io" denied the request: managed chart rancher-monitoring-crd is not ready, please wait for it to be ready+`.

De plus, lorsque vous recherchez les objets marqués comme `+missing+`, vous constaterez qu'ils existent dans le cluster.
...
Les objets marqués comme `+missing+` n'ont pas les annotations et étiquettes requises par l'objet `+ManagedChart+`.
...
. Appliquez un correctif à l'objet ClusterRole `+rancher-monitoring-crd-manager+` pour ajouter l'opération `+update+`.
+
[,console]
----
...
. Appliquez un correctif aux objets marqués comme `+missing+` pour ajouter les annotations et étiquettes requises.
+
[,console]
----
...
. Vérifiez l'objet `+rancher-monitoring-crd+` ManagedChart.
+
Après quelques secondes, le statut de l'objet `+rancher-monitoring-crd+` ManagedChart change en `+Ready+`.
+
[,console]
----
...
----
+
De plus, les indicateurs d'erreur ne sont plus affichés pour les objets en aval.
+
[,console]
----
 $ kubectl bundle -A
...
. (Optionnel) Réessayez la mise à niveau (si elle a échoué précédemment à cause de ce problème).

=== Problème connexe

https://github.com/harvester/harvester/issues/5505[#5505]

== Certains `+rancher-monitoring+` pods du produit complémentaire sont abruptement terminés

=== Description du problème

Lorsque le `+rancher-monitoring+` produit complémentaire est activé, les pods liés à Prometheus, Alertmanager et Grafana sont terminés peu après leur création.
...
Les journaux du `+prometheus+` pod contiennent le message `+level=warn msg="Received SIGTERM, exiting gracefully..."+`.
...
L'objet CRD `+prometheus+` inclut `+storage-network.settings.harvesterhci.io/replica : "1" +` annotation.
...
Les journaux du pod Harvester ('déploiement harvester-system/harvester') indiquent que la tentative de changement du `+storage-network+` paramètre a été bloquée.
...
Lorsque vous apportez des modifications au paramètre xref:networking/storage-network.adoc#_storage_network_setting[storage-network], le contrôleur {harvester-product-name} attend que les volumes attachés soient détachés avant d'appliquer les modifications. De plus, le contrôleur termine automatiquement les pods liés à Prometheus, Alertmanager et Grafana car ces pods utilisent des volumes pour stocker des données.

Ce processus prend généralement peu de temps à compléter, mais peut être perturbé lorsque les éléments suivants se produisent :

* Les volumes attachés empêchent le contrôleur Harvester d'appliquer les modifications au paramètre.
* Un utilisateur ou le `+monitoring-operator+` tente d'activer le produit complémentaire `+rancher-monitoring+`.
* Le contrôleur Harvester termine les pods.

=== Solution provisoire

. Désactivez le produit complémentaire `+rancher-monitoring+`.

. Vérifiez si le paramètre xref:networking/storage-network.adoc#_storage_network_setting[storage-network] est activé ou désactivé.

. Vérifiez les indicateurs d'erreur dans les journaux des pods Harvester. Si des volumes sont encore attachés, arrêtez les machines virtuelles associées jusqu'à ce qu'aucune erreur n'apparaisse après le message `+storage network change+`.

. Activez le produit complémentaire `+rancher-monitoring+`.

=== Problème lié

https://github.com/harvester/harvester/issues/8565[#8565]

== {harvester-product-name} L'interface utilisateur cesse de signaler les métriques des machines virtuelles après la mise à niveau.

=== Description du problème

Après une mise à niveau, l'interface utilisateur {harvester-product-name} cesse de signaler les métriques des machines virtuelles tandis que les métriques du cluster restent disponibles. Désactiver et réactiver le `+rancher-monitoring+` produit complémentaire ne résout pas le problème.

L'objet `+prometheus-kubevirt-rules+` ServiceMonitor est manquant dans l'espace de noms `+cattle-monitoring-system+`. Vous ne pouvez pas ajouter cet objet manuellement car l'opérateur KubeVirt le supprime automatiquement.
...
Lorsque KubeVirt est nouvellement installé ou mis à niveau, il génère un nouvel objet ConfigMap pour stocker la configuration. Une condition de concurrence se produit au sein de l'opérateur KubeVirt si l'objet `+rancher-monitoring-operator+` ServiceAccount est manquant/non synchronisé depuis l'espace de noms `+cattle-monitoring-system+` pendant ce processus. Par conséquent, la configuration du ServiceMonitor peut être exclue de l'objet ConfigMap résultant. 

Pendant le processus de mise à niveau, KubeVirt peut déterminer incorrectement l'état de surveillance. Une fois l'objet ConfigMap généré, KubeVirt ne le réconcilie ni ne le régénère jusqu'à la prochaine mise à niveau, sauf si un déclencheur manuel est effectué.

=== Solution provisoire

La solution de contournement consiste à s'assurer que l'objet `+rancher-monitoring-operator+` ServiceAccount existe, à supprimer les objets ConfigMap orphelins et à redémarrer l'opérateur KubeVirt.

. Récupérez la liste des objets ConfigMap.
+
[,console]
----
...
La liste inclut le ConfigMap de la dernière version et tous les objets hérités restants.

. Vérifiez si le dernier objet ConfigMap contient la configuration ServiceMonitor.
+
[,console]
----
...
----
+
Lorsque la sortie est vide, le problème existe dans votre environnement.

. Vérifiez que les champs `+monitorAccount+` et `+monitorNamespace+` existent.
+
[,console]
----
...
. Vérifiez que l'objet ServiceAccount existe.
+
Cet objet est créé lors de l'installation et ne doit pas être supprimé.
+
[,console]
----
...
. Si l'objet ServiceAccount n'existe pas, créez-le manuellement. Sinon, passez à l'étape suivante.
+
[,console]
----
$ cat > rmo.yaml << 'EOF'
...
. Supprimez tous les objets ConfigMap (`+kubevirt-install-strategy-*+`).

. Déployez le `+virt-operator+` déploiement.
+
KubeVirt recrée le ConfigMap.
+
[,console]
----
