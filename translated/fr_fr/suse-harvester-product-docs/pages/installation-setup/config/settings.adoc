...
* Docker Hub (docker.io) : C'est l'option par défaut lorsque vous ne configurez pas de registre privé dans {rancher-product-name}.
...
*Valeur par défaut* : `+https://releases.rancher.com/harvester+`
...
*Définition* : Version de {harvester-product-name} installée sur les nœuds.
...
*Définition* : Réseau de stockage séparé pour le trafic {longhorn-product-name}.
...
*Valeur par défaut* : image `+support-bundle-kit+` qui est intégrée dans l'ISO {harvester-product-name} et est spécifique à chaque version {harvester-product-name}.
...
*Définition* : Espaces de noms supplémentaires que vous pouvez utiliser lors de xref:troubleshooting/cluster.adoc#_generate_a_support_bundle[la génération d'un bundle de support].
...
*Définition* : Nombre de minutes que {harvester-product-name} permet pour l'achèvement du processus de génération du bundle de support.
...
*Définition* : Nombre de minutes que {harvester-product-name} attend avant de supprimer un bundle de support qui a été conditionné mais non téléchargé (soit délibérément, soit sans succès) ou conservé.
...
*Définition* : Nombre de minutes que {harvester-product-name} permet pour la collecte des journaux et des configurations sur les nœuds pour le bundle de support.
...
*Valeur par défaut* : `+https://harvester-upgrade-responder.rancher.io/v1/checkupgrade+`
...
*Valeur par défaut* : `+https://releases.rancher.com/harvester-ui/dashboard/latest/index.html+`
