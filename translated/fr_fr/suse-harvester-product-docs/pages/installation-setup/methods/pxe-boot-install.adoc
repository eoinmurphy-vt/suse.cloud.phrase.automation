...
Créez un xref:installation-setup/config/configuration-file.adoc[fichier de configuration] appelé `+config-create.yaml+` pour le mode `+CREATE+`. Modifiez les valeurs si nécessaire :
...
Pour effectuer un démarrage PXE sur un réseau VLAN tagué, les hôtes {harvester-product-name} doivent être configurés avec les paramètres BIOS/UEFI suivants :
