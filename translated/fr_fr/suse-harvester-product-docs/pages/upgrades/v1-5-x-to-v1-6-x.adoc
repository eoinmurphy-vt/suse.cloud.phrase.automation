...
Problème connexe : https://github.com/harvester/harvester/issues/9033[#9033]
...
. Vérifiez les VLAN attachés à `+mgmt-br+` et `+mgmt-bo+` en exécutant la commande suivante sur les hôtes de gestion :
+
[,shell]
----
bridge vlan show
...
. Ajoutez manuellement les VLAN secondaires requis au pont `+mgmt-br+` et à l'interface `+mgmt-bo+` en ajoutant les commandes suivantes au fichier `+/oem/90_custom.yaml+` :
+
...
. Vérifiez que tous les VLAN requis ont été ajoutés en exécutant la commande suivante sur les hôtes :
+
[,shell]
----
bridge vlan show
...
Dans l'exemple suivant, un cluster v1.5.x a été initialement installé avec xref:installation-setup/config/configuration-file.adoc#_install_management_interface[une interface VLAN principale] (ID VLAN : `+2021+`). Pour ajouter une interface VLAN secondaire (ID VLAN : `+2113+`), le fichier `+/oem/99_vlan-ifcfg.yaml+` a été créé sur les hôtes de gestion avec le contenu suivant :
...
L'interface utilisateur {harvester-product-name} peut afficher le message `+Restart action is required for the virtual machine configuration change to take effect+` pour certaines machines virtuelles en cours d'exécution dans les situations suivantes :
...
Problème connexe : https://github.com/harvester/harvester/issues/9739[#9739]
...
Pour résoudre ce problème, effectuez l'une des actions suivantes _avant de commencer la mise à niveau_ :
...
Problème connexe : https://github.com/harvester/harvester/issues/8816[#8816]