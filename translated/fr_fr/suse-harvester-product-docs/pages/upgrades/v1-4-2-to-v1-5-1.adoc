= Mettez à niveau de v1.4.2 ou v1.4.3 à v1.5.1
:revdate: 2025-07-07
:page-revdate: {revdate}

== informations générales

Un bouton *Mettre à niveau* apparaît sur l'écran *Tableau de bord* chaque fois qu'une nouvelle {harvester-product-name} version à laquelle vous pouvez mettre à niveau est disponible. Pour plus d'informations, consultez xref:./upgrades.adoc#_start_an_upgrade[Démarrer une mise à niveau].

Vous pouvez directement mettre à niveau de v1.4.2 à v1.5.1 car {harvester-product-name} permet un maximum d'une mise à niveau mineure pour les composants sous-jacents. {harvester-product-name} v1.4.2 et v1.4.3 utilisent la même version mineure de {rke2-product-name} (v1.31), tandis que {harvester-product-name} v1.5.0 et v1.5.1 utilisent la prochaine version mineure (v1.32).
...
=== Mettez à jour l'extension de l'interface utilisateur Harvester sur {rancher-product-name} v2.11.0

Vous devez utiliser **v1.5.1** de l'extension de l'interface utilisateur Harvester pour importer des clusters {harvester-product-name} v1.5.1 sur {rancher-short-name} v2.11.0.

. Sur l'interface utilisateur {rancher-short-name}, allez à *local -> Applications -> Dépôts*.
...
=== 1. Mise à niveau isolée physiquement bloquée avec l'erreur `+ImagePullBackOff+` dans les pods Fluentd et Fluent Bit

La mise à niveau peut être bloquée au tout début du processus, comme l'indique un progrès de 0 % et des éléments marqués *En attente* dans la boîte de dialogue *Mise à niveau* de l'interface utilisateur {harvester-product-name}.
...
Cela se produit parce que les images de conteneur suivantes ne sont ni préchargées dans les nœuds du cluster ni téléchargées depuis Internet :
...
* Mettez à jour le CR de journalisation pour utiliser les images déjà préchargées dans les nœuds du cluster. Pour ce faire, exécutez les commandes suivantes sur le cluster :
+
[,bash]
----
...
----
+
Le statut des pods Fluentd et Fluent Bit devrait changer en `+Running+` dans un instant et le processus de mise à niveau devrait se poursuivre après la mise à jour du CR Logging. Si le statut du pod Fluentd est toujours `+ImagePullBackOff+`, vous pouvez supprimer le pod pour forcer son redémarrage.
+
[,bash]
----
...
* Sur un ordinateur avec accès à Internet, récupérez les images de conteneur requises puis exportez-les dans un fichier TAR. Ensuite, transférez le fichier TAR vers les nœuds du cluster puis importez les images en exécutant les commandes suivantes sur chaque nœud :
+
[,bash]
----
...
Problème connexe : https://github.com/harvester/harvester/issues/7955[#7955]

=== 2. Les machines virtuelles utilisant des volumes RWX migrables redémarrent de manière inattendue
...
Problèmes connexes : https://github.com/harvester/harvester/issues/8534[#8534] et https://github.com/longhorn/longhorn/issues/11158[#11158]