...
Vous pouvez également vérifier les journaux des tâches de pré-drain. Veuillez vous référer à xref:./troubleshooting.adoc#_phase_4_upgrade_nodes[Phase 4 : Mettre à niveau les nœuds] dans le guide de dépannage.
...
* Problème lié :
 ** https://github.com/harvester/harvester/issues/2894[[BUG\] Mise à niveau bloquée lors de la mise à niveau du premier nœud : le travail a été actif plus longtemps que le délai spécifié]
* Solution de contournement :
...
. Vérifiez les noms des pods `+instance-manager-e+` ou `+instance-manager-r+` sur le nœud bloqué :
+
----
...
. Vérifiez les journaux de Rancher et assurez-vous que le pod `+instance-manager-e+` ou `+instance-manager-r+` ne peut pas être vidé :
+
----
...
. Supprimez de force le pod bloqué avec le statut `+Terminating+` avec la commande suivante :
+
----
...
--
. Trouvez le nom du job `+apply-manifest+` avec la commande :
+
----
...
. Trouvez le nom d'hôte de la machine avec la commande suivante, et suivez le https://github.com/harvester/harvester/issues/3863#issuecomment-1539681311[solution de contournement] pour voir si les certificats de service expirent sur un nœud :
+
----
...
. Le pod `+hvst-upgrade-vq4hl-apply-manifests-65vv8+` a le journal de boucle suivant :
+
----
...
. Vérifiez l'état de tous les bundles. Notez qu'un couple de bundles est `+OutOfSync+` :
+
----
...
. Le pod `+fleet-agent-*+` a le journal d'erreurs suivant :
+
----
 fleet-agent pod log:
...
. Vérifiez les paramètres `+ssl-certificates+` dans Harvester :
+
...
. Vérifiez le paramètre `+server-url+`, c'est la valeur de VIP :
+
----
