...
{harvester-product-name} fournit deux façons d'injecter des clés SSH publiques dans les machines virtuelles. En général, ces méthodes se classent en deux catégories. <<Static SSH Key Injection via cloud-init,Injection de clé statique>>, qui place les clés dans le script cloud-init lorsque la machine virtuelle est d'abord mise sous tension ; <<Dynamic SSH Key Injection via Qemu guest agent,injection dynamique>>, qui permet de mettre à jour les clés ou l'authentification de base dynamiquement à l'exécution.
...
Vous pouvez injecter vos identifiants d'accès via l'interface utilisateur {harvester-product-name} en effectuant les étapes suivantes :
