= Kube-OVN Operator (Expérimental)
:revdate: 2025-09-15
:page-revdate: {revdate}

[NOTE]
====
**kubeovn-operator** est un *produit complémentaire expérimental*. Pour plus d'informations sur les fonctionnalités expérimentales, consultez xref:introduction/document-conventions.adoc#_feature_labels[Étiquettes de Fonctionnalité].
====

https://github.com/harvester/kubeovn-operator[`+kubeovn-operator+`] est utilisé pour gérer le cycle de vie de https://github.com/kubeovn/kube-ovn[Kube-OVN] en tant que CNI secondaire sur les clusters sous-jacents {harvester-product-name}.

== Activation de `+kubeovn-operator+`

Vous devez activer `+kubeovn-operator+` pour déployer Kube-OVN sur un cluster {harvester-product-name} pour des capacités SDN avancées telles que le cloud privé virtuel (VPC) et les sous-réseaux pour les charges de travail des machines virtuelles.

. Dans l'interface utilisateur {harvester-product-name}, allez à *Avancés -> produits complémentaires*.

. Sélectionnez *kubeovn-operator (Expérimental)*, puis sélectionnez *⋮ -> Activer*.
+
image::addons/kubeovn-operator.png[Kube-OVN Operator]

Le produit complémentaire déploie `+kubeovn-operator+` et crée l'objet par défaut `+Configuration+` nommé `+configuration.kubeovn.io+`, qui utilise des valeurs par défaut spécifiques à {harvester-product-name} pour configurer le CNI Kube-OVN.

Voici un exemple d'un objet `+Configuration+` :
...
[CAUTION]
====
Cet objet `+Configuration+` a été validé contre des installations {harvester-product-name}. Vous pouvez affiner l'objet en modifiant le YAML du produit complémentaire `+kubeovn-operator+`, mais faites preuve de prudence lors des modifications du champ `+configurationSpec+`.

Assurez-vous que les blocs CIDR de pod et de service IPv4 de Kube-OVN ne se chevauchent pas avec les blocs CIDR de pod et de service de Harvester.
====

== Désactivation de `+kubeovn-operator+`

[IMPORTANT]
====
Assurez-vous qu'aucune machine virtuelle n'utilise des réseaux VM basés sur les composants SDN de Kube-OVN.

La désactivation du produit complémentaire kubeovn-operator est un processus perturbateur.
====

Vous pouvez désactiver `+kubeovn-operator+` en utilisant les commandes suivantes :
...
Vous devez redémarrer chaque nœud pour compléter le processus de désinstallation. Une fois les nœuds redémarrés, vous pouvez désactiver le produit complémentaire `+kubeovn-operator+` depuis l'interface utilisateur de Harvester.