...
Le champ `+defaultDiskBusType+` vous permet de spécifier le type de bus pour les disques importés. {harvester-product-name} utilise ce champ de la manière suivante :

* Sources VMware : La valeur est utilisée uniquement si {harvester-product-name} est incapable de détecter automatiquement le type de bus.
* Sources OpenStack : La valeur est utilisée pour tous les disques importés.
* Sources Open Virtual Appliance (OVA) : La valeur est utilisée uniquement si {harvester-product-name} est incapable de détecter automatiquement le type de bus.
...
Lorsque vous tentez d'importer une machine virtuelle basée sur VMware dans {harvester-product-name} v1.6.0, les problèmes suivants se produisent si https://knowledge.broadcom.com/external/article/315382/overview-of-vmware-tools.html[VMware Tools] n'est pas installé sur la machine virtuelle :
