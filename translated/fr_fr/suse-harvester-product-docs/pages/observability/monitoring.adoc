...
De plus, Grafana est fourni par `+rancher-monitoring+`, donc le mot de passe administrateur par défaut est : prom-operator

Référence : https://github.com/rancher/charts/tree/dev-v2.7/charts/rancher-project-monitoring[values.yaml]
====
...
* Métriques en temps réel : Ces métriques sont générées par Prometheus et sont conservées pendant _cinq jours_.
+
|===
...
* Événements de migration : Ces enregistrements d'événements spécifiques à la machine virtuelle sont générés par le serveur API Kubernetes (kube-apiserver) et sont conservés pendant _une heure_.
