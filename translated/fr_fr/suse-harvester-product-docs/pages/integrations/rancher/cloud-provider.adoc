...
. Générez des données de configuration cloud en utilisant le script `+generate_addon.sh+`, puis placez les données sur chaque nœud personnalisé (répertoire : `+/etc/kubernetes/cloud-config+`).
+
[,bash]
----
...
. Désactivez le fournisseur de cloud `+in-tree+` de plusieurs manières :
