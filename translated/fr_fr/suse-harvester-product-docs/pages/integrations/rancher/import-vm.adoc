= Importer un cluster construit sur une VM SUSE® Virtualization
:revdate: 2025-05-09
:page-revdate: {revdate}

Rancher vous permet d'importer des machines virtuelles SUSE® Virtualization dans lesquelles vous avez installé Kubernetes.

== Déploiement

=== Conditions préalables

* Le cluster Kubernetes est construit sur des VMs SUSE® Virtualization.

=== Déployer des clusters invités sur des machines virtuelles SUSE® Virtualization

. Générez le fichier de configuration cloud du pilote csi à l'aide du script https://raw.githubusercontent.com/harvester/harvester-csi-driver/master/deploy/generate_addon_csi.sh[generate_addon_csi.sh], qui est disponible dans le dépôt https://github.com/harvester/harvester-csi-driver[harvester/harvester-csi-driver].
+
Exemple :
+
[,shell]
----
 ./generate_addon_csi.sh <serviceaccount name> <namespace> RKE2
----
+
La sortie générée sera similaire à la suivante :
+
[,yaml]
----
 ########## cloud-config ############
 apiVersion: v1
 clusters:
 - cluster: <token>
     server: https://<YOUR HOST HARVESTER VIP>:6443
   name: default
 contexts:
 - context:
     cluster: default
     namespace: default
     user: rke2-guest-01-default-default
   name: rke2-guest-01-default-default
 current-context: rke2-guest-01-default-default
 kind: Config
 preferences: {}
 users:
 - name: rke2-guest-01-default-default
   user:
     token: <token>

 ########## cloud-init user data ############
 write_files:
   - encoding: b64
     content: 
     owner: root:root
     path: /var/lib/rancher/rke2/etc/config-files/cloud-provider-config
     permissions: '0644'
----

. Générez la configuration cloud du fournisseur cloud à l'aide du script https://raw.githubusercontent.com/harvester/cloud-provider-harvester/master/deploy/generate_addon.sh[generate_addon.sh], qui est disponible dans le dépôt https://github.com/harvester/cloud-provider-harvester[harvester/cloud-provider-harvester].
+
Exemple :
+
[,shell]
----
 ./generate_addon.sh <serviceaccount name> <namespace>
----
+
La sortie générée sera similaire à la suivante :
 La sortie ressemblera à ce qui suit :
+
[,yaml]
----
 ########## cloud config ############
 apiVersion: v1
 clusters:
 - cluster:
     certificate-authority-data: <CACERT>
     server: https://HARVESTER-ENDPOINT/k8s/clusters/local
   name: local
 contexts:
 - context:
     cluster: local
     namespace: default
     user: harvester-cloud-provider-default-local
   name: harvester-cloud-provider-default-local
 current-context: harvester-cloud-provider-default-local
 kind: Config
 preferences: {}
 users:
 - name: harvester-cloud-provider-default-local
   user:
     token: <TOKEN>

 ########## cloud-init user data ############
 write_files:
 - encoding: b64
   content: <CONTENT>
   owner: root:root
   path: /etc/kubernetes/cloud-config
   permissions: '0644'
----

. Créer une VM avec deux fichiers de configuration cloud.
+
image::rancher/vm-with-cloud-config.png[VM avec configuration cloud]

. Installer RKE2 dans la VM.
+
[,shell]
----
 sudo mkdir -p /etc/rancher/rke2
 echo "cni: calico
 disable-kube-proxy: false
 etcd-expose-metrics: false" | sudo tee /etc/rancher/rke2/config.yaml
 curl -sfL https://get.rke2.io | sudo sh -
 sudo systemctl enable rke2-server.service
 sudo systemctl start rke2-server.service
----

. Vérifiez que RKE2 fonctionne dans la VM.
+
[,shell]
----
 sudo /var/lib/rancher/rke2/bin/kubectl --kubeconfig /etc/rancher/rke2/rke2.yaml get nodes
----

. Importer la VM dans Rancher.
+
Dans l'interface utilisateur de Rancher, allez à *Gestion des clusters* > *Clusters* > *Importer existant* > *Générique* > *Créer*.
+
image::rancher/import-existing-cluster.png[Importer un cluster existant]
+
[,shell]
----
 # Run the command in the VM
 curl --insecure -sfL https://192.168.0.181:6443/v3/import/g5p2g2gtxw4564nktdl4nr5cwwvtwqp9zxd6dmhm5nc7vpnxmr9cfk_c-m-mzf28skd.yaml | sudo /var/lib/rancher/rke2/bin/kubectl --kubeconfig /etc/rancher/rke2/rke2.yaml apply -f -
----

. Installer le fournisseur cloud Harvester.
+
Sur le tableau de bord du cluster RKE2, allez à *Apps* > *Charts* > *Harvester Cloud Provider* > *Installer*.

. Installer le pilote CSI Harvester.
+
Sur le tableau de bord du cluster RKE2, allez à *Apps* > *Charts* > *Harvester CSI Driver* > *Installer*.
