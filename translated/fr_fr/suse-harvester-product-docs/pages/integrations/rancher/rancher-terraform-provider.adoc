...
Le https://registry.terraform.io/providers/rancher/rancher2/[Rancher Terraform Provider] permet aux administrateurs de créer et de gérer des clusters invités RKE2 en utilisant Terraform.
...
+
image:rancher/create-api-key.png[Créer une clé API]
+
image:rancher/access-and-secret-keys.png[« Clés d'accès & clés secrètes »]

. Obtenez l'ID du cluster Harvester.
+
...
+
image::rancher/harvester-cluster-id.png[ID du cluster Harvester]

. Obtenez le kubeconfig pour le Harvester Cloud Provider et le Harvester CSI Driver.
+
--
[tabs]
======
...
. Préparez un fichier `+provider.tf+` avec le contenu suivant:
+
[,hcl]
----
 terraform {
...
. Préparez un fichier `+main.tf+` avec le contenu suivant:
+
[,hcl]
----
