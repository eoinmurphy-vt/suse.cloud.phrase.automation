...
|===
| Version du pilote CSI Harvester | {harvester-product-name} Version | Affecté ?
...
Assurez-vous que le `+serviceaccount name+` et le `+namespace+` correspondent aux paramètres de votre fournisseur de cloud. Effectuez les étapes suivantes pour récupérer ces détails.
 ...
 .. Trouvez le `+rolebinding+` associé à votre fournisseur de cloud :
+
----
...
----
 ...
 .. Extrayez les informations `+subjects+` de ce `+rolebinding+` :
+
----
 $ kubectl get rolebinding default-rke2-guest-01 -n default -o yaml |yq -e '.subjects'
----

 .. Identifiez les informations `+ServiceAccount+` :
+
----
 - kind: ServiceAccount
   name: rke2-guest-01
...
+
** Debian et Ubuntu : `+apt-get install -y nfs-common+`
** CentOS et RHEL : `+yum install -y nfs-utils+`
** SUSE et OpenSUSE : `+zypper install -y nfs-client+`
...
* RKE2 (recommandé) : Le contrôleur d'instantané CSI est déployé par défaut.
* Autres distributions Kubernetes : Vous devez installer le contrôleur d'instantané CSI sur le cluster invité avant de mettre à niveau le pilote CSI Harvester vers *v0.1.25*. Pour les instructions d'installation, consultez la documentation de votre distribution (exemple : https://documentation.suse.com/cloudnative/storage/latest/en/snapshots-backups/csi-snapshots/enable-csi-snapshot-creation.html({longhorn-product-name}).
====
