...
+
Chaque réseau superposé est considéré comme un fournisseur dans Kube-OVN. Lorsque vous créez un sous-réseau sur l'interface utilisateur {harvester-product-name}, vous pouvez sélectionner ces réseaux superposés dans la liste *Fournisseur* (type : `+OverlayNetwork+`) sur l'écran *Sous-réseau : Créer*.
...
* Séparation claire des préoccupations : {harvester-product-name} gère la virtualisation ; Kube-OVN gère le SDN
* Évolutivité : Les nouveaux VPC, sous-réseaux et peering ne nécessitent pas de modifications dans le noyau de {harvester-product-name}
...
. xref:networking/vm-network.adoc#_creating_an_overlay_network[Créer des réseaux superposés] avec les paramètres suivants :
+
...
. Créez deux sous-réseaux dans `+vpc-1+` avec les paramètres suivants :
+
|===
...
. Ouvrez les consoles série de `+vm1-vswitch1+` et `+vm1-vswitch2+`, puis ajoutez une route par défaut sur chacune (s'il n'en existe pas) en utilisant les commandes suivantes :
+
