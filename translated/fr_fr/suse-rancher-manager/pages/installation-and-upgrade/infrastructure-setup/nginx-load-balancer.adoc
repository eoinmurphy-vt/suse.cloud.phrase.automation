...
[NOTE]
====
Voir https://docs.nginx.com/nginx/admin-guide/load-balancer/tcp-udp-load-balancer/[Documentation NGINX : Équilibrage de la charge TCP et UDP] pour toutes les options de configuration.
====
+
...
. Enregistrez `+nginx.conf+` sur votre équilibreur de charge à l'emplacement suivant : `+/etc/nginx/nginx.conf+`.
. Chargez les mises à jour de votre configuration NGINX en exécutant la commande suivante :
+
----
# nginx -s reload
----
