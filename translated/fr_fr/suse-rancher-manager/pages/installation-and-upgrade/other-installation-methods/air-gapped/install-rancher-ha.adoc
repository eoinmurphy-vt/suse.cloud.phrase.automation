...
** Dernière : Recommandé pour essayer les fonctionnalités les plus récentes.
+
----
...
.. Récupérez une version spécifique en spécifiant le paramètre `+--version+` :
+
[,shell,subs="attributes+"]
----
...
Téléchargez le fichier CRD requis pour cert-manager :
...
*Facultatif* : Pour installer une version spécifique de Rancher, définissez la valeur `+image.tag+`, exemple : `+--set image.tag=v{current-patch-version}+`
...
Ces ressources pourraient être utiles lors de l'installation de Rancher :
