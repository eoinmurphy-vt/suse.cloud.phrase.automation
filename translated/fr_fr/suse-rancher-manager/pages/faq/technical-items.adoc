...
== Comment puis-je activer la journalisation de débogage ?

Voir {xref-logging-troubleshooting}[Dépannage : Journalisation]
...
* Installation de Docker : dans l'etcd intégré du conteneur `+rancher/rancher+`, situé à `+/var/lib/rancher+`.
* Installation de Kubernetes : l'emplacement par défaut se trouve dans les répertoires `+/var/lib/rancher/rke2+` ou `+/var/lib/rancher/k3s+` du cluster RKE2/K3s respectif créé pour exécuter Rancher.
...
== Puis-je utiliser des raccourcis clavier dans l'interface utilisateur ?
