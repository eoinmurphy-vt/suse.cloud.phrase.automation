...
. *Région* (optionnel) : La https://aws.amazon.com/about-aws/global-infrastructure/regions_az/[région] AWS où se trouve le bucket S3. Ce champ n'est pas nécessaire pour configurer MinIO.
...
. *Point de terminaison* : Le https://docs.aws.amazon.com/general/latest/gr/s3.html[point de terminaison] qui est utilisé pour accéder à S3 dans la région de votre bucket.
. *Point de terminaison CA* (optionnel) : Ceci doit être le certificat CA encodé en Base64. Pour un exemple, référez-vous à la <<_example_s3_storage_configuration,configuration d'exemple compatible S3>>.
