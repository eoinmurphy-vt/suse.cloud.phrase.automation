...
Si cela s'applique, fournissez le nom de l'objet `+Secret+` contenant le fichier de configuration de chiffrement. Si vous n'avez que le fichier de configuration de chiffrement, mais que vous n'avez pas le secret créé dans ce cluster, utilisez les étapes suivantes pour créer le secret :
 ...
 .. La commande ci-dessous utilise un fichier nommé `+encryption-provider-config.yaml+`, avec l'option `+--from-file+`. Exécutez ci-dessous une fois que le `+EncryptionConfiguration+` est enregistré dans un fichier appelé `+encryption-provider-config.yaml+` :
+
[,bash]
----
...
.. Appliquez la ressource objet `+Restore+` :
+
[,bash]
----
