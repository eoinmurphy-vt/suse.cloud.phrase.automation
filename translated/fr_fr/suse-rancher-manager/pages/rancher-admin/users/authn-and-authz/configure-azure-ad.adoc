...
====
 ...
 .. Dans la section https://docs.microsoft.com/en-us/azure/active-directory/develop/reply-url[*URI de redirection*], assurez-vous que *Web* est sélectionné dans le menu déroulant et entrez l'URL de votre serveur Rancher dans la zone de texte à côté du menu déroulant. Cette URL de serveur Rancher doit être complétée par le chemin de vérification : `+<MY_RANCHER_URL>/verify-auth-azure+`.
+
...
. Dans l'API Microsoft Graph, sélectionnez les *Autorisations d'application* suivantes : `+Directory.Read.All+`
+
...
Plus précisément, Rancher a besoin d'autorisations qui permettent les actions suivantes :
...
Voici quelques exemples de combinaisons d'autorisations qui satisfont les besoins de Rancher :
...
* Si vous ne souhaitez pas mettre à niveau vers v2.7.0+ après la retraite de l'API Graph d'Azure AD, vous devrez soit :
