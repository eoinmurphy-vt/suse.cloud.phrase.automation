...
URL des métadonnées : `+\https://<rancher-server>/v1-saml/shibboleth/saml/metadata+` URL du service consommateur d'assertion (ACS) : `+\https://<rancher-server>/v1-saml/shibboleth/saml/acs+`
...
----
 ...
 .. *Métadonnées IDP* : Le fichier `+metadata.xml+` que vous avez exporté depuis votre serveur IdP.
