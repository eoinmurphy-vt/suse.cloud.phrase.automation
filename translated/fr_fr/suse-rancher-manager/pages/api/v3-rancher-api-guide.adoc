...
L'API v3 précédente a sa propre interface utilisateur accessible depuis un <<_enable_view_in_api,navigateur web>>. C'est un moyen facile de voir les ressources, d'effectuer des actions et de voir l'équivalent `+curl+` ou la requête et la réponse HTTP. Pour y accéder :
