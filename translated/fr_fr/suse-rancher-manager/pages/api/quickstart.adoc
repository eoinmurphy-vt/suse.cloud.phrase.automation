...
. Créez un fichier `+kubeconfig.yaml+`. Remplacez `+$SERVER_URL+` par l'URL du serveur et `+$API_KEY+` par votre clé API Rancher :
+
[,yaml]
----
 apiVersion: v1
...
. Définissez votre variable d'environnement KUBECONFIG sur le fichier kubeconfig que vous venez de créer :
...
. Utilisez `+kubectl explain+` pour afficher les champs disponibles pour les projets, ou les sous-champs complexes des ressources :
