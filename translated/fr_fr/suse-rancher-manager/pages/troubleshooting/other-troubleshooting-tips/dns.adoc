...
. Attendez que `+kubectl rollout status ds/dnstest -w+` renvoie : `+daemon set "dnstest" successfully rolled out+`.
...
. Lorsque cette commande a terminé son exécution, la sortie indiquant que tout est correct est :
+
----
