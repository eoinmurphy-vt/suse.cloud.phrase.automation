...
* Certains composants internes de Kubernetes sont récupérés via un proxy déployé dans le cadre de Monitoring V2 appelé *PushProx*. Les composants Kubernetes qui exposent des métriques à Prometheus via PushProx sont les suivants :
...
Les métriques sont collectées différemment en fonction de la distribution Kubernetes. Pour de l'aide avec la terminologie, référez-vous <<_terminology,ici>>. Pour plus de détails, consultez le tableau ci-dessous :
