...
. Si vous importez un cluster Kubernetes générique dans Rancher, effectuez les étapes suivantes pour la configuration :
...
L'interface utilisateur de Rancher exposera les fonctionnalités disponibles pour <<_features_for_all_registered_clusters,tous les clusters enregistrés>>, ainsi que les options suivantes pour modifier et mettre à niveau le cluster :
...
Les nœuds sont mis à niveau par l'Upgrade Controller fonctionnant dans le cluster en aval. En fonction de la configuration du cluster, Rancher déploie deux https://github.com/rancher/system-upgrade-controller#example-upgrade-plan[plans] pour mettre à niveau les nœuds : un pour les nœuds de plan de contrôle et un pour les nœuds de travail. L'Upgrade Controller suit les plans et met à niveau les nœuds.
...
. Créez un fichier à `+/var/lib/rancher/{rke2,k3s}/kube-api-authn-webhook.yaml+` avec le contenu suivant :
+
[,yaml]
----
 apiVersion: v1
...
. Ajoutez ce qui suit au fichier de configuration (ou créez-en un s'il n'existe pas) ; notez que l'emplacement par défaut est `+/etc/rancher/{rke2,k3s}/config.yaml+` :
+
[,yaml]
----
 kube-apiserver-arg:
...
Pour résoudre ce problème, activez les comptes locaux avant d'essayer d'xref:#_registering_a_cluster[importer le cluster] à nouveau :
