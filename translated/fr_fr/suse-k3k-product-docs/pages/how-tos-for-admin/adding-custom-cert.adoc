= Ajout de certificats personnalisés à k3s depuis le cluster k3k
:revdate: 2025-09-16
:page-revdate: {revdate}

== Générer et ajouter les certificats personnalisés

. Utilisez le https://github.com/k3s-io/k3s/blob/master/contrib/util/generate-custom-ca-certs.sh[script de certificat personnalisé] pour générer des certificats.

. Après avoir généré les certificats, créez un répertoire local car le répertoire par défaut ne sera pas accessible. Par exemple : `+k3k/custom-certs+`.

. Copiez les certificats générés dans le répertoire - `+$ sudo cp -r /var/lib/rancher/k3s/server/tls k3k/custom-certs/+`

. Utilisez le chemin `+/var/lib/rancher/k3s/server/tls k3k/custom-certs/+` pour créer les clusters virtuels en 2 modes :
+
* Mode virtuel 
* Mode partagé
...
. Générez des certificats personnalisés via un script :
+
----
...
. Vérifiez les certificats sur le pod du serveur :
+
----
...
. Copiez et vérifiez les certificats dans le répertoire local :
+
----
...
. Vérifiez le cluster k3k nouvellement créé :
+
----
