...
____
Nouveau sur Kubernetes ? La documentation officielle de Kubernetes propose déjà d'excellents tutoriels décrivant les bases https://kubernetes.io/docs/tutorials/kubernetes-basics/[ici].
____
...
{product-name} fournit un script d'installation qui est un moyen pratique de l'installer en tant que service sur des systèmes basés sur systemd. Ce script est disponible sur https://get.rke2.io.. Pour installer {product-name} en utilisant cette méthode, procédez comme suit :
...
.  Pour tirer des images du registre Rancher Prime, définissez la valeur suivante dans `+etc/rancher/rke2/config.yaml+` :
+
[,yaml]
----
...
. Suivez les journaux avec la commande suivante :
+
[,sh]
----
