...
====
Il est important de faire correspondre les drapeaux critiques sur vos nœuds serveur. Par exemple, si vous utilisez le drapeau `+cluster-cidr: 10.200.0.0/16+` sur votre premier nœud de serveur, mais que vous ne le définissez pas sur d'autres nœuds de serveur, les nœuds échoueront à rejoindre. Ils afficheront des erreurs telles que : `+failed to validate server configuration: critical configuration value mismatch.+` Voir xref:reference/server_config.adoc#_critical_configuration_values[Configuration du serveur] pour plus d'informations sur les drapeaux qui doivent être définis de manière identique sur les nœuds de serveur.
====
