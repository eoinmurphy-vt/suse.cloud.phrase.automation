...
. Déployez RKE2 avec les options suivantes :
+
[,yaml]
----
...
. Créez le fichier `+azure.json+` et enregistrez-le dans `+/etc/kubernetes/azure.json+`. Assurez-vous qu'il contient les deux options suivantes :
+
[,json]
----
...
.. Ensuite, exécutez les `+helm+` commandes suivantes :
+
[,bash]
----
