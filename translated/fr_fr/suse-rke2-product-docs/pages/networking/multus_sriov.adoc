...
+
--
Multus fournit un daemonset optionnel pour déployer le démon DHCP nécessaire au fonctionnement du https://www.cni.dev/plugins/current/ipam/dhcp/[plugin IPAM DHCP]. Vous pouvez le faire en utilisant le xref:add-ons/helm.adoc#_customizing_packaged_components_with_helmchartconfig[HelmChartConfig] suivant :
...
Un cas d'utilisation du "plugin épais" de Multus est de déployer le https://github.com/k8snetworkplumbingwg/multus-dynamic-networks-controller[Contrôleur de réseaux dynamiques]. Cela se fait via le HelmChartConfig suivant :
...
Le plugin CNI SR-IOV ne peut pas être utilisé comme plugin CNI par défaut pour Multus ; il doit être déployé aux côtés de Multus et d'un plugin CNI traditionnel. Le chart CNI SR-IOV peut être trouvé dans le dépôt `+rancher-charts+` Helm. Pour plus d'informations, consultez https://documentation.suse.com/cloudnative/rancher-manager/latest/en/cluster-admin/helm-charts-in-rancher/helm-charts-in-rancher.html[la documentation des charts Helm Rancher].
