...
Il est également possible de renouveler un service individuel en passant l’option `+--service+`, par exemple : `+rke2 certificate rotate --service api-server+`. Consultez xref:security/certificates.adoc#_rotating_client_and_server_certificates_manually[Gestion des certificats] pour plus de détails.
...
. Assurez-vous que RKE2 sait où envoyer les requêtes API pour les services `+ec2+` et `+elasticloadbalancing+` en créant un fichier `+cloud.conf+`, ci-dessous un exemple pour la région `+us-iso-east-1+` (C2S) :
+
[,yaml]
----
...
. Configurez RKE2 pour utiliser le fournisseur de cloud `+aws+` avec le `+cloud.conf+` personnalisé créé à l'étape 1 :
+
[,yaml]
----
# /etc/rancher/rke2/config.yaml
...
Les valeurs d’unité pour l’UC/mémoire sont identiques aux unités de ressources Kubernetes (Voir : https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#resource-units-in-kubernetes[Limites de ressources dans Kubernetes]).
