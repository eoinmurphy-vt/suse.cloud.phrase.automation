...
.Correction:
======
Créez des comptes de service explicites chaque fois qu'une charge de travail Kubernetes nécessite un accès spécifique au serveur API Kubernetes. Modifiez la configuration de chaque compte de service par défaut pour inclure cette valeur automountServiceAccountToken: false. Ou en utilisant kubectl :
----
...
.Correction:
======
Modifiez la définition des ServiceAccounts et des Pods qui n'ont pas besoin de monter des jetons de compte de service pour le désactiver, avec `+automountServiceAccountToken: false+`. RKE2 accorde des exceptions aux ServiceAccounts dans l'espace de noms kube-system, car ceux-ci sont censés contacter l'apiserver. Lorsqu'il est en mode cis, RKE2 désactive l'automountServiceAccountToken pour les espaces de noms : - kube-public, default, kube-node-lease. Si à la fois le ServiceAccount et le .spec du Pod définissent une valeur pour automountServiceAccountToken, celle du Pod prévaut. Condition : Le Pod est considéré comme conforme (vrai) lorsque : - le ServiceAccount est configuré avec automountServiceAccountToken défini sur false et le Pod avec automountServiceAccountToken défini sur false ou non défini ; - ou lorsque le ServiceAccount est configuré avec automountServiceAccountToken défini sur true ou non défini et que le Pod est configuré avec automountServiceAccountToken défini sur false.
...
**Correction:** Utilisez `+securityContext+` pour activer le profil seccomp docker/default dans vos définitions de Pod. Un exemple est le suivant : securityContext: seccompProfile: type: RuntimeDefault
