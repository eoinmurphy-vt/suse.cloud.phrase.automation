...
* *Non applicable* - Le contrôle n'est pas applicable (type : skip) à RKE2 en raison de la manière dont il est conçu pour fonctionner. La section de justification expliquera pourquoi cela est le cas.
...
Si cette vérification échoue, supprimez toute configuration personnalisée autour de `+tls-cipher-suites+` ou mettez à jour le fichier /etc/rancher/rke2/config.yaml pour correspondre à la configuration par défaut en ajoutant ce qui suit : kube-apiserver-arg :
