...
* *RÉUSSITE* - Le contrôle est automatisé (score : vrai). Le cluster RKE2 testé a réussi l'audit décrit dans le benchmark.
* *Non applicable* - Le contrôle n'est pas applicable (type : ignorer) à RKE2 en raison de la manière dont il est conçu pour fonctionner. La section de justification expliquera pourquoi cela est le cas.
* *AVERTISSEMENT* - Le contrôle est manuel (score : faux) dans le benchmark CIS et dépend de l'intervention manuelle de l'opérateur. La section de remédiation fournira des conseils sur la manière d'obtenir un résultat de RÉUSSITE.
...
Si ce contrôle échoue, supprimez toute configuration personnalisée autour de `+tls-cipher-suites+` ou mettez à jour le fichier /etc/rancher/rke2/config.yaml pour correspondre à la configuration par défaut en ajoutant ce qui suit : kube-apiserver-arg:
...
Modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et définissez le paramètre --service-account-extend-token-expiration sur false avant de recharger rke2, comme ci-dessous :
 kube-apiserver-arg:
...
Si vous utilisez un fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, modifiez le fichier pour définir `+seccomp-default+` sur kubelet-arg :
...
RKE2 donne des exceptions aux pods suivants, qui sont nécessaires pour les opérations régulières :

* kube-flannel, calico-node, kube-proxy : l'audit liste tous les conteneurs des pods pour récupérer leur valeur .securityContext.privileged.
...
RKE2 donne des exceptions aux pods suivants dans l'espace de noms kube-system, qui sont nécessaires pour les opérations régulières :

* cloud-controller-manager, etcd, rke2-canal, rke2-coredns, kube-apiserver,
* kube-scheduler, kube-proxy, kube-controller-manager : l'audit récupère le spec.hostNetwork de chaque Pod.
...
* rke2-ingress-ngninx-controller, coredns : l'audit récupère les capacités ajoutées des conteneurs de chaque Pod.
