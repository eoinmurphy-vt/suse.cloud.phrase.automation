...
Ce guide est spécifique à la ligne de version *v1.28-v1.31* de K3s et à la version *v1.10* du CIS Kubernetes Benchmark.
...
Chaque contrôle du CIS Kubernetes Benchmark a été évalué sur un cluster K3s configuré selon le guide de renforcement de la sécurité associé.
...
=== 1.1.10 Assurez-vous que la propriété du fichier de l'interface réseau du conteneur est définie sur root:root (Automatisé)
...
Par défaut, K3s ne configure pas le --enable-admission-plugins sur AlwaysAdmit.
Si cette vérification échoue, modifiez le fichier de configuration K3s /etc/rancher/k3s/config.yaml, supprimez toutes les lignes comme ci-dessous.
...
=== 1.2.12 Assurez-vous que le plugin de contrôle d'admission ServiceAccount est configuré (Automatisé)
...
Par défaut, K3s ne configure pas le --disable-admission-plugins sur quoi que ce soit.
...
=== 1.2.13 Assurez-vous que le plugin de contrôle d'admission NamespaceLifecycle est configuré (Automatisé)
...
=== 1.2.14 Assurez-vous que le plugin de contrôle d'admission NodeRestriction est configuré (Automatisé)
...
Par défaut, K3s configure le --enable-admission-plugins sur NodeRestriction.
...
*Résultat attendua:* '--audit-log-maxsize' est supérieur ou égal à 100
...
*Résultat attendua:* '--service-account-key-file' est présent
...
Il est généré et situé à /var/lib/rancher/k3s/server/tls/client-ca.crt.
Si pour une raison quelconque vous devez fournir votre propre certificat CA, envisagez d'utiliser l'outil de ligne de commande de certificat k3s.
Si cette vérification échoue, modifiez le fichier de configuration K3s /etc/rancher/k3s/config.yaml et supprimer toutes les lignes comme ci-dessous.
...
Par défaut, le kube-apiserver K3s respecte ce test. Les modifications apportées à ces valeurs peuvent entraîner des régressions, assurez-vous donc que tous les clients apiserver prennent en charge la nouvelle configuration TLS avant de l'appliquer dans des déploiements en production.
...
Lors de l'utilisation d'embedded-etcd, K3s génère des fichiers de certificat et de clé pour etcd.
...
Si vous utilisez sqlite ou une base de données externe, les vérifications etcd ne sont pas applicables.
Lors de l'utilisation d'embedded-etcd, K3s définit le paramètre --client-cert-auth sur true.
...
*Résultat attendu:* 'auto-tls' est présent OU 'auto-tls' n'est pas présent
...
Si vous utilisez sqlite ou une base de données externe, les vérifications etcd ne sont pas applicables.
Lors de l'utilisation d'embedded-etcd, K3s ne définit pas le paramètre --auto-tls.
...
Si vous utilisez sqlite ou une base de données externe, les vérifications etcd ne sont pas applicables.
Lors de l'utilisation d'embedded-etcd, K3s génère des fichiers de certificat et de clé pour le peer d'etcd.
Ils se trouvent dans /var/lib/rancher/k3s/server/tls/etcd/.
Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/k3s/server/db/etcd/config n'a pas été modifié pour utiliser des fichiers de certificat et de clé peer personnalisés.
======
...
Si vous utilisez sqlite ou une base de données externe, les vérifications etcd ne sont pas applicables.
Lors de l'utilisation d'embedded-etcd, K3s définit le paramètre --peer-cert-auth sur true.
Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/k3s/server/db/etcd/config n'a pas été modifié pour désactiver l'authentification par certificat client pour le peer.
======
...
Le kubelet est intégré dans le processus k3s. Il n'y a pas de fichier de service kubelet, toute la configuration est passée en tant qu'arguments lors de l'exécution.
...
*Résultat attendu:* les permissions doivent être fixées à 600 ou plus restrictives
...
Par exemple, `+chmod 600 /var/lib/rancher/k3s/agent/kubeproxy.kubeconfig+`
======
...
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail.
Par exemple, `+chmod 600 /var/lib/rancher/k3s/agent/kubelet.kubeconfig+`
======
...
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail.
Par exemple, `+chown root:root /var/lib/rancher/k3s/agent/kubelet.kubeconfig+`
======
...
*Résultat attendu:* les permissions ont des permissions 600, attendu 600 ou plus restrictives
...
Le kubelet est intégré dans le processus k3s. Il n'y a pas de fichier de configuration kubelet, toute la configuration est passée en tant qu'arguments lors de l'exécution.
...
Si vous utilisez la ligne de commande, éditez le fichier de service K3s et supprimez l'argument ci-dessous. --kubelet-arg="anonymous-auth=true" En fonction de votre système, redémarrez le service k3s. Par exemple, systemctl daemon-reload systemctl restart k3s.service
======
...
Selon votre système, redémarrez le service k3s. Par exemple, systemctl daemon-reload systemctl restart k3s.service
======
...
Par défaut, K3s définit le --read-only-port sur 0. Si vous l'avez défini sur une valeur différente, vous
...
Selon votre système, redémarrez le service k3s. Par exemple,
...
Par défaut, K3s ne définit pas l'argument --rotate-certificates. Si vous avez défini ce drapeau avec une valeur de `+false+`, vous devez soit le définir sur `+true+`, soit complètement supprimer le drapeau.
...
Si vous utilisez la ligne de commande, ajoutez le drapeau K3s --kubelet-arg="tls-cipher-suites=<same values as above>". Selon votre système, redémarrez le service k3s. Par exemple,
systemctl restart k3s.service
======
...
=== Assurez-vous que le service de métriques kube-proxy est lié à l'hôte local (Automatisé).
...
=== 5.1.1 Assurez-vous que le rôle cluster-admin n'est utilisé que là où cela est nécessaire (Automatisé)
...
Identifiez tous les clusterrolebindings au rôle cluster-admin. Vérifiez s'ils sont utilisés et s'ils ont besoin de ce rôle ou s'ils pourraient utiliser un rôle avec moins de privilèges. K3s fait des exceptions pour les clusterrolebindings helm-kube-system-traefik et helm-kube-system-traefik-crd car ils sont nécessaires pour l'installation de traefik dans l'espace de noms kube-system pour les opérations régulières.
Lorsque cela est possible, liez d'abord les utilisateurs à un rôle de moindre privilège, puis supprimez le clusterrolebinding au rôle cluster-admin :
...
*Remédiation:*
Lorsque cela est possible, supprimez l'accès en lecture, en liste et en observation aux objets Secret dans le cluster.
...
K3s fait des exceptions pour les rôles de cluster suivants, qui sont nécessaires pour les opérations régulières :
...
Modifiez la configuration de chaque compte de service par défaut pour inclure cette valeur automountServiceAccountToken : false Ou en utilisant kubectl :
...
Si à la fois le ServiceAccount et le .spec du Pod spécifient une valeur pour automountServiceAccountToken, la spécification du Pod prévaut.
Condition\&nbsp;: Le Pod is_compliant vaut true lorsque
...
* Le ServiceAccount is automountServiceAccountToken vaut true (ou n'est pas défini) et le Pod is automountServiceAccountToken vaut false. K3s accorde des exceptions aux comptes de service suivants, qui sont nécessaires pour les opérations régulières :
...
*Remédiation\&nbsp;:*
Lorsque cela est possible, supprimez les droits Bind, Impersonate et Escalate des sujets.
...
*Remédiation\&nbsp;:*
Lorsque cela est possible, supprimez l'accès à la création d'objets PersistentVolume dans le cluster.
...
*Remédiation\&nbsp;:*
Lorsque cela est possible, supprimez l'accès à la sous-ressource proxy des objets nœuds.
...
*Remédiation\&nbsp;:*
Lorsque cela est possible, supprimer l'accès aux objets validatingwebhookconfigurations ou mutatingwebhookconfigurations.
...
Audit\&nbsp;: l'audit récupère spec.hostNetwork de chaque Pod.
...
Audit : l'audit récupère chaque conteneur de Pod `+.securityContext.allowPrivilegeEscalation+`.
...
K3s donne des exceptions aux pods suivants, qui sont nécessaires pour les opérations régulières :
