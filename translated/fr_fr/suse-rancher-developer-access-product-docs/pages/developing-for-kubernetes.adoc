...
----
. *PATH* – sur macOS, vérifiez que `+~/.rd/bin/+` est dans votre `+$PATH+` (ajouté automatiquement par l'installateur) :
+
[,bash]
----
...
Si vous avez https://scoop.sh[Scoop] installé, le script l'utilisera automatiquement. Sinon, vous devrez peut-être ajouter le répertoire d'installation à votre `+$PATH+`. Vérifiez :
...
Le code source complet est disponible sur GitHub : https://github.com/fxHouard/Rancher-Developer-Access-Demo[fxHouard/Rancher-Developer-Access-Demo].
