= {elemental-product-name} Soutien
:revdate: 2025-07-03
:page-revdate: {revdate}

`+elemental-support+` est un programme utilitaire qui recueille toutes les informations relatives à la machine sur laquelle tourne la pile {elemental-product-name}.

Lors du dépannage d'un nœud {elemental-product-name}, il est possible de générer un fichier compressé contenant les journaux et les fichiers de configuration. +
Attention, *le fichier tar.gz généré peut contenir des informations sensibles*, comme des jetons d'accès au cluster de gestion, etc.

== Exigences

En option, l'utilitaire `+elemental-support+` a besoin que `+kubectl+` soit disponible sur la machine cible, afin de collecter des informations sur les nœuds. Cela peut être très utile pour déboguer les problèmes de mise à niveau {elemental-product-name} d'un nœud. +
...
Une fois lancé, le site `+elemental-support+` créera l'archive compressée dans le même répertoire.
