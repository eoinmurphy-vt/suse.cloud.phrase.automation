...
[IMPORTANT]
====
Cette fonctionnalité est en pré-production et non publiée. Elle n'est disponible que si la variable d'environnement `+STS_EXPERIMENTAL_STACKPACK+` est définie. Des changements incompatibles peuvent survenir.
====

== Objectif

En tant que développeur, vous souhaitez intégrer votre technologie à {stackstate-product-name} en construisant un StackPack personnalisé. Ce didacticiel vous guide dans le processus en utilisant le StackPack CLI.
...
- Créer un nouveau projet StackPack en utilisant la CLI :
+
[source,bash]
----
...
- Vérifiez les fichiers générés dans le répertoire cible .
- Modifiez `+stackpack.yaml+` et d'autres fichiers pour définir votre logique d'intégration.
  * Intégrer des données. Pour mapper les composants et ingérer les métriques, voir xref:setup/custom-integrations/otelmappings/README.adoc[Ajout de mappages de télémétrie Otel]
  * xref:setup/custom-integrations/monitors/index.adoc[Ajouter des moniteurs]
  * xref:setup/custom-integrations/metric-bindings/index.adoc[Ajouter des liaisons de métriques]
...
La sous-commande `+test+` qui rationalise le flux de travail de développement de StackPack en automatisant la séquence package → téléchargement → installation/mise à niveau avec gestion automatique des versions pour les itérations de test.
...
- La sous-commande `+test+` emballe, télécharge et installe votre StackPack avec un suffixe de version de test.
- Itérez sur votre StackPack, en utilisant la commande `+test+` pour un retour rapide, apportez des modifications et observez le processus d'installation / mise à niveau pour qu'il soit exécuté avec succès. Examinez la topologie et la télémétrie ingérées dans l’interface utilisateur de Suse Observability.
...
- Téléchargez l'archive vers {stackstate-product-name} :
+
[source,bash]
----
