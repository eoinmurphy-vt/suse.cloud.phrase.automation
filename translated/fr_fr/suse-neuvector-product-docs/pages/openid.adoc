...
App. Nom de l'intégration : Nom pour cette intégration. Choisissez librement n'importe quel nom
Type de flux (cochez) :
...
Dans la section &quot;`+Client Credentials+`&quot;, cliquez sur modifier et modifiez la section &quot;`+Client Authentication+`&quot; de &quot;`+Use PKCE (for public clients)+`&quot; à &quot;`+Use Client Authentication+`&quot;, puis cliquez sur sauvegarder. Cela générera automatiquement un nouveau secret dont nous aurons besoin dans les prochaines {product-name} étapes de configuration :
...
En mode classique, l'URL de l'émetteur se trouve dans l'onglet Connexion de la page d'application Okta. Pour que l'appartenance au groupe de l'utilisateur soit renvoyée dans la revendication, vous devez ajouter la portée "groups" dans {product-name} la page de configuration OpenID Connect :
