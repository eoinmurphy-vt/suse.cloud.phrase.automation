...
+
Si vos fichiers de certificat ont été créés avant la version {product-name} 5.3, vous devez mettre à jour le certificat avec au moins un Nom Alternatif de Sujet, ou SAN en abrégé. Si vous avez toujours les fichiers `+ca.key+` et `+ca.crt+` accessibles, exécutez les commandes comme suit :
+
[source,bash]
----
kubectl delete secret internal-cert -n neuvector 
...
+
Si vos fichiers de certificat ont été créés avant la version {product-name} 5.3, vous devez mettre à jour le certificat avec au moins un Nom Alternatif de Sujet, ou SAN en abrégé. Si vous n'avez plus les fichiers `+ca.key+` et `+ca.crt+`, suivez les étapes ci-dessous : 
+
