...
** comportement : bloquer_accès / surveiller_changement  #Cela bloque l'accès au filtre défini ci-dessous.  Si surveiller_changement est choisi, alors un événement de sécurité sera généré à partir de la page Notifications > Événements de sécurité de la console web de {product-name}.
...
. La première partie est une NvClusterSecurityRule pour le groupe nommé conteneurs :
