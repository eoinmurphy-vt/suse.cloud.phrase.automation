...
Si vous déployez avec le chart Helm actuel {product-name} de (v1.8.9+), les modifications suivantes doivent être apportées à values.yml :
...
----
--
. Créez les ressources personnalisées (CRD) pour {product-name} les règles de sécurité. Pour Kubernetes 1.19+ : 
+
[NOTE]
====
...
C'est tout ! Vous devriez pouvoir accéder à la console {product-name} et vous connecter avec admin:admin, par exemple `+https://<public-ip>:8443+`.
--
