...
Il existe de nombreuses fonctionnalités d'automatisation dans {product-name} pour soutenir l'ensemble du flux de travail CI/CD, y compris :
...
*Fichiers python d'exemple* Contiennent config.py, client.py et multipart.py. Télécharger des fichiers d'exemple : xref:attachment$ImportExport.zip[ImportExport]. Veuillez mettre les trois fichiers dans un seul dossier pour exécuter les commandes ci-dessus. Vous devrez peut-être installer certains modules Python afin d'exécuter le script.
...
En général, l'énumération des membres de la fédération peut utiliser une requête GET vers le point de terminaison suivant (voir les exemples pour la syntaxe spécifique) :
