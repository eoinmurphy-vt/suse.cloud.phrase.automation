...
La logique de correspondance pour plusieurs critères dans une règle de contrôle d'admission est :
...
Les critères suivants sont liés aux résultats dans {product-name} Actifs > Page d'analyse du registre :
...
* Meilleure pratique PSP. Règles équivalentes pour PSP (note : PSP est complètement supprimé de Kubernetes 1.25+, cependant cet {product-name} équivalent peut encore être utilisé dans 1.25+). Inclut l'exécution en tant qu'utilisateur privilégié, l'exécution en tant qu'utilisateur root, le partage des espaces de noms PID de l'hôte, le partage des espaces de noms IPC de l'hôte, le partage du réseau de l'hôte, et l'autorisation de l'escalade de privilèges. 
