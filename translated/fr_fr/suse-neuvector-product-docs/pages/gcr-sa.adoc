...
[upperalpha]
. Voyez le &quot;`+gcr.io+`&quot; sous Nom d'hôte ? C'est ce qui doit figurer dans l'élément n°2, Registre dans l'interface {product-name}. (N'oubliez pas la partie \https://)
...
Comme vous l'avez déjà conclu, JSON est la solution ici. Sélectionner &quot;`+CREATE+`&quot; entraînera un fichier que vous pourrez télécharger dans votre navigateur. Le contenu de ce fichier doit être collé dans le champ Clé JSON 3 dans {product-name} ; voir la figure 1.
