...
* Mettre à jour pour remédier à CVE-2022-0778, une vulnérabilité OpenSSL trouvée dans l'image de base Alpine utilisée par les images {product-name}. Brève description : Il est possible de déclencher une boucle infinie en créant un certificat ayant des paramètres de courbe elliptique invalides. Étant donné que l'analyse du certificat se produit avant la vérification de la signature du certificat, tout processus qui analyse un certificat fourni de l'extérieur peut être sujet à une attaque par déni de service. Plus de détails peuvent être trouvés aux liens suivants.
...
Pour préserver l'adresse IP source d'origine, l'utilisateur doit ajouter la ligne suivante aux services exposés, dans la section 'spec' de l'équilibreur de charge ou du contrôleur d'entrée. (Réf : https://kubernetes.io/docs/tutorials/services/source-ip/)
