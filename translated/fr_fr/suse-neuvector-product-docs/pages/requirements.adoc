...
* Systèmes de gestion de conteneurs conformes à Kubernetes et Docker officiellement supportés. Les plateformes suivantes sont testées avec chaque version de {product-name} : Kubernetes 1.19-1.32, SUSE Rancher (RKE, RKE2, K3s, etc.), RedHat OpenShift 4.6-4.16 (3.x à 4.12 supporté avant {product-name} 5.2.x), Google GKE, Amazon EKS, Microsoft Azure AKS, IBM IKS, docker natif, docker swarm. Les plateformes conformes à Kubernetes et Docker suivantes sont supportées et ont été vérifiées pour fonctionner avec {product-name} : VMware Photon et Tanzu, SUSE CaaS, Oracle OKE, Mirantis Kubernetes Engine, Nutanix Kubernetes Engine, docker UCP/DataCenter, docker Cloud.
