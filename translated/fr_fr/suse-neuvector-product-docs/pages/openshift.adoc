...
Pour déployer manuellement, commencez par tirer les conteneurs {product-name} appropriés depuis le registre {product-name} vers votre registre local. Remarque : l'image du scanner doit être tirée régulièrement pour les mises à jour de la base de données CVE depuis {product-name}.
...
Si vous déployez avec le {product-name} chart Helm actuel (v1.8.9+), les modifications suivantes doivent être apportées à values.yml :
...
Les informations suivantes seront ajoutées dans les utilisateurs du SCC privilégié :
...
Puis créez le(s) service(s) approprié(s) :
