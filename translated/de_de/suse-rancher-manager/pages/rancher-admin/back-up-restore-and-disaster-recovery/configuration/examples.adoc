...
NOTE: Siehe die {xref-rancher-backup-config}[Sicherungs-Konfigurationsreferenzseite] für weitere Informationen zur Konfiguration der untenstehenden Optionen.
...
Dieses Beispiel zeigt, dass das AWS-Anmeldegeheimnis nicht bereitgestellt werden muss, um eine Sicherung zu erstellen, wenn die Knoten, die `+rancher-backup+` ausführen, {xref-rancher-backup-config}#_iam_permissions_for_ec2_nodes_to_access_s3[über diese Berechtigungen für den Zugriff auf S3 verfügen.]
...
NOTE: Siehe die {xref-rancher-restore-config}[Wiederherstellungs-Konfigurationsreferenzseite] für weitere Informationen zur Konfiguration der untenstehenden Optionen.
...
Dieses Beispiel zeigt, dass das AWS-Anmeldegeheimnis nicht bereitgestellt werden muss, um aus der Sicherung wiederherzustellen, wenn die Knoten, die `+rancher-backup+` ausführen, {xref-rancher-backup-config}#_iam_permissions_for_ec2_nodes_to_access_s3[diese Berechtigungen für den Zugriff auf S3] haben.
