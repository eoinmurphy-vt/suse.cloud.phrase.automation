...
==== Stellen Sie das Konfigurationsbackup wieder her.

Jetzt, da SUSE Observability installiert ist, kann das Konfigurationsbackup wiederhergestellt werden. Das SUSE Observability Helm-Chart enthält eine ähnliche Reihe von Sicherungs-Tools xref:/setup/data-management/backup_restore/configuration_backup.adoc[die hier dokumentiert sind]. *Diese sind nicht die gleichen wie für StackState 6.x*, also stellen Sie sicher, dass Sie die Skripte aus dem `+restore+` Verzeichnis des *SUSE Observability Helm-Charts* für die Wiederherstellung der Sicherung erhalten.

Aus dem Verzeichnis `+restore+` des SUSE Observability Helm-Charts führen Sie bitte die folgenden Befehle aus, um die Sicherung wiederherzustellen:
...
. Laden Sie die zuvor erstellte Konfigurationsbackup-Datei hoch, in diesem Fall `+sts-backup-20241024-1423.sty+` (stellen Sie sicher, den vollständigen Pfad zu verwenden, falls erforderlich):
+
[,bash]
----
