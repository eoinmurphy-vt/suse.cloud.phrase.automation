...
* `+historicWindowMinutes+`: zum Beispiel `+120+` (2 Stunden) – um den aktuellen Zeitpunkt herum, aber dann vor ein oder mehreren Wochen – also von einer Stunde vor dem aktuellen Zeitpunkt bis eine Stunde danach.  Auch die zwei Stunden vor dem Prüfintervall werden verwendet.  Der dynamische Schwellenmonitor vergleicht die Verteilung dieser historischen Daten mit den Datenpunkten im Prüfintervall.
...
Der Monitor kann mithilfe des Leitfadens unter xref:use/alerting/k8s-dynamic-threshold-monitors.adoc[Fügen Sie einen Schwellenwertmonitor zu Komponenten mit der Kommandozeilenschnittstelle hinzu] implementiert werden.
