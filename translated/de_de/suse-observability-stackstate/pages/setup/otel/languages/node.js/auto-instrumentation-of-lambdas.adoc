= Automatische Instrumentierung einer NodeJS-Lambda-Funktion
:revdate: 2025-07-10
...
Dieses Dokument führt Sie durch das automatische Instrumentieren von NodeJS Lambda-Funktionen mit OpenTelemetry. Die automatische Instrumentierung vereinfacht den Prozess, Ihren Lambda-Funktionen Einblicke zu verleihen, indem automatisch Leistungsmetriken und Tracing-Informationen erfasst werden.
...
== Werte, die von der Umgebung bereitgestellt werden

OpenTelemetry ist auf verschiedene Konfigurationswerte angewiesen, um korrekt zu funktionieren. Diese Werte steuern Aspekte wie Datensammlung, Export und Kommunikation mit Backend-Systemen. Um Ihre OpenTelemetry-Bereitstellung flexibel und anpassungsfähig an verschiedene Umgebungen zu gestalten, können Sie diese Einstellungen über Umgebungsvariablen bereitstellen. Dieser Ansatz bietet mehrere Vorteile:
...
*Ein vollständiges Beispiel: Seien Sie sich bewusst, dass Sie Ihre eigenen Werte eingeben müssen.*
...
Die OTEL-Sammelkonfiguration legt fest, wie die gesammelten Daten verteilt werden sollen.  Dies geschieht in der collector.yaml-Datei, die im src-Verzeichnis platziert ist, wo die Lambda-Dateien zu finden sind.  Nachfolgend finden Sie ein Beispiel für eine collector.yaml-Datei.
...
Je nach gewünschter Funktionalität oder basierend auf Faktoren wie den von auf diese Weise instrumentierten Lambdas erzeugten Datenmengen können Collector für Batching, Tail-Sampling und andere Vorverarbeitungstechniken eingerichtet werden, um die Auswirkungen auf SUSE Observability zu reduzieren.

Siehe diese Seite für xref:/setup/otel/proxy-collector.adoc[Hinweise und Anleitungen] zur Einrichtung eines Batching-Collectors, der als Sicherheitsproxy für SUSE Observability fungiert.
Siehe diese Seite für xref:/setup/otel/collector.adoc[Anleitungen] zur Einrichtung eines Collectors, der ebenfalls Tail-Sampling durchführt.
Für weitere Informationen zur Prozessor-Konfiguration im OpenTelemetry Collector siehe die https://github.com/open-telemetry/opentelemetry-collector/blob/main/processor/README.md[offizielle Dokumentation].
...
Für weitere Informationen zur Konfiguration der Authentifizierung für den OpenTelemetry-Collector konsultieren Sie bitte die https://github.com/open-telemetry/opentelemetry-collector/blob/main/config/configauth/README.md[offizielle Dokumentation].
