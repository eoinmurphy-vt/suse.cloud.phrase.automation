...
Bundles werden automatisch von {product_name} erstellt, wenn ein `+GitRepo+` erstellt wird.

Der Inhalt der Ressource entspricht der xref:reference/ref-crds.adoc#_bundlespec[BundleSpec].
Weitere Informationen finden Sie in der Anleitung zur Verwendung der Bundle-Ressource: xref:how-tos-for-users/bundle-add.adoc[Erstellen Sie eine Bundle-Ressource].
