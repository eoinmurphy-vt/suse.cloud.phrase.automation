...
Die `+Schedule+` Ressource definiert Zeitfenster, während derer {product_name} Änderungen an `+Clusters+` akzeptiert.

Weitere Informationen dazu, wie Sie die `+Schedule+` Ressource verwenden, finden Sie in der Anleitung unter xref:experimental-features/scheduling.adoc[Planung].
