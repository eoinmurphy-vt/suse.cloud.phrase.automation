= Hinzufügen von benutzerdefinierten Zertifikaten zu k3s aus dem k3k-Cluster
:revdate: 2025-09-16
:page-revdate: {revdate}

== Generieren und hinzufügen der benutzerdefinierten Zertifikate

. Verwenden Sie das https://github.com/k3s-io/k3s/blob/master/contrib/util/generate-custom-ca-certs.sh[benutzerdefinierte Zertifikatsskript] zum Generieren von Zertifikaten.

. Nach der Generierung der Zertifikate erstellen Sie ein lokales Verzeichnis, da das Standardverzeichnis nicht zugänglich sein wird. Beispiel: `+k3k/custom-certs+`. 

. Kopieren Sie die generierten Zertifikate in das Verzeichnis - `+$ sudo cp -r /var/lib/rancher/k3s/server/tls k3k/custom-certs/+`

. Verwenden Sie den Pfad `+/var/lib/rancher/k3s/server/tls k3k/custom-certs/+`, um die virtuellen Cluster in 2 Modi zu erstellen:
+
* Virtueller Modus 
* Geteilter Modus
...
. Generieren Sie benutzerdefinierte Zertifikate über das Skript:
+
----
...
. Überprüfen Sie die Zertifikate im Server-Pod:
+
----
...
. Kopieren und überprüfen Sie die Zertifikate im lokalen Verzeichnis:
+
----
...
. Überprüfen Sie den neu erstellten k3k-Cluster:
+
----
