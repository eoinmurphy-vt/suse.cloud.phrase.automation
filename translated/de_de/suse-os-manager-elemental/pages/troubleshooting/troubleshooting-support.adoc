= {elemental-product-name} Unterstützung
:revdate: 2025-07-03
:page-revdate: {revdate}

Das `+elemental-support+` ist ein Dienstprogramm, das alle Informationen über die Maschine sammelt, auf der der {elemental-product-name} Stack läuft.

Bei der Fehlerbehebung an einem {elemental-product-name}-Knoten ist es möglich, eine komprimierte Datei zu erstellen, die Logs und Konfigurationsdateien enthält. +
Bitte beachten Sie, dass *die generierte tar.gz-Datei sensible Informationen enthalten kann*, wie Token zum Verwaltungscluster und mehr.

== Anforderungen

Optional muss das `+elemental-support+` Dienstprogramm `+kubectl+` auf der Zielmaschine verfügbar sein, um Knoteninformationen zu sammeln. Dies kann sehr nützlich sein, um {elemental-product-name} Upgrade-Probleme mit einem Knoten zu debuggen. +
Die verwendete kubectl-Konfiguration wird aus der `+KUBECONFIG+` Umgebungsvariable abgerufen, oder in deren Abwesenheit werden entweder die `+/etc/rancher/k3s/k3s.yaml+` oder `+/etc/rancher/rke2/rke2.yaml+` Dateien verwendet.

== Informationen sammeln

Sobald es ausgeführt wird, erstellt das `+elemental-support+` das komprimierte Archiv im selben Verzeichnis.
