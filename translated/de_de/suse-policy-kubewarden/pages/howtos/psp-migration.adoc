...
:title: Migration der PodSecurityPolicy
:description: Konfigurieren Sie {project-name} Richtlinien in Ihren Kubernetes-Clustern, um eine granulare Kontrolle über die Richtlinienkonfiguration zu erhalten, und ersetzen Sie PodSecurityPolicy (PSP).
...
https://kubernetes.io/docs/concepts/security/pod-security-policy/[PodSecurityPolicy](PSP) wird entfernt. Jetzt können Sie {project-name} für die Zulassungskontrolle in Ihren Kubernetes-Clustern verwenden.

{short-project-name} verfügt über separate Richtlinien, um das gleiche Ziel wie eine monolithische PSP-Konfiguration zu erreichen. Jede {short-project-name} Policy-Definition fungiert als ein anderer Konfigurationsabschnitt in der Spezifikation eines PSP. Die Zuordnung der PSP-Konfigurationsfelder zu ihren jeweiligen {short-project-name} Richtlinien ist in der nachstehenden<<_mapping_kubewarden_policies_to_psp_fields,Zuordnungstabelle>> aufgeführt.

Mit {short-project-name} haben die Betreiber eine detaillierte Kontrolle über die Richtlinienkonfiguration in ihren Clustern.
...
* ein PSP, das die Eskalation von Privilegien verhindert
* privilegierte Container
* Blockieren von Pods, die als Root laufen
* Erzwingen einer bestimmten Benutzergruppe
* Sperrung von Host-Namespaces
* Erlaubt einem Pod, nur Port 443 zu verwenden
...
== {short-project-name} Ersatzprodukte für PSP

Jetzt werden wir {short-project-name} Richtlinien erstellen, um das gleiche Ziel zu erreichen.
Sie setzen jede Regel mit einer eigenen {short-project-name} Richtlinie durch.
In diesem Beispiel benötigen Sie also für die Durchsetzung jedes dieser Ziele eine eigene Richtlinie:

* Privilegieneskalation
* Benutzer- und Gruppenkonfiguration
* Host-Namensräume
* privilegierte Container-Konfiguration.

=== Blockierung der Ausweitung von Container-Privilegien

Sie können eine Richtlinie wie nachstehend gezeigt bereitstellen:

.kubectl-Befehl für die Bereitstellung von Richtlinien
[%collapsible]
======
...
In diesem Befehl haben wir `+default_allow_privilege_escalation+` als`+false+` angegeben. Diese Richtlinie schränkt Pods ein, die versuchen, mit mehr Rechten als der Elterncontainer zu laufen.

.Ausgabe von kubectl, die versucht, das Privileg zu erhöhen
[%collapsible]
======
...
Um die Benutzer- und Gruppenkonfiguration durchzusetzen, können Sie nun dielink:https://github.com/kubewarden/user-group-psp-policy[Richtlinie] verwenden.

.`+kubectl+` Befehl zur Verwendung von user-group-psp-policy
[%collapsible]
======
[subs="+attributes",shell]
----
...
Sie sollten die Richtlinie mit `+mutation: true+` konfigurieren. Sie ist erforderlich, weil die Richtliniehttps://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups[supplementalGroups]hinzufügt, wenn der Benutzer sie nicht definiert.

Daher können Benutzer jetzt keine Pods als Root installieren:

.Beispiel für eine Ausgabe mit runAsNonRoot: false
[%collapsible]
======
...
.Beispiel für die Ausgabe von runAsUser: 0
[%collapsible]
======
...
Das folgende Beispiel zeigt die Hinzufügung einer https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups[Ergänzungsgruppe], obwohl sie nicht von uns definiert wurde.
...
Sie müssen die ältere PSP-Konfiguration ersetzen, die privilegierte Container blockiert. Es ist notwendig, die https://github.com/kubewarden/pod-privileged-policy[pod-privileged policy] einzusetzen. Für diese Richtlinie sind keine Einstellungen erforderlich. Sobald er läuft, blockiert er privilegierte Pods.

.Anwenden der pod-privileged-Richtlinie
[%collapsible]
======
...
.Pod-Lauf mit aktivierter privilegierter Konfiguration
[%collapsible]
======
...
=== Konfiguration des Host-Namensraums

Um die PSP-Migrationsübung abzuschließen, müssen Sie die gemeinsame Nutzung von Host-Namespaces deaktivieren. Dazu werden wir die https://github.com/kubewarden/host-namespaces-psp-policy[`+host-namespace-psp+`policy] verwenden. Es ermöglicht dem Cluster-Administrator, IPC-, PID- und Netzwerk-Namensräume individuell zu sperren. Sie legt auch die Ports fest, an denen die Pods auf der Host-IP geöffnet werden können.

.Deaktivieren der gemeinsamen Nutzung von Namensräumen und Festlegen von Ports
[%collapsible]
======
...
Wir können die Police validieren.
Der Pod sollte nicht in der Lage sein, Host-Namespaces gemeinsam zu nutzen:

.Beispiel für einen blockierten Namensraum
[%collapsible]
======
...
In diesem letzten Beispiel sollte der Pod nur den Port 443 freigeben können.
...
Diese Tabelle ordnet PSP-Konfigurationsfelder den entsprechenden {short-project-name} Richtlinien zu.

|===
| PSP-Feld | {short-project-name} gleichwertige Politik
...
| https://kubernetes.io/docs/concepts/security/pod-security-policy/#volumes-and-file-systems[Bände]
| https://github.com/kubewarden/volumes-psp-policy[volumen-psp-richtlinie]
...
| https://github.com/kubewarden/selinux-psp-policy[selinux-psp-Richtlinie]
...
| https://kubernetes.io/docs/concepts/security/pod-security-policy/#apparmor[Abrüstung]
| https://github.com/kubewarden/apparmor-psp-policy[apparmor-psp-Richtlinie]
...
== Skript zur PSP-Migration

Das Team von {short-project-name} hat ein Skript für die PSP-Migration entwickelt. Es verwendet das Migrationstool von https://github.com/appvia/psp-migration[AppVia]. Das AppVia-Tool liest eine PSP-YAML-Konfiguration. Anschließend erstellt es die entsprechenden Richtlinien. Dies gilt für {short-project-name} und andere Policy Engines.

[CAUTION]
====

Das AppVia-Migrationswerkzeug ist außerhalb der Kontrolle der Kuberwarden-Maintainer.
Das bedeutet, dass es möglicherweise veraltete {short-project-name} Richtlinien erstellt. Mit Vorsicht zu verwenden. Wir benötigen einen Pull-Request für AppVia, an dem bereits gearbeitet wird.
Wenden Sie sich an uns, wenn Sie weitere Informationen benötigen.
====


Das Skript ist im Repository {short-project-name}https://github.com/kubewarden/utils/blob/main/scripts/psp-to-kubewarden[ utils]verfügbar. Es lädt das AppVia-Migrationstool in das zu verwendende Arbeitsverzeichnis herunter. Er verarbeitet die im Standardkontext `+kubectl+` definierten PSPs. Anschließend werden die Definitionen der Kuberwarden-Richtlinien auf der Standardausgabe ausgegeben. Die Nutzer können den Inhalt an eine Datei oder direkt an `+kubectl+` weiterleiten.
...
Schauen wir uns ein Beispiel an. In einem Cluster mit dem PSP:

* Sperrung des Zugangs zu Host-Namensräumen
* Sperrung privilegierter Container
* keine Privilegieneskalation zulassen
* Abwurfmöglichkeiten für Container
* Auflistung der zulässigen Datenträgertypen
* Festlegung der zulässigen Benutzer und Gruppen, die verwendet werden dürfen
* Kontrolle der auf die Bände angewendeten zusätzlichen Gruppe
* Erzwingen der Ausführung von Containern in einem schreibgeschützten Root-Dateisystem
...
{short-project-name} Richtlinien können mit dem folgenden Befehl direkt auf einen Cluster angewendet werden:
...
Wenn Benutzer die Richtlinien vor der Anwendung überprüfen möchten, können sie den Inhalt in eine Datei umleiten oder direkt in der Konsole überprüfen

So speichern Sie die erstellten Richtlinien und zeigen sie an:
...
[TIP]
====
Die Namen der Richtlinien werden vom PSP-Migrationswerkzeug generiert.
Vielleicht sollten Sie den Namen in etwas Sinnvolleres ändern.
====
