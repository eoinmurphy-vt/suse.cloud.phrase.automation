...
:description: Implementieren Sie Richtliniengruppen mit spezifischen Regeln, die nicht konforme Pods auf der Grundlage von Bildsignaturen und anderen Kriterien zurückweisen.
...
[TIP]
====
Bevor Sie mit Richtliniengruppen arbeiten, sollten Sie diexref:explanations/policy-groups.adoc[Erklärung] lesen.
====


Verwenden Sie das Beispiel aus der xref:../explanations/policy-groups.adoc[Erläuterung der Richtliniengruppen] und implementieren Sie es mit diesen Befehlen.
...
Um mehr Informationen über die Bewertung der Richtlinien zu erhalten, die Teil der Gruppe sind, erhöhen Sie die Ausführlichkeitsstufe von `+kubectl+`:
...
[NOTE]
====
Die von den Richtlinien, die Teil der Gruppe sind, erzeugte Bewertungsausgabe ist nur sichtbar, wenn die Ausführlichkeitsstufe von `+kubectl+` erhöht wird.

Eine Ausführlichkeitsstufe von `+4+` reicht aus, um die Bewertungsergebnisse der Richtlinien zu sehen.
====
