...
Este documento es un complemento a la xref:security/hardening-guide.adoc[guía de protección de K3s]. La guía de protección proporciona orientación prescriptiva para proteger una instalación de producción de K3s, y esta guía de referencia está destinada a ayudarte a evaluar el nivel de seguridad del clúster protegido frente a cada control en el Benchmark de Kubernetes CIS. Está destinada a ser utilizada por operadores de K3s, equipos de seguridad, auditores y tomadores de decisiones.

Esta guía es específica para la línea de versiones **v1.29-v1.34** de K3s y la versión **v1.11** del Benchmark de Kubernetes CIS.
...
Por defecto, K3s incrusta el servidor API dentro del proceso de k3s. No hay archivo de especificación del pod del servidor API.
...
Por defecto, K3s incrusta el administrador de control dentro del proceso de k3s. No hay archivo de especificación del pod del administrador de control.
...
Por defecto, K3s incrusta el programador dentro del proceso de k3s. No hay archivo de especificación del pod del programador.
...
Por defecto, K3s incrusta etcd dentro del proceso de k3s. No hay archivo de especificación del pod de etcd.
...
Por ejemplo, `+chown root:root /var/lib/cni/networks/<filename>+`
======
...
No es aplicable para el clúster sin etcd. Si se ejecuta solo el maestro sin rol de etcd, esta verificación no es aplicable.
...
Ejecuta el siguiente comando (basado en el directorio de datos de etcd encontrado arriba). Por ejemplo, `+chmod 700 /var/lib/rancher/k3s/server/db/etcd+`
======
...
Ejecuta el siguiente comando (basado en la ubicación del archivo en tu sistema) en el nodo del plano de control.
Por ejemplo, `+chmod 600 /var/lib/rancher/k3s/server/cred/scheduler.kubeconfig+`
======
...
Por defecto, k3s establece los permisos del archivo de certificado PKI en 644, se admiten permisos más restrictivos como 600.
======
...
Si esta comprobación falla, edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml y elimina cualquier cosa similar a lo siguiente.
----
kube-apiserver-arg:
  - "anonymous-auth=true"
----
...
Si esta comprobación falla, edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml y elimina cualquier línea como la siguiente.
----
kube-apiserver-arg:
...
Por defecto, K3s no configura el --authorization-mode en AlwaysAllow.
Si esta comprobación falla, edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml. Elimina cualquier línea como la siguiente.
----
kube-apiserver-arg:
...
Si esta verificación falla, edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml. Asegúrate de que no estás sobrescribiendo authorization-mode.
======
...
**Remediación:**
Permisivo, según las directrices de CIS, +
"Esta configuración podría afectar a clústeres fuera de línea o aislados, que tienen imágenes pre-cargadas y no tienen acceso a un registro para obtener imágenes en uso. Esta configuración no es apropiada para clústeres que utilizan esta configuración."
...
Por defecto, K3s no establece el --disable-admission-plugins en nada.
...
=== 1.2.15 Asegúrate de que el argumento --profiling esté configurado en false (Automatizado)
...
=== 1.2.16 Asegúrate de que el argumento --audit-log-path esté configurado (Manual)
...
=== 1.2.17 Asegúrate de que el argumento --audit-log-maxage esté configurado en 30 o según corresponda (Manual)
...
Edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml en el nodo del plano de control y establece el parámetro audit-log-maxage en 30 o en un número de días adecuado, por ejemplo,
----
kube-apiserver-arg:
  - "audit-log-maxage=30"
----
...
=== 1.2.18 Asegúrate de que el argumento --audit-log-maxbackup esté configurado en 10 o según corresponda (Manual)
...
Edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml en el nodo del plano de control y establece el parámetro audit-log-maxbackup en 10 o en un valor adecuado. Por ejemplo,
----
kube-apiserver-arg:
...
=== 1.2.19 Asegúrate de que el argumento --audit-log-maxsize esté configurado en 100 o según corresponda (Manual)
...
Edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml en el nodo del plano de control y establece el parámetro audit-log-maxsize a un tamaño apropiado en MB. Por ejemplo,
----
kube-apiserver-arg:
...
=== 1.2.20 Asegúrate de que el argumento --request-timeout esté configurado de manera apropiada (Manual)

**Resultado:** WARN (ADVERTENCIA)

**Remediación:**
Permisivo, según las directrices de CIS, "se recomienda establecer este límite de manera apropiada y cambiar el límite predeterminado de 60 segundos solo si es necesario".
...
=== 1.2.21 Asegúrate de que el argumento --service-account-lookup esté configurado en verdadero (Automatizado)
...
----
Alternativamente, puedes eliminar el parámetro service-account-lookup de este archivo de modo que se aplique el valor predeterminado.
======

=== 1.2.22 Asegúrate de que el argumento --service-account-key-file esté configurado de manera apropiada (Automatizado)
...
=== 1.2.23 Asegúrate de que los argumentos --etcd-certfile y --etcd-keyfile estén configurados de manera apropiada (Automatizado)
...
K3s genera y establece automáticamente los archivos de certificado y clave de etcd.
...
=== 1.2.24 Asegúrate de que los argumentos --tls-cert-file y --tls-private-key-file estén configurados de manera apropiada (Automatizado)
...
Se generan y se encuentran en /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.crt y /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.key +
Si esta comprobación falla, edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml y elimina cualquier línea como la siguiente.
----
kube-apiserver-arg:
  - "tls-cert-file=<path>"
...
Si por alguna razón necesitas proporcionar tu propio certificado CA, considera usar las herramientas shell de k3s.
Si esta comprobación falla, edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml y elimina cualquier línea como la siguiente.
----
kube-apiserver-arg:
  - "client-ca-file=<path>"
...
Edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml en el nodo del plano de control y establece el siguiente parámetro. secrets-encryption: true. El cifrado de secretos se puede gestionar luego con la herramienta shell k3s secrets-encrypt.
...
=== 1.2.29 Asegúrate de que el API Server solo utilice cifrados criptográficos fuertes (Automatizado)
...
Por defecto, el kube-apiserver de K3s cumple con esta prueba. Los cambios en estos valores pueden causar regresiones, por lo tanto, asegúrate de que todos los clientes del apiserver soporten la nueva configuración de TLS antes de aplicarla en despliegues de producción.
...
Si esta verificación falla, elimina cualquier configuración personalizada alrededor de `+tls-cipher-suites+` o actualiza el archivo /etc/rancher/k3s/config.yaml para que coincida con la configuración predeterminada añadiendo lo siguiente:
----
kube-apiserver-arg:
...
Edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml y establece el parámetro --service-account-extend-token-expiration en false antes de recargar k3s, como se muestra a continuación:
kube-apiserver-arg:
 - "service-account-extend-token-expiration=false"
Por defecto, este parámetro está configurado en true.
======

== 1.3 Administrador de Controladores

=== 1.3.1 Asegúrate de que el argumento --terminated-pod-gc-threshold esté configurado de manera apropiada (Manual)
...
Edita el archivo de configuración de K3s /etc/rancher/k3s/config.yaml en el nodo del plano de control
y establece el --terminated-pod-gc-threshold en un umbral apropiado,
----
...
=== 1.3.2 Asegúrate de que el argumento --profiling esté configurado en false (Automatizado)
...
=== 1.3.3 Asegúrate de que el argumento --use-service-account-credentials esté configurado en true (Automatizado)
...
=== 1.3.4 Asegúrate de que el argumento --service-account-private-key-file esté configurado de manera apropiada (Automatizado)
...
Por defecto, K3s no establece la puerta de características RotateKubeletServerCertificate.
Si has habilitado esta puerta de características, deberías eliminarla.
...
Al ejecutarse con embedded-etcd, K3s genera archivos de certificado y clave para etcd.
Estos se encuentran en /var/lib/rancher/k3s/server/tls/etcd/.
Si esta comprobación falla, asegúrate de que el archivo de configuración /var/lib/rancher/k3s/server/db/etcd/config no haya sido modificado para usar archivos de certificado y clave personalizados.
======
...
Si esta comprobación falla, asegúrate de que el archivo de configuración /var/lib/rancher/k3s/server/db/etcd/config no haya sido modificado para desactivar la autenticación del certificado del cliente.
======

=== 2.3 Asegúrate de que el argumento --auto-tls no esté configurado en true (Manual)
...
Si esta comprobación falla, edita el archivo de especificación del pod etcd /var/lib/rancher/k3s/server/db/etcd/config en el nodo maestro y elimina el parámetro --auto-tls o configúralo en false. client-transport-security: auto-tls: false
======
...
Si se ejecuta con sqlite o una base de datos externa, las comprobaciones de etcd no son aplicables.
Al ejecutarse con embedded-etcd, K3s genera archivos de certificado y de clave para pares en etcd.
Estos se encuentran en /var/lib/rancher/k3s/server/tls/etcd/.
Si esta verificación falla, asegúrate de que el archivo de configuración /var/lib/rancher/k3s/server/db/etcd/config no haya sido modificado para usar archivos de certificado y clave de pares personalizados.
======

=== 2.5 Asegúrate de que el argumento --peer-client-cert-auth esté configurado en true (Manual)
...
Si se ejecuta con sqlite o una base de datos externa, las comprobaciones de etcd no son aplicables.
Al ejecutarse con embedded-etcd, K3s establece el parámetro --peer-cert-auth en true.
Si esta verificación falla, asegúrate de que el archivo de configuración /var/lib/rancher/k3s/server/db/etcd/config no haya sido modificado para deshabilitar la autenticación del certificado del cliente de pares.
======

=== 2.6 Asegúrate de que el argumento --peer-auto-tls no esté configurado en true (Manual)
...
Si se ejecuta con sqlite o una base de datos externa, las comprobaciones de etcd no son aplicables.
Al ejecutarse con embedded-etcd, K3s no establece el parámetro --peer-auto-tls.
Si esta verificación falla, edita el archivo de especificación del pod etcd /var/lib/rancher/k3s/server/db/etcd/config en el nodo maestro y elimina el parámetro --peer-auto-tls o configúralo en false. peer-transport-security: auto-tls: false
======
...
Si se ejecuta con sqlite o una base de datos externa, las comprobaciones de etcd no son aplicables.
Al ejecutar con etcd embebido, K3s genera una autoridad de certificación única para etcd.
Esto se encuentra en /var/lib/rancher/k3s/server/tls/etcd/peer-ca.crt.
Si esta verificación falla, asegúrate de que el archivo de configuración /var/lib/rancher/k3s/server/db/etcd/config no haya sido modificado para usar una autoridad de certificación compartida.
======
...
=== 4.1.2 Asegúrate de que la propiedad del archivo de servicio kubelet esté configurada en root:root (Automatizado)
...
=== 4.1.4 Si existe el archivo kubeconfig del proxy, asegúrate de que la propiedad esté configurada en root:root (Automatizado)
...
=== Asegúrate de que la propiedad del archivo --kubeconfig kubelet.conf esté configurada en root:root (Automatizado).
...
=== Asegúrate de que los permisos del archivo de autoridades de certificación estén configurados en 600 o más restrictivos (Automatizado).
...
Ejecute el siguiente comando para modificar los permisos del archivo del
...
Ejecute el siguiente comando para modificar la propiedad del --client-ca-file.
`+chown root:root /var/lib/rancher/k3s/agent/client-ca.crt+`
======

=== Asegúrate de que el archivo de configuración --config del kubelet tenga permisos configurados en 600 o más restrictivos (Automatizado).
...
=== Asegúrate de que la propiedad del archivo de configuración --config del kubelet esté configurada en root:root (Automatizado).
...
=== Asegúrate de que el argumento --anonymous-auth esté configurado en false (Automatizado).
...
Por defecto, K3s establece el --anonymous-auth en false. Si has establecido esto a un valor diferente, debes volver a configurarlo en false. Si utilizas el archivo de configuración de K3s /etc/rancher/k3s/config.yaml, elimina cualquier línea similar a la siguiente.
----
kubelet-arg:
  - "anonymous-auth=true"
----
...
Por defecto, K3s establece el --read-only-port en 0. Si has establecido esto a un valor diferente, debes volver a configurarlo en 0. Si utilizas el archivo de configuración de K3s /etc/rancher/k3s/config.yaml, elimina cualquier línea similar a la siguiente.
----
kubelet-arg:
  - "read-only-port=XXXX"
----
...
Si utilizas el archivo de configuración de K3s /etc/rancher/k3s/config.yaml, establece el siguiente parámetro a un valor apropiado.
----
kubelet-arg:
...
=== 4.2.6 Asegúrate de que el argumento --make-iptables-util-chains esté establecido en verdadero (Automatizado)
...
**Resultado Esperado:** '--make-iptables-util-chains' es igual a 'verdadero' o '--make-iptables-util-chains' no está presente
...
=== 4.2.8 Asegúrate de que el argumento eventRecordQPS esté establecido a un nivel que asegure una captura de eventos apropiada (Manual)
...
Por defecto, K3s establece event-qps en 0. Si deseas cambiar esto,
Si utilizas el archivo de configuración de K3s /etc/rancher/k3s/config.yaml, establece el siguiente parámetro a un valor apropiado.
----
kubelet-arg:
  - "event-qps=<value>"
...
Por defecto, K3s no establece el argumento --rotate-certificates. Si has establecido este flag con un valor de `+false+`, deberías establecerlo en `+true+` o eliminar completamente la bandera.
...
Si utilizas la línea de comandos, elimina la bandera de K3s --kubelet-arg="rotate-certificates".
Según tu sistema, reinicia el servicio k3s. Por ejemplo,
systemctl restart k3s.service
======

=== 4.2.11 Verifica que el argumento RotateKubeletServerCertificate esté establecido en verdadero (Automatizado)
...
Si utilizas la línea de comandos, elimina la bandera de K3s --kubelet-arg="feature-gate=RotateKubeletServerCertificate".
Según tu sistema, reinicia el servicio k3s. Por ejemplo,
systemctl restart k3s.service
======
...
Si utilizas la línea de comandos, añade la bandera de K3s --kubelet-arg="tls-cipher-suites=&lt;mismos valores que se indican arriba&gt;". Según tu sistema, reinicia el servicio k3s. Por ejemplo,
systemctl restart k3s.service
======
...
Si utilizas un archivo de configuración de K3s /etc/rancher/k3s/config.yaml, edita el archivo para establecer `+seccomp-default+` en kubelet-arg: - "seccomp-default=verdadero"
...
Modifica o elimina cualquier valor que vincule el servicio de métricas a una dirección que no sea host local.
...
Donde sea posible, eliminar el acceso de creación a objetos pod en el clúster.
======
...
**Remediación:**
Crear cuentas de servicio explícitas siempre que una carga de trabajo de Kubernetes requiera acceso específico al servidor API de Kubernetes.
K3s hace una excepción para la cuenta de servicio predeterminada en el espacio de nombres kube-system.
Modificar la configuración de cada cuenta de servicio predeterminada para incluir este valor automountServiceAccountToken: false O usando kubectl:
[,bash]
----
...
Modificar la definición de ServiceAccounts y Pods que no necesitan montar tokens de cuenta de servicio para desactivarlo, con `+automountServiceAccountToken: false+`.
...
Condición: El Pod es_compliant a true cuando - ServiceAccount es automountServiceAccountToken: false y el Pod es automountServiceAccountToken: false o notset - ServiceAccount es automountServiceAccountToken: true notset y el Pod es automountServiceAccountToken: false K3s da excepciones a las siguientes cuentas de servicio, que son necesarias para operaciones regulares: - coredns, helm-traefik, helm-traefik-crd, traefik, metrics-server, svclb, local-path-provisioner-service-account
======
...
**Remediación:**
Eliminar el grupo system:masters de todos los usuarios en el clúster.
...
**Remediación:**
Donde sea posible, eliminar los permisos de impersonate, bind y escalate de los sujetos.
...
**Remediación:**
Donde sea posible, eliminar el acceso para crear objetos PersistentVolume en el clúster.
...
**Remediación:**
Donde sea posible, eliminar el acceso al sub-recurso proxy de los objetos de nodo.
...
**Remediación:**
Donde sea posible, eliminar el acceso al sub-recurso de aprobación de los objetos certificatesigningrequests.
...
**Remediación:**
Donde sea posible, eliminar el acceso a los objetos validatingwebhookconfigurations o mutatingwebhookconfigurations.

=== 5.1.13 Minimizar el acceso a la creación de tokens de cuentas de servicio (Manual)

**Resultado:** WARN (ADVERTENCIA)

**Remediación:**
Donde sea posible, eliminar el acceso al sub-recurso token de los objetos ServiceAccount.

== 5.2 Normas de seguridad de Pods

=== 5.2.1 Asegurarse de que el clúster tenga al menos un mecanismo de control de políticas activo (Manual)
...
Añadir políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores privilegiados.
Auditoría: la auditoría lista todos los contenedores de los pods para recuperar su valor .securityContext.privileged.
Condición: is_compliant es falso si el `+.securityContext.privileged+` del contenedor está establecido en `+true+`.
Por defecto: por defecto, no hay restricciones sobre la creación de contenedores privilegiados.
======

=== 5.2.3 Minimizar la admisión de contenedores que deseen compartir el espacio de nombres del ID de proceso del host (Manual)
...
Añadir políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores `+hostPID+`.
Auditoría: la auditoría recupera el spec.hostPID de cada Pod.
Condición: is_compliant es falso si el spec.hostPID del Pod está establecido en `+true+`.
Por defecto, no hay restricciones en la creación de contenedores hostPID.
======
...
Añadir políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores `+hostIPC+`.
Auditoría: la auditoría recupera el spec.IPC de cada Pod.
Condición: is_compliant es falso si el spec.hostIPC del Pod está establecido en `+true+`.
Por defecto, no hay restricciones en la creación de contenedores hostIPC.
======

=== 5.2.5 Minimizar la admisión de contenedores que deseen compartir el espacio de nombres de red del host (Manual)
...
Añadir políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores `+hostNetwork+`.
Auditoría: la auditoría recupera el spec.hostNetwork de cada Pod.
Condición: is_compliant es falso si el spec.hostNetwork del Pod está establecido en `+true+`.
...
Añadir políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores con `+.securityContext.allowPrivilegeEscalation+` establecido en `+true+`.
Auditoría: la auditoría recupera el/los contenedor(es) de cada Pod `+.securityContext.allowPrivilegeEscalation+`.
Condición: is_compliant es falso si `+.securityContext.allowPrivilegeEscalation+` del contenedor está establecido en `+true+`.
Por defecto: Si no está establecido, se permite la escalada de privilegios (por defecto es verdadero). Sin embargo, si se utiliza PSP/PSA con un perfil de `+restricted+`, la escalada de privilegios está explícitamente prohibida a menos que se configure de otra manera.
======

=== 5.2.7 Minimizar la admisión de contenedores [root] (Manual)

**Resultado:** WARN (ADVERTENCIA)

**Solución:**
Crear una política para cada espacio de nombres en el clúster, asegurando que ya sea `+MustRunAsNonRoot+` o `+MustRunAs+` con el rango de UIDs que no incluya 0, esté establecido.
...
**Solución:**
Añadir políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores con la `+NET_RAW+` capacidad.
...
Asegúrate de que `+allowedCapabilities+` no esté presente en las políticas para el clúster a menos que esté establecido en un array vacío.
Auditoría: la auditoría recupera el/los contenedor(es) de cada Pod con capacidades añadidas.
Condición: is_compliant es falso si se añaden capacidades para un contenedor dado.
Por defecto: Los contenedores se ejecutan con un conjunto predeterminado de capacidades asignadas por el entorno de ejecución de contenedor.
K3s da excepciones a los siguientes pods, que son necesarios para operaciones regulares: - coredns, svclb-traefik
======
...
**Solución:**
Revisa el uso de capacidades en las aplicaciones que se ejecutan en tu clúster. Donde un espacio de nombres contiene aplicaciones que no requieren ninguna capacidad de Linux para operar, considera añadir un PSP que prohíba la admisión de contenedores que no eliminen todas las capacidades.

=== 5.2.11 Minimiza la admisión de contenedores de Windows HostProcess (Manual)

**Resultado:** WARN (ADVERTENCIA)

**Solución:**
Añade políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores que tengan `+.securityContext.windowsOptions.hostProcess+` establecido en `+true+`.
...
**Solución:**
Añade políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores con `+hostPath+` volúmenes.
...
**Solución:**
Añade políticas a cada espacio de nombres en el clúster que tenga cargas de trabajo de usuario para restringir la admisión de contenedores que utilizan `+hostPort+` secciones.
...
**Solución:**
Si el plugin CNI en uso no soporta políticas de red, se debe considerar el uso de un plugin diferente o encontrar un mecanismo alternativo para restringir el tráfico en el clúster de Kubernetes.

=== 5.3.2 Asegúrate de que todos los Espacios de Nombres tengan definidas NetworkPolicies (Manual)

**Resultado:** WARN (ADVERTENCIA)

**Solución:**
Sigue la documentación y crea objetos NetworkPolicy según los necesites.

== 5.4 Gestión de Secretos

=== 5.4.1 Prefiere usar Secretos como archivos en lugar de Secretos como variables de entorno (Manual)

**Resultado:** WARN (ADVERTENCIA)

**Solución:**
Si es posible, reescribe el código de la aplicación para leer Secretos de archivos secretos montados, en lugar de variables de entorno.
...
**Solución:**
Consulta las opciones de gestión de secretos ofrecidas por tu proveedor de nube o una solución de gestión de secretos de terceros.
...
**Solución:**
Sigue la documentación de Kubernetes y aplica SecurityContexts a tus Pods. Para una lista sugerida de SecurityContexts, puedes consultar el CIS Security Benchmark para Contenedores Docker.
...
**Solución:**
Asegúrate de que se creen espacios de nombres para permitir la segregación adecuada de los recursos de Kubernetes y que todos los nuevos recursos se creen en un espacio de nombres específico.
