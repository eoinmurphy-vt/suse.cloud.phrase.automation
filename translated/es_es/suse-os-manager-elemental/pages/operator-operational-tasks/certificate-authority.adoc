= Verificación de la autoridad de certificación
:revdate: 2025-09-08
:page-revdate: {revdate}

`+elemental-register+` y `+elemental-system-agent+` se basan en la https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/resources/update-rancher-certificate.html[configuración de la Autoridad de Certificación de] para verificar la URL xref:references/machineregistration-reference.adoc#_config_elemental_registration[MachineRegistration], y para vigilar remotamente los planes.

Dependiendo de si la autoridad de certificación es privada o pública, es posible que desee indicar al agente que aplique la verificación de CA en `+strict+` o que utilice el almacén de confianza del sistema en su lugar.

Desde Rancher `+2.9+`, la configuración global https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/references/tls-settings.html#_agent_tls_enforcement[agent-tls-mode] también se aplicará a la instalación de agentes Elementales. Tenga en cuenta que si cambia la configuración de `+agent-tls-mode+`, será necesario xref:node-operational-tasks/reset.adoc[reiniciar] las máquinas Elementales para que se aplique la configuración.

== Ciclo de vida del certificado de CA privada

Cuando se utiliza una CA privada, la recomendación es asegurarse siempre de que la misma CA se utiliza también para Rancher. Elemental hará uso de la `+cacerts+`, al incluir el certificado de CA en el que confiarán los agentes. Es el mismo valor que aparece en la URL `+https://my.rancher.example/cacerts+`.

Tenga en cuenta, sin embargo, que no será posible actualizar este valor después de que se haya instalado una máquina Elemental. Reemplazar el certificado CA en Rancher puede llevar a que las máquinas Elemental no puedan reconectarse a Rancher y operar normalmente, cuando el `+agent-tls-mode+` está configurado en `+strict+`.

Por esta razón, la recomendación es utilizar la configuración `+agent-tls-mode: system-store+` en su lugar y gestionar el ciclo de vida de los certificados de CA en las máquinas Elemental directamente, cuando una Autoridad de Certificación privada está en uso.

El certificado de CA puede instalarse directamente en xref:operator-operational-tasks/custom-images.adoc[imágenes de SO personalizadas] o pasarse como una configuración de inicio en la nube en los recursos de Elemental. Por ejemplo, el certificado de CA inicial puede incluirse en xref:references/machineregistration-reference.adoc#_config_cloud_config[MachineRegistration] `+cloud-config+`:

[,yaml]
----
apiVersion: elemental.cattle.io/v1beta1
kind: MachineRegistration
metadata:
  name: fire-nodes
  namespace: fleet-default
spec:
  config:
    cloud-config:
      write_files:
        - path: /etc/pki/trust/anchors/rancher-ca.pem
          permission: 0444
          content: |-
            -----BEGIN CERTIFICATE-----
            
            12xVJHQTSN8XWh0++9jNG0eSMe75
            -----END CERTIFICATE-----
      runcmd:
        - update-ca-certificates
----

Antes de reemplazar el CA cert en Rancher, el nuevo CA cert puede ser incluido en las máquinas Elemental actualizándolas. La *nueva* CA cert puede configurarse en xref:references/managedosimage-reference.adoc#_cloudconfig[ManagedOSImage] `+cloudConfig+`:

[,yaml]
----
apiVersion: elemental.cattle.io/v1beta1
kind: ManagedOSImage
metadata:
  name: ca-cert-upgrade
  namespace: fleet-default
spec:
  # The cloudConfig will be applied after node reboot
  cloudConfig:
    write_files:
      - path: /etc/pki/trust/anchors/rancher-ca-new.pem
        permission: 0444
        content: |-
          -----BEGIN CERTIFICATE-----
          
          -----END CERTIFICATE-----
    runcmd:
      - update-ca-certificates
  osImage: "registry.suse.com/suse/sl-micro/6.0/baremetal-os-container:2.1.2-3.59"
  clusterTargets:
    - clusterName: volcano
  upgradeContainer:
    envs:
      # Use FORCE to force an upgrade.
      # This is convenient when the `+osImage+` is the same, and only the `+cloudConfig+` changed.
      - name: FORCE
        value: "false"
----
