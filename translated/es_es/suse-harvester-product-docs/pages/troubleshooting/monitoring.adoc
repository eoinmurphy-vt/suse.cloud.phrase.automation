...
Cuando el {harvester-product-name} Panel de control no muestra ninguna métrica de monitorización, puede deberse a las siguientes razones.

=== La monitorización es inutilizable debido a que el Pod está atascado en estado `+Terminating+`

{harvester-product-name}Los pods de monitorización se despliegan aleatoriamente en los nodos del clúster. Cuando el nodo que aloja los pods se cae accidentalmente, los pods relacionados pueden quedar atascados en el estado `+Terminating+`, lo que hace que la monitorización sea inutilizable desde la WebUI.
...
Espera unos minutos para que se creen y preparen los nuevos pods para que el panel de monitorización sea utilizable de nuevo.
...
{harvester-product-name} integra {longhorn-product-name} como el proveedor de almacenamiento predeterminado.

{harvester-product-name}La monitorización utiliza Volumen Persistente (PV) para almacenar datos en ejecución. Cuando un clúster ha estado funcionando durante un cierto tiempo, el Volumen Persistente puede necesitar expandir su tamaño.

Para información sobre cómo aumentar el tamaño del volumen, consulta https://documentation.suse.com/cloudnative/storage/1.8/en/volumes/volume-expansion.html[Expansión de Volumen] en la documentación de {longhorn-product-name}.

=== Ver Volumen

==== Desde la UI embebida de {longhorn-product-name}

Accede a la UI embebida de {longhorn-product-name} según xref:./cluster.adoc#_access_embedded_rancher_and_longhorn_dashboards[este documento].

La vista predeterminada del {longhorn-product-name} panel.
...
En la interfaz de usuario de {longhorn-product-name}, el volumen relacionado se convierte en `+Detached+`. Haz clic en el icono en la columna `+Operation+` y selecciona `+Expand Volume+`.

image::troubleshooting/4-select-volume-to-expand.png[]

Introduce un nuevo tamaño y {longhorn-product-name} expandirá el volumen a este tamaño.
...
Puedes encontrar esto cuando instalas {harvester-product-name} v1.3.0 o posterior en un clúster con el xref:../installation-setup/requirements.adoc#_hardware_requirements[tamaño mínimo de disco requerido].

=== Reproducir pasos

. Instala el {harvester-product-name} clúster.
. Habilita el `+rancher-monitoring+` xref:../add-ons/add-ons.adoc[complemento], observarás:

* El POD `+prometheus-rancher-monitoring-prometheus-0+` en el espacio de nombres `+cattle-monitoring-system+` no puede iniciarse debido a que el PVC adjunto ha fallado.
+
----
  $ kubectl get pods -n cattle-monitoring-system
  NAME                                                     READY   STATUS      RESTARTS   AGE
...
. Desactiva el complemento `+rancher-monitoring+` si ya lo has habilitado.
+
Todos los pods en `+cattle-monitoring-system+` han sido eliminados, pero los PVCs se mantienen. Para obtener más información, consulta [Complementos].
+
----
 $ kubectl get pods -n cattle-monitoring-system
...
. Elimina el PVC llamado `+prometheus+`, pero conserva el PVC llamado `+alertmanager+`.
+
----
...
. En la pantalla de *Complementos* de la interfaz de usuario {harvester-product-name}, selecciona *⋮* (icono de menú) y luego selecciona *Editar YAML*.
+
image::troubleshooting/edit-rancher-monitoring.png[]

. Como se indica a continuación, cambia las dos ocurrencias del número `+50+` a `+30+` bajo prometheusSpec, y luego guarda. La función `+prometheus+` utilizará un disco de 30GiB para almacenar datos.
+
image::troubleshooting/edit-rancher-monitoring-yaml.png[]
+
Alternativamente, puedes usar `+kubectl+` para editar el objeto.
+
...
. Habilita el complemento `+rancher-monitoring+` y espera unos minutos.
. Todos los pods se han desplegado con éxito, y la función `+rancher-monitoring+` está disponible.
+
----
 $ kubectl get pods -n cattle-monitoring-system
 NAME                                                     READY   STATUS      RESTARTS   AGE
...
Cuando existe el problema y tú xref:../upgrades/upgrades.adoc#_start_an_upgrade[inicias una actualización], {harvester-product-name} puede devolver el siguiente mensaje de error: `+admission webhook "validator.harvesterhci.io" denied the request: managed chart rancher-monitoring-crd is not ready, please wait for it to be ready+`.
...
. Aplica un parche al objeto ClusterRole `+rancher-monitoring-crd-manager+` para añadir la operación `+update+`.
+
[,console]
----
...
. Aplica un parche a los objetos marcados como `+missing+` para añadir las anotaciones y etiquetas requeridas.
+
[,console]
----
...
. Revisa el `+rancher-monitoring-crd+` objeto ManagedChart.
+
Después de unos segundos, el estado del `+rancher-monitoring-crd+` objeto ManagedChart cambia a `+Ready+`.
+
[,console]
----
...
----
+
Además, los indicadores de error ya no se muestran para los objetos descendentes.
+
[,console]
----
 $ kubectl bundle -A
...
Los registros del pod `+prometheus+` contienen el mensaje `+level=warn msg="Received SIGTERM, exiting gracefully..."+`.
...
Los registros del pod Harvester ('ampliación harvester-system/harvester') indican que el intento de cambiar `+storage-network+` la configuración fue bloqueado.
...
Cuando realizas cambios en la configuración xref:networking/storage-network.adoc#_storage_network_setting[storage-network], el controlador {harvester-product-name} espera a que los volúmenes adjuntos sean desasignados antes de aplicar los cambios. Además, el controlador termina automáticamente los pods relacionados con Prometheus, Alertmanager y Grafana porque esos pods utilizan volúmenes para almacenar datos.
...
* Un usuario o el `+monitoring-operator+` intenta habilitar el complemento `+rancher-monitoring+`.
* El controlador de Harvester termina los pods.

=== Solución

. Deshabilita el complemento `+rancher-monitoring+`.

. Verifica si la configuración xref:networking/storage-network.adoc#_storage_network_setting[storage-network] está habilitada o deshabilitada.

. Revisa si hay indicadores de error en los registros de los pods de Harvester. Si los volúmenes siguen adjuntos, detén las máquinas virtuales relacionadas hasta que no aparezcan errores después del mensaje `+storage network change+`.

. Habilita el complemento `+rancher-monitoring+`.

=== Problema relacionado

https://github.com/harvester/harvester/issues/8565[#8565]

== {harvester-product-name}La interfaz de usuario deja de informar métricas de máquinas virtuales después de la actualización

=== Descripción del problema

Después de una actualización, {harvester-product-name}la interfaz de usuario deja de informar métricas de máquinas virtuales mientras que las métricas del clúster siguen disponibles. Deshabilitar y volver a habilitar el complemento `+rancher-monitoring+` no resuelve el problema.
...
----
+
Cuando la salida está vacía, el problema existe en tu entorno.

. Verifica que los campos `+monitorAccount+` y `+monitorNamespace+` existan.
+
[,console]
----
...
. Elimina todos los objetos ConfigMap (`+kubevirt-install-strategy-*+`).

. Despliega la `+virt-operator+` ampliación.
+
Kubevirt recrea el ConfigMap.
+
[,console]
----
