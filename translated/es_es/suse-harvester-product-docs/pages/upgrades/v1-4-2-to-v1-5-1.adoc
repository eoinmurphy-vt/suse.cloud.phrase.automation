= Actualiza de v1.4.2 o v1.4.3 a v1.5.1
:revdate: 2025-07-07
:page-revdate: {revdate}

== Información general

Un *botón de actualizar versión* aparece en la pantalla *Panel* siempre que una nueva {harvester-product-name} versión a la que puedes actualizar esté disponible. Para más información, consulta xref:./upgrades.adoc#_start_an_upgrade[Iniciar una actualización de versión].

Puedes actualizar directamente de v1.4.2 a v1.5.1 porque {harvester-product-name} permite un máximo de una actualización menor de versión para los componentes subyacentes. {harvester-product-name} v1.4.2 y v1.4.3 utilizan la misma versión menor de {rke2-product-name} (v1.31), mientras que {harvester-product-name} v1.5.0 y v1.5.1 utilizan la siguiente versión menor (v1.32).

Para información sobre la actualización de versión de {harvester-product-name} en entornos aislados, consulta xref:./upgrades.adoc#_prepare_an_air_gapped_upgrade[Preparar una actualización de versión en entorno aislado].
...
=== 1. La actualización en entorno aislado se queda atascada con el error `+ImagePullBackOff+` en los pods de Fluentd y Fluent Bit
...
----
+
El proceso de actualización de versión debería continuar después de que las imágenes estén precargadas.

- (No recomendado) Reinicia el proceso de actualización de versión con el registro deshabilitado. Asegúrate de que la casilla *Habilitar Registro* en el diálogo de *Actualización* no esté seleccionada.

Problema relacionado: https://github.com/harvester/harvester/issues/7955[#7955]

=== 2. Las máquinas virtuales que utilizan volúmenes RWX migrables se reinician inesperadamente.
...
La solución alternativa es deshabilitar la configuración https://documentation.suse.com/cloudnative/storage/1.8/en/longhorn-system/settings.html#_automatically_delete_workload_pod_when_the_volume_is_detached_unexpectedly[Eliminar Automáticamente el Pod de Carga de Trabajo Cuando el Volumen se Desconecta Inesperadamente] en la UI {longhorn-product-name} antes de iniciar la actualización de versión. Debes habilitar la configuración nuevamente una vez que se complete la actualización de versión.
