...
[CAUTION]
====
Cuando la vinculación de métricas no especifica una agregación, SUSE Observability utilizará automáticamente la agregación `+last_over_time+` para reducir el número de puntos de datos para un gráfico. Consulta también xref:/use/metrics/k8s-writing-promql-for-charts.adoc#_why[¿Por qué es esto necesario?] para una explicación.
====
...
Pruébalo tú mismo en el https://play.stackstate.com/#_/metrics?promql=last_over_time%28container_cpu_usage%7Bnamespace%3D%22sock_shop%22%2C%20pod_name%3D~%22carts.%2A%22%7D%5B%24%7B%5F%5Finterval%7D%5D%29%20%2F%201000000000&timeRange=LAST_7_DAYS[SUSE Observability playground].
...
Por lo tanto, `+${__interval}+` nunca se reducirá a menos de dos veces el intervalo de raspado por defecto (el intervalo de raspado por defecto es de 30 segundos) del agente de SUSE Observability.
