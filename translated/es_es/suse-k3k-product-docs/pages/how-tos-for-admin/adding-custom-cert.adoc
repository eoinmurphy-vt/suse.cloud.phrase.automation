= Añadiendo certificados personalizados a k3s desde el clúster k3k
:revdate: 2025-09-16
:page-revdate: {revdate}

== Generar y añadir los certificados personalizados

. Utiliza el https://github.com/k3s-io/k3s/blob/master/contrib/util/generate-custom-ca-certs.sh[guion de certificado personalizado] para generar certificados.

. Después de generar los certificados, crea un directorio local ya que el directorio predeterminado no será accesible. Por ejemplo, `+k3k/custom-certs+`.

. Copia los certificados generados al directorio - `+$ sudo cp -r /var/lib/rancher/k3s/server/tls k3k/custom-certs/+`

. Utiliza la vía `+/var/lib/rancher/k3s/server/tls k3k/custom-certs/+` para crear los clústeres virtuales en 2 modos:
+
* Modo virtual 
* Modo compartido
...
. Generar certificados personalizados a través del guion:
+
----
...
. Verifica los certificados en el pod del servidor:
+
----
...
. Copia y verifica los certificados en el directorio local:
+
----
...
. Verifica el clúster k3k recién creado:
+
----
