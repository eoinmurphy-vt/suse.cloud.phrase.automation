...
[IMPORTANT]
====
Este recurso está em pré-produção e não foi lançado. Está disponível apenas se a variável de ambiente `+STS_EXPERIMENTAL_STACKPACK+` estiver definida. Alterações incompatíveis podem ocorrer.
====

== Meta

Como desenvolvedor, você deseja integrar sua tecnologia com {stackstate-product-name} ao construir um StackPack personalizado. Este tutorial o guia pelo processo usando o StackPack CLI.
...
O subcomando `+scaffold+` cria uma nova estrutura de projeto StackPack a partir de modelos configuráveis, agilizando o processo de configuração inicial para o desenvolvimento do StackPack. O comando suporta tanto modelos de diretório local quanto modelos hospedados no GitHub.
...
- Para usar um modelo específico do GitHub (padrão: https://github.com/StackVista/stackpack-templates[stackvista/stackpack-templates]):
+
[source,bash]
----
...
- Edite `+stackpack.yaml+` e outros arquivos para definir sua lógica de integração.
  ...
  * xref:setup/custom-integrations/metric-bindings/index.adoc[Adicionando Vínculos de Métrica]

[NOTE]
====
O modelo padrão: https://github.com/StackVista/stackpack-templates[stackvista/stackpack-templates] é um ótimo ponto de partida, pois contém exemplos para a maioria dos pontos de extensão, como Monitores e Vínculos de Métricas.
====

=== Teste seu StackPack

O subcomando `+test+` que simplifica o fluxo de trabalho de desenvolvimento do StackPack, automatizando a sequência pacote → upload → instalação/atualização, com gerenciamento automático de versões para iterações de teste.

- Teste rapidamente seu StackPack em um ambiente de pré-produção (requer uma instância SUSE Observability em execução):
+
[source,bash]
----
...
- Itere sobre seu StackPack, usando o comando `+test+` para feedback rápido, faça alterações e observe que o processo de instalação/atualização foi executado com sucesso. Revise a topologia e a telemetria ingeridas na interface do usuário do SUSE Observability UI.
