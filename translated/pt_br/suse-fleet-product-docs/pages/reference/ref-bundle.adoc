...
Os pacotes são criados automaticamente por {product_name} quando um `+GitRepo+` é criado.

O conteúdo do recurso corresponde ao xref:reference/ref-crds.adoc#_bundlespec[BundleSpec].
Para mais informações sobre como usar o recurso de pacote xref:how-tos-for-users/bundle-add.adoc[Criar um Recurso de Pacote].
