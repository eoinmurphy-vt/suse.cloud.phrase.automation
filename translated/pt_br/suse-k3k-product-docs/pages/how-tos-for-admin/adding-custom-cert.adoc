= Adicionando certificados personalizados ao k3s do cluster k3k
:revdate: 2025-09-16
:page-revdate: {revdate}

== Gerar e adicionar os certificados personalizados

. Use o https://github.com/k3s-io/k3s/blob/master/contrib/util/generate-custom-ca-certs.sh[script de certificado personalizado] para gerar certificados.

. Após gerar os certificados, crie um diretório local, pois o diretório padrão não será acessível. Por exemplo, `+k3k/custom-certs+`.

. Copie os certificados gerados para o diretório - `+$ sudo cp -r /var/lib/rancher/k3s/server/tls k3k/custom-certs/+`

. Use o caminho `+/var/lib/rancher/k3s/server/tls k3k/custom-certs/+` para criar os clusters virtuais em 2 modos:
+
* Modo virtual 
* Modo compartilhado
...
. Gere certificados personalizados via script:
+
----
...
. Verifique os certificados no pod do servidor:
+
----
...
. Copie e verifique os certificados no diretório local:
+
----
...
. Verifique o novo cluster k3k criado:
+
----
