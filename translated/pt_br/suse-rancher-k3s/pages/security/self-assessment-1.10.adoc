...
Este guia é específico para a linha de lançamento *v1.28-v1.31* do K3s e a versão *v1.10* do Benchmark do CIS Kubernetes.

Para mais informações sobre cada controle, incluindo descrições detalhadas e remediações para testes que falham, você pode consultar a seção correspondente do Benchmark do CIS Kubernetes v1.9. Você pode baixar o benchmark, após criar uma conta gratuita, no https://www.cisecurity.org/benchmark/kubernetes[Centro para Segurança da Internet (CIS)].

=== Metodologia de testes de controles

Cada controle no Benchmark do CIS Kubernetes foi avaliado em um cluster K3s que foi configurado de acordo com o guia de proteção acompanhante.
...
=== 1.1.9 Garantir que as permissões do arquivo da interface de rede de contêiner estejam definidas como 600 ou mais restritivas (Automatizado)
...
*Resultado Esperado:* as permissões devem ser 700 ou mais restritivas
...
Para o K3s, o etcd está embutido dentro do processo k3s. Não há um processo etcd separado.
...
Se esta verificação falhar, edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml e remova qualquer coisa semelhante ao abaixo.
...
Para habilitar esta flag, edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml como abaixo.
...
Se por algum motivo você precisar fornecer seu próprio certificado CA, considere usar a ferramenta de linha de comando do k3s.
...
Se esta verificação falhar, edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml, assegurando que você não está sobrescrevendo o authorization-mode.
======
...
Por padrão, o K3s não configura o --habilitar-plugins-de-admissão para AlwaysAdmit.
Se esta verificação falhar, edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml, remova quaisquer linhas como abaixo.
...
=== 1.2.12 Assegure-se de que o plugin de controle de admissão ServiceAccount esteja configurado (Automatizado)
...
Por padrão, o K3s não configura o --desabilitar-plugins-de-admissão para nada.
...
=== 1.2.13 Assegure-se de que o plugin de controle de admissão NamespaceLifecycle esteja configurado (Automatizado)
...
=== 1.2.14 Assegure-se de que o plugin de controle de admissão NodeRestriction esteja configurado (Automatizado)
...
*Resultado Esperado:* '--enable-admission-plugins' contém 'NodeRestriction'
...
Edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml e defina o parâmetro audit-log-path para um caminho e um arquivo adequados onde você gostaria que os logs de auditoria fossem gravados, por exemplo,
...
Edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml no nó de controle e defina o parâmetro audit-log-maxage como 30 ou como um número apropriado de dias, por exemplo,
...
Edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml no nó de controle e defina o parâmetro audit-log-maxbackup como 10 ou como um valor apropriado. Por exemplo,
...
Edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml no nó de controle e defina o parâmetro audit-log-maxsize como um tamanho apropriado em MB. Por exemplo,
...
O K3s gera automaticamente e define os arquivos de certificado e chave etcd.
...
=== 1.2.25 Certifique-se de que o argumento --client-ca-file esteja definido conforme apropriado (Automatizado)
...
Edite o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml no nó de controle e defina o parâmetro abaixo.
secrets-encryption: true
A criptografia de segredos pode ser gerenciada com a ferramenta de linha de comando k3s secrets-encrypt.
...
*Resultado Esperado:* 'provedor' contém elementos válidos de 'aescbc,kms,secretbox'
...
Por padrão, o kube-apiserver do K3s cumpre este teste. Mudanças nesses valores podem causar regressão, portanto, assegure-se de que todos os clientes do apiserver suportem a nova configuração TLS antes de aplicá-la em implantações de produção.
...
Se essa verificação falhar, remova qualquer configuração personalizada em torno de `+tls-cipher-suites+` ou atualize o arquivo /etc/rancher/k3s/config.yaml para corresponder ao padrão, adicionando o seguinte:
...
Por padrão, o K3s fornece automaticamente o arquivo root-ca-file.
...
Ao executar com embedded-etcd, o K3s gera arquivos de certificado e chave para o etcd.
Estes estão localizados em /var/lib/rancher/k3s/server/tls/etcd/.
Se esta verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/k3s/server/db/etcd/config não foi modificado para usar arquivos de certificado e chave personalizados.
======
...
Se esta verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/k3s/server/db/etcd/config não foi modificado para desativar a autenticação de certificado do cliente.
======

=== 2.3 Certifique-se de que o argumento --auto-tls não está definido como verdadeiro (Manual)
...
Se esta verificação falhar, edite o arquivo de especificação do pod etcd /var/lib/rancher/k3s/server/db/etcd/config no nó mestre e remova o parâmetro --auto-tls ou defina-o como falso. client-transport-security: auto-tls: falso
======
...
Se estiver executando com sqlite ou um banco de dados externo, as verificações do etcd não são aplicáveis.
Ao executar com embedded-etcd, o K3s gera arquivos de certificado e chave do peer para o etcd.
Estes estão localizados em /var/lib/rancher/k3s/server/tls/etcd/.
Se esta verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/k3s/server/db/etcd/config não foi modificado para usar arquivos de certificado e chave do peer personalizados.
======
...
Se esta verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/k3s/server/db/etcd/config não foi modificado para desativar a autenticação de certificado do cliente do peer.
======
...
Se essa verificação falhar, edite o arquivo de especificação do pod etcd /var/lib/rancher/k3s/server/db/etcd/config no nó mestre e remova o parâmetro --peer-auto-tls ou defina-o como false.
peer-transport-security:
  auto-tls: false
======
...
Se essa verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/k3s/server/db/etcd/config não foi modificado para usar uma autoridade certificadora compartilhada.
======
...
*Resultado Esperado:* As permissões devem estar definidas como 600, ou serem mais restritivas.
...
=== 4.2.1 Certifique-se de que o argumento --anonymous-auth esteja definido como falso (Automatizado)
...
Por padrão, o K3s define o --anonymous-auth como falso. Se você definiu isso como um valor diferente, deve configurá-lo de volta para falso. Se estiver usando o arquivo de configuração do K3s /etc/rancher/k3s/config.yaml, remova quaisquer linhas semelhantes às abaixo.
...
=== 4.2.7 Assegure-se de que o argumento --hostname-override não esteja definido (Automatizado)
...
=== 4.2.8 Assegure-se de que o argumento eventRecordQPS esteja definido em um nível que assegure a captura apropriada de eventos (Manual)
...
=== 4.2.9 Assegure-se de que os argumentos --tls-cert-file e --tls-private-key-file estejam definidos conforme apropriado (Automatizado)
...
=== 4.2.10 Assegure-se de que o argumento --rotate-certificates não esteja definido como false (Automatizado)
...
Se estiver usando a linha de comando, remova a flag K3s --kubelet-arg="rotate-certificates".
Com base no seu sistema, reinicie o serviço k3s. Por exemplo,
systemctl restart k3s.service
======
...
Por padrão, o K3s não define o feature gate RotateKubeletServerCertificate.
Se você ativou esse feature gate, deve removê-lo.
Se estiver usando o arquivo de configuração K3s /etc/rancher/k3s/config.yaml, remova qualquer parâmetro feature-gate=RotateKubeletServerCertificate.
Se estiver usando a linha de comando, remova a flag K3s --kubelet-arg="feature-gate=RotateKubeletServerCertificate".
Com base no seu sistema, reinicie o serviço k3s. Por exemplo,
systemctl restart k3s.service
======
...
Se estiver usando a linha de comando, adicione a flag K3s --kubelet-arg="tls-cipher-suites=<same values as above>" +
Com base no seu sistema, reinicie o serviço k3s. Por exemplo,
systemctl restart k3s.service
======
...
=== 4.3.1 Certifique-se de que o serviço de métricas kube-proxy esteja vinculado ao host local (Automatizado)
...
Identifique todos os clusterrolebindings para o papel de cluster-admin. Verifique se eles estão sendo usados e se precisam desse papel ou se poderiam usar um papel com menos privilégios. K3s dá exceções aos clusterrolebindings helm-kube-system-traefik e helm-kube-system-traefik-crd, pois são necessários para a instalação do traefik no namespace kube-system para operações regulares.
Sempre que possível, primeiro vincule os usuários a um papel de menor privilégio e, em seguida, remova o clusterrolebinding para o papel cluster-admin:
...
*Remediação:*
Sempre que possível, remova o acesso get, list e watch a objetos Secret no cluster.
...
K3s dá exceções para as seguintes funções de cluster, que são necessárias para operações regulares:
...
=== 5.1.5 Assegure-se de que contas de serviço padrão não estejam sendo usadas ativamente. (Automatizado)
...
K3s faz uma exceção para a conta de serviço padrão no namespace kube-system.
...
* A ServiceAccount tem automountServiceAccountToken: true (não definido) e o Pod tem automountServiceAccountToken: false. K3s dá exceções às seguintes contas de serviço, que são necessárias para operações regulares:
...
=== 5.1.8 Limitar o uso das permissões Bind, Impersonate e Escalate no cluster Kubernetes (Manual)
...
=== 5.1.10 Minimizar o acesso ao sub-recurso proxy de nós (Manual)
...
=== 5.1.11 Minimizar o acesso ao sub-recurso de aprovação de objetos certificatesigningrequests (Manual)

*Resultado:* AVISO

*Remediação:*
Sempre que possível, remova o acesso ao sub-recurso de aprovação de objetos certificatesigningrequests.

=== 5.1.12 Minimizar o acesso a objetos de configuração de webhook (Manual)
...
*Remediação:*
Sempre que possível, remova o acesso ao sub-recurso token dos objetos serviceaccount.
...
Auditoria: a auditoria lista todos os contêineres dos pods para recuperar seu valor .securityContext.privileged.
...
=== 5.2.3 Minimize a admissão de contêineres que desejam compartilhar o namespace de ID de processo do host (Manual)
...
Condição: is_compliant é falso se o spec.hostPID de cada Pod estiver definido como `+true+`.
Padrão: por padrão, não há restrições na criação de contêineres com hostPID.
======
...
Condição: is_compliant é falso se o spec.hostIPC de cada Pod estiver definido como `+true+`.
Padrão: por padrão, não há restrições na criação de contêineres com hostIPC.
======
...
Condição: is_compliant é falso se o spec.hostNetwork de cada Pod estiver definido como `+true+`.
Padrão: por padrão, não há restrições na criação de contêineres com hostNetwork.
======
...
Adicione políticas a cada namespace no cluster que possui cargas de trabalho de usuários para restringir a admissão de contêineres com `+.securityContext.allowPrivilegeEscalation+` definido como `+true+`.
Auditoria: a auditoria recupera o(s) contêiner(es) de cada Pod `+.securityContext.allowPrivilegeEscalation+`.
Condição: is_compliant é falso se o `+.securityContext.allowPrivilegeEscalation+` do contêiner estiver definido como `+true+`.
Padrão: Se não estiver definido, a elevação de privilégios é permitida (o padrão é verdadeiro). No entanto, se PSP/PSA for usado com um perfil `+restricted+`, a elevação de privilégios é explicitamente proibida, a menos que configurado de outra forma.
======
...
*Remediação:*
Crie uma política para cada namespace no cluster, garantindo que `+MustRunAsNonRoot+` ou `+MustRunAs+` com a faixa de UIDs não incluindo 0 estejam definidos.
...
Certifique-se de que `+allowedCapabilities+` não esteja presente nas políticas do cluster, a menos que esteja definido como um array vazio.
Auditoria: a auditoria recupera as capacidades adicionadas de cada contêiner de Pod.
Condição: is_compliant é falso se capacidades adicionais forem adicionadas para um determinado contêiner.
Padrão: Os contêineres são executados com um conjunto padrão de capacidades conforme atribuído pelo tempo de execução do contêiner.
K3s dá exceções aos seguintes pods, que são necessários para operações regulares:
...
*Remediação:*
Revise o uso de capacidades em aplicativos que estão sendo executados no seu cluster. Onde um namespace contém aplicações que não requerem nenhuma capacidade do Linux para operar, considere adicionar um PSP que proíba a admissão de contêineres que não descartem todas as capacidades.
...
*Remediação:*
Adicione políticas a cada namespace no cluster que possui cargas de trabalho de usuários para restringir a admissão de contêineres que utilizam `+hostPort+` seções.
...
=== 5.7.2 Assegure-se de que o arquivo de controle seccomp esteja definido como docker/default nas definições do seu Pod (Manual)

*Resultado:* AVISO

*Remediação:*
Use `+securityContext+` para habilitar o arquivo de controle seccomp docker/default nas definições do seu pod.
Um exemplo é o seguinte: +
  ContextoDeSegurança: +
    arquivo de controle seccomp: +
      tipo: PadrãoDeExecução

=== 5.7.3 Aplique o ContextoDeSegurança aos seus Pods e Contêineres (Manual)

*Resultado:* AVISO

*Remediação:*
Siga a documentação do Kubernetes e aplique ContextosDeSegurança aos seus Pods. Para uma lista sugerida de ContextosDeSegurança, você pode consultar o Benchmark de Segurança CIS para Contêineres Docker.
...
*Remediação:*
Assegure-se de que os namespaces sejam criados para permitir a segregação apropriada dos recursos do Kubernetes e que todos os novos recursos sejam criados em um namespace específico.
