= Guia de autoavaliação do CIS 1.10
...
Este guia é específico para a linha de versão *v1.28-v1.31* do K3s e a versão *v1.10* do CIS Kubernetes Benchmark.
...
O K3s oferece exceções para as seguintes funções de cluster, que são necessárias para operações regulares:
...
Condição: Pod is_compliant para true quando

* ServiceAccount é automountServiceAccountToken: false e Pod é automountServiceAccountToken: false ou notset
* ServiceAccount is automountServiceAccountToken: true notset e Pod is automountServiceAccountToken: false O K3s faz exceções às seguintes contas de serviço, que são necessárias para operações regulares:
...
O K3s oferece exceções para os pods a seguir, que são necessários para operações regulares:
