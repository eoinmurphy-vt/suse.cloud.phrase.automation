...
Para mais informações sobre o uso de certificados CA personalizados, consulte a xref:cli/certificate.adoc[`+k3s certificate+` documentação]. +
Para mais informações sobre como fazer backup do seu cluster, consulte a xref:datastore/backup-restore.adoc[Backup e Restauração] documentação.
...
.Portão de Versão
====
O suporte para o comando `+k3s token+` e a capacidade de juntar nós com tokens de bootstrap está disponível a partir das versões de 2023-02 (v1.26.2+k3s1, v1.25.7+k3s1, v1.24.11+k3s1, v1.23.17+k3s1).
====

K3s suporta tokens de bootstrap de agente https://kubernetes.io/docs/reference/access-authn-authz/bootstrap-tokens/[bootstrap tokens] gerados dinamicamente e que expiram automaticamente.
...
* O ciclo de vida dos tokens de bootstrap, usando o mesmo código de geração e validação que os `+kubeadm token+` tokens de bootstrap. Observe que ambas as CLIs são semelhantes.
