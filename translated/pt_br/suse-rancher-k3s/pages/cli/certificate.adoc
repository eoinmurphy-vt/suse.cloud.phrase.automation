...
Os certificados de cliente e servidor K3s são válidos por 365 dias a partir da data de emissão.
Quaisquer certificados que estejam expirados ou dentro de 120 dias de expiração são renovados automaticamente toda vez que o K3s é iniciado.
Essa renovação reutiliza as chaves existentes e estende a validade dos certificados existentes.
Se você deseja gerar novos certificados e chaves, em vez de estender a validade dos certificados existentes, use o subcomando xref:#_rotating_client_and_server_certificates[`+rotate+`] documentado abaixo.
...
Os certificados e chaves CA autoritativos são armazenados dentro da chave de inicialização do datastore, criptografados usando o xref:cli/token.adoc#_server[token do servidor] como a frase secreta PBKDF2 com AES256-GCM e HMAC-SHA1.
Cópias dos certificados e chaves CA são extraídas para o disco durante a inicialização do servidor K3s.
Qualquer servidor pode gerar certificados de entidade final para os nós à medida que se juntam ao cluster, e os controladores do Kubernetes https://kubernetes.io/docs/reference/access-authn-authz/certificate-signing-requests/[API de Certificados] podem emitir certificados adicionais em tempo de execução.

Para rotacionar certificados e chaves CA, use o comando `+k3s certificate rotate-ca+`.
O comando realiza verificações de integridade para confirmar que os certificados e chaves atualizados são utilizáveis.
Se os dados atualizados forem aceitáveis, a chave de inicialização criptografada do datastore é atualizada, e os novos certificados e chaves serão usados na próxima vez que o K3s iniciar.
Se problemas forem encontrados ao validar os certificados e chaves, um erro é relatado ao log do sistema e a operação é cancelada sem alterações.
...
Da mesma forma, os certificados de servidor emitidos por um cluster também são confiáveis em todos os outros clusters e por toda a infraestrutura ou clientes que confiam naquela CA raiz.
...
Um script de exemplo para pré-criar os certificados e chaves apropriados está disponível https://github.com/k3s-io/k3s/blob/main/contrib/util/generate-custom-ca-certs.sh[no repositório K3s em `+contrib/util/generate-custom-ca-certs.sh+`].
Este script deve ser executado antes de iniciar o K3s pela primeira vez e criará um conjunto completo de certificados CA de entidade final assinados por certificados CA raiz e intermediários comuns.
Se você tiver uma CA raiz ou intermediária existente, este script pode ser usado (ou usado como ponto de partida) para criar os certificados CA corretos para provisionar um cluster K3s com PKI enraizada em uma autoridade existente.
...
_// nota: arquivos etcd são necessários mesmo que o etcd embutido não esteja em uso._
* `+etcd/peer-ca.crt+`
* `+etcd/peer-ca.key+`
* `+etcd/server-ca.crt+`
* `+etcd/server-ca.key+` +
_// nota: Esta é a chave privada usada para assinar tokens de conta de serviço. Não possui um certificado correspondente._
* `+service.key+`
...
.Importante
====
Se você quiser assinar os certificados CA do cluster com uma CA raiz existente usando o script de exemplo, deve colocar os arquivos raiz e intermediários no diretório de destino antes de executar o script.
...
Se o script gerou arquivos CA raiz e/ou intermediários, você deve fazer backup desses arquivos para que possam ser reutilizados caso seja necessário rotacionar os certificados CA em uma data posterior.
...
Os arquivos atualizados devem ser organizados em um diretório temporário, carregados no datastore e o K3s deve ser reiniciado em todos os nós para usar os certificados atualizados.
...
Se o comando `+rotate-ca+` retornar um erro, verifique o log do serviço em busca de erros.
Se o comando for concluído com sucesso, reinicie o K3s em todos os nós do cluster - servidores primeiro, depois agentes.

Se você usou a opção `+--force+` ou alterou o CA raiz, certifique-se de que quaisquer nós que foram unidos com um xref:cli/token.adoc#_secure[token seguro] sejam reconfigurados para usar o novo valor do token, antes de serem reiniciados.
O token pode ser armazenado em um `+.env+` arquivo, unidade systemd ou config.yaml, dependendo de como o nó foi configurado durante a instalação inicial.
...
Se o cluster foi iniciado com certificados CA autoassinados padrão, a rotação será disruptiva. Todos os nós que foram adicionados com um xref:cli/token.adoc#_secure[token seguro] precisarão ser reconfigurados para confiar no novo hash CA.
...
Um script de exemplo para criar certificados CA e chaves atualizados cruzados pelos CAs existentes está disponível https://github.com/k3s-io/k3s/blob/main/contrib/util/rotate-default-ca-certs.sh[no repositório K3s em `+contrib/util/rotate-default-ca-certs.sh+`].
...
Certifique-se de que quaisquer nós que foram adicionados com um xref:cli/token.adoc#_secure[token seguro], incluindo outros nós de servidor, sejam reconfigurados para usar o novo valor do token antes de serem reiniciados.
O token pode ser armazenado em um `+.env+` arquivo, unidade systemd ou config.yaml, dependendo de como o nó foi configurado durante a instalação inicial.
...
Ela pode ser rotacionada independentemente das CAs do cluster usando o `+k3s certificate rotate-ca+` para instalar apenas um arquivo `+service.key+` atualizado que inclua tanto as chaves novas quanto as antigas.
...
É normal ver avisos para arquivos que não estão sendo atualizados. Se o comando `+rotate-ca+` retornar um erro, verifique o log do serviço em busca de erros.
Se o comando for concluído com sucesso, reinicie o K3s em todos os servidores do cluster. Não é necessário reiniciar agentes ou reiniciar quaisquer pods.
