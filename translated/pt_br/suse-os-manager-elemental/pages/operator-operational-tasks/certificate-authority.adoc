= Verificação de autoridade de certificação
:revdate: 2025-09-08
:page-revdate: {revdate}

O `+elemental-register+` e o `+elemental-system-agent+` dependem da Rancher https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/resources/update-rancher-certificate.html[configuração da Autoridade de Certificação do] para verificar o URL xref:references/machineregistration-reference.adoc#_config_elemental_registration[de MachineRegistration] e para observar remotamente os planos.

Dependendo do fato de a Autoridade de Certificação ser privada ou pública, talvez você queira instruir o agente a impor a verificação da `+strict+` CA ou a usar o armazenamento de confiança do sistema.

No Rancher `+2.9+`, a configuração global https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/references/tls-settings.html#_agent_tls_enforcement[agent-tls-mode] também se aplicará à instalação de agentes Elemental. Observe que, se a configuração `+agent-tls-mode+` for alterada, as máquinas Elemental precisarão ser xref:node-operational-tasks/reset.adoc[reiniciadas] para que a configuração seja aplicada.

== Ciclo de vida do certificado de AC privada

Ao usar uma CA privada, a recomendação é sempre garantir que a mesma CA também seja usada para o Rancher. A Elemental usará o endereço `+cacerts+`, ao incluir o certificado da CA que será confiável para os agentes. Esse é o mesmo valor que aparece no URL `+https://my.rancher.example/cacerts+`.

Observe, entretanto, que não será possível atualizar esse valor depois que uma máquina Elemental tiver sido instalada. A substituição do certificado CA no Rancher pode fazer com que as máquinas Elemental não consigam se reconectar ao Rancher e operar normalmente, quando o `+agent-tls-mode+` estiver definido como `+strict+`.

Por esse motivo, a recomendação é usar a configuração `+agent-tls-mode: system-store+` e gerenciar diretamente o ciclo de vida dos certificados CA nos computadores Elemental, quando uma autoridade de certificação privada estiver em uso.

O certificado da CA pode ser instalado diretamente nas xref:operator-operational-tasks/custom-images.adoc[imagens personalizadas do sistema operacional] ou passado como uma configuração de inicialização na nuvem nos recursos do Elemental. Por exemplo, o certificado CA inicial pode ser incluído no xref:references/machineregistration-reference.adoc#_config_cloud_config[MachineRegistration] `+cloud-config+`:

[,yaml]
----
apiVersion: elemental.cattle.io/v1beta1
kind: MachineRegistration
metadata:
  name: fire-nodes
  namespace: fleet-default
spec:
  config:
    cloud-config:
      write_files:
        - path: /etc/pki/trust/anchors/rancher-ca.pem
          permission: 0444
          content: |-
            -----BEGIN CERTIFICATE-----
            
            12xVJHQTSN8XWh0++9jNG0eSMe75
            -----END CERTIFICATE-----
      runcmd:
        - update-ca-certificates
----

Antes de o certificado CA ser substituído no Rancher, o novo certificado CA pode ser incluído nas máquinas Elemental, atualizando-as. O *novo* certificado da CA pode ser configurado na xref:references/managedosimage-reference.adoc#_cloudconfig[ManagedOSImage] `+cloudConfig+`:

[,yaml]
----
apiVersion: elemental.cattle.io/v1beta1
kind: ManagedOSImage
metadata:
  name: ca-cert-upgrade
  namespace: fleet-default
spec:
  # The cloudConfig will be applied after node reboot
  cloudConfig:
    write_files:
      - path: /etc/pki/trust/anchors/rancher-ca-new.pem
        permission: 0444
        content: |-
          -----BEGIN CERTIFICATE-----
          
          -----END CERTIFICATE-----
    runcmd:
      - update-ca-certificates
  osImage: "registry.suse.com/suse/sl-micro/6.0/baremetal-os-container:2.1.2-3.59"
  clusterTargets:
    - clusterName: volcano
  upgradeContainer:
    envs:
      # Use FORCE to force an upgrade.
      # This is convenient when the `+osImage+` is the same, and only the `+cloudConfig+` changed.
      - name: FORCE
        value: "false"
----
