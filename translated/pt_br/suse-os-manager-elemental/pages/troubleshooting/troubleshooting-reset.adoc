= Redefinição de solução de problemas
:revdate: 2025-07-03
:page-revdate: {revdate}

Cada `+MachineInventory+` com a anotação `+elemental.cattle.io/resettable: "true"+` acionará a execução de um plano de redefinição após a exclusão. +
O finalizador `+machineinventory.elemental.cattle.io+` será removido somente depois que o plano tiver sido executado com êxito pelo `+elemental-system-agent+` em execução na máquina.

Você pode investigar por que um `+MachineInventory+` ainda não foi excluído, examinando-o:

[,yaml]
----
apiVersion: elemental.cattle.io/v1beta1
kind: MachineInventory
metadata:
  # deletionTimestamp has been set. This object has been marked for deletion.
  deletionTimestamp: "2023-08-04T08:41:25Z"
  annotations:
    # `+resettable+` annotation is enabled.
    # This means the machine has to go through reset, before deletion of this object.
    elemental.cattle.io/resettable: "true"
  # `+machineinventory.elemental.cattle.io+` finalizer is set.
  #  The `+elemental-operator+` is going to create a reset plan for the machine to execute.
  #  After successful execution of the reset plan, the finalizer is removed and the object will be deleted.
  finalizers:
  - machineinventory.elemental.cattle.io
status:
  conditions:
  # Most recent condition shows that the MachineInventory is waiting for a plan to be applied.
  - lastTransitionTime: "2023-08-04T08:41:25Z"
    message: waiting for plan to be applied
    reason: WaitingForPlan
    status: "False"
    type: Ready
  # The plan to be executed is referenced.
  # Normally it has the same name of the MachineInventory and lives within the same namespace.
  plan:
    checksum: 5aba8b6b3161bc52d8953b2428e54ecda3b59e8e0043b49d761d1e79174eded6
    secretRef:
      name: m-bf1008a1-61d6-4355-b5f5-f7d1c527affe
      namespace: fleet-default
----

Você também pode examinar o plano referenciado `+Secret+`. +
Observe que o `+elemental-system-agent+` em execução na máquina está observando esse segredo e deve executar o plano. +
Você também pode monitorar seu progresso nos registros da máquina: `+journalctl -u elemental-system-agent -f+`.

[,yaml]
----
apiVersion: v1
kind: Secret
# This is a `+elemental.cattle.io/plan+` secret plan.
type: elemental.cattle.io/plan
metadata:
  annotations:
    # This is a `+reset+` plan type.
    elemental.cattle.io/plan.type: reset
  labels:
    elemental.cattle.io/managed: "true"
  name: m-bf1008a1-61d6-4355-b5f5-f7d1c527affe
  namespace: fleet-default
  # It is owned by the `+MachineInventory+` waiting for deletion.
  ownerReferences:
  - apiVersion: elemental.cattle.io/v1beta1
    controller: true
    kind: MachineInventory
    name: m-bf1008a1-61d6-4355-b5f5-f7d1c527affe
    uid: 5aa3863c-63a5-4cb9-91fd-7a45191d4842
data:
  # The plan has not been applied yet.
  applied-checksum: ""
  # It also hasn't failed.
  failed-checksum: ""
  # The actual plan to be executed, base64 encoded.
  plan: 
----

O plano criado pelo site `+elemental-operator+` deve conter as seguintes instruções:

[,json]
----
{
  "files": [
    // A cloud-init config file is created on the default /oem directory.
    // This config will be executed once in recovery mode.
    {
      "content": "",
      "path": "/oem/reset-cloud-config.yaml",
      "permissions": "0600"
    }
  ],
  "instructions": [
    {
      "name": "configure next boot to recovery mode",
      "args": [
        "/oem/grubenv",
        "set",
        "next_entry=recovery"
      ],
      "command": "grub2-editenv"
    },
    {
      "name": "schedule reboot",
      "args": [
        "-r",
        "+1"
      ],
      "command": "shutdown"
    }
  ]
}
----

Se o `+elemental-system-agent+` executar o plano com êxito, o finalizador `+machineinventory.elemental.cattle.io+` no `+MachineInventory+` será removido e o `+MachineInventory+` será excluído. +
Observe que isso não é uma indicação de que a máquina já tenha sido totalmente reinicializada. +
Essa é uma limitação da implementação atual e, eventualmente, ela será aprimorada, de modo que será possível rastrear completamente o status de redefinição.

No entanto, nesse estágio, esperamos que o host seja reinicializado e reinicializado no modo de recuperação. +
Uma vez no modo de recuperação, o `+cos-setup-network+` deve executar a configuração cloud-init que foi gravada em `+/oem/reset-cloud-config.yaml+`. +
Você pode monitorar o status com `+journalctl -u cos-setup-network -f+`.

As instruções de instalação na nuvem devem ser parecidas com as seguintes:

[,yaml]
----
name: {elemental-product-name} Reset
stages:
    network.after:
        - if: '[ -f /run/cos/recovery_mode ]'
          name: Runs elemental reset
          commands:
            - systemctl start elemental-register-reset
----

O `+elemental-register+` cli será registrado no `+elemental-operator+` como uma nova máquina. Isso levará à criação de um novo objeto `+MachineInventory+`. +
A configuração remota do `+MachineRegistration+` também será obtida para aplicar as opções de redefinição, por exemplo, `+reset-persistent+`, `+reset-oem+`, ou as configurações de energia, `+reboot+` ou `+power-off+`. +
Após a redefinição, dependendo das configurações, a máquina deverá ser desligada ou reinicializada e estará pronta para ser adotada em um novo cluster.

== Exclusão forçada de um MachineInventory durante a reinicialização

Se a máquina não conseguir executar as instruções de redefinição e o site `+MachineInventory+` relacionado não for excluído, há duas maneiras equivalentes de corrigir o problema manualmente.

* Remova a anotação `+elemental.cattle.io/resettable: "true"+` do site `+MachineInventory+`.
* Remova o finalizador `+machineinventory.elemental.cattle.io+` do site `+MachineInventory+`.

Lembre-se também de cuidar da própria máquina, reprovisionando-a totalmente ou reiniciando no modo de recuperação e usando o comando `+elemental reset+` diretamente.
