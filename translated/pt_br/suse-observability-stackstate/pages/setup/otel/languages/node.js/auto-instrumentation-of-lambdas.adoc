...
* *MemÃ³ria:* Memória suficiente para executar a Lambda, incluindo a instrumentação.
...
* *Correspondência de Arquitetura:* A camada do OpenTelemetry Collector é específica da arquitetura. Escolha o ARN correto para a arquitetura da sua função Lambda (por exemplo, `+amd64+` ou `+arm64+`).
...
Veja esta página para xref:/setup/otel/proxy-collector.adoc[orientações e instruções] sobre como configurar um coletor em lote que atua como um proxy de segurança para SUSE Observability.
Veja esta página para xref:/setup/otel/collector.adoc[instruções] sobre como configurar um coletor que também faz amostragem de cauda.
Para mais informações sobre a configuração do processador no coletor opentelemetry, consulte a https://github.com/open-telemetry/opentelemetry-collector/blob/main/processor/README.md[documentação oficial].
