...
Você pode usar a CLI do {stackstate-product-name} para inspecionar, executar e modificar monitores:
...
== Monitores de depuração

Você pode usar vários comandos da CLI do site {stackstate-product-name} para depurar um monitor.

=== Monitores de listagem
...
A execução de um monitor lhe dá uma visão dos estados de verificação que são produzidos:
...
=== Ativação/desativação de monitores
...
=== Monitorar detalhes da execução

As estatísticas das execuções do monitor e a latência no processamento dos estados de verificação resultantes podem ser obtidas com o comando `+status+`.
...
== Modificação de monitores

[NOTE]
====
A maneira recomendada de trabalhar é armazenar os monitores (e quaisquer outros recursos personalizados criados em {stackstate-product-name}) como arquivos YAML em xref:/setup/custom-integrations/overview.adoc[um StackPack]. A partir daí, as alterações podem ser aplicadas manualmente ou podem ser totalmente automatizadas usando a CLI do SUSE Observability em um sistema de CI/CD, como ações do GitHub ou pipelines do GitLab.
====

=== Criar um monitor

Você pode criar um monitor aplicando um arquivo YAML, por exemplo, `+monitor.yaml+`.  Ele deve ter o seguinte esboço:
...
Use a xref:/setup/cli/cli-sts.adoc[CLI do SUSE Observability] para criar ou atualizar o monitor:
...
Para editar um monitor, edite o original do monitor que foi aplicado e aplique-o novamente. Ou há um comando `+sts monitor edit+` para editar diretamente o monitor configurado na instância {stackstate-product-name}:
