...
Devido à renomeação do produto e também às mudanças drásticas no formato dos dados de topologia, não é possível fazer upgrade do StackState para o SUSE Observability via um comando padrão de upgrade do Helm. Este guia de migração ajudará você a configurar o SUSE Observability exatamente da mesma forma que o StackState.
...
* A versão do Helm chart deve ser `+1.12.1+`
* A versão do aplicativo deve ser `+6.0.0-snapshot.20241023094532-stackstate-6.x-7be52ad+`

Se você não tiver essa versão, faça o upgrade primeiro seguindo os passos padrão https://docs.stackstate.com/6.0/self-hosted-setup/upgrade-stackstate/steps-to-upgrade#_minor_or_maintenance_stackstate_release[para fazer upgrade].

== Crie um backup da configuração e faça o download do backup

Primeiro, criamos um backup da configuração do StackState, após isso você não deve mais fazer alterações na configuração do StackState (elas não serão transferidas para o SUSE Observability). Para fazer isso, familiarize-se primeiro com o backup da configuração e obtenha os scripts necessários usando a https://docs.stackstate.com/6.0/self-hosted-setup/data-management/backup_restore/configuration_backup#_working_with_configuration_backups[documentação do backup da configuração para StackState 6.x].
...
Portanto, a etapa de geração de valores fica assim (usando nosso exemplo novamente, com o menor arquivo de controle):
...
. Escale todas as implantações de volta para cima:
+
[,bash]
----
./scale-up.sh
...
Agora o SUSE Observability tem a mesma configuração exata do StackState e estamos prontos para começar a usá-lo. Observe que, como a mesma URL é usada, pode ser necessário atualizar o navegador na primeira vez.
...
Em algum momento, o tráfego precisará ser transferido do StackState para o SUSE Observability. A solução que limita o impacto sobre seus usuários e os agentes instalados é configurar o SUSE Observability com a URL originalmente usada pelo StackState. Este guia reutilizará a URL do StackState (`+stackstate.demo.stackstate.io+`) enquanto o "antigo" StackState estará acessível sob uma nova URL `+stackstate-old.demo.stackstate.io+`. Ao usar um provedor OIDC para autenticação, a URL `+stackstate-old+` precisará ser adicionada/atualizada na configuração do provedor OIDC e na configuração do StackState.

Também é possível instalar o SUSE Observability sob uma nova URL; nesse caso, você precisará atualizar o agente e os coletores do Open Telemetry para usar a nova URL ou usar outro método de redirecionamento do tráfego.

Para resumir, antes da migração, a configuração é o StackState rodando no namespace `+stackstate+` com a URL `+https://stackstate.demo.stackstate.io+`. Isso será migrado para:
...
Portanto, a etapa de geração de valores fica assim (usando nosso exemplo novamente, com o menor perfil):
...
----
+
Verifique se você está no namespace suse-observability e não mais no namespace StackState, só então responda `+yes+` para confirmar que a remoção de todos os dados está ok.
...
Agora o SUSE Observability tem a mesma configuração exata do StackState e estamos prontos para começar a usá-lo.

==== Prepare-se para reduzir o StackState.

Para garantir que nada mais mude na antiga configuração do "StackState" e também para reduzir seu uso de recursos, um número de implantações do StackState deve ser reduzido para 0 réplicas. A melhor maneira de fazer isso é através dos valores do Helm, assim qualquer outra mudança de configuração não aumentará acidentalmente algumas das implantações novamente.
...
Este arquivo será usado ao alterar o ingress para o StackState. Quando nenhum agente ou dados do Open Telemetry forem mais recebidos, esses serviços do StackState não são mais necessários.
...
. Execute o fazer upgrade do helm para o StackState e inclua a configuração de ingress atualizada para que comece a usar o ingress `+stackstate-old.demo.stackstate.io+`. Inclua também os valores `+scaled-down.yaml+` da etapa anterior e certifique-se de incluir todos os arquivos de valores usados durante a instalação do StackState:
+
----
  helm upgrade \
   --install \
...
O SUSE Observability tem uma mudança em sua autenticação. O StackState usava um token de portador com o esquema `+StackState+`, mas o SUSE Observability usa o esquema `+SUSEObservability+`. Atualize os valores dos seus Coletores Open Telemetry instalados para mudar de:
...
A etapa final na migração para o SUSE Observability é atualizar todos os seus agentes instalados. Isso não precisa ser feito imediatamente, mas pode ser feito em um momento conveniente para cada cluster específico, porque o SUSE Observability é compatível com versões anteriores com o agente StackState.

Migrar é um processo fácil de 2 etapas:

. Desinstale o agente StackState
. Instale o agente SUSE Observability
