...
Um botão de *fazer upgrade* aparece na tela do *Dashboard* sempre que uma nova {harvester-product-name} versão para a qual você pode fazer upgrade se torna disponível. Para mais informações, veja xref:./upgrades.adoc#_start_an_upgrade[Iniciar um upgrade].
...
. Verifique as condições e os status dos nós do recurso personalizado mais recente `+Upgrade+`.
+
...
----
+
. Verifique os pods no nó trabalhador.
+
...
Para mais informações, veja https://github.com/harvester/harvester/issues/6432#issuecomment-2325488465[Problema #6432].
