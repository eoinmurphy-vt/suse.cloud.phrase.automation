= Faça upgrade de v1.4.2 ou v1.4.3 para v1.5.1
:revdate: 2025-07-07
:page-revdate: {revdate}

== Informações gerais

Um *Upgrade* aparece na tela *Dashboard* sempre que uma nova {harvester-product-name} versão para a qual você pode fazer upgrade se torna disponível. Para mais informações, veja xref:./upgrades.adoc#_start_an_upgrade[Iniciar um upgrade].

Você pode fazer upgrade diretamente de v1.4.2 para v1.5.1 porque {harvester-product-name} permite um máximo de um upgrade de versão menor para componentes subjacentes. {harvester-product-name} v1.4.2 e v1.4.3 usam a mesma versão menor de {rke2-product-name} (v1.31), enquanto {harvester-product-name} v1.5.0 e v1.5.1 usam a próxima versão menor (v1.32).
...
Você deve usar **v1.5.1** da Extensão da UI do Harvester para importar clusters {harvester-product-name} v1.5.1 na {rancher-short-name} v2.11.0.
...
A atualização pode ficar travada no início do processo, conforme indicado por 0% de progresso e itens marcados como *Pendente* na caixa de diálogo *Upgrade* da {harvester-product-name} UI.
...
----
+
O status dos pods Fluentd e Fluent Bit deve mudar para `+Running+` em breve e o processo de upgrade deve continuar após a atualização do CR de Logging. Se o status do pod Fluentd ainda estiver `+ImagePullBackOff+`, você pode excluir o pod para forçá-lo a reiniciar.
+
[,bash]
----
...
- (Não recomendado) Reinicie o processo de upgrade com o logging desativado. Certifique-se de que a caixa de seleção *Ativar Logging* no diálogo *Upgrade* não esteja selecionada.

Problema relacionado: https://github.com/harvester/harvester/issues/7955[#7955]

=== 2. Máquinas virtuais que usam volumes RWX migráveis reiniciam inesperadamente
...
A solução alternativa é desativar a configuração https://documentation.suse.com/cloudnative/storage/1.8/en/longhorn-system/settings.html#_automatically_delete_workload_pod_when_the_volume_is_detached_unexpectedly[Excluir Automaticamente o Pod de Trabalho Quando o Volume for Desconectado Inesperadamente] na {longhorn-product-name} UI antes de iniciar o upgrade. Você deve habilitar a configuração novamente assim que o upgrade for concluído.

O problema será corrigido nas versões {longhorn-product-name} v1.8.3, v1.9.1 e versões posteriores. {harvester-product-name} v1.6.0 incluirá {longhorn-product-name} v1.9.1. 
