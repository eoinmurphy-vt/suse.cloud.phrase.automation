= Kube-OVN Operator (Experimental)
:revdate: 2025-09-15
:page-revdate: {revdate}

[NOTE]
====
**kubeovn-operator** é um *complemento* experimental. Para mais informações sobre recursos experimentais, veja xref:introduction/document-conventions.adoc#_feature_labels[Rótulos de Recursos].
====

https://github.com/harvester/kubeovn-operator[`+kubeovn-operator+`] é usado para gerenciar o ciclo de vida de https://github.com/kubeovn/kube-ovn[Kube-OVN] como um CNI secundário em {harvester-product-name} clusters subjacentes.

== Habilitar `+kubeovn-operator+`

Você deve habilitar `+kubeovn-operator+` para implantar o Kube-OVN em um cluster {harvester-product-name} para capacidades avançadas de SDN, como nuvem privada virtual (VPC) e sub-redes para cargas de trabalho de máquinas virtuais.

. Na interface do {harvester-product-name}, vá para *Avançados -> Complementos*.

. Selecione *kubeovn-operator (Experimental)*, e então selecione *⋮ -> Habilitar*.
+
image::addons/kubeovn-operator.png[Kube-OVN Operator]

O complemento implanta `+kubeovn-operator+` e cria o objeto padrão `+Configuration+` nomeado `+configuration.kubeovn.io+`, que usa padrões específicos do {harvester-product-name} para configurar o CNI Kube-OVN.

Veja a seguir um exemplo de um objeto `+Configuration+`:
...
[CAUTION]
====
Este objeto `+Configuration+` foi validado contra instalações {harvester-product-name}. Você pode ajustar o objeto editando o YAML do complemento `+kubeovn-operator+`, mas tenha cautela ao fazer alterações no campo `+configurationSpec+`.

Certifique-se de que os blocos CIDR de pod e serviço IPv4 do Kube-OVN não se sobreponham aos blocos CIDR de pod e serviço do Harvester.
====

== Desabilitando `+kubeovn-operator+`

[IMPORTANT]
====
Certifique-se de que nenhuma máquina virtual esteja usando redes de VM suportadas por componentes SDN do Kube-OVN.

Desabilitar o complemento kubeovn-operator é um processo disruptivo.
====

Você pode desabilitar `+kubeovn-operator+` usando os seguintes comandos:
...
Você deve reiniciar cada nó para completar o processo de desinstalação. Uma vez que os nós sejam reiniciados, você pode desabilitar o complemento `+kubeovn-operator+` na interface do Harvester.