...
* 用户。允许或不允许在运行时定义的https://kubernetes.io/docs/reference/access-authn-authz/rbac/#referring-to-subjects[由kubernetes绑定的用户]，在userInfo字段中可见。注意：yaml（上传）审计功能将无法检查此项，因为它在运行时绑定。
* 用户组。允许或不允许在运行时定义的https://kubernetes.io/docs/reference/access-authn-authz/rbac/#referring-to-subjects[由kubernetes绑定的用户组]，在userInfo字段中可见。 注意：yaml（上传）审计功能将无法检查此项，因为它在运行时绑定。
