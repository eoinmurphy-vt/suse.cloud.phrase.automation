...
. 注册表 - 这是您需要确保从GCR实例收集正确数据的第一处。虽然https://gcr.io的示例是最常见的，但我们需要确保它准确反映您在GCP中设置的GCR。例如，它可能是https://us.gcr.io。我们将在下一部分检查它。
...
[upperalpha]
. 请查看主机名下的&quot;`+gcr.io+`&quot;？这就是在{product-name}界面中属于项目#2，注册表的内容。（不要忘记https://部分）
