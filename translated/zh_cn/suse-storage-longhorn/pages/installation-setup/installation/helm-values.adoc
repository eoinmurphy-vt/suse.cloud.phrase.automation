...
| list
| `+[]+`
| 配置在摄取之前应用于样本的重新标记规则。有关格式详细信息，请参阅https://prometheus-operator.dev/docs/api-reference/api/#monitoring.coreos.com/v1.Endpoint[Prometheus Operator文档]。

| `+metrics.serviceMonitor.relabelings+`
| list
| `+[]+`
| 配置应用于目标元数据标签的重新标记规则。有关格式详细信息，请参阅https://prometheus-operator.dev/docs/api-reference/api/#monitoring.coreos.com/v1.Endpoint[Prometheus Operator文档]。
...
| 设置允许{longhorn-product-name}定期收集匿名使用数据以用于产品改进。{longhorn-product-name}将收集的数据发送到https://github.com/longhorn/upgrade-responder[升级响应器]服务器，这是Longhorn公共指标仪表板（https://metrics.longhorn.io）的数据源。升级响应器服务器不存储可用于识别客户端的数据，包括IP地址。
