...
应用聚合通常意味着在指标中强调某些模式而牺牲其他模式。例如，对于较大的时间窗口，`+max_over_time+` 会显示所有峰值，但不会显示所有谷值。而 `+min_over_time+` 则完全相反，`+avg_over_time+` 会平滑处理峰值和谷值。为了展示这种行为，这里是一个使用 Pod CPU 使用率的示例指标绑定。要自己尝试，请将其复制到 YAML 文件中，并在您自己的 SUSE Observability 中使用 xref:/use/metrics/k8s-add-charts.adoc#_create_or_update_the_metric_binding_in_stackstate[CLI 来应用它]（您可以稍后去除它）。
...
[CAUTION]
====
当指标绑定未指定聚合时，SUSE Observability 将自动使用 `+last_over_time+` 聚合来减少图表的数据点数量。另请参见 xref:/use/metrics/k8s-writing-promql-for-charts.adoc#_why[为什么这是必要的？] 以获取解释。
====
...
您可以在 https://play.stackstate.com/#_/metrics?promql=last_over_time%28container_cpu_usage%7Bnamespace%3D%22sock_shop%22%2C%20pod_name%3D~%22carts.%2A%22%7D%5B%24%7B%5F%5Finterval%7D%5D%29%20%2F%201000000000&timeRange=LAST_7_DAYS[SUSE Observability playground] 上亲自尝试。
