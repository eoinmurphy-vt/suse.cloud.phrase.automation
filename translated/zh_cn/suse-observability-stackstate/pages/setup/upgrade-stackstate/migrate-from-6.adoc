...
安装的最大变化是现在支持控制文件，请选择与您观察到的集群匹配的控制文件，使用 xref:/setup/install-stackstate/requirements.adoc#_resource_requirements[requirements] 并根据安装指南生成值。StackState 的自定义 Helm 值与 SUSE Observability 兼容。但是，必须删除自定义资源的值，以支持新的控制文件，我们将这些保存在一个名为 `+custom-values-no-resources.yaml+` 的文件中。您可以使用相同的入口设置，这样 SUSE Observability 从用户和代理的角度来看将有效地替代 StackState。
