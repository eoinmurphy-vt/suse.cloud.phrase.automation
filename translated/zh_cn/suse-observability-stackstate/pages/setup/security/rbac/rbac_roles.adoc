...
SUSE Observability 中的每个用户都需要分配一个主题和一组xref:/setup/security/rbac/rbac_permissions.adoc[权限]；这种组合称为角色。角色描述了可以访问特定数据集的一组用户。SUSE Observability 随附一组预定义的角色，您也可以根据自己的需要创建角色。

== 预定义角色

SUSE Observability 预定义了四种角色：

* *管理员*- 可完全访问所有视图并拥有所有权限。
* *高级用户*- 通常授予需要为团队配置 SUSE Observability 但不会管理整个 SUSE Observability 安装的用户。
* *Kubernetes 故障*排除程序 - 拥有使用 SUSE Observability 进行故障排除所需的所有权限，包括启用/禁用监控器、创建自定义视图和使用 Cli 的能力。
* *访客*- 只读访问 SUSE Observability。

分配给每个预定义 SUSE Observability 角色的权限如下。有关不同权限以及如何使用`+sts+` CLI 管理这些权限的详细信息，请参阅xref:/setup/security/rbac/rbac_permissions.adoc[基于角色的访问控制 (RBAC) 权限]。
...
下文列出了分配给预定义管理员角色 (`+stackstate-admin+`) 的权限，这些权限是使用`+sts+` CLI 获取的。有关不同权限以及如何使用`+sts+` CLI 管理这些权限的详细信息，请参阅xref:/setup/security/rbac/rbac_permissions.adoc[RBAC 权限]。
...
高级用户角色 (`+stackstate-power-user+`) 拥有除以下权限外的所有管理员权限：

* `+execute-restricted-scripts+`
* `+update-permissions+`
* `+update-stackpacks+`

下文列出了分配给预定义的 "高级用户 "角色 (`+stackstate-power-user+`) 的权限，这些权限是通过`+sts+` CLI 获取的。有关不同权限以及如何使用`+sts+` CLI 管理这些权限的详细信息，请参阅xref:/setup/security/rbac/rbac_permissions.adoc[RBAC 权限]。
...
故障排除员角色 (`+stackstate-k8s-troubleshooter+`) 可以访问 SUSE Observability 中的所有可用数据，并能创建视图和启用/禁用监控器。

下文列出了分配给预定义故障排除者角色的权限，这些权限是通过`+sts+` CLI 获取的。有关不同权限以及如何使用`+sts+` CLI 管理这些权限的详细信息，请参阅xref:/setup/security/rbac/rbac_permissions.adoc[RBAC 权限]。
...
访客角色 (`+stackstate-guest+`) 只读访问 SUSE Observability。

分配给预定义访客角色的权限如下，这些权限是使用`+sts+` CLI 获取的。有关不同权限以及如何使用`+sts+` CLI 管理这些权限的详细信息，请参阅xref:/setup/security/rbac/rbac_permissions.adoc[RBAC 权限]。
...
除了始终可用的预定义角色 (`+stackstate-admin+`,`+stackstate-power-user+`,`+stackstate-k8s-troubleshooter+`,`+stackstate-guest+`) 之外，还可以添加自定义角色。添加自定义角色有多种方法：

. 通过配置文件，权限与预定义角色相同
. 通过配置文件，使用自定义权限
. 使用`+sts+` CLI，主体及其权限存储在数据库中，可在运行时修改

通过配置文件添加的角色需要重新启动，因此会导致短时间停机。使用 CLI 创建的角色存储在数据库中，可在运行时修改。

=== 预定义角色的自定义名称

当预定义的 SUSE Observability 角色非常适合，但外部身份验证提供商对角色使用了不同的名称时，请使用此选项。例如，当 LDAP 身份验证提供程序具有相似但名称不同的角色时，可在`+authentication.yaml+` 中包含此 YAML 代码段，以赋予来自 LDAP 的角色与预定义的同等角色相同的权限和作用域。
...
要将其用于 SUSE Observability 安装（或已在运行的实例，请注意它将重启 API），请将其用于 SUSE Observability 安装（或已在运行的实例，请注意它将重启 API）：
...
要设置名为`+development-troubleshooter+` 的新角色，该角色的权限与预定义的故障排除者角色相同，但仅限于`+dev-test+` 群集，请在`+authentication.yaml+` 中包含此 YAML 代码段：
...
=== 通过 CLI 自定义角色（Observability RBAC）

要设置一个名为`+development-troubleshooter+` 的新角色，其权限与普通故障排除者角色相同，但仅限于`+dev-test+` 群集，需要创建一个新主题。此外，还需要为该主题分配所需的权限：

. 创建主题（与角色名称相同，角色-主题匹配基于名称且区分大小写）：
+
[,text]
----
...
. 配置的主体需要有访问用户界面部分内容和在其中执行操作的权限。要授予与故障排除者角色相同的权限，请参考下面的示例：
+
[,text]
----
