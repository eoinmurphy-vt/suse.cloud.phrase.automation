...
每个带有`+elemental.cattle.io/resettable: "true"+` 注释的`+MachineInventory+` 都会在删除后触发重置计划的执行。 +
`+machineinventory.elemental.cattle.io+` finalizer 只有在机器上运行的`+elemental-system-agent+` 成功执行计划后才会被移除。

您可以通过检查`+MachineInventory+` 来了解尚未删除的原因：
...
您还可以查看参考计划`+Secret+` 。 +
请注意，机器上运行的`+elemental-system-agent+` 正在监视这个秘密，它应该执行该计划。 +
您还可以从机器日志中监控其进度：`+journalctl -u elemental-system-agent -f+`.
...
`+elemental-operator+` 创建的计划应包含以下说明：
...
如果`+elemental-system-agent+` 成功执行了计划，则`+MachineInventory+` 上的`+machineinventory.elemental.cattle.io+` 终结器将被移除，`+MachineInventory+` 也将被删除。 +
请注意，这并不表明机器已经完全复位。 +
这是当前实施的局限性，最终会有所改进，从而可以完全跟踪重置状态。

不过，在这一阶段，我们确实希望主机进行重启，并在恢复模式下重启。 +
进入恢复模式后，`+cos-setup-network+` 应执行写入`+/oem/reset-cloud-config.yaml+` 的云启动配置。 +
您可以通过`+journalctl -u cos-setup-network -f+` 监控状态。
...
`+elemental-register+` cli 将作为新机器在`+elemental-operator+` 注册。这将导致创建一个新的`+MachineInventory+` 对象。 +
还将获取远程`+MachineRegistration+` 配置，以应用重置选项，如`+reset-persistent+`,`+reset-oem+`, 或电源设置，如`+reboot+` 或`+power-off+` 。 +
重置后，根据设置的不同，机器应关闭或重启，并准备好在新群集中采用。

== 强制删除正在重置的机器信息库

如果机器无法执行重置指令，且相关的`+MachineInventory+` 也未删除，则有两种等效的方法可以手动解决问题。

* 从`+MachineInventory+` 中删除`+elemental.cattle.io/resettable: "true"+` 注释。
* 从`+MachineInventory+` 上卸下`+machineinventory.elemental.cattle.io+` 定稿器。

切记也要对机器本身进行保护，方法是对其进行完全重置或重启至恢复模式，然后直接使用`+elemental reset+` 命令。
