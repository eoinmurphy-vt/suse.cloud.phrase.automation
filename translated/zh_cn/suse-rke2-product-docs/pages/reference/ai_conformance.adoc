...
作为参考，我们解释如何使用上游自动扩缩器https://github.com/kubernetes/autoscaler[autoscaler]，以Azure为例。

. 创建一个https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/overview[虚拟机规模集（VMSS）]，其中包含配备GPU的虚拟机。
. 使用以下选项部署RKE2：
+
[,yaml]
----
...
根据与AI/ML工作负载相关的自定义指标扩缩Pod的能力，是通过https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/[水平Pod自动扩缩器（HPA）]实现的，该功能在Kubernetes中默认包含。
