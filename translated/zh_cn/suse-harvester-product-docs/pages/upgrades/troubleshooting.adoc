...
然而，这些组件继续消耗集群资源，并可能阻止某些操作，例如更新存储网络设置（请参见https://github.com/harvester/harvester/issues/9599[问题 #9599]）。要释放资源并解除阻塞操作，请执行以下任一操作：

* 在 {harvester-product-name} 用户界面上点击 *忽略它* 按钮。
* 通过 kubectl 删除相关的 `+UpgradeLog+` 自定义资源。
====

有关更多详细信息，请参阅https://github.com/harvester/harvester/blob/master/enhancements/20221207-upgrade-observability.md[升级日志HEP]。
