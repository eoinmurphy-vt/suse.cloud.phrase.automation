...
解决方法是在升级开始之前，在{longhorn-product-name} UI上禁用设置https://documentation.suse.com/cloudnative/storage/1.8/en/longhorn-system/settings.html#_automatically_delete_workload_pod_when_the_volume_is_detached_unexpectedly[意外分离卷时自动删除工作负载Pod]。升级完成后，您必须再次启用该设置。

该问题将在{longhorn-product-name} v1.8.3、v1.9.1及更高版本中修复。{harvester-product-name} v1.6.0将包含{longhorn-product-name} v1.9.1。

相关问题：https://github.com/harvester/harvester/issues/8534[#8534]和https://github.com/longhorn/longhorn/issues/11158[#11158]