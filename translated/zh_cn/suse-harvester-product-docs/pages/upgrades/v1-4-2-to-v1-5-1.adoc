...
您可以直接从 v1.4.2 升级到 v1.5.1，因为 {harvester-product-name} 允许对基础组件进行最多一次小版本升级。{harvester-product-name} v1.4.2 和 v1.4.3 使用相同的小版本 {rke2-product-name} (v1.31)，而 {harvester-product-name} v1.5.0 和 v1.5.1 使用下一个小版本 (v1.32)。
...
您必须使用 **v1.5.1** 的 Harvester UI 扩展，在 {rancher-short-name} v2.11.0 上导入 {harvester-product-name} v1.5.1 集群。
...
=== 1.隔离升级在 Fluentd 和 Fluent Bit pod 中卡住，出现 `+ImagePullBackOff+` 错误

升级可能在过程的最开始就卡住，进度显示为 0%，并且在 {harvester-product-name} UI 的 *升级* 对话框中标记为 *待处理* 的项目。
...
* 更新日志 CR，以使用已经在集群节点中预加载的镜像。要执行此操作，请对集群运行以下命令：
+
[,bash]
----
...
----
+
Fluentd 和 Fluent Bit pod 的状态应在片刻内变为 `+Running+`，并且在更新 Logging CR 后，升级过程应继续。如果 Fluentd pod 的状态仍为 `+ImagePullBackOff+`，您可以删除该 pod 以强制其重新启动。
+
[,bash]
----
