...
上面的例子显示同一个PVC存在两个引擎，这是Longhorn中的一个已知问题https://github.com/longhorn/longhorn/issues/6642[#6642]。为了解决这个问题，请删除冗余的引擎，以便继续升级。
...
. 使用以下命令查找机器的主机名，并按照https://github.com/harvester/harvester/issues/3863#issuecomment-1539681311[解决方法]查看节点上的服务证书是否过期：
+
----
...
[NOTE]
====
如果多个节点在同一时间加入集群，您应该在所有这些节点上执行https://github.com/harvester/harvester/issues/3863#issuecomment-1539681311[解决方法]。
====
--
* 相关问题：
