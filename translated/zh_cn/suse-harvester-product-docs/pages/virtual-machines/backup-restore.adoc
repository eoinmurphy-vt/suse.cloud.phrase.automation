...
但是，删除虚拟机的最新备份或启用 {longhorn-product-name} 设置 https://documentation.suse.com/cloudnative/storage/latest/en/longhorn-system/settings.html#_auto_cleanup_snapshot_when_delete_backup[删除备份时自动清理快照] 会阻止对相关卷的所有后续操作。这是一个https://github.com/longhorn/longhorn/issues/9064[已知问题]，影响卷快照、备份和实时迁移等操作。
...
然而，删除虚拟机的最新备份会阻止对相关卷的所有后续操作。这是一个https://github.com/longhorn/longhorn/issues/9064[已知问题]，影响卷快照、备份和实时迁移等操作。
...
当来宾虚拟机与*QEMU来宾代理*连接时，{harvester-product-name}控制器通过Kubevirt的https://github.com/kubevirt/kubevirt/blob/main/docs/freeze.md#virt-freezer[virt-freezer]应用程序执行文件系统冻结操作，以确保在虚拟机备份和快照期间文件系统的一致性。
