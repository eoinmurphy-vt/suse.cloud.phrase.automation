...
{harvester-product-name} 附加产品允许您根据需求启用和禁用特定的产品和第三方组件。附加产品作为 https://documentation.suse.com/cloudnative/rke2/latest/en/helm.html#_using_the_helm_crd[{rke2-product-name} HelmChart 资源定义 (CRD)] 的封装程序。

== 先决条件

* 现有的 {harvester-product-name} 集群
...
[NOTE]
====
实验性附加产品未直接打包在 {harvester-product-name} 软件包中。一个例子是 https://github.com/harvester/experimental-addons/blob/main/rancher-vcluster/rancher-vcluster.yaml[`+rancher-vcluster+`] 附加产品。
====
...
对 `+repo+`、`+version+`、`+chart+` 或 `+valueContent+` 字段的更改将触发 `+helm upgrade+`，这会强制升级现有的 HelmChart。
