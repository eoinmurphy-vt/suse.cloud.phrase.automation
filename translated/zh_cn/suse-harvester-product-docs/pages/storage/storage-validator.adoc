...
+
存储供应商可以使用{harvester-product-name}来验证其解决方案，以确保更好的互操作性。您可以在{harvester-product-name}文档中找到经过认证与{rancher-product-name-tm}兼容的企业级存储解决方案的信息，该文档可通过https://scc.suse.com/home[SUSE客户中心]访问。

* 第三方存储合作伙伴：通过与{harvester-product-name}参考架构验证您的存储解决方案，获得https://www.suse.com/product-certification/suse-certified/virtualization-certification[SUSE虚拟化认证存储]认证。要完成认证要求，您可以在https://www.suse.com/suse-harvester/support-matrix/all-supported-versions[最新稳定版本]上运行此工具，并将结果提交给SUSE。
+
[NOTE]
====
该工具执行的测试将在未来的版本中扩展，以满足https://www.suse.com/product-certification/suse-certified/data-protection-for-virtualization-certification[SUSE虚拟化认证数据保护]认证的要求。
====
