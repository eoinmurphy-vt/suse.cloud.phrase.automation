= 导入基于SUSE® Virtualization虚拟机的集群
:revdate: 2025-05-09
:page-revdate: {revdate}

Rancher允许您导入安装了Kubernetes的SUSE® Virtualization虚拟机。

== 部署

=== 先决条件

* Kubernetes集群构建在SUSE® Virtualization虚拟机之上。

=== 在SUSE® Virtualization虚拟机上部署客户集群

. 使用https://raw.githubusercontent.com/harvester/harvester-csi-driver/master/deploy/generate_addon_csi.sh[generate_addon_csi.sh]脚本生成csi-driver云配置文件，该脚本可在https://github.com/harvester/harvester-csi-driver[harvester/harvester-csi-driver]储存库中找到。
+
示例：
+
[,shell]
...
. 使用https://raw.githubusercontent.com/harvester/cloud-provider-harvester/master/deploy/generate_addon.sh[generate_addon.sh]脚本生成云提供商云配置，该脚本可在https://github.com/harvester/cloud-provider-harvester[harvester/cloud-provider-harvester]储存库中找到。
+
示例：
+
[,shell]
...
. 创建具有两个云配置文件的虚拟机。
+
image::rancher/vm-with-cloud-config.png[具有云配置的虚拟机]
...
. 将虚拟机导入Rancher。
+
在Rancher UI中，转到*集群管理* > *集群* > *导入现有* > *通用* > *创建*。
+
image::rancher/import-existing-cluster.png[导入现有集群]
+
[,shell]
----
...
. 安装Harvester云提供商。
+
在RKE2集群仪表板中，转到*应用* > *图表* > *Harvester云提供商* > *安装*。

. 安装Harvester CSI驱动程序。
+
在RKE2集群仪表板中，转到*应用* > *图表* > *Harvester CSI驱动程序* > *安装*。
