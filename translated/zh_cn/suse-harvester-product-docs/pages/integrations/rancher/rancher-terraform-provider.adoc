...
https://registry.terraform.io/providers/rancher/rancher2/[Rancher Terraform Provider] 允许管理员使用 Terraform 创建和管理 RKE2 客户集群。

== 部署

=== 先决条件

* Kubernetes 集群构建在 Harvester 虚拟机之上。
* 作为客户 Kubernetes 节点运行的 Harvester 虚拟机位于同一名称空间中。
...
+
image:rancher/create-api-key.png[创建 API 密钥]
+
image:rancher/access-and-secret-keys.png[访问密钥与秘密密钥]

. 获取 Harvester 集群 ID。
+
在 Rancher UI 中，转到 *虚拟化管理* > *管理* > *相关资源* > *管理集群名称*。
+
image::rancher/harvester-cluster-id.png[Harvester 集群 ID]
