...
默认情况下，`+macvlan+` 内核模块不包含在 SUSE Linux Enterprise 15 服务包 4/5/6 最小云映像中（请参见 https://github.com/harvester/harvester/issues/6418[问题 #6418]）。这些映像包含https://software.opensuse.org/package/kernel-default-base[`+kernel-default-base+`]软件包，其中仅包括基本模块。但是，当您安装 `+kernel-default+` 软件包时，`+macvlan+` 内核驱动程序将可用。
...
有关更多信息，请参见https://github.com/harvester/harvester/issues/5348#issuecomment-2055453709[此 GitHub 问题评论]。要解决此问题，请手动删除旧的`+harvester-cloud-provider+` pod。您可能需要多次执行此操作，直到新的 pod 能够成功调度。
====
...
从Harvester云提供商v0.2.0开始，客人Kubernetes集群内的`+LoadBalancer+`服务的额外健康检查不再必要。相反，您可以为您的工作负载配置https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#define-a-tcp-liveness-probe[活跃性]和https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-startup-probes/#define-readiness-probes[就绪]探针。因此，任何不可用的Pod将自动从负载均衡器端点中去除，以实现相同的预期结果。