...
. 生成 `+cloud-config+`。您可以使用 https://raw.githubusercontent.com/harvester/harvester-csi-driver/master/deploy/generate_addon_csi.sh[generate_addon_csi.sh] 脚本生成 `+cloud-config+` 文件。它可以在https://github.com/harvester/harvester-csi-driver[harvester/harvester-csi-driver]仓库中找到。
+
...
Harvester CSI驱动程序是https://github.com/harvester/charts/tree/master/charts/harvester-csi-driver[打包为Helm图表]。
有关更多信息，请参见https://charts.harvesterhci.io.
====
...
从*v0.1.25*开始，Harvester CSI 驱动程序支持https://kubernetes.io/docs/concepts/storage/volume-snapshots/[卷快照]，为在来宾 Kubernetes 集群上运行的工作负载提供时间点备份和恢复功能。
