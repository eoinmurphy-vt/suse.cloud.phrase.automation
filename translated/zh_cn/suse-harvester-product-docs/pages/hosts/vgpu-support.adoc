...
要确定您的GPU是否支持SR-IOV，请检查设备文档。有关创建支持SR-IOV的NVIDIA vGPU的更多信息，请参见https://docs.nvidia.com/grid/15.0/grid-vgpu-user-guide/index.html#creating-sriov-vgpu-device-red-hat-el-kvm[NVIDIA文档]。
...
[NOTE]
====
控制文件列表取决于GPU和主机的/sys树。有关可用控制文件及其功能的更多信息，请参见https://docs.nvidia.com/grid/15.0/grid-vgpu-user-guide/index.html#supported-gpus-grid-vgpu[NVIDIA文档]。
...
{harvester-product-name}允许https://docs.nvidia.com/datacenter/tesla/mig-user-guide[MIG]支持的vGPU在虚拟机之间共享。MIG支持的vGPU存在于支持MIG的物理GPU的GPU实例上。每个驻留的 MIG 支持的 vGPU 独占访问 GPU 实例的引擎，包括计算和视频解码引擎。
