...
{harvester-product-name}提供了一个内置的监控集成，使用https://prometheus.io/[Prometheus]。安装期间监控会自动启用。

在`+Dashboard+`页面，用户可以分别查看集群指标和使用最多的前10个虚拟机指标。
此外，用户可以点击http://grafana.com/[Grafana]仪表板链接，在Grafana用户界面上查看更多仪表板。
...
有关详细的配置说明，请参见https://documentation.suse.com/cloudnative/rancher-manager/v2.11/en/observability/monitoring-and-dashboards/configuration/receivers.html[Receiver Configuration]中的{rancher-short-name}文档。
...
我们已经创建了一个https://github.com/harvester/harvester/issues/2760[GitHub问题]来跟踪上游更改。一旦该功能可用，{harvester-product-name}将采用它。
