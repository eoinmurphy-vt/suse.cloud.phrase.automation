...
. 使用https://github.com/k3s-io/k3s/blob/master/contrib/util/generate-custom-ca-certs.sh[自定义证书脚本]生成证书。

. 生成证书后，创建一个本地目录，因为默认目录将无法访问。例如，`+k3k/custom-certs+`。

. 将生成的证书复制到目录 - `+$ sudo cp -r /var/lib/rancher/k3s/server/tls k3k/custom-certs/+`

. 使用路径`+/var/lib/rancher/k3s/server/tls k3k/custom-certs/+`以两种模式创建虚拟集群：
+
* 虚拟模式 
* 共享模式
