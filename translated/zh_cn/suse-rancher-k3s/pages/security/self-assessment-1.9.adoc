...
本文件是 xref:security/hardening-guide.adoc[K3s 安全强化指南] 的补充。安全强化指南为 K3s 的生产安装提供了安全强化的建议指导，而本基准指南旨在帮助您评估安全强化集群在 CIS Kubernetes 基准中的每个控制项的安全级别。该指南供 K3s 操作员、安全团队、审计员和决策者使用。

本指南特定于 K3s 的 *v1.27-v1.29* 版本线和 CIS Kubernetes 基准的 *v1.9* 版本。
...
=== 1.1.5 确保调度器 pod 规范文件的权限设置为 600 或更严格（自动化）

*结果：*不适用

*说明:*

默认情况下，K3s 将调度器嵌入到 k3s 进程中。没有调度器 pod 规范文件。

=== 1.1.6 确保调度器 pod 规范文件的所有权设置为 root:root（自动化）
...
[%collapsible]
======
默认情况下，K3s 将 CNI 文件权限设置为 600。请注意，对于许多 CNI，会创建一个权限为 750 的锁定文件。这是预期的，可以忽略。如果您修改 CNI 配置，请确保权限设置为 600。例如，`+chmod 600 /var/lib/cni/networks/<filename>+`
======
...
[%collapsible]
======
不适用于非 etcd 集群。如果仅运行主节点而没有 etcd 角色，则此检查不适用。如果控制平面和 etcd 角色在同一节点上，但此检查为警告，则在 etcd 服务器节点上，从命令 'ps -ef | grep etcd' 获取 etcd 数据目录，作为参数 --data-dir。运行以下命令（根据上面找到的 etcd 数据目录）。例如，`+chmod 700 /var/lib/rancher/k3s/server/db/etcd+`
======
...
*说明：*对于 K3s，etcd 嵌入在 k3s 进程中。没有单独的 etcd 进程。因此，etcd 数据目录的所有权由 k3s 进程管理，应该是 root:root。
...
*修正：*在主节点上运行以下命令（根据您系统上的文件位置）。例如，`+chmod -R 600 /var/lib/rancher/k3s/server/tls/*.crt+`
...
[%collapsible]
======
请遵循文档并配置其他身份验证机制。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并删除类似以下内容。
...
*修正：*默认情况下，K3s 不设置 DenyServiceExternalIPs。要启用此标志，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，如下所示。
...
*预期结果:* '--kubelet-client-certificate' 存在，并且 '--kubelet-client-key' 存在 
...
[%collapsible]
======
默认情况下，K3s 自动提供 kubelet 客户端证书和密钥。它们被生成并位于 /var/lib/rancher/k3s/server/tls/client-kube-apiserver.crt 和 /var/lib/rancher/k3s/server/tls/client-kube-apiserver.key。如果出于某种原因需要提供自己的证书和密钥，可以在 K3s 配置文件 /etc/rancher/k3s/config.yaml 中设置以下参数。
...
[%collapsible]
======
默认情况下，K3s 自动提供 kubelet CA 证书文件，位于 /var/lib/rancher/k3s/server/tls/server-ca.crt。如果出于某种原因需要提供自己的 CA 证书，请考虑使用 k3s 证书命令行工具。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并删除任何类似以下的行。
...
[%collapsible]
======
默认情况下，K3s 不将 --authorization-mode 设置为 AlwaysAllow。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除任何类似以下的行。
...
[%collapsible]
======
默认情况下，K3s 将 --authorization-mode 设置为 Node 和 RBAC。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，确保未覆盖 authorization-mode。
======
...
*修正：*遵循 Kubernetes 文档并在配置文件中设置所需的限制。然后，编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并设置以下参数。
...
=== 1.2.10 确保未设置 admission control 插件 AlwaysAdmit（自动化）
...
[%collapsible]
======
默认情况下，K3s 不将 --enable-admission-plugins 设置为 AlwaysAdmit。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除任何类似以下的行。
...
=== 1.2.11 确保设置 admission control 插件 AlwaysPullImages（手动）

*结果：*警告

*修正：*根据 CIS 指南，"此设置可能会影响离线或隔离的集群，这些集群已预加载镜像且无法访问注册表以拉取正在使用的镜像。"此设置不适合使用此配置的集群。"编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并设置以下参数。
...
[%collapsible]
======
默认情况下，K3s 不将 --disable-admission-plugins 设置为任何值。遵循文档并根据您的环境创建 ServiceAccount 对象。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除任何类似以下的行。
...
[%collapsible]
======
默认情况下，K3s 不将 --NamespaceLifecycle 设置为任何值。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除任何类似以下的行。
...
[%collapsible]
======
默认情况下，K3s 将 --enable-admission-plugins 设置为 NodeRestriction。如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请检查您是否没有覆盖 admission 插件。如果有，请将 NodeRestriction 包含在列表中。
...
[%collapsible]
======
默认情况下，K3s 将 --profiling 参数设置为 false。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除任何类似以下的行。
...
*预期结果:* '--service-account-lookup' 不存在 或 '--service-account-lookup' 存在 
...
[%collapsible]
======
默认情况下，K3s 不设置 --service-account-lookup 参数。编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并设置 service-account-lookup。例如，
...
[%collapsible]
======
K3s 会自动生成并设置服务账户密钥文件。它位于 /var/lib/rancher/k3s/server/tls/service.key。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并删除任何类似以下的行。
...
[%collapsible]
======
K3s 会自动生成并设置 etcd 证书和密钥文件。它们位于 /var/lib/rancher/k3s/server/tls/etcd/client.crt 和 /var/lib/rancher/k3s/server/tls/etcd/client.key。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并删除任何类似以下的行。
...
[%collapsible]
======
默认情况下，K3s 会自动生成并提供 apiserver 的 TLS 证书和私钥。它们生成并位于 /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.crt 和 /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.key。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并删除任何类似以下的行。
...
[%collapsible]
======
默认情况下，K3s 会自动提供客户端证书颁发机构文件。它生成并位于 /var/lib/rancher/k3s/server/tls/client-ca.crt。如果出于某种原因需要提供自己的 CA 证书，请考虑使用 k3s 证书命令行工具。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并删除任何类似以下的行。
...
[%collapsible]
======
默认情况下，K3s 会自动提供 etcd 证书颁发机构文件。它生成并位于 /var/lib/rancher/k3s/server/tls/client-ca.crt。如果出于某种原因需要提供自己的 CA 证书，请考虑使用 k3s 证书命令行工具。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并删除任何类似以下的行。
...
[%collapsible]
======
K3s 可以配置为使用加密提供程序对静态存储中的机密数据进行加密。在控制平面节点上编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并设置以下参数： secrets-encryption: true。然后可以使用 k3s secrets-encrypt 命令行工具管理机密数据加密。如有需要，您可以在 /var/lib/rancher/k3s/server/cred/encryption-config.json 找到生成的加密配置。
======
...
[%collapsible]
======
K3s 可以配置为使用加密提供程序对静态存储中的机密数据进行加密。K3s 将使用 aescbc 提供程序。在控制平面节点上编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并设置以下参数： secrets-encryption: true。然后可以使用 k3s secrets-encrypt 命令行工具管理机密数据加密。如有需要，您可以在 /var/lib/rancher/k3s/server/cred/encryption-config.json 找到生成的加密配置。
======
...
[%collapsible]
======
默认情况下，K3s kube-apiserver 符合此测试。对这些值的更改可能会导致回归，因此在生产部署中应用之前，请确保所有 apiserver 客户端支持新的 TLS 配置。如果需要自定义 TLS 配置，请考虑创建与您的要求相符的此规则的自定义版本。如果此检查失败，请去除与 `+tls-cipher-suites+` 相关的任何自定义配置，或更新 /etc/rancher/k3s/config.yaml 文件以通过添加以下内容来匹配默认值：
...
=== 1.3.3 确保 --use-service-account-credentials 参数设置为 true（自动化）
...
[%collapsible]
======
默认情况下，K3s 将 --use-service-account-credentials 参数设置为 true。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除任何类似以下的行。
...
=== 1.3.4 确保 --service-account-private-key-file 参数设置为适当（自动化）
...
[%collapsible]
======
默认情况下，K3s 自动提供服务账户私钥文件。它在 /var/lib/rancher/k3s/server/tls/service.current.key 生成并存放。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除任何类似以下的行。
...
=== 1.3.5 确保 --root-ca-file 参数设置为适当（自动化）
...
[%collapsible]
======
默认情况下，K3s 自动提供 root CA 文件。它在 /var/lib/rancher/k3s/server/tls/server-ca.crt 生成并存放。如果出于某种原因您需要提供自己的 CA 证书，请考虑使用 k3s 证书命令行工具。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除如下所示的行。
...
[%collapsible]
======
默认情况下，K3s 不会设置 RotateKubeletServerCertificate 功能开关。如果您已启用此功能开关，则应将其去除。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，去除如下所示的行。
...
[%collapsible]
======
默认情况下，K3s 将 --bind-address 参数设置为 127.0.0.1。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除如下所示的行。
...
[%collapsible]
======
默认情况下，K3s 将 --profiling 参数设置为 false。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，并去除如下所示的行。
...
[%collapsible]
======
如果在使用sqlite或外部数据库上运行，etcd检查不适用。当使用嵌入式etcd时，K3s会为etcd生成证书和密钥文件。这些文件位于/var/lib/rancher/k3s/server/tls/etcd/。如果此检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改为使用自定义证书和密钥文件。
======
...
[%collapsible]
======
如果在使用sqlite或外部数据库上运行，etcd检查不适用。当使用嵌入式etcd时，K3s 将 --client-cert-auth 参数设置为 true。如果此检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改为禁用客户端证书认证。
======
...
[%collapsible]
======
如果在使用sqlite或外部数据库上运行，etcd检查不适用。当使用嵌入式etcd时，K3s 不会设置 --auto-tls 参数。如果此检查失败，请在主节点上编辑 etcd pod 规范文件 /var/lib/rancher/k3s/server/db/etcd/config，删除 --auto-tls 参数或将其设置为 false。 client-transport-security: auto-tls: false
======

=== 2.4 确保 --peer-cert-file 和 --peer-key-file 参数设置为适当值（手动）。
...
[%collapsible]
======
如果在使用sqlite或外部数据库上运行，etcd检查不适用。当使用嵌入式etcd时，K3s 为etcd生成对等证书和密钥文件。这些文件位于/var/lib/rancher/k3s/server/tls/etcd/。如果此检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改为使用自定义对等证书和密钥文件。
======
...
[%collapsible]
======
如果使用sqlite或外部数据库，etcd检查不适用。当使用嵌入式etcd时，K3s 将 --peer-cert-auth 参数设置为 true。如果此检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改为禁用对等客户端证书认证。
======
...
[%collapsible]
======
如果使用sqlite或外部数据库，etcd检查不适用。在使用嵌入式 etcd 时，K3s 不会设置 --peer-auto-tls 参数。如果此检查失败，请编辑主节点上的 etcd pod 规范文件 /var/lib/rancher/k3s/server/db/etcd/config，删除 --peer-auto-tls 参数或将其设置为 false。 peer-transport-security: auto-tls: false
======
...
*基本原理:*kubelet 嵌入在 k3s 进程中。没有 kubelet 服务文件，所有配置在运行时作为参数传递。

=== 4.1.2 确保 kubelet 服务文件的所有权设置为 root:root（自动化）

*结果：*不适用

*基本原理:*kubelet 嵌入在 k3s 进程中。没有 kubelet 服务文件，所有配置在运行时作为参数传递。所有配置在容器运行时作为参数传递。
...
*基本原理:*kubelet 嵌入在 k3s 进程中。没有 kubelet 配置文件，所有配置在运行时作为参数传递。

=== 4.1.10 确保 kubelet --config 配置文件的所有权设置为 root:root（自动化）

*结果：*不适用

*基本原理:*kubelet 嵌入在 k3s 进程中。没有 kubelet 配置文件，所有配置都在运行时作为参数传递。 +
## 4.2 Kubelet
...
[%collapsible]
======
默认情况下，K3s 将 --anonymous-auth 设置为 false。如果您将其设置为其他值，则应将其设置回 false。如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请删除类似以下的行。
...
*预期结果:* '--authorization-mode' 没有 'AlwaysAllow' 
...
[%collapsible]
======
默认情况下，K3s 将 --read-only-port 设置为 0。如果您将其设置为其他值，则应将其设置回 0。如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请删除类似以下的行。
...
如果使用命令行，请运行 K3s，使用 --kubelet-arg="make-iptables-util-chains=true"。根据您的系统，重启 k3s 服务。例如，systemctl restart k3s.service
======
...
*基本原理:*默认情况下，K3s 会设置 --hostname-override 参数。根据 CIS 指南，这是为了遵守要求此标志的云服务提供商，以确保主机名与节点名称匹配。
...
如果使用命令行，请运行 K3s，使用 --kubelet-arg="event-qps=<value>"。根据您的系统，重启 k3s 服务。例如，systemctl restart k3s.service
======
...
[%collapsible]
======
默认情况下，K3s 会自动提供 Kubelet 的 TLS 证书和私钥。它们会生成并位于 /var/lib/rancher/k3s/agent/serving-kubelet.crt 和 /var/lib/rancher/k3s/agent/serving-kubelet.key。如果出于某种原因您需要提供自己的证书和密钥，可以在 K3s 配置文件 /etc/rancher/k3s/config.yaml 中设置以下参数。
...
[%collapsible]
======
默认情况下，K3s 不会设置 --rotate-certificates 参数。如果您已将此标志设置为 `+false+`，则应将其设置为 `+true+` 或完全删除该标志。如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请删除任何 rotate-certificates 参数。如果使用命令行，请删除 K3s 标志 --kubelet-arg="rotate-certificates"。根据您的系统，重启 k3s 服务。例如，systemctl restart k3s.service
======
...
*预期结果：*'RotateKubeletServerCertificate' 存在或者 'RotateKubeletServerCertificate' 不存在 
...
[%collapsible]
======
默认情况下，K3s 不会设置 RotateKubeletServerCertificate 功能开关。如果您已启用此功能开关，则应将其删除。如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请删除任何 feature-gate=RotateKubeletServerCertificate 参数。如果使用命令行，请删除 K3s 标志 --kubelet-arg="feature-gate=RotateKubeletServerCertificate"。根据您的系统，重启 k3s 服务。例如，systemctl restart k3s.service
======
...
[%collapsible]
======
如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请编辑该文件将 `+TLSCipherSuites+` 设置为
...
或这些值的子集。如果使用命令行，请添加 K3s 标志 --kubelet-arg="tls-cipher-suites=<same values as above>" 根据您的系统，重启 k3s 服务。例如，systemctl restart k3s.service
======
...
[%collapsible]
======
识别所有与 cluster-admin 角色相关的 clusterrolebindings。检查它们是否被使用，以及是否需要此角色，或者是否可以使用权限更少的角色。K3s 对 helm-kube-system-traefik 和 helm-kube-system-traefik-crd clusterrolebindings 给予例外，因为这些是将 traefik 安装到 kube-system 命名空间以进行常规操作所必需的。在可能的情况下，首先将用户绑定到权限较低的角色，然后去除对 cluster-admin 角色的 clusterrolebinding：
...
[%collapsible]
======
在可能的情况下，将集群角色和角色中的任何通配符使用替换为特定对象或操作。K3s 对以下集群角色给予例外，这些角色是常规操作所必需的： - k3s-cloud-controller-manager, local-path-provisioner-role, cluster-admin - system:kube-controller-manager, system:kubelet-api-admin, system:controller:namespace-controller, - system:controller:disruption-controller, system:controller:generic-garbage-collector, - system:controller:horizontal-pod-autoscaler, system:controller:resourcequota-controller
======
...
[%collapsible]
======
在 Kubernetes 工作负载需要特定访问 Kubernetes API 服务器的地方创建显式服务帐户。K3s 对 kube-system 名称空间中的默认服务账户给予例外。修改每个默认服务账户的配置，以包含此值 automountServiceAccountToken: false 或使用 kubectl：
...
[%collapsible]
======
修改不需要挂载服务账户令牌的 ServiceAccounts 和 Pods 的定义，以禁用该功能，使用 `+automountServiceAccountToken: false+`.如果 ServiceAccount 和 Pod 的 .spec 都指定了 automountServiceAccountToken 的值，则 Pod 的 spec 优先。条件：当 - ServiceAccount 的 automountServiceAccountToken 为 false 且 Pod 的 automountServiceAccountToken 为 false 或未设置时，Pod 为合规；当 - ServiceAccount 的 automountServiceAccountToken 为 true 或未设置，而 Pod 的 automountServiceAccountToken 为 false 时，K3s 对以下服务账户给予例外，这些服务账户是正常操作所必需的：- coredns，helm-traefik，helm-traefik-crd，traefik，metrics-server，svclb，local-path-provisioner-service-account。
======
...
*修正:*在可能的情况下，从集群中去除 system:masters 组。

=== 5.1.8 限制在 Kubernetes 集群中使用 Bind、Impersonate 和 Escalate 权限（手动）

*结果：*警告

*修正:*在可能的情况下，从主体中去除 impersonate、bind 和 escalate 权限。
...
*修正:*在可能的情况下，去除对 validatingwebhookconfigurations 或 mutatingwebhookconfigurations 对象的访问权限。

=== 5.1.13 最小化对服务账户词元创建的访问权限（手动）

*结果：*警告

*修正:*在可能的情况下，去除对服务账户对象的词元子资源的访问权限。## 5.2 Pod 安全标准
...
*修正:*为集群中每个包含用户工作负载的名称空间添加策略，以限制使用 `+hostPort+` 部分的容器的准入。## 5.3 网络策略和 CNI
...
*修正:*按照文档创建所需的 NetworkPolicy 对象。## 5.4 秘密管理

=== 5.4.1 优先使用文件中的 Secrets 而不是环境变量中的 Secrets（手动）
...
*修正:*参考您的云服务提供商或第三方 Secrets 管理解决方案提供的管理选项。## 5.5 可扩展的准入控制

=== 5.5.1 使用 ImagePolicyWebhook 准入控制器配置镜像溯源（手动）

*结果：*警告

*修正:*遵循 Kubernetes 文档并设置镜像溯源。## 5.7 一般政策
...
*修正:*确保创建名称空间以实现 Kubernetes 资源的适当隔离，并确保所有新资源都在特定的名称空间内创建。
