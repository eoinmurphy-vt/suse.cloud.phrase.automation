...
本文件是 xref:security/hardening-guide.adoc[K3s 安全强化指南] 的补充。安全强化指南为 K3s 的生产安装提供了强化的建议指导，而本基准指南旨在帮助您评估强化集群在 CIS Kubernetes 基准中的每个控制项的安全级别。该指南供 K3s 操作员、安全团队、审计员和决策者使用。

本指南特定于 K3s 的 *v1.26* 版本线和 CIS Kubernetes 基准的 *v1.8* 版本。

有关每个控制项的更多信息，包括详细描述和针对未通过测试的修复措施，您可以参考 CIS Kubernetes 基准 v1.8 的相应部分。您可以在 https://www.cisecurity.org/benchmark/kubernetes/[互联网安全中心 (CIS)] 创建免费账户后下载基准。
...
* *不适用* - 由于 K3s 的设计方式，该控制项不适用于 K3s。修正部分将解释原因。
...
=== 1.1.11 确保 etcd 目录的权限设置为 700 或更严格（自动化）
...
[%collapsible]
======
在 etcd 服务器节点上，获取 etcd 数据目录，作为参数 --data-dir，从命令 'ps -ef | grep etcd' 中获取。
运行以下命令（根据上面找到的 etcd 数据目录）。例如，`+chmod 700 /var/lib/etcd+`
======
...
它们在 /var/lib/rancher/k3s/server/tls/client-kube-apiserver.crt 和 /var/lib/rancher/k3s/server/tls/client-kube-apiserver.key 生成和存放。如果出于某种原因需要提供自己的证书和密钥，可以在 K3s 配置文件 /etc/rancher/k3s/config.yaml 中设置以下参数。
...
[%collapsible]
======
默认情况下，K3s 不将 --authorization-mode 设置为 AlwaysAllow。
如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除以下类似的行。
...
如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，确保未覆盖 authorization-mode。
======
...
=== 1.2.9 确保已设置准入控制插件 EventRateLimit（手动）
...
=== 1.2.10 确保未设置准入控制插件 AlwaysAdmit（自动化）
...
=== 1.2.11 确保已设置准入控制插件 AlwaysPullImages（手动）

*结果：*警告

*修正:*
按照 CIS 指南，这是一个宽松的设置，“此设置可能会影响预先加载了镜像且无法访问注册中心以拉取正在使用镜像的离线或孤立集群。”此设置不适合使用此配置的集群。"
...
=== 1.2.12 确保如果未使用 PodSecurityPolicy，则已设置准入控制插件 SecurityContextDeny（手动）

*结果：*不适用

*说明:*

在 K3s v1.25+ 上不再支持启用 Pod 安全策略，这将导致应用程序意外失败。

=== 1.2.13 确保已设置准入控制插件 ServiceAccount（自动化）
...
=== 1.2.14 确保已设置准入控制插件 NamespaceLifecycle（自动化）
...
=== 1.2.15 确保已设置准入控制插件 NodeRestriction（自动化）
...
=== 1.2.16 确保 --profiling 参数设置为 false（自动化）
...
如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，去除如下类似的行。
...
=== 1.2.17 确保 --audit-log-path 参数已设置（手动）
...
=== 1.2.18 确保 --audit-log-maxage 参数设置为 30 或适当的值（手动）
...
=== 1.2.19 确保 --audit-log-maxbackup 参数设置为 10 或适当的值（手动）
...
=== 1.2.20 确保 --audit-log-maxsize 参数设置为 100 或适当的值（手动）
...
=== 1.2.21 确保 --request-timeout 参数设置为适当的值（手动）
...
=== 1.2.23 确保 --service-account-key-file 参数设置为适当值（自动）
...
=== 1.2.24 确保 --etcd-certfile 和 --etcd-keyfile 参数设置为适当值（自动）
...
=== 1.2.25 确保 --tls-cert-file 和 --tls-private-key-file 参数设置为适当值（自动）
...
[%collapsible]
======
默认情况下，K3s 自动生成并提供 apiserver 的 TLS 证书和私钥。
它们生成并位于 /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.crt 和 /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.key。如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并删除以下类似的行。
...
=== 1.2.26 确保 --client-ca-file 参数设置为适当值（自动）
...
它位于 /var/lib/rancher/k3s/server/tls/client-ca.crt。
如果出于某种原因需要提供自己的 CA 证书，请考虑使用 k3s 证书命令行工具。
如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除以下类似的行。
...
=== 1.2.27 确保 --etcd-cafile 参数设置正确（自动）
...
[%collapsible]
======
默认情况下，K3s会自动提供etcd证书授权文件。
该文件在/var/lib/rancher/k3s/server/tls/client-ca.crt生成并存放。
如果出于某种原因需要提供自己的 CA 证书，请考虑使用 k3s 证书命令行工具。
如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除以下类似的行。
...
=== 1.2.28 确保 --encryption-provider-config 参数设置正确（手动）
...
[%collapsible]
======
默认情况下，K3s kube-apiserver符合此测试。对这些值的更改可能会导致回归，因此在生产部署中应用之前，请确保所有apiserver客户端支持新的TLS配置。
如果需要自定义TLS配置，请考虑创建与您的要求相符的此规则的自定义版本。
如果此检查失败，请去除与`+tls-cipher-suites+`相关的任何自定义配置，或更新/etc/rancher/k3s/config.yaml文件以通过添加以下内容来匹配默认值：
...
[%collapsible]
======
默认情况下，K3s 自动提供服务账户私钥文件。
它被生成并位于 /var/lib/rancher/k3s/server/tls/service.current.key。
如果此检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，去除以下类似的行。
...
如果出于某种原因需要提供自己的CA证书，请考虑使用k3s证书命令行工具。
如果此检查失败，请编辑K3s配置文件/etc/rancher/k3s/config.yaml，并去除以下类似的行。
...
=== 1.3.6 确保RotateKubeletServerCertificate参数设置为true（自动）
...
[%collapsible]
======
默认情况下，K3s不设置RotateKubeletServerCertificate功能开关。
如果您已启用此功能开关，则应将其去除。
如果此检查失败，请编辑K3s配置文件/etc/rancher/k3s/config.yaml，去除以下类似的行。
...
=== 1.3.7 确保--bind-address参数设置为127.0.0.1（自动）
...
[%collapsible]
======
默认情况下，K3s将--bind-address参数设置为127.0.0.1。如果此检查失败，请编辑K3s配置文件/etc/rancher/k3s/config.yaml，并去除以下类似的行。
...
=== 1.4.1 确保--profiling参数设置为false（自动）
...
[%collapsible]
======
默认情况下，K3s将--profiling参数设置为false。
如果此检查失败，请编辑K3s配置文件/etc/rancher/k3s/config.yaml，并去除以下类似的行。
...
=== 1.4.2 确保--bind-address参数设置为127.0.0.1（自动）
...
在使用嵌入式 etcd 时，K3s 为 etcd 生成证书和密钥文件。
...
======
如果使用 sqlite 或外部数据库，etcd 检查不适用。
在使用嵌入式 etcd 时，K3s 将 --client-cert-auth 参数设置为 true。
...
*预期结果：* '.client-transport-security.auto-tls' 存在 或者 '.client-transport-security.auto-tls' 不存在
...
======
如果使用 sqlite 或外部数据库，etcd 检查不适用。
在使用嵌入式 etcd 时，K3s 不会设置 --auto-tls 参数。
如果此检查失败，请编辑主节点上的 etcd pod 规格文件 /var/lib/rancher/k3s/server/db/etcd/config，去除 --auto-tls 参数或将其设置为 false。
client-transport-security:
  auto-tls: false
======
...
======
如果使用 sqlite 或外部数据库，etcd 检查不适用。
在使用嵌入式 etcd 时，K3s 为 etcd 生成对等证书和密钥文件。
这些文件位于 /var/lib/rancher/k3s/server/tls/etcd/。
...
如果此检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改以禁用对等客户端证书身份验证。
======
...
如果此检查失败，请编辑主节点上的 etcd pod 规格文件 /var/lib/rancher/k3s/server/db/etcd/config，并去除 --peer-auto-tls 参数或将其设置为 false。peer-transport-security: auto-tls: false
======
...
=== 4.1.4 如果存在代理 kubeconfig 文件，请确保所有权设置为 root:root（自动化）
...
[%collapsible]
======
默认情况下，K3s 将 --anonymous-auth 设置为 false。如果您将其设置为其他值，您应该将其重新设置为 false。如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请去除任何类似于以下的行。
...
======
默认情况下，K3s 不将 --authorization-mode 设置为 AlwaysAllow。
如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请删除任何类似于以下的行。
...
[%collapsible]
======
默认情况下，K3s 自动为 Kubelet 提供客户端 CA 证书。
它生成并位于 /var/lib/rancher/k3s/agent/client-ca.crt
======
...
[%collapsible]
======
默认情况下，K3s 将 --read-only-port 设置为 0。如果您将其设置为其他值，您应该将其重新设置为 0。如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请去除任何类似于以下的行。
...
如果使用命令行，请使用 --kubelet-arg="streaming-connection-idle-timeout=5m" 运行 K3s。
...
=== 4.2.9 确保 --tls-cert-file 和 --tls-private-key-file 参数设置为适当（自动）
...
[%collapsible]
======
默认情况下，K3s 自动提供 Kubelet 的 TLS 证书和私钥。
它们在 /var/lib/rancher/k3s/agent/serving-kubelet.crt 和 /var/lib/rancher/k3s/agent/serving-kubelet.key 生成和存放。如果出于某种原因您需要提供自己的证书和密钥，可以在 K3s 配置文件 /etc/rancher/k3s/config.yaml 中设置以下参数。
...
[%collapsible]
======
默认情况下，K3s 不会设置 --rotate-certificates 参数。如果您已将此标志设置为 `+false+`，则应将其设置为 `+true+` 或完全删除该标志。
...
[%collapsible]
======
如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请编辑文件以将 `+TLSCipherSuites+` 设置为
...
或这些值的子集。
如果使用命令行，请添加 K3s 标志 --kubelet-arg="tls-cipher-suites=<same values as above>"。根据您的系统，重启 k3s 服务。例如，systemctl restart k3s.service
======

=== 4.2.13 确保对 Pod 的进程 ID 设置了限制（手动）
...
=== 5.1.1 确保仅在需要时使用 cluster-admin 角色（手动）

*结果：*警告

*修正:*
识别所有与 cluster-admin 角色相关的 clusterrolebindings。检查它们是否被使用，以及它们是否需要此角色，或者是否可以使用具有较少权限的角色。
在可能的情况下，首先将用户绑定到较低权限的角色，然后删除与 cluster-admin 角色的 clusterrolebinding：kubectl delete clusterrolebinding [name]
...
*修正:*
在可能的情况下，去除对集群中 Secret 对象的获取、列出和监视访问权限。

=== 5.1.3 在角色和集群角色中最小化通配符的使用（手动）
...
修改每个默认服务帐户的配置，以包含此值 automountServiceAccountToken: false
...
=== 5.1.11 最小化对证书签名请求对象审批子资源的访问（手动）
...
*修正:*
在可能的情况下，去除对 validatingwebhookconfigurations 或 mutatingwebhookconfigurations 对象的访问
...
*修正:*
为集群中每个包含用户工作负载的名称空间添加策略，以限制希望共享主机进程 ID 名称空间的 `+hostPID+` 容器的准入。
...
*修正:*
为集群中每个包含用户工作负载的名称空间添加策略，以限制希望共享主机 IPC 名称空间的 `+hostIPC+` 容器的准入。
...
=== 5.2.6 最小化具有允许特权提升的容器的准入（自动化）

*结果：*警告

*修正:*
为集群中每个包含用户工作负载的名称空间添加策略，以限制具有 `+.spec.allowPrivilegeEscalation+` 设置为 `+true+` 的容器的准入。
...
*修正:*
确保 `+allowedCapabilities+` 在集群的策略中不存在，除非它被设置为空数组。

=== 5.2.10 最小化具有分配的能力的容器的准入（手动）

*结果：*警告

*修正:*
审查在您的集群上运行的应用程序中权限的使用。如果一个命名空间包含不需要任何 Linux 权限来操作的应用程序，请考虑添加一个禁止入场不丢弃所有权限的容器的 PSP。
...
=== 5.3.1 确保使用的 CNI 支持网络策略（手动）

*结果：*警告

*修正:*
如果使用的 CNI 插件不支持网络策略，则应考虑使用不同的插件，或寻找限制 Kubernetes 集群中流量的替代机制。

=== 5.3.2 确保所有名称空间都有定义的网络策略（手动）
...
*修正:*
如果可能，重写应用程序代码以从挂载的秘密文件中读取 Secrets，而不是从环境变量中读取。
...
*修正:*
请遵循文档并根据需要为您的部署中的对象创建名称空间。

=== 5.7.2 确保在您的 Pod 定义中将 seccomp 控制文件设置为 docker/default（手动）

*结果：*警告

*修正:*
使用 `+securityContext+` 在您的 Pod 定义中启用 docker/default seccomp 控制文件。
示例如下： +
  securityContext: +
    seccompProfile: +
      type:RuntimeDefault
...
*修正:*
确保创建名称空间以允许适当隔离 Kubernetes 资源，并且所有新资源都在特定名称空间中创建。
