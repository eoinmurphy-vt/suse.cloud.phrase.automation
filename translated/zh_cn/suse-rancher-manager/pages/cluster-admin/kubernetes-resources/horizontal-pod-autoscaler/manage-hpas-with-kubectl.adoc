...
本节描述了如何使用 `+kubectl+` 管理 HPA。本文件提供了以下操作的说明：

* 创建 HPA
* 获取 HPA 信息
* 删除 HPA
* 配置您的 HPA 以根据 CPU 或内存使用情况进行扩展
* 如果您使用 Prometheus 等第三方工具进行指标监控，可以配置 HPA 根据自定义指标进行扩展

您可以通过 Rancher UI 创建、查看和删除 HPA。您还可以通过 Rancher UI 配置它们根据 CPU 或内存使用情况进行扩展。有关更多信息，请参阅 {xref-hpas-with-ui}[使用 Rancher UI 管理 HPA]。要根据 CPU 或内存以外的其他指标扩展 HPA，您仍然需要 `+kubectl+`。

[#_basic_kubectl_command_for_managing_hpas]
== 管理 HPA 的基本 kubectl 命令

如果您有 HPA 清单文件，可以使用 `+kubectl+` 创建、管理和删除 HPA：

* 创建 HPA
 ** 使用清单： `+kubectl create -f <HPA_MANIFEST>+`
 ** 不使用清单（仅支持 CPU）： `+kubectl autoscale deployment hello-world --min=2 --max=5 --cpu-percent=50+`
* 获取 HPA 信息
...
HPA 清单是用于通过 `+kubectl+` 管理 HPA 的配置文件。

以下代码片段演示了在 HPA 清单中使用不同指令的情况。请查看示例下方的列表，以了解每个指令的目的。
...
| `+apiVersion: autoscaling/v2beta1+`
| 正在使用的 Kubernetes `+autoscaling+` API 组的版本。此示例清单使用的是 beta 版本，因此启用了基于 CPU 和内存的扩展。

| `+name: hello-world+`
| 指示 HPA 正在对 `+hello-word+` 部署执行自动扩展。
...
| `+targetAverageUtilization: 50+`
| 指示当平均运行的 Pod 使用超过 50% 的请求 CPU 时，部署将扩展 Pod。

| `+targetAverageValue: 100Mi+`
| 指示当平均运行的 Pod 使用超过 100Mi 的内存时，部署将扩展 Pod。
|===

[#_configuring_hpa_to_scale_using_resource_metrics_cpu_and_memory]
== 配置 HPA 以使用资源指标（CPU 和内存）进行扩展

在 Rancher v2.0.7 及更高版本中创建的集群具备使用水平 Pod 自动扩展器所需的所有要求（metrics-server 和 Kubernetes 集群配置）。

运行以下命令以检查您的安装中是否可用指标：
...
== 配置 HPA 以使用 Prometheus 的自定义指标进行扩展

您可以配置 HPA 以根据第三方软件提供的自定义指标进行自动扩展。使用第三方软件进行自动扩展的最常见用例是基于应用程序级别的指标（即每秒 HTTP 请求）。HPA使用`+custom.metrics.k8s.io+` API来获取这些指标。通过部署自定义指标适配器来启用此API，以便进行指标收集。

在本示例中，我们将使用https://prometheus.io/[Prometheus]。我们从以下假设开始：

* Prometheus已在集群中部署。
* Prometheus已正确配置，并从Pod、节点、命名空间等收集了适当的指标。
* Prometheus在以下URL和端口上可用：`+http://prometheus.mycompany.io:80+`

Prometheus可以在Rancher v2.0目录中进行部署。如果它尚未在您的集群中运行，请从Rancher目录中部署它。

为了让 HPA 使用 Prometheus 的自定义指标，您需要在集群的 `+kube-system+` 命名空间中安装软件包 https://github.com/DirectXMan12/k8s-prometheus-adapter[k8s-prometheus-adapter]。要安装`+k8s-prometheus-adapter+`，我们使用位于https://github.com/banzaicloud/banzai-charts[banzai-charts]的Helm图表。
...
. 从GitHub克隆`+banzai-charts+`仓库：
+
----
  # git clone https://github.com/banzaicloud/banzai-charts
----

. 安装`+prometheus-adapter+`图表，指定Prometheus的URL和端口号。
+
----
...
. 检查`+prometheus-adapter+`是否正常运行。检查`+kube-system+`命名空间中的服务Pod和日志。
. 检查服务Pod是否为`+Running+`。输入以下命令。
    `
...
. 检查指标API是否可以从kubectl访问。

* 如果您直接访问集群，请在kubectl配置中以以下格式输入您的服务器URL：`+\https://<Kubernetes_URL>:6443+`。
+
----
# kubectl get --raw /apis/custom.metrics.k8s.io/v1beta1
----
+
如果API可访问，您应该收到类似于以下内容的输出。
+
.API响应
[%collapsible]
======
...
* 如果您通过Rancher访问集群，请在kubectl配置中以以下格式输入您的服务器URL：`+\https://<RANCHER_URL>/k8s/clusters/<CLUSTER_ID>+`。在 API 路径中添加后缀`+/k8s/clusters/<CLUSTER_ID>+`。
+
----
