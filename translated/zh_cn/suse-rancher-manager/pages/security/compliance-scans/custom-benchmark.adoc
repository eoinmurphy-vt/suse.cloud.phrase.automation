...
每个基准版本定义了一组测试配置文件，这些文件定义了由https://github.com/aquasecurity/kube-bench[kube-bench]工具运行的合规性测试。
...
. 创建一个名为`+foo+`的目录，并在该目录中放置https://github.com/aquasecurity/kube-bench[kube-bench]工具查找的所有配置YAML文件。例如，这里是通用CIS 1.5基准版本https://github.com/aquasecurity/kube-bench/tree/master/cfg/cis-1.5的配置YAML文件。
