...
然而，用户可以选择在必要时以https://grafana.com/docs/grafana/latest/permissions/organization_roles/#admin-role[管理员]身份登录 Grafana。Grafana 实例的默认管理员用户名和密码将是`+admin+`/`+prom-operator+`，但在部署或升级图表时也可以提供其他凭据。
