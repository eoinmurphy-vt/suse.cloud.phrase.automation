= {elemental-product-name} サポート
:revdate: 2025-07-03
:page-revdate: {revdate}

`+elemental-support+`は、{elemental-product-name}スタックが実行されているマシンに関するすべての情報を収集するユーティリティプログラムです。

任意の{elemental-product-name}ノードのトラブルシューティングを行う際には、ログや設定ファイルを含む圧縮ファイルを生成することが可能です。 +
*生成されたtar.gzファイルには、管理クラスターへのアクセス・トークンなどの機密情報が含まれている可能性があることに注意してください*。

== 要件

オプションとして、`+elemental-support+`ユーティリティは、ノード情報を収集するためにターゲットマシン上で`+kubectl+`が利用可能である必要があります。これは、ノードの{elemental-product-name}アップグレードの問題をデバッグするのに非常に役立ちます。 +
使用されるkubectl設定は、`+KUBECONFIG+`環境変数から取得され、存在しない場合は`+/etc/rancher/k3s/k3s.yaml+`または`+/etc/rancher/rke2/rke2.yaml+`ファイルが代わりに使用されます。

== 情報の収集

実行されると、`+elemental-support+`は同じディレクトリに圧縮アーカイブを作成します。
