...
`+elemental-support+` は、{elemental-product-name} スタックが動作しているマシンに関するすべての情報を収集するユーティリティ・プログラムである。

{elemental-product-name} ノードの問題をトラブルシューティングする場合、ログと設定ファイルを含む圧縮ファイルを生成することができます。 +
*生成されたtar.gzファイルには、*管理クラスタへのアクセストークンなど、*機密情報が含まれている可能性がある*ことに注意してほしい。

== 必要条件

オプションとして、`+elemental-support+` ユーティリティは、ノード情報を収集するために、ターゲット・マシン上で`+kubectl+` が利用可能である必要がある。これはノードの{elemental-product-name} アップグレードの問題をデバッグするのに非常に便利です。 +
使用される kubectl config は`+KUBECONFIG+` 環境変数から検索され、それがない場合は`+/etc/rancher/k3s/k3s.yaml+` または`+/etc/rancher/rke2/rke2.yaml+` ファイルが代わりに使用されます。

== 情報収集

実行すると、`+elemental-support+` 、同じディレクトリ内に圧縮アーカイブが作成される。
