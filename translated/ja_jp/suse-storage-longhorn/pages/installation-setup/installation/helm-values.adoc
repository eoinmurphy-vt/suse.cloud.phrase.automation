...
| {longhorn-product-name}が製品改善のために匿名の使用データを定期的に収集できるようにする設定です。{longhorn-product-name}は収集したデータをhttps://github.com/longhorn/upgrade-responder[アップグレードレスポンダー]サーバーに送信し、これはLonghorn Public Metrics Dashboard (https://metrics.longhorn.io)のデータソースです。アップグレードレスポンダーサーバーは、IPアドレスを含むクライアントを特定できるデータを保存しません。
