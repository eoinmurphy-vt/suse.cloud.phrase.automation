...
https://gitlab.com/cryptsetup/cryptsetup[Cryptsetup]は、`+dm-crypt+`ベースのデバイスマッパーターゲットを便利に設定するために使用されるオープンソースユーティリティであり、{longhorn-product-name}はLinuxディスク暗号化の標準であるhttps://gitlab.com/cryptsetup/cryptsetup#luks-design[LUKS2]（Linux Unified Key Setup）形式を使用してボリューム暗号化をサポートします。
