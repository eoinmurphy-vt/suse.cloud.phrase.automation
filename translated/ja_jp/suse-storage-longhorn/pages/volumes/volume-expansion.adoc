...
+
ファイルシステムが`+ext4+`の場合、ファイルシステムを手動でリサイズする前に、ボリュームをhttps://linux.die.net/man/8/mount[マウント]し、https://linux.die.net/man/8/umount[アンマウント]する必要があるかもしれません。そうでない場合、`+resize2fs+`を実行するとエラーが発生する可能性があります：
+
----
...
* Kubernetesはv1.29からhttps://kubernetes.io/blog/2023/12/15/csi-node-expand-secret-support-ga/[認証されたCSIストレージのリサイズ]をネイティブにサポートしています。
