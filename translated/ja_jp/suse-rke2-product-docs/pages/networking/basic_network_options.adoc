...
spec:
  valuesContent: |-
    flannel:
      iface: "eth1"
----
フランネルのhttps://github.com/flannel-io/flannel/blob/master/Documentation/backends.md#wireguard[ワイヤガードバックエンド]（https://www.wireguard.com/install/[カーネル < 5.6 のユーザーはモジュールをインストールする必要があります]）を使用するには、次のチャート値を使用する必要があります：
