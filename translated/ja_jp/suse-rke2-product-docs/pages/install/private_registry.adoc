...
プライベートレジストリが別のレジストリのミラーとして使用される場合、たとえばhttps://docs.docker.com/registry/recipes/mirror/[プルスルーキャッシュ]を構成する際には、イメージのプルがリストされたエンドポイントに透過的にリダイレクトされます。元のレジストリ名は、`+ns+`クエリパラメータを介してミラーエンドポイントに渡されます。
