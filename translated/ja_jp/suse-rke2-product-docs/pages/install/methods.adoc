...
署名されたRPMは、`+rpm-testing.rancher.io+`および`+rpm.rancher.io+`のRPMリポジトリ内でRKE2のために公開されています。RPMをサポートするノードでhttps://get.rke2.ioスクリプトを実行すると、デフォルトでこれらのRPMリポジトリを使用します。しかし、あなた自身でインストールすることもできます。
