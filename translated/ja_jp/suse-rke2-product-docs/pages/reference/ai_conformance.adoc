...
参考のために、Azureを例として、アップストリームオートスケーラーhttps://github.com/kubernetes/autoscaler[autoscaler]の使用方法を説明します。

. GPUを搭載したVMでhttps://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/overview[仮想マシンスケールセット（VMSS）]を作成します。
...
AI/MLワークロードに関連するカスタムメトリクスに基づいてポッドをスケールする能力は、Kubernetesにデフォルトで含まれているhttps://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/[HorizontalPodAutoscaler (HPA)]を使用して実現されます。
