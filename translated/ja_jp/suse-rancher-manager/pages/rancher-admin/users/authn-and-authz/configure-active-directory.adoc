...
| 無効ステータスビットマスク
| これは、無効なユーザーアカウントを示す`+User Enabled Attribute+`の値です。通常は、Microsoft Active Directoryスキーマで指定されているデフォルト値の"2"に設定しておくべきです（詳細はhttps://docs.microsoft.com/en-us/windows/desktop/adschema/a-useraccountcontrol#remarks[こちら]を参照）。
|===
...
| 検索フィルタ
| このフィルターは、Rancherがサイトアクセスリストにグループを追加しようとしたり、クラスターやプロジェクトにグループを追加しようとしたときに検索されるグループのリストに適用されます。例えば、グループ検索フィルターは`+(\|(cn=group1)(cn=group2))+`のようになります。注意:検索フィルターがhttps://docs.microsoft.com/en-us/windows/win32/adsi/search-filter-syntax[有効なAD検索構文,]を使用しない場合、グループのリストは空になります。
