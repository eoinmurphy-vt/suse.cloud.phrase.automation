...
Rancher Backupsチャートには、チャートがインストールされるときに1つの大きなresourceSetに追加されるYAMLファイルの組み合わせであるhttps://github.com/rancher/backup-restore-operator/tree/release/v3.0/charts/rancher-backup/files/default-resourceset-contents[デフォルトのresourceSet]が含まれています。ファイルの順序は重要ではありません。resourceSetsはバージョンによって異なる場合があります。
...
* 復元時に注意すべきことの一つは、Rancherリソースのクラスターを&quot;`+wipe+`&quot;する必要があるかもしれないということです。これは、クラスターにジョブとしてhttps://github.com/rancher/rancher-cleanup[Rancherクリーンアップスクリプト]をデプロイすることで行うことができます。これにより、Rancherバックアップを再度インストールし、完全に新しいクラスターに復元することができます。
