...
[NOTE]
====
Kubernetesは、シークレット、証明書、レジストリをすべてhttps://kubernetes.io/docs/concepts/configuration/secret/[シークレット]として分類し、プロジェクトまたはネームスペース内の2つのシークレットが同じ名前を持つことはできません。したがって、競合を防ぐために、レジストリはワークスペース内のすべてのシークレットの中で一意の名前を持つ必要があります。
====
