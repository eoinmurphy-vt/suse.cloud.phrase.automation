...
HPAがPrometheusからカスタムメトリクスを使用するには、クラスターの`+kube-system+`ネームスペースにhttps://github.com/DirectXMan12/k8s-prometheus-adapter[k8s-prometheus-adapter]パッケージが必要です。`+k8s-prometheus-adapter+`をインストールするために、https://github.com/banzaicloud/banzai-charts[banzai-charts]で利用可能なHelmチャートを使用します。
