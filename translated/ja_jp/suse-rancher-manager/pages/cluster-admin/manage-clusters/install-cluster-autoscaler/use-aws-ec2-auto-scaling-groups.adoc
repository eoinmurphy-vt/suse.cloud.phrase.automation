...
このガイドでは、AWS EC2 Auto Scaling Groupsを使用してRancherカスタムクラスターにhttps://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/[Kubernetes cluster-autoscaler]をインストールして使用する方法を示します。
...
Rancherサーバー上で、カスタムk8sクラスターを作成する必要があります。バージョンの互換性を確認するにはhttps://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/[こちら]を参照してください。
 ...
 ** IAMプロファイル:Kubernetesのcloud_provider統合で必要です。オプションで、`+AWS_ACCESS_KEY+`と`+AWS_SECRET_KEY+`はhttps://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/cloudprovider/aws/README.md#using-aws-credentials[using-aws-credentials.]の代わりに使用できます。このプロファイルは`+K8sMasterProfile+`と呼ばれます。
...
詳細は{xref-cloud-provider-amazon}[AWS上のRKE2クラスター]およびhttps://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/cloudprovider/aws/README.md[AWS上のクラスターオートスケーラー]にあります。
...
https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/cloudprovider/aws/examples/cluster-autoscaler-run-on-control-plane.yaml[cluster-autoscaler-run-on-control-plane.yaml]の例に基づいて、好ましいhttps://github.com/kubernetes/autoscaler/tree/master/cluster-autoscaler/cloudprovider/aws#auto-discovery-setup[自動検出セットアップ]を使用するために、独自の`+cluster-autoscaler-deployment.yaml+`を作成し、トレランス、nodeSelector、イメージバージョン、コマンド設定を更新しました：
