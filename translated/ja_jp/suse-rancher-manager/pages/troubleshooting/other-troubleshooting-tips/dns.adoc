...
クエリログを有効にするには、configmap `+coredns+`のCorefile設定でhttps://coredns.io/plugins/log/[ログプラグイン]を有効にします。`+kubectl -n kube-system edit configmap coredns+`を使用するか、以下のコマンドを使用して設定をそのまま置き換えることができます。
