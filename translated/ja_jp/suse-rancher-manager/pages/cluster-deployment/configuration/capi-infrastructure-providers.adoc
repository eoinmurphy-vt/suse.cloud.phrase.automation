...
Rancher 2.14は、https://cluster-api-aws.sigs.k8s.io/[CAPA]（Cluster API Provider AWS）やhttps://github.com/kubernetes-sigs/cluster-api-provider-vsphere[CAPV]（Cluster API Provider vSphere）などのネイティブCAPIインフラストラクチャプロバイダーを使用してRKE2クラスターをプロビジョニングできます。
...
* Kubernetesクラウドプロバイダーの要件：ダウンストリームクラスターが実行されるインフラストラクチャ用のクラウド特有のKubernetesプロバイダーが必要です（例：CAPA用のhttps://cloud-provider-aws.sigs.k8s.io/[Kubernetes AWS Cloud Provider]またはCAPV用の`+rancher-vsphere-cpi+`チャート）。
...
これを行うために、CAPAは必要なオブジェクトを生成して適用するための`+clusterawsadm+`ツールを提供します。詳細については、CAPAマニュアルを参照してくださいhttps://cluster-api-aws.sigs.k8s.io/topics/using-clusterawsadm-to-fulfill-prerequisites[詳細]。

次に、CAPAプロバイダーがAWS上でリソースを作成できるように、アップストリームクラスタでプロバイダーアイデンティティを構成します。すべてのhttps://cluster-api-aws.sigs.k8s.io/topics/multitenancy[オプション]については、マニュアルを参照してください。
...
まず、CAPVプロバイダーがあなたのvSphereサーバー上でリソースを作成できるように、アップストリームクラスターでプロバイダーアイデンティティを構成します。すべてのアイデンティティhttps://github.com/kubernetes-sigs/cluster-api-provider-vsphere/blob/v1.15.2/docs/identity_management.md[オプション]についてはマニュアルを参照してください。また、一般的なvSpherehttps://github.com/kubernetes-sigs/cluster-api-provider-vsphere/blob/v1.15.2/docs/getting_started.md[要件]についても参照してください。
...
CAPAと同様に、ダウンストリームクラスターにhttps://github.com/kubernetes/cloud-provider-vsphere[vSphere用のクラウドプロバイダー]をインストールする必要があります。
