...
{harvester-product-name}はブータブルアプライアンスイメージとして出荷され、ISOイメージを使用してベアメタルサーバーに直接インストールできます。ISOイメージを取得するには、*💿 harvester-v1.x.x-amd64.iso*をhttps://github.com/harvester/harvester/releases[リリース]ページからダウンロードしてください。

インストール中に、*新しいクラスターを作成する*か、*既存のクラスターにノードを参加させる*かを選択できます。

以下のhttps://youtu.be/X0VIGZ_lExQ[ビデオ]は、ISOインストールの簡単な概要を示しています。
...
. クラスターをインストールするディスクと、VMデータを保存するデータディスクを選択します。デフォルトでは、{harvester-product-name}はUEFIおよびBIOSの両方に対してhttps://en.wikipedia.org/wiki/GUID_Partition_Table[GUIDパーティションテーブル (GPT)]パーティショニングスキーマを使用します。BIOSブートを使用する場合、https://en.wikipedia.org/wiki/Master_boot_record[マスターブートレコード (MBR)]を選択するオプションがあります。
+
[IMPORTANT]
====
