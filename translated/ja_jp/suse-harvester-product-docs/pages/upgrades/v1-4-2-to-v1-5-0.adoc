...
回避策は、アップグレードを開始する前に{longhorn-product-name} UIで設定https://documentation.suse.com/cloudnative/storage/1.8/en/longhorn-system/settings.html#_automatically_delete_workload_pod_when_the_volume_is_detached_unexpectedly[ボリュームが予期せず切り離されたときにワークロードポッドを自動的に削除する]を無効にすることです。アップグレードが完了したら、再度設定を有効にする必要があります。
