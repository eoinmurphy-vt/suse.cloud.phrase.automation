...
+
レプリカを手動で切り離すか、他のノードに移動し、その後https://documentation.suse.com/cloudnative/storage/1.8/en/troubleshooting-maintenance/maintenance.html#_updating_the_node_os_or_container_runtime[ノードをドレインする]ためにコマンド`+kubectl drain --ignore-daemonsets <node name>+`を使用する必要があります。オプション`+--ignore-daemonsets+`は、{longhorn-product-name}がLonghorn Manager、Longhorn CSIプラグイン、Longhorn Engineイメージなどのデーモンセットをデプロイするために必要です。
+
