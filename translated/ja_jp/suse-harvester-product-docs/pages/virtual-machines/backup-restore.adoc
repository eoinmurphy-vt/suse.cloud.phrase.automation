...
ただし、仮想マシンの最新バックアップを削除するか、{longhorn-product-name}設定https://documentation.suse.com/cloudnative/storage/latest/en/longhorn-system/settings.html#_auto_cleanup_snapshot_when_delete_backup[Auto Cleanup Snapshot When Delete Backup]を有効にすると、関連するボリュームに対するすべての後続の操作がブロックされます。これはhttps://github.com/longhorn/longhorn/issues/9064[既知の問題]であり、ボリュームスナップショット、バックアップ、ライブマイグレーションなどの操作に影響を与えます。
...
ただし、仮想マシンの最新のバックアップを削除すると、関連するボリュームに対するすべての後続の操作がブロックされます。これは、ボリュームスナップショット、バックアップ、およびライブマイグレーションなどの操作に影響を与えるhttps://github.com/longhorn/longhorn/issues/9064[既知の問題]です。
...
ゲスト仮想マシンが*QEMUゲストエージェント*に接続されているとき、{harvester-product-name}コントローラーはKubevirtのhttps://github.com/kubevirt/kubevirt/blob/main/docs/freeze.md#virt-freezer[virt-freezer]アプリケーションを通じてファイルシステムフリーズ操作を実行し、仮想マシンのバックアップとスナップショット中のファイルシステムの整合性を確保します。
