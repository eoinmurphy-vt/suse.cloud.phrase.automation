...
SUSEは、https://www.suse.com/download/sles/[SUSE Linux Enterprise (SLE)]およびhttps://get.opensuse.org/leap/[openSUSE Leap]の仮想マシンイメージを提供しており、{harvester-product-name}での使用に適しています。これらのイメージは、https://build.opensuse.org/[openSUSE Build Service] (OBS)上でhttps://osinside.github.io/kiwi/[Kiwi]イメージビルディングツールを使用して構築されており、ダウンロード後すぐに使用できます。
...
あなたのイメージはhttps://download.opensuse.org/（`+repositories/home:/YOUR_USER_NAME:/branches:/SUSE:/Templates:/Images:/+`の下）に公開されます。
