...
仮想マシンオブジェクトを作成する際、vm-import-controllerアドオンはソース仮想マシンの名前を使用しますが、これはKubernetesオブジェクトのhttps://kubernetes.io/docs/concepts/overview/working-with-objects/names/#dns-subdomain-names[命名基準]を満たさない可能性があります。インポートの成功を完了させるために、ソース仮想マシンの名前を変更する必要があるかもしれません。

===== VMware ToolsなしのVMwareベースの仮想マシンは移行されません

{harvester-product-name} v1.6.0でVMwareベースの仮想マシンをインポートしようとすると、仮想マシンにhttps://knowledge.broadcom.com/external/article/315382/overview-of-vmware-tools.html[VMware Tools]がインストールされていない場合、次の問題が発生します：
...
* VMware Toolsまたはhttps://knowledge.broadcom.com/external/article?legacyId=2073803[open-vm-tools]をインストールしてください。
