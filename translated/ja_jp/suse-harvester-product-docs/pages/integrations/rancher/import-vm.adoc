= SUSE® Virtualization VM上に構築されたクラスターをインポートします。
:revdate: 2025-05-09
:page-revdate: {revdate}

Rancherは、KubernetesをインストールしたSUSE® Virtualization仮想マシンをインポートすることを許可します。

== 展開

=== 前提条件

* Kubernetesクラスターは、SUSE® Virtualization VMの上に構築されています。

=== SUSE® Virtualization仮想マシンにゲストクラスターをデプロイします。

. https://raw.githubusercontent.com/harvester/harvester-csi-driver/master/deploy/generate_addon_csi.sh[generate_addon_csi.sh]スクリプトを使用してcsi-driver cloud-configファイルを生成します。このスクリプトは、https://github.com/harvester/harvester-csi-driver[harvester/harvester-csi-driver]リポジトリにあります。
+
例:
+
[,shell]
----
 ./generate_addon_csi.sh <serviceaccount name> <namespace> RKE2
----
+
生成された出力は、以下のようになります：
+
[,yaml]
----
 ########## cloud-config ############
 apiVersion: v1
 clusters:
 - cluster: <token>
     server: https://<YOUR HOST HARVESTER VIP>:6443
   name: default
 contexts:
 - context:
     cluster: default
     namespace: default
     user: rke2-guest-01-default-default
   name: rke2-guest-01-default-default
 current-context: rke2-guest-01-default-default
 kind: Config
 preferences: {}
 users:
 - name: rke2-guest-01-default-default
   user:
     token: <token>

 ########## cloud-init user data ############
 write_files:
   - encoding: b64
     content: 
     owner: root:root
     path: /var/lib/rancher/rke2/etc/config-files/cloud-provider-config
     permissions: '0644'
----

. https://raw.githubusercontent.com/harvester/cloud-provider-harvester/master/deploy/generate_addon.sh[generate_addon.sh]スクリプトを使用してcloud-provider cloud-configを生成します。このスクリプトは、https://github.com/harvester/cloud-provider-harvester[harvester/cloud-provider-harvester]リポジトリにあります。
+
例:
+
[,shell]
----
 ./generate_addon.sh <serviceaccount name> <namespace>
----
+
生成された出力は、以下のようになります：
 出力は以下のようになります：
+
[,yaml]
----
 ########## cloud config ############
 apiVersion: v1
 clusters:
 - cluster:
     certificate-authority-data: <CACERT>
     server: https://HARVESTER-ENDPOINT/k8s/clusters/local
   name: local
 contexts:
 - context:
     cluster: local
     namespace: default
     user: harvester-cloud-provider-default-local
   name: harvester-cloud-provider-default-local
 current-context: harvester-cloud-provider-default-local
 kind: Config
 preferences: {}
 users:
 - name: harvester-cloud-provider-default-local
   user:
     token: <TOKEN>

 ########## cloud-init user data ############
 write_files:
 - encoding: b64
   content: <CONTENT>
   owner: root:root
   path: /etc/kubernetes/cloud-config
   permissions: '0644'
----

. 2つのcloud-configファイルを持つVMを作成します。
+
image::rancher/vm-with-cloud-config.png[cloud-configを持つVM]

. VMにRKE2をインストールします。
+
[,shell]
----
 sudo mkdir -p /etc/rancher/rke2
 echo "cni: calico
 disable-kube-proxy: false
 etcd-expose-metrics: false" | sudo tee /etc/rancher/rke2/config.yaml
 curl -sfL https://get.rke2.io | sudo sh -
 sudo systemctl enable rke2-server.service
 sudo systemctl start rke2-server.service
----

. VMでRKE2が実行されていることを確認します。
+
[,shell]
----
 sudo /var/lib/rancher/rke2/bin/kubectl --kubeconfig /etc/rancher/rke2/rke2.yaml get nodes
----

. VMをRancherにインポートします。
+
Rancher UIで、*クラスター管理* > *クラスター* > *既存のインポート* > *一般* > *作成*に移動します。
+
image::rancher/import-existing-cluster.png[既存のクラスターをインポート]
+
[,shell]
----
 # Run the command in the VM
 curl --insecure -sfL https://192.168.0.181:6443/v3/import/g5p2g2gtxw4564nktdl4nr5cwwvtwqp9zxd6dmhm5nc7vpnxmr9cfk_c-m-mzf28skd.yaml | sudo /var/lib/rancher/rke2/bin/kubectl --kubeconfig /etc/rancher/rke2/rke2.yaml apply -f -
----

. Harvester Cloud Providerをインストールします。
+
RKE2クラスターダッシュボードで、*アプリ* > *チャート* > *Harvester Cloud Provider* > *インストール*に移動します。

. Harvester CSIドライバーをインストールします。
+
RKE2クラスターダッシュボードで、*アプリ* > *チャート* > *Harvester CSIドライバー* > *インストール*に移動します。
