...
[NOTE]
====
現在、Harvester CSI ドライバーは単一ノードの読み書き（RWO）ボリュームのみをサポートしています。可能なマルチノードの読み取り専用（ROX）および読み書き（RWX）サポートに関する情報はhttps://github.com/harvester/harvester/issues/1992[issue #1992]を確認してください。
====
...
*v0.1.25*以降、Harvester CSIドライバーはhttps://kubernetes.io/docs/concepts/storage/volume-snapshots/[ボリュームスナップショット]をサポートし、ゲストKubernetesクラスターで実行されているワークロードのための時点バックアップおよび復元機能を提供します。
