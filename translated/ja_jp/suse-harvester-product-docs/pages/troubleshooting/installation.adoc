...
{rancher-short-name}はhttps://documentation.suse.com/en-us/cloudnative/continuous-delivery/[{fleet-product-name}]を使用してターゲットクラスターにチャートをインストールします。{harvester-product-name}にはターゲットクラスターが1つだけあります（`+fleet-local/local+`）。
