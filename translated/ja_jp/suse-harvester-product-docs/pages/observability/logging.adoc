...
HarvesterとRancherの両方は、内部ログインフラストラクチャの特定のコンポーネントと操作を管理するためにhttps://kube-logging.dev/docs/#overview[Logging Operator]を使用します。
...
Harvesterのログインフラストラクチャは、Harvesterのログをhttps://www.graylog.org[Graylog]、https://www.elastic.co/elasticsearch/[Elasticsearch]、https://www.splunk.com/[Splunk]、https://grafana.com/oss/loki/[Grafana Loki]などの外部サービスに集約することを可能にします。
...
ログオペレーターの下には、ログの収集とルーティングを担当するhttps://www.fluentd.org/[Fluentd]とhttps://fluentbit.io/[Fluent Bit]が存在します。必要に応じて、これらのコンポーネントに割り当てるリソースの数を変更できます。
...
ログ操作は、https://kube-logging.dev/docs/#overview[ログオペレーター]によってサポートされ、Fluentdリソース、特にhttps://kube-logging.dev/docs/configuration/flow/[フローおよびクラスターフロー]とhttps://kube-logging.dev/docs/configuration/output/[出力およびクラスター出力]を使用して制御されます。これらのCRDをHarvesterクラスターに適用することで、ログをルーティングおよびフィルタリングできます。
...
フォームには、選択したhttps://kube-logging.dev/docs/configuration/plugins/outputs/[出力]に利用可能なフィールドが表示されます。

====== 出力バッファ

エディタでは、さまざまなhttps://kube-logging.dev/docs/configuration/plugins/outputs/buffer/[フィールド]を使用して、好ましい出力バッファの動作を説明できます。
...
フィルターを使用すると、ログを変換、処理、変更することができます。詳細については、サポートされているhttps://kube-logging.dev/docs/configuration/plugins/filters/[フィルター]のリストをご覧ください。
...
`+kube-apiserver+`をガイドするためのポリシーファイルはhttps://github.com/harvester/harvester-installer/blob/5991dcf6307aa5da79c5d6926566541f48105778/pkg/config/templates/rke2-92-harvester-kube-audit-policy.yaml[こちら]です。
