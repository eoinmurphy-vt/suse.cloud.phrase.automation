...
このガイドは、K3sの**v1.29-v1.34**リリースラインとCIS Kubernetes Benchmarkの**v1.11**リリースに特化したものです。
...
非etcdクラスタには適用されない。etcdロールを使用せずにmasterのみを実行している場合、このチェックは適用されない。
controlplaneロールとetcdロールが同じノードに存在するが、このチェックが警告される場合、etcdサーバーノードで、コマンド「ps -ef | grep etcd」から、引数--data-dirとして渡されたetcdデータディレクトリを取得する。
...
=== 1.1.20 Kubernetes PKI証明書ファイルのパーミッションが644以上に設定されていることを確認する(自動化)
...
**期待される結果：**パーミッションに644のパーミッションがある。
...
たとえば、`+chmod -R 644 /var/lib/rancher/k3s/server/tls/*.crt+`デフォルトでは、k3sはPKI証明書ファイルのパーミッションを644に設定している。
======
...
このフラグを有効にするには、K3sの設定ファイル/etc/rancher/k3s/config.yamlを以下のように編集します。
----
kube-apiserver-arg:
...
=== 1.2.30--service-account-extend-token-expirationパラメータがfalseに設定されていることを確認する（自動化）。
...
**期待される結果：**'--service-account-extend-token-expiration' is equal to 'false'
...
K3sの設定ファイル/etc/rancher/k3s/config.yamlを編集し、k3sをリロードする前に、--service-account-extend-token-expirationパラメータを以下のようにfalseに設定する：kube-apiserver-arg: - "service-account-extend-token-expiration=false" デフォルトでは、このパラメータはtrueに設定されています。
======
...
=== 4.2.4 定義されている場合、--read-only-port引数が0（自動化）に設定されていることを確認する。
...
----
あるいは、これらの値のサブセットである。
コマンドラインを使用する場合は、K3sフラグを追加する--kubelet-arg="tls-cipher-suites=&lt;上記と同じ値&gt;" あなたのシステムに基づいて、k3sサービスを再起動します。例えば、systemctl restart k3s.service
======
...
=== 4.2.14--seccomp-default パラメータが真（Manual）に設定されていることを確認する。

**結果**ウォーン

**修復：**
有効にすると、kubelet はデフォルトの Unconfined (seccomp 無効) モードではなく、コンテナランタイムによって定義される RuntimeDefault seccomp プロファイルを使用します。
K3sの設定ファイル/etc/rancher/k3s/config.yamlを使用している場合は、`+seccomp-default+` をkubelet-arg: - "seccomp-default=true "に設定するようにファイルを編集します。
...
メ ト リ ッ ク ・ サービ ス を ロ ーカルホ ス ト 以外のア ド レ ス にバ イ ン ド す る 値を変更ま たは削除 し ます。
デフォルト値は127.0.0.1:10249である。
======
...
cluster-adminロールへのすべてのclusterrolebindingsを識別します。そのロールが使用されているかどうか、そのロールが必要かどうか、あるいは、より少ない権限のロールを使用できるかどうかを確認する。K3sは、helm-kube-system-traefikおよびhelm-kube-system-traefik-crdクラスタロールバインディングを例外としています。これらは、通常の操作でtraefikをkube-system名前空間にインストールするために必要だからです。
可能であれば、まずユーザをより低い権限のロールにバインドし、次にクラスタ管理ロールのクラスタロールバインドを削除します：
----
...
**期待される結果：**'canGetListWatchSecretsAsSystemAuthenticated' is equal to 'no'
...
**期待される結果：**'canCreatePodsAsSystemAuthenticated' is equal to 'no'
...
K3sは、kube-system名前空間のデフォルトのサービスアカウントを例外とします。
各デフォルトサービスアカウントの設定を変更して、この値を含める automountServiceAccountToken: false または kubectl を使用する：
[,bash]
----
...
**修復：**
可能であれば、certificatesigningrequests オブジェクトの承認サブリソースへのア クセスを削除する。
...
監査：監査は、すべてのポッドのコンテナを一覧表示して、その .securityContext.privileged 値を取得します。
条件：コンテナの`+.securityContext.privileged+` が`+true+` に設定されている場合、is_compliantはfalseとなる。
デフォルト：デフォルトでは、特権コンテナの作成に制限はない。
======
...
監査：監査は、各 Pod の spec.hostPID を取得します。
条件：Podのspec.hostPIDが`+true+` に設定されている場合、is_compliantはfalseになる。
デフォルト：デフォルトでは、hostPIDコンテナの作成に制限はない。
======
...
監査：監査は各 Pod の spec.IPC を取得します。
条件：Podのspec.hostIPCが`+true+` に設定されている場合、is_compliantはfalseになる。
デフォルト：デフォルトでは、hostIPCコンテナの作成に制限はない。
======
...
監査：監査は、各 Pod の spec.hostNetwork を取得します。
条件：Podのspec.hostNetworkが`+true+` に設定されている場合、is_compliantはfalseになる。
デフォルト：デフォルトでは、hostNetwork コンテナの作成に制限はない。
======
...
ユーザー・ワークロードを持つクラスタ内の各ネームスペースにポリシーを追加し、`+.securityContext.allowPrivilegeEscalation+` が`+true+` に設定されたコンテナの入場を制限する。
監査：監査は各 Pod のコンテナを検索する`+.securityContext.allowPrivilegeEscalation+` 。
条件：コンテナの`+.securityContext.allowPrivilegeEscalation+` が`+true+` に設定されている場合、is_compliantはfalseとなる。
デフォルト：notsetの場合、特権の昇格が許可される（デフォルトはtrue）。しかし、PSP/PSAが`+restricted+` プロファイルとともに使用される場合、特に設定されない限り、特権の昇格は明示的に禁止される。
======
...
監査：監査は、各Podのコンテナに追加された機能を検索します。
条件：指定されたコンテナに能力が追加された場合、is_compliantはfalseとなる。
デフォルト：コンテナは、コンテナ・ランタイムによって割り当てられたデフォルトの機能セットで実行される。
K3sは以下のポッドに例外を与えるが、これらは通常のオペレーションに必要である：- coredns, svclb-traefik
======
...
=== 5.6.1 名前空間を使用してリソース間の管理境界を作成する（手動）
...
=== 5.6.2 Pod定義でseccompプロファイルがdocker/defaultに設定されていることを確認する（手動）
...
=== 5.6.3 ポッドとコンテナにSecurityContextを適用する（手動）
...
=== 5.6.4 デフォルトの名前空間を使用すべきではない（Manual）
