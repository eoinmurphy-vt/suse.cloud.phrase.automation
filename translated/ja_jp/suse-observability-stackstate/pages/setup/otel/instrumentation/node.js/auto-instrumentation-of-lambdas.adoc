...
データをSUSE Observabilityに送信するためのコレクターを設定するには、xref:/setup/otel/getting-started/getting-started-lambda.adoc[スタートガイド]に従ってください。サンプリングやフィルタリングなどのコレクターの設定をカスタマイズする方法は、xref:/setup/otel/collector.adoc[コレクターのドキュメント]に記載されています。
