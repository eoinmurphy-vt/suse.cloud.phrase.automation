...
適用後、SUSE Observabilityでポッドのメトリクスの視点を開いてください（できればCPU使用率にスパイクと谷があるポッド）。右上隅のアイコンを使用してチャートを拡大すると、より見やすくなります。今、異なる集約の影響を確認するために時間ウィンドウを変更することもできます（例えば、30分と24時間の場合）。

[CAUTION]
====
メトリックバインディングが集約を指定しない場合、SUSE Observabilityは自動的に`+last_over_time+`集約を使用してチャートのデータポイントの数を減らします。説明については、xref:/use/metrics/k8s-writing-promql-for-charts.adoc#_why[なぜこれが必要なのか？]も参照してください。
====
...
https://play.stackstate.com/#_/metrics?promql=last_over_time%28container_cpu_usage%7Bnamespace%3D%22sock_shop%22%2C%20pod_name%3D~%22carts.%2A%22%7D%5B%24%7B%5F%5Finterval%7D%5D%29%20%2F%201000000000&timeRange=LAST_7_DAYS[SUSE Observabilityプレイグラウンド]で自分で試してみてください。
