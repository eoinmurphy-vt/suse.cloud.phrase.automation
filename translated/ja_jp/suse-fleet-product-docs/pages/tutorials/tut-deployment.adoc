...
Helm::
+
Helmを使用した例。私たちは、ローカルクラスタにhttps://github.com/rancher/fleet-examples/tree/master/single-cluster/helm[helm example]をデプロイしています。
+
...
Helmマルチチャート::
+
単一のリポジトリから複数のチャートをデプロイするhttps://github.com/rancher/fleet-examples/blob/master/single-cluster/helm-multi-chart[例]。これは前の例に似ていますが、サブフォルダーから3つのHelmチャートをデプロイし、それぞれが独自の`+fleet.yaml+`で構成されています。
+
[source,bash]
----
...
マニフェスト::
+
生のKubernetes YAMLを使用したhttps://github.com/rancher/fleet-examples/tree/master/single-cluster/manifests[例]です。
+
[source,bash]
----
kubectl apply -n fleet-local -f - <<EOF
...
Helm外部::
+
サードパーティのソースからダウンロードされたHelmチャートを使用し、ターゲットクラスターごとにカスタマイズしたhttps://github.com/rancher/fleet-examples/blob/master/multi-cluster/helm-external[例]です。カスタマイズは前の例と似ています。
+
...
HelmとKustomize::
+
サードパーティのHelmチャートを修正するためにhttps://github.com/rancher/fleet-examples/blob/master/multi-cluster/helm-kustomize[Kustomizeを使用した例]です。Kubernetesのサンプルゲストブックアプリケーションを、サードパーティのソースからダウンロードしたHelmチャートとしてパッケージ化し、Kustomizeを使用してHelmチャートを変更します。アプリは、fleet-helm-kustomize-exampleネームスペースにデプロイされます。
+
