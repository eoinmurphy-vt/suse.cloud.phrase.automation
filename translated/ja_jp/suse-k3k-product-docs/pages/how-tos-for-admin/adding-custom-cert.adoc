= k3k クラスターから k3s へカスタム証明書を追加する
:revdate: 2025-09-16
:page-revdate: {revdate}

== カスタム証明書を生成して追加する

. https://github.com/k3s-io/k3s/blob/master/contrib/util/generate-custom-ca-certs.sh[カスタム証明書スクリプト]を使用して証明書を生成します。

. 証明書を生成した後、デフォルトのディレクトリにアクセスできないため、ローカルディレクトリを作成します。たとえば、`+k3k/custom-certs+`と指定します。

. 生成した証明書をディレクトリ - `+$ sudo cp -r /var/lib/rancher/k3s/server/tls k3k/custom-certs/+` にコピーします。

. パス `+/var/lib/rancher/k3s/server/tls k3k/custom-certs/+` を使用して、2 つのモードで仮想クラスターを作成します：
+
* 仮想モード 
* 共有モード
