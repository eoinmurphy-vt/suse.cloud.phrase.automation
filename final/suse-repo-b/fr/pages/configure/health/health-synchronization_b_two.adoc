...
La synchronisation de l'état de santé de SUSE Observability s'appuie sur différents modèles de cohérence pour garantir que les données envoyées par un système de surveillance externe correspondent à ce que SUSE Observability ingère et affiche. Le modèle de cohérence est spécifié dans la propriété `"health"` de l'xref:/configure/health/send-health-data/send-health-data.adoc#_common_json_object[objet JSON commun] ou en tant qu'argument dans l'interface CLI de SUSE Observability lorsque les données de santé sont envoyées à SUSE Observability. Les modèles pris en charge sont les suivants : `REPEAT_SNAPSHOTS` et `TRANSACTIONAL_INCREMENTS`.
[tabs]
====
Modèle de répétition d'instantanés::
+
--
