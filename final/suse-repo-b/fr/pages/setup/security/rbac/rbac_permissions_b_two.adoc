...
|api-tokens
|obtenir
|La clé API à utiliser pour l'authentification avec la CLI de SUSE Observability.  Il s'agit d'une donnée unique par utilisateur.
...
|update\^[1]^
|Gérer les autorisations de portée, accorder (ou révoquer) l'accès à la topologie, aux métriques et aux traces à partir d'un seul cluster.
...
[NOTE]
====
`+Le sujet `+SOURCE` indique qui possède et gère le sujet. Il peut s'agir d'une de nos sources RBAC (Configuration|Static, Observability, Kubernetes).
