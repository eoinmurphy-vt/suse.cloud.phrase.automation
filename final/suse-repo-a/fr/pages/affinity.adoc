...
:description: {stackstate-product-name} Configuration de l'affinité

Les configurations d'affinité contrôlent le comportement de l'ordonnancement des pods dans {stackstate-product-name}. Les valeurs d'affinité permettent d'optimiser l'utilisation des ressources et d'assurer une haute disponibilité en contrôlant l'ordonnancement des pods.
...
Supposons que vous définissiez `sizing.profile` sur l'un des profils HA. Les valeurs ci-dessous configurent nodeAffinity pour planifier les composants de {stackstate-product-name} dans la zone `us-west-2a` et imposer la planification des répliques des mêmes services de données sur des hôtes différents (`affinity.podAntiAffinity.requiredDuringSchedulingIgnoredDuringExecution`=`true` par défaut).
[,yaml]
----
affinity:
...
Ajoutez la configuration de l'affinité directement à votre fichier `values.yaml` sous `global.suseObservability.affinity`:
...
Il suffit ensuite d'une seule commande pour procéder à l'installation ou à la mise à niveau :
...
TIP: Pour les profils HA, l'anti-affinité des pods est automatiquement appliquée à tous les services de données avec état. Vous ne devez le configurer que si vous souhaitez modifier le comportement par défaut.

Pour plus de détails sur la migration à partir de l'ancienne méthode, voir le xref:/setup/upgrade-stackstate/migrate-to-global-mode.adoc[guide de migration].
--
