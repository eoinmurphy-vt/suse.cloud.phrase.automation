...
|xref:/setup/security/authentication/service_tokens.adoc[Service-Tokens] authentifizieren einen externen Dienst und können für die Dateneingabe verwendet werden.
...
[NOTE]
====
Das Thema `+SOURCE` zeigt, wer das Thema besitzt und verwaltet. Es kann eine unserer RBAC-Quellen sein (Configuration|Static, Observability, Kubernetes)
...
Nachfolgend sehen Sie ein Beispiel dafür, wie die SUSE Observability-Benutzeroberfläche für einen Benutzer ohne Berechtigungen aussehen würde:
