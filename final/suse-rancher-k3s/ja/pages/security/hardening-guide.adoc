...
+
--
K3s v1.25以降は、ポッドセキュリティを制御するための https://kubernetes.io/docs/concepts/security/pod-security-admission/[ポッドセキュリティアドミッション（PSA）]をサポートしています。PSAは、次のフラグをK3sサーバーに渡すことで有効になります：
...
+
--
K3s v1.24以前は、ポッドセキュリティを制御するための https://kubernetes.io/docs/concepts/security/pod-security-policy/[ポッドセキュリティポリシー（PSP）]をサポートしています。PSPは、次のフラグをK3sサーバーに渡すことで有効になります：
...
リクエストメタデータをログに記録するためのスターター監査ポリシーは以下に示されています。ポリシーは、`/var/lib/rancher/k3s/server`ディレクトリ内の`audit.yaml`という名前のファイルに書き込む必要があります。APIサーバーのポリシー設定に関する詳細情報は、Kubernetesの https://kubernetes.io/docs/tasks/debug-application-cluster/audit/[ドキュメント]で確認できます。
