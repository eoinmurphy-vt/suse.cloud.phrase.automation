...
このガイドは、K3sの*v1.28-v1.31*リリースラインとCIS Kubernetes Benchmarkの*v1.10*リリースに特化したものです。
...
K3sは、通常の運用に必要な以下のクラスタ・ロールに例外を与える：
...
コンディションPodのis_compliantがtrueに設定されている場合

* ServiceAccount が automountServiceAccountToken: false で、かつ Pod が automountServiceAccountToken: false か notset である。
* ServiceAccountはautomountServiceAccountToken: true notset、PodはautomountServiceAccountToken: false K3sは、通常の運用に必要な以下のサービスアカウントに例外を与える：
...
K3sは、通常のオペレーションに必要な以下のポッドを例外としている：
