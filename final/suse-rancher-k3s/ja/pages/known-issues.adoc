...
. 通常通りアップグレードを行ってください。xref:upgrades/automated.adoc[自動アップグレード]を行う場合は、`system-upgrade-controller`ポッドが実行されている名前空間が https://kubernetes.io/docs/concepts/security/pod-security-admission/#pod-security-levels[ポッドセキュリティレベル]に従って特権として設定されていることを確認してください。
+
[,yaml]
----
apiVersion: v1
