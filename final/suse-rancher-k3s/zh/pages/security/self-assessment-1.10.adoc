...
本指南适用于 K3s 的*v1.28-v1.31*版本系列和 CIS Kubernetes Benchmark 的*v1.10*版本。
...
K3s 默认将 CNI 文件权限设置为 600。
请注意，许多 CNI 都会创建一个权限为 750 的锁定文件。这是意料之中的，可以忽略不计。
如果修改了 CNI 配置，请确保权限设置为 600。
...
不适用于非 ETCD 群组。如果只运行主站而没有 etcd 角色，则此检查不适用。
如果 controlplane 和 etcd 角色出现在同一节点上，但该检查被警告，那么在 etcd 服务器节点上，从 "ps -ef | grep etcd "命令中获取 etcd 数据目录（作为参数传递）--data-dir。
运行以下命令（基于上面找到的 etcd 数据目录）。例如
...
对于 K3s，etcd 被嵌入到 k3s 进程中。没有单独的 etcd 进程。
...
默认情况下，K3s 会将--anonymous-auth 参数设置为 false。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的内容。
...
*补救措施：*
默认情况下，K3s 不设置 DenyServiceExternalIPs。
要启用此标记，请像下面这样编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml。
...
它们生成后位于 /var/lib/rancher/k3s/server/tls/client-kube-apiserver.crt 和 /var/lib/rancher/k3s/server/tls/client-kube-apiserver.key 中。如果出于某种原因需要提供自己的证书和密钥，可以在 K3s 配置文件 /etc/rancher/k3s/config.yaml 中设置以下参数。
...
默认情况下，K3s 会自动提供 kubelet CA 证书文件，地址为 /var/lib/rancher/k3s/server/tls/server-ca.crt。
如果出于某种原因，您需要提供自己的 ca 证书，请使用 k3s 证书命令行工具。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
默认情况下，K3s 不会将--授权模式设置为 AlwaysAllow。
如果检查失败，请编辑 K3 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
默认情况下，K3s 会将--授权模式设置为节点和 RBAC。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，确保没有覆盖授权模式。
======
...
*补救措施：*
按照 Kubernetes 文档，在配置文件中设置所需的限制。
...
默认情况下，K3s 不会将--disable-admission-plugins 设置为任何内容。
按照文档要求，根据您的环境创建 ServiceAccount 对象。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
默认情况下，K3s 会将--enable-admission-plugins 设置为 NodeRestriction。
如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请检查是否覆盖了接纳插件。
如果是，请将 NodeRestriction 列入列表。
...
默认情况下，K3s 会将--profiling 参数设置为 false。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
它们位于 /var/lib/rancher/k3s/server/tls/etcd/client.crt 和 /var/lib/rancher/k3s/server/tls/etcd/client.key 中。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
默认情况下，K3s 会自动为 apiserver 生成并提供 TLS 证书和私钥。
它们已生成并位于 /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.crt 和 /var/lib/rancher/k3s/server/tls/serving-kube-apiserver.key 中。如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml 并删除类似下面的行。
...
默认情况下，K3s 会自动提供客户端证书授权文件。
它生成后位于 /var/lib/rancher/k3s/server/tls/client-ca.crt。
如果出于某种原因，您需要提供自己的 ca 证书，请使用 k3s 证书命令行工具。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
K3 可以配置为使用加密提供商对静态机密进行加密。
编辑控制平面节点上的 K3s 配置文件 /etc/rancher/k3s/config.yaml，并设置以下参数 secrets-encryption：true 可使用 k3s secrets-encrypt 命令行工具管理秘密加密。
如有需要，可在 /var/lib/rancher/k3s/server/cred/encryption-config.json 中找到生成的加密配置。
======
...
K3 可以配置为使用加密提供商对静态机密进行加密。K3 将使用 aescbc 提供商。
编辑控制平面节点上的 K3s 配置文件 /etc/rancher/k3s/config.yaml，并设置以下参数 secrets-encryption：true 可使用 k3s secrets-encrypt 命令行工具管理秘密加密。
如有需要，您可以在 /var/lib/rancher/k3s/server/cred/encryption-config.json 中找到生成的加密配置。
======
...
默认情况下，K3s kube-apiserver 符合该测试。更改这些值可能会导致回归，因此在生产部署中应用新的 TLS 配置之前，请确保所有 apiserver 客户端都支持该配置。
如果需要自定义 TLS 配置，也可考虑创建一个符合您要求的自定义版本。
如果检查失败，请移除`tls-cipher-suites` 附近的任何自定义配置，或更新 /etc/rancher/k3s/config.yaml 文件，添加以下内容以匹配默认配置：
...
默认情况下，K3s 会将--use-service-account-credentials 参数设置为 true。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
它生成后位于 /var/lib/rancher/k3s/server/tls/service.current.key。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
默认情况下，K3s 会自动提供 CA 根文件。
它生成后位于 /var/lib/rancher/k3s/server/tls/server-ca.crt。
如果出于某种原因，您需要提供自己的 ca 证书，请使用 k3s 证书命令行工具。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
默认情况下，K3s 不会设置旋转 KubeletServerCertificate 功能门。
如果您已启用该功能网关，则应将其删除。
如果检查失败，请编辑 K3s 配置文件 /etc/rancher/k3s/config.yaml，删除类似下面的行。
...
如果使用 sqlite 或外部数据库运行，则不适用 etcd 检查。
使用 embedded-etcd 运行时，K3s 会为 etcd 生成证书和密钥文件。
它们位于 /var/lib/rancher/k3s/server/tls/etcd/ 中。
如果检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改为使用自定义证书和密钥文件。
======
...
如果使用 sqlite 或外部数据库运行，则不适用 etcd 检查。
使用 embedded-etcd 运行时，K3s 会将--client-cert-auth 参数设置为 true。
如果检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改以禁用客户端证书验证。
======
...
如果使用 sqlite 或外部数据库运行，则不适用 etcd 检查。
使用 embedded-etcd 运行时，K3s 不会设置--auto-tls 参数。
如果检查失败，请编辑主节点上的 etcd pod 规范文件/var/lib/rancher/k3s/server/db/etcd/config，删除--auto-tls 参数或将其设置为 false。 client-transport-security: auto-tls: false
======
...
如果使用 sqlite 或外部数据库运行，则不适用 etcd 检查。
使用 embedded-etcd 运行时，K3s 会为 etcd 生成对等证书和密钥文件。
它们位于 /var/lib/rancher/k3s/server/tls/etcd/ 中。
如果检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改为使用自定义对等证书和密钥文件。
======
...
如果使用 sqlite 或外部数据库运行，则不适用 etcd 检查。
使用 embedded-etcd 运行时，K3s 会将--peer-cert-auth 参数设置为 true。
如果检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改以禁用对等客户端证书验证。
======
...
如果使用 sqlite 或外部数据库运行，则不适用 etcd 检查。
使用 embedded-etcd 运行时，K3s 不会设置--peer-auto-tls 参数。
如果检查失败，请编辑主节点上的 etcd pod 规范文件 /var/lib/rancher/k3s/server/db/etcd/config 并删除--peer-auto-tls 参数或将其设置为 false。 peer-transport-security: auto-tls: false
======
...
如果使用 sqlite 或外部数据库运行，则不适用 etcd 检查。
使用 embedded-etcd 运行时，K3s 会为 etcd 生成唯一的证书授权。
它位于 /var/lib/rancher/k3s/server/tls/etcd/peer-ca.crt。
如果检查失败，请确保配置文件 /var/lib/rancher/k3s/server/db/etcd/config 未被修改为使用共享证书授权。
======
...
kubelet 嵌入了 k3s 进程。没有 kubelet 服务文件，所有配置都在运行时作为参数传递。
...
运行以下命令修改--client-ca-file 文件的所有权。
...
kubelet 嵌入了 k3s 进程。没有 kubelet 配置文件，所有配置都在运行时作为参数传递。
...
默认情况下，K3s 不会将--授权模式设置为 AlwaysAllow。
如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请删除与下面类似的任何行。
...
默认情况下，K3s 会自动为 Kubelet 提供 ca 客户端证书。
它已生成并位于 /var/lib/rancher/k3s/agent/client-ca.crt 中
======
...
如果使用命令行，请运行 K3s，--kubelet-arg="streaming-connection-idle-timeout=5m"。
根据系统情况，重新启动 k3s 服务。例如，systemctl restart k3s.service
======
...
如果使用命令行，请使用--kubelet-arg="make-iptables-util-chains=true" 运行 K3s。
根据系统情况，重新启动 k3s 服务。例如，systemctl restart k3s.service
======
...
默认情况下，K3s 会设置--主机名覆盖参数。根据 CIS 指导原则，这是为了符合云提供商的要求，因为云提供商要求使用此标记以确保主机名与节点名称相匹配。
...
如果使用命令行，请使用--kubelet-arg="event-qps=<value>" 运行 K3s。
根据系统情况，重新启动 k3s 服务。例如，systemctl restart k3s.service
======
...
默认情况下，K3s 会自动为 Kubelet 提供 TLS 证书和私钥。
它们生成后位于 /var/lib/rancher/k3s/agent/serving-kubelet.crt 和 /var/lib/rancher/k3s/agent/serving-kubelet.key 中。如果出于某种原因需要提供自己的证书和密钥，可以在 K3s 配置文件 /etc/rancher/k3s/config.yaml 中设置以下参数。
...
默认情况下，K3s 不会设置--rotate-certificates 参数。如果该标志的值为`false` ，则应将其设置为`true` 或完全删除该标志。
如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请删除任何 rotate-certificates 参数。
如果使用命令行，请移除 K3s 标志--kubelet-arg="rotate-certificates"。
根据系统情况，重新启动 k3s 服务。例如，systemctl restart k3s.service
======
...
默认情况下，K3s 不会设置旋转 KubeletServerCertificate 功能门。
如果您已启用该功能网关，则应将其删除。
如果使用 K3s 配置文件 /etc/rancher/k3s/config.yaml，请移除任何 feature-gate=RotateKubeletServerCertificate 参数。
如果使用命令行，请移除 K3s 标志--kubelet-arg="feature-gate=RotateKubeletServerCertificate"。
根据系统情况，重新启动 k3s 服务。例如，systemctl restart k3s.service
======
...
或这些值的子集。
如果使用命令行，请添加 K3s 标志--kubelet-arg="tls-cipher-suites=<same values as above>" 根据系统情况，重新启动 k3s 服务。例如，systemctl restart k3s.service
======
...
确定与群集管理员角色的所有群集绑定。检查他们是否被使用，是否需要这个角色，或者是否可以使用权限较小的角色。K3s 给出了 helm-kube-system-traefik 和 helm-kube-system-traefik-crd 集群绑定的例外情况，因为这些是将 traefik 安装到 kube-system 命名空间进行常规操作所必需的。
在可能的情况下，首先将用户绑定到权限较低的角色，然后删除群集管理员角色的群集绑定：
...
*补救措施：*
在可能的情况下，删除对群集中秘密对象的获取、列表和监视访问权限。
...
K3s 为以下集群角色提供了例外情况，这些角色是正常运行所必需的：
...
K3s 为 kube-system 命名空间中的默认服务帐户设置了一个例外。
...
修改不需要挂载服务账户令牌的 ServiceAccounts 和 Pod 的定义，禁用服务账户令牌，`automountServiceAccountToken: false` 。
如果 ServiceAccount 和 Pod 的 .spec 都指定了 automountServiceAccountToken 的值，则 Pod spec 优先。
条件：当

* 服务帐户为 automountServiceAccountToken: false，且 Pod 为 automountServiceAccountToken: false 或 notset
* ServiceAccount 是 automountServiceAccountToken: true notset，Pod 是 automountServiceAccountToken: false K3s 为以下服务帐户提供了例外，这些帐户是常规操作所必需的：
...
*补救措施：*
在可能的情况下，删除对群集中 PersistentVolume 对象的创建访问权限。
...
*补救措施：*
在可能的情况下，取消对证书签名请求对象的批准子资源的访问权限。
...
*补救措施：*
确保为每个包含用户工作负载的命名空间安装 Pod 安全接入或外部策略控制系统。
...
为群集中每个有用户工作负载的命名空间添加策略，以限制特权容器的进入。
审核：审核会列出所有 pod 的容器，以检索其 .securityContext.privileged 值。
...
为群集中每个有用户工作负载的命名空间添加策略，以限制`hostPID` 容器的进入。
审计：审计会检索每个 Pod 的 spec.hostPID。
条件：如果 Pod 的 spec.hostPID 设置为`true` ，则 is_compliant 为 false。
默认：默认情况下，创建 hostPID 容器不受限制。
======
...
为群集中每个有用户工作负载的命名空间添加策略，以限制`hostIPC` 容器的进入。
审计：审计检索每个 Pod 的 IPC 规格。
条件：如果 Pod 的 spec.hostIPC 设置为`true` ，则 is_compliant 为 false。
默认：默认情况下，创建 hostIPC 容器不受限制。
======
...
为群集中每个有用户工作负载的命名空间添加策略，以限制`hostNetwork` 容器的进入。
审计：审计会检索每个 Pod 的 spec.hostNetwork。
条件：如果 Pod 的 spec.hostNetwork 设置为`true` ，则 is_compliant 为 false。
默认：默认情况下，创建 hostNetwork 容器不受限制。
======
...
在群集中有用户工作负载的每个命名空间中添加策略，限制`.securityContext.allowPrivilegeEscalation` 设置为`true` 的容器进入。
...
默认值：如果 notset，则允许权限升级（默认为 true）。不过，如果 PSP/PSA 与`restricted` 配置文件一起使用，除非另有配置，否则将明确禁止权限升级。
======
...
*补救措施：*
为群集中的每个命名空间创建策略，确保设置`MustRunAsNonRoot`或`MustRunAs` ，UID 范围不包括 0。
...
*补救措施：*
为群集中具有用户工作负载的每个命名空间添加策略，以限制具有`NET_RAW` 功能的容器进入。
...
确保`allowedCapabilities` 不出现在群集的策略中，除非它被设置为空数组。
审计：审计检索每个 Pod 的容器添加功能。
条件：如果为给定容器添加了功能，则 is_compliant 为 false。
默认值：容器以容器运行时分配的默认能力集运行。
K3s 允许以下 pod 例外运行，这些 pod 是正常运行所必需的：
...
*补救措施：*
检查群集上运行的应用程序中功能的使用情况。如果命名空间中包含的应用程序不需要任何 Linux 功能就能运行，则可考虑添加一个 PSP，禁止使用不丢弃所有功能的容器。
...
*补救措施：*
为群集中每个有用户工作负载的命名空间添加策略，以限制接纳`.securityContext.windowsOptions.hostProcess` 设置为`true` 的容器。
...
*补救措施：*
在群集中有用户工作负载的每个命名空间中添加策略，限制使用`hostPath` 卷的容器进入。
...
*补救措施：*
为群集中每个有用户工作负载的命名空间添加策略，以限制使用`hostPort` 部分的容器进入。
...
*补救措施：*
如果使用的 CNI 插件不支持网络策略，则应考虑使用不同的插件，或寻找其他机制来限制 Kubernetes 集群中的流量。
...
*补救措施：*
按照文档要求，根据需要创建 NetworkPolicy 对象。
...
*补救措施：*
如果可能，重写应用程序代码，从挂载的秘密文件而不是环境变量中读取 "秘密"。
...
示例如下： securityContext: seccompProfile: type：运行时默认值
...
*补救措施：*
按照 Kubernetes 文档，将 SecurityContexts 应用于 Pod。有关 SecurityContexts 的建议列表，请参阅 CIS Security Benchmark for Docker Containers。
...
*补救措施：*
确保创建命名空间，以便适当隔离 Kubernetes 资源，并确保在特定命名空间中创建所有新资源。
