...
Les résultats sont résumés comme suit{nbsp}:
...
Les exigences en matière d'UC sont{nbsp}:
...
Les exigences en matière de mémoire sont{nbsp}:
...
Les exigences en matière de disque sont{nbsp}:
...
À la fin de chaque test, deux serveurs supplémentaires ont été ajoutés (formant un cluster HA de base à 3 nœuds) pour observer quel effet cela avait sur les ressources du serveur d'origine. L'effet était{nbsp}:
