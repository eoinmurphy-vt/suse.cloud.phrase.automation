...
Voici un exemple de ConfigMap qui redirige les recherches vers `example.com` vers un serveur de noms à `10.0.0.1` et sert `example.net` à partir d'un fichier texte conforme à https://datatracker.ietf.org/doc/html/rfc1035#section-5[RFC 1035]{nbsp}:
...
Si vous souhaitez configurer les paramètres du proxy pour containerd sans affecter K3s et le Kubelet, vous pouvez préfixer les variables avec `CONTAINERD_`{nbsp}:
...
Pour utiliser Docker au lieu de containerd{nbsp}:

. Installez Docker sur le nœud K3s. L'un des https://github.com/rancher/install-docker[scripts d'installation de Docker] de Rancher peut être utilisé pour installer Docker{nbsp}:
+
[,bash]
----
...
. Installez K3s en utilisant l'option `--docker`{nbsp}:
+
[,bash]
----
...
. Confirmez que le cluster est disponible{nbsp}:
+
[,bash]
----
...
. Confirmez que les conteneurs Docker sont en cours d'exécution{nbsp}:
+
[,bash]
----
$ sudo docker ps
...
Vous pouvez ensuite utiliser etcdctl en le configurant pour utiliser les certificats et clés gérés par K3s pour l'authentification{nbsp}:
...
.Version Gate
====
K3s inclut containerd 2.0 à partir des versions de février 2025{nbsp}: v1.31.6+k3s1 et v1.32.2+k3s1.
...
K3s détectera automatiquement les environnements d'exécution de conteneur alternatifs s'ils sont présents lorsque K3s démarre. Les environnements d'exécution de conteneur pris en charge sont{nbsp}:
...
Si un environnement d'exécution de conteneur installé n'est pas détecté par K3s, assurez-vous qu'il est présent dans un chemin système, qui inclut généralement{nbsp}: +
`/usr/local/sbin /usr/local/bin /usr/sbin /usr/bin /sbin /bin`

=== Environnement d'exécution de conteneur NVIDIA

Les GPU NVIDIA nécessitent l'installation de l'environnement d'exécution de conteneur NVIDIA afin de planifier et d'exécuter des charges de travail accélérées dans des Pods. Pour utiliser les GPU NVIDIA avec K3s, effectuez les étapes suivantes{nbsp}:

. Installez le dépôt de paquets nvidia-container sur le nœud en suivant les instructions à{nbsp}:
 https://nvidia.github.io/libnvidia-container/
. Installez les paquets de l'environnement d'exécution de conteneur NVIDIA. Par exemple{nbsp}:
...
. Confirmez que l'environnement d'exécution de conteneur NVIDIA a été trouvé par k3s{nbsp}:
...
Si vous n'avez pas modifié l'environnement d'exécution par défaut sur vos nœuds GPU, vous devez explicitement demander l'environnement d'exécution NVIDIA en définissant `runtimeClassName: nvidia` dans la spécification du Pod{nbsp}:
...
* *Cgroups*
+
Cgroup v1 et Hybrid v1/v2 ne sont pas pris en charge{nbsp}; seul Cgroup v2 pur est pris en charge. Si K3s échoue à démarrer en raison de cgroups manquants lors de l'exécution sans racine, il est probable que votre nœud soit en mode hybride, et les cgroups "manquants" sont toujours liés à un contrôleur v1.
...
Cette étape est requise{nbsp}; le kubelet sans racine échouera à démarrer sans les cgroups appropriés délégués.
...
| builtin
| Sélectionne le pilote de port sans racine{nbsp}; soit `builtin` soit `slirp4netns`. Intégré est plus rapide, mais masque l'adresse source d'origine des paquets entrants.
...
Un exemple de désactivation du démarrage automatique et de l'activation du service avec le script d'installation{nbsp}:
...
Il existe plusieurs façons d'exécuter K3s dans Docker{nbsp}:
...
Utilisation de la commande `docker run`{nbsp}:
...
[NOTE]
=====
Vous devez spécifier une version K3s valide comme tag{nbsp}; le tag `latest` n'est pas maintenu. +
Les images Docker n'autorisent pas l'utilisation du signe `+` dans les tags, utilisez à la place un `-`.
=====

Une fois K3s opérationnel, vous pouvez copier le kubeconfig admin hors du conteneur Docker pour l'utiliser{nbsp}:
...
+
--
Les politiques nécessaires peuvent être installées avec les commandes suivantes{nbsp}:
...
Pour forcer le script d'installation à enregistrer un avertissement plutôt qu'à échouer, vous pouvez définir la variable d'environnement suivante{nbsp}: `INSTALL_K3S_SELINUX_WARN=true`.
--
======
...
=== Qu'est-ce que le lazy pulling et eStargz{nbsp}?
...
La journalisation https://documentation.suse.com/cloudnative/rancher-manager/latest/en/observability/logging/logging-helm-chart-options.html[Rancher logging] pour K3s peut être installée sans utiliser Rancher. Les instructions suivantes doivent être exécutées pour ce faire{nbsp}:
...
S'il y a beaucoup de trafic, le nombre de messages de journal pourrait être très élevé. Pour contrôler le taux de journalisation par politique, définissez les paramètres iptables `limit` et `limit-burst` en ajoutant les annotations suivantes à la politique réseau en question{nbsp}:
...
Les paquets envoyés au socket netlink NFLOG peuvent également être lus en utilisant des outils en ligne de commande comme tcpdump ou tshark{nbsp}:
