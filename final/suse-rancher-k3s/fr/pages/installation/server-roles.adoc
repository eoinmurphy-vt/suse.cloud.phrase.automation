...
Pour créer un serveur avec uniquement le rôle `etcd`, démarrez K3s avec tous les composants du plan de contrôle désactivés{nbsp}:
...
Pour créer un serveur avec uniquement le rôle `control-plane`, démarrez k3s avec etcd désactivé{nbsp}:
