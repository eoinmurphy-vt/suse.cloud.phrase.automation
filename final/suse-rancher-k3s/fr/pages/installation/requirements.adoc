...
K3s est disponible pour les architectures suivantes{nbsp}:
...
Certains systèmes d'exploitation ont des exigences de configuration supplémentaires{nbsp}:
...
Il est recommandé de désactiver firewalld{nbsp}:
...
Si vous souhaitez garder firewalld activé, par défaut, les règles suivantes sont requises{nbsp}:
...
Sur RHEL 10, un paquet supplémentaire est requis{nbsp}:
...
====
Les versions du système d'exploitation antérieures à RHEL 8.4 utilisent NetworkManager avec un bug connu qui interfère avec le réseau K3s. Si vous utilisez une ancienne version, il est nécessaire de désactiver nm-cloud-setup et de redémarrer le nœud{nbsp}:
...
Il est recommandé de désactiver ufw (pare-feu simplifié){nbsp}:
...
Si vous souhaitez garder ufw activé, par défaut, les règles suivantes sont requises{nbsp}:
...
Les exigences matérielles varient en fonction de la taille de vos déploiements. Les exigences minimales sont{nbsp}:
...
Si vous déployez K3s sur un Raspberry Pi ou d'autres appareils ARM, il est recommandé d'utiliser un SSD externe. etcd est intensif en écriture{nbsp}; les cartes SD et eMMC ne peuvent pas gérer la charge d'E/S.
...
Les exigences minimales en matière de CPU et de mémoire pour les nœuds d'un serveur K3s à haute disponibilité sont les suivantes{nbsp}:
...
Voici un guide de dimensionnement pour les ressources de base de données dont vous avez besoin pour exécuter de grands clusters{nbsp}:
