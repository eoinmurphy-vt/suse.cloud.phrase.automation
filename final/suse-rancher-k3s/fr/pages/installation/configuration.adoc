...
Pour illustrer cela, les commandes suivantes aboutissent toutes au même comportement d'enregistrement d'un serveur sans flannel et avec un token{nbsp}:
...
Lors de l'enregistrement d'un agent, les commandes suivantes aboutissent toutes au même comportement{nbsp}:
...
Vous pouvez passer la configuration en définissant des variables d'environnement `K3S_`{nbsp}:
...
L'agent k3s peut également être configuré de cette manière{nbsp}:
...
Ceci est équivalent aux arguments CLI suivants{nbsp}:
...
Un exemple de valeurs fusionnées à partir de plusieurs fichiers de configuration est ci-dessous{nbsp}:
...
Un fichier `config.yaml` est créé à `/etc/rancher/k3s/config.yaml`{nbsp}:
...
Ensuite, le script d'installation est exécuté avec une combinaison de variables d'environnement et d'indicateurs{nbsp}:
...
Ou si vous avez déjà installé le binaire K3s{nbsp}:
...
Cela donne un serveur avec{nbsp}:
...
K3s utilise une configuration kubelet par défaut qui est stockée sous `/var/lib/rancher/k3s/agent/etc/kubelet.conf.d/00-k3s-defaults.conf`. Si vous souhaitez modifier les paramètres de configuration par défaut, il existe trois façons de le faire{nbsp}:
