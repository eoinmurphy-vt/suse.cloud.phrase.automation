...
=== 1.2.16 Assurez-vous que l'argument --secure-port n'est pas défini sur 0 - Note{nbsp}: cette recommandation est obsolète et sera supprimée selon le processus de consensus (Automatisé)
...
Dans la mesure du possible, liez d'abord les utilisateurs à un rôle de moindre privilège, puis supprimez le clusterrolebinding au rôle de cluster-admin{nbsp}: kubectl delete clusterrolebinding [nom]
...
Modifiez la configuration de chaque compte de service par défaut pour inclure cette valeur automountServiceAccountToken{nbsp}: false
