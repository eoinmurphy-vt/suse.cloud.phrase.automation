...
Si cette vérification échoue, supprimez toute configuration personnalisée autour de `tls-cipher-suites` ou mettez à jour le fichier /etc/rancher/k3s/config.yaml pour correspondre à la valeur par défaut en ajoutant ce qui suit{nbsp}:
----
kube-apiserver-arg:
...
Modifiez le fichier de configuration K3s /etc/rancher/k3s/config.yaml et définissez le paramètre --service-account-extend-token-expiration sur false avant de recharger k3s, comme ci-dessous{nbsp}: +
kube-apiserver-arg{nbsp}: +
...
Lorsque cela est possible, liez d'abord les utilisateurs à un rôle moins privilégié, puis supprimez le clusterrolebinding au rôle de cluster-admin{nbsp}:
----
...
Condition\&nbsp;: Le pod est conforme à 'vrai' lorsque{nbsp}: - le compte de service est défini avec automountServiceAccountToken{nbsp}: false et le pod est défini avec automountServiceAccountToken{nbsp}: false ou n'est pas défini, - le compte de service est défini avec automountServiceAccountToken{nbsp}: true (ou n'est pas défini) et le pod est défini avec automountServiceAccountToken{nbsp}: false. K3s donne des exceptions aux comptes de service suivants, qui sont nécessaires pour les opérations régulières{nbsp}: - coredns, helm-traefik, helm-traefik-crd, traefik, metrics-server, svclb, local-path-provisioner-service-account
======
...
Audit{nbsp}: l'audit liste tous les conteneurs des pods pour récupérer leur valeur .securityContext.privileged.
Condition{nbsp}: is_compliant est faux si `.securityContext.privileged` du conteneur est défini sur `true`.
...
Audit{nbsp}: l'audit récupère le champ spec.hostPID de chaque Pod.
Condition{nbsp}: is_compliant est faux si le champ spec.hostPID du Pod est défini sur `true`.
...
Audit{nbsp}: l'audit récupère le champ spec.IPC de chaque Pod.
Condition{nbsp}: is_compliant est faux si le champ hostIPC du Pod est défini sur `true`.
...
Audit{nbsp}: l'audit récupère le champ hostNetwork de chaque Pod.
Condition{nbsp}: is_compliant est faux si le champ hostNetwork du Pod est défini sur `true`.
...
Audit{nbsp}: l'audit récupère le(s) conteneur(s) de chaque Pod `.securityContext.allowPrivilegeEscalation`.
...
Audit{nbsp}: l'audit récupère les capacités ajoutées de chaque conteneur de Pod.
Condition{nbsp}: is_compliant est faux si des capacités ajoutées sont ajoutées pour un conteneur donné.
...
K3s donne des exceptions aux pods suivants, qui sont nécessaires pour les opérations régulières{nbsp}: - coredns, svclb-traefik
======
...
Un exemple est le suivant{nbsp}: +
  securityContext: +
    seccompProfile: +
      type: RuntimeDefault
