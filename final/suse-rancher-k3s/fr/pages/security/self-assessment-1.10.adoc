...
Si ce contrôle échoue, supprimez toute configuration personnalisée autour de `tls-cipher-suites` ou mettez à jour le fichier /etc/rancher/k3s/config.yaml pour correspondre à la valeur par défaut en ajoutant ce qui suit{nbsp}:
...
K3s fait des exceptions pour les rôles de cluster suivants, qui sont nécessaires pour les opérations régulières{nbsp}:
...
K3s donne des exceptions aux pods suivants, qui sont nécessaires pour les opérations régulières{nbsp}:
