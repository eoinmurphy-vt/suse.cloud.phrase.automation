...
[%collapsible]
======
Si vous utilisez sqlite ou une base de données externe, les vérifications etcd ne sont pas applicables. Lors de l'utilisation d'embedded-etcd, K3s ne définit pas le paramètre --peer-auto-tls. Si cette vérification échoue, modifiez le fichier de spécification du pod etcd /var/lib/rancher/k3s/server/db/etcd/config sur le nœud maître et soit supprimez le paramètre --peer-auto-tls, soit définissez-le sur false{nbsp}: peer-transport-security: auto-tls: false
======
...
[%collapsible]
======
Créez des comptes de service explicites chaque fois qu'une charge de travail Kubernetes nécessite un accès spécifique au serveur API Kubernetes. K3s fait une exception pour le compte de service par défaut dans l'espace de noms kube-system. Modifiez la configuration de chaque compte de service par défaut pour inclure cette valeur automountServiceAccountToken: false. Ou en utilisant kubectl{nbsp}:
...
[%collapsible]
======
Modifiez la définition des ServiceAccounts et des Pods qui n'ont pas besoin de monter des jetons de compte de service pour le désactiver, avec `automountServiceAccountToken: false`. Si à la fois le ServiceAccount et le Pod .spec spécifient une valeur pour automountServiceAccountToken, la spécification du Pod prévaut. Condition{nbsp}: Le Pod est conforme à vrai lorsque - le ServiceAccount a automountServiceAccountToken : faux et le Pod a automountServiceAccountToken : faux ou non défini - le ServiceAccount a automountServiceAccountToken : vrai (non défini) et le Pod a automountServiceAccountToken : faux. K3s donne des exceptions aux comptes de service suivants, qui sont nécessaires pour les opérations régulières : - coredns, helm-traefik, helm-traefik-crd, traefik, metrics-server, svclb, local-path-provisioner-service-account
======
...
*Remédiation:* Utilisez `securityContext` pour activer le profil seccomp docker/default dans vos définitions de Pod. Un exemple est le suivant{nbsp}: securityContext: seccompProfile: type: RuntimeDefault
