...
Les valeurs du Chart sont utilisées dans l'ordre suivant, de la moindre à la plus grande priorité{nbsp}:
...
Chaque entrée `valuesSecrets` répertoriée a les champs suivants{nbsp}:
...
Un exemple de déploiement d'un Helm chart utilisant un Secret existant avec deux clés{nbsp}:
...
| spec.failurePolicy
| Définir sur `abort`{nbsp}; dans ce cas, l'opération Helm est annulée, en attendant une intervention manuelle de l'opérateur.
|===
...
Par exemple, pour personnaliser la configuration d'ingress de Traefik empaqueté, vous pouvez créer un fichier nommé `/var/lib/rancher/k3s/server/manifests/traefik-config.yaml` et le remplir avec le contenu suivant{nbsp}:
