...
| Temps d'intervalle de snapshot dans la spécification cron. Par exemple, toutes les 5 heures `0 */5 * * *` (par défaut{nbsp}: `0 */12 * * *`).
|===
...
Pour supprimer manuellement les instantanés programmés ou les instantanés à la demande, vous pouvez utiliser la commande `k3s etcd-snapshot delete`{nbsp}:
...
.Version Gate
====
Le support du secret de configuration S3 est disponible à partir des versions d'août 2024{nbsp}: v1.30.4+k3s1, v1.29.8+k3s1, v1.28.13+k3s1.
====
...
K3s passe par plusieurs étapes lors de la restauration d'un instantané{nbsp}:
...
Lors de la restauration d'un instantané, vous n'avez pas besoin d'utiliser la même version de K3s qui l'a créé{nbsp}; une version mineure supérieure est également acceptable.
...
----
+
**Résultat :** K3s restaure l'instantané et réinitialise l'appartenance au cluster, puis affiche un message indiquant qu'il est prêt à être redémarré{nbsp}: `Managed etcd cluster membership has been reset, restart without --cluster-reset flag now.`

. Redémarrer K3s à nouveau{nbsp}:
+
[,bash]
----
systemctl start k3s
...
. Interrompre K3s sur tous les serveurs{nbsp}:
+
[,bash]
----
systemctl stop k3s
...
----
+
**Résultat :** K3s restaure l'instantané et réinitialise l'appartenance au cluster, puis affiche un message indiquant qu'il est prêt à être redémarré{nbsp}:
...
. Sur S1, redémarrez K3s à nouveau{nbsp}:
+
[,bash]
----
systemctl start k3s
...
. Sur S2 et S3, supprimez le répertoire de données, `/var/lib/rancher/k3s/server/db/`{nbsp}:
+
[,bash]
----
...
. Sur S2 et S3, redémarrez K3s pour rejoindre le cluster restauré{nbsp}:
+
[,bash]
----
systemctl start k3s
...
Il est possible de restaurer un instantané etcd sur un hôte différent de celui sur lequel il a été pris. Lorsque vous le faites, vous devez passer le xref:cli/token.adoc#_server[token du serveur] qui a été utilisé à l'origine lors de la prise de l'instantané, car il sert à déchiffrer les données de bootstrap contenues dans l'instantané. Le processus est le même que ci-dessus mais en changeant l'étape 2 en{nbsp}:

. Dans le nœud qui a pris l'instantané, enregistrez la valeur de{nbsp}: `/var/lib/rancher/k3s/server/token`. Ceci est `<BACKED-UP-TOKEN-VALUE>` à l'étape 3.
...
.Version Gate
====
Les ETCDSnapshotFiles sont disponibles depuis les versions de novembre 2023{nbsp}: v1.28.4+k3s2, v1.27.8+k3s2, v1.26.11+k3s2, v1.25.16+k3s4.
====
