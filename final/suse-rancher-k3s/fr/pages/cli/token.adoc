...
K3s prend en charge trois types de tokens. Seul le token du serveur est disponible par défaut{nbsp}; des types de tokens supplémentaires doivent être configurés ou créés par l'administrateur.
...
L'outil CLI de token k3s gère{nbsp}:
...
| `--data-dir` valeur
| Dossier pour contenir l'état (par défaut{nbsp}: /var/lib/rancher/k3s ou ${HOME}/.rancher/k3s si ce n'est pas root)
...
| `--groups` valeur
| Groupes supplémentaires que ce token authentifiera lors de son utilisation pour l'authentification. (par défaut{nbsp}: Par défaut{nbsp}: "system:bootstrappers:k3s:default-node-token")
...
| `--server` valeur
| Serveur auquel se connecter (par défaut{nbsp}: "https://127.0.0.1:6443") [$K3S_URL]
