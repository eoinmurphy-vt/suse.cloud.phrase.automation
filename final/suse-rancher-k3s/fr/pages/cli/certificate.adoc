...
Si vous souhaitez générer de nouveaux certificats et clés, au lieu de prolonger la validité des certificats existants, utilisez la commande xref:#_rotating_client_and_server_certificates[`rotate`] documentée ci-dessous.
...
.Version Gate
====
Le format de sortie est configurable depuis les versions de janvier 2025{nbsp}: v1.32.0+k3s1, v1.31.5+k3s1, v1.30.9+k3s1, v1.30.13+k3s1.

Un nouveau format avec plus d'informations est disponible depuis les versions de mai 2025{nbsp}: v1.33.1+k3s1, v1.32.5+k3s1, v1.31.9+k3s1, v1.29.13+k3s1.
====
...
Les certificats suivants peuvent être renouvelés{nbsp}: `admin`, `api-server`, `controller-manager`, `scheduler`, `k3s-controller`, `k3s-server`, `cloud-controller`, `etcd`, `auth-proxy`, `kubelet`, `kube-proxy`.
...
Les certificats CA et les clés autoritaires sont stockés dans la clé de démarrage du datastore, chiffrés à l'aide du xref:cli/token.adoc#_server[token serveur] comme phrase de passe PBKDF2 avec AES256-GCM et HMAC-SHA1.
Des copies des certificats CA et des clés sont extraites sur le disque lors du démarrage du serveur K3s.
Tout serveur peut générer des certificats de nœud pour les nœuds lorsqu'ils rejoignent le cluster, et les contrôleurs Kubernetes https://kubernetes.io/docs/reference/access-authn-authz/certificate-signing-requests/[API des certificats] peuvent émettre des certificats supplémentaires à l'exécution.
...
.Version Gate
====
Le support de la commande `k3s certificate rotate-ca` et la possibilité d'utiliser des certificats CA signés par une CA externe sont disponibles à partir des versions 2023-02 (v1.26.2+k3s1, v1.25.7+k3s1, v1.24.11+k3s1, v1.23.17+k3s1).
====
...
Cela signifie que tout utilisateur disposant d'un certificat client valide (ou kubeconfig) pour un cluster peut également s'authentifier auprès d'autres clusters, et tout RBAC correspondant à un utilisateur ou groupe de client est également appliqué dans ces autres clusters.
De même, les certificats de serveur émis par un cluster sont également approuvés dans tous les autres clusters, ainsi que par toute autre infrastructure ou client qui fait confiance à cette autorité de certification racine.
...
Un script d'exemple pour pré-créer les certificats et clés appropriés est disponible https://github.com/k3s-io/k3s/blob/main/contrib/util/generate-custom-ca-certs.sh[dans le dépôt K3s à `contrib/util/generate-custom-ca-certs.sh`].
...
Les fichiers d'autorité de certification personnalisés doivent être placés dans `/var/lib/rancher/k3s/server/tls`. Les fichiers suivants sont requis{nbsp}:
...
_// note{nbsp}: les fichiers etcd sont requis même si etcd intégré n'est pas utilisé._
* `etcd/peer-ca.crt`
* `etcd/peer-ca.key`
* `etcd/server-ca.crt`
* `etcd/server-ca.key` +
_// note{nbsp}: Ceci est la clé privée utilisée pour signer les jetons de compte de service. Elle n'a pas de certificat correspondant._
* `service.key`

==== Topologie CA personnalisée

Les certificats CA personnalisés générés par le script d'exemple ont la topologie suivante{nbsp}:
...
.Important
====
Si vous souhaitez signer les certificats CA du cluster avec une CA racine existante en utilisant le script d'exemple, vous devez placer les fichiers racine et intermédiaires dans le répertoire cible avant d'exécuter le script.
Si les fichiers n'existent pas, le script créera de nouveaux certificats CA racines et intermédiaires.
====
...
Pour utiliser le script d'exemple afin de générer des certificats et des clés personnalisés avant de démarrer K3s, exécutez les commandes suivantes{nbsp}:
...
Si le script a généré des fichiers CA racine et/ou intermédiaires, vous devez sauvegarder ces fichiers afin qu'ils puissent être réutilisés si nécessaire pour faire tourner les certificats CA à une date ultérieure.
...
[CAUTION]
====
Vous ne devez pas écraser les données actuellement utilisées dans `/var/lib/rancher/k3s/server/tls`. +
Placez les certificats et clés mis à jour dans un répertoire séparé.
====
...
Pour utiliser le script d'exemple afin de générer des certificats et des clés mis à jour, exécutez les commandes suivantes{nbsp}:
...
Si vous avez utilisé l'option `--force` ou changé le CA racine, assurez-vous que tous les nœuds qui ont été joints avec un xref:cli/token.adoc#_secure[token sécurisé] sont reconfigurés pour utiliser la nouvelle valeur de token, avant d'être redémarrés.
...
Si le cluster a été démarré avec des certificats CA auto-signés par défaut, la rotation sera perturbatrice. Tous les nœuds qui ont été joints avec un xref:cli/token.adoc#_secure[token sécurisé] devront être reconfigurés pour faire confiance au nouveau hachage CA.
...
Un exemple de script pour créer des certificats CA et des clés mis à jour signés croisés par les CA existants est disponible https://github.com/k3s-io/k3s/blob/main/contrib/util/rotate-default-ca-certs.sh[dans le dépôt K3s à `contrib/util/rotate-default-ca-certs.sh`].
...
Assurez-vous que tous les nœuds qui ont été joints avec un xref:cli/token.adoc#_secure[token sécurisé], y compris d'autres nœuds serveurs, sont reconfigurés pour utiliser la nouvelle valeur de token avant d'être redémarrés.
Le token peut être stocké dans un fichier `.env`, une unité systemd, ou config.yaml, selon la façon dont le nœud a été configuré lors de l'installation initiale.
...
====
Vous ne devez pas écraser les données actuellement utilisées dans `/var/lib/rancher/k3s/server/tls`. +
Mettez la clé mise à jour dans un répertoire séparé.
====
