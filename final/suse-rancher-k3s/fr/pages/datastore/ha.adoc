...
Les clusters à serveur unique peuvent répondre à une variété de cas d'utilisation, mais pour les environnements où la disponibilité du plan de contrôle Kubernetes est critique, vous pouvez exécuter K3s dans une configuration HA. Un cluster K3s HA est composé de{nbsp}:
...
Par exemple, une commande comme celle-ci pourrait être utilisée pour installer le serveur K3s avec une base de données MySQL comme magasin de données externe et xref:cli/server.adoc#_cluster_options[définir un token]{nbsp}:
...
La même commande d'exemple de l'étape 2 peut être utilisée pour rejoindre des nœuds de serveur supplémentaires, où le jeton du premier nœud doit être utilisé{nbsp}:

Si le premier nœud de serveur a été démarré sans le drapeau CLI `--token` ou la variable `K3S_TOKEN`, la valeur du jeton peut être récupérée à partir de tout serveur déjà rejoint au cluster{nbsp}:
