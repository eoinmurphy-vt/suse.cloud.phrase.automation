...
テストスイートを再利用するための主な参照は https://github.com/rancher-sandbox/turtles-integration-suite-example[このリポジトリ]であり、特定のCAPIプロバイダーを{product_name}と統合する方法の例が含まれており、GitOpsワークフローに基づいて一連のチェックを適用します。
...
`isolated-kind`はローカルテストに使用した値であり、`kind`は同等のローカル環境をデプロイする。違いは、`kind`が https://ngrok.com/[ngrok]を介して公開アクセス可能なエンドポイントも構成することです。無料の（制限付き）`ngrok`エンドポイントを取得し、テストの実行に使用できます。`make test`を実行する前に、次の環境変数を設定する必要があります。
