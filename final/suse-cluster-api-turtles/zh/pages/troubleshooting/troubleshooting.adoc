...
本指南提供 Rancher Turtles 和集群 API (CAPI) 集群的故障排除步骤。它涵盖了常见问题及其解决方法，以帮助您维护一个稳定和正常运行的环境。

在阅读本指南之前，您可以先查看 CAPI 项目的官方https://cluster-api.sigs.k8s.io/user/troubleshooting[`troubleshooting`]CAPI 项目的官方文档。

了解 Rancher Turtles 和 CAPI 的复杂性对于有效排除故障至关重要。本指南旨在使您掌握诊断和解决 Rancher Turtles 和 CAPI 基本问题的知识。

== 先决条件

要有效排除 Rancher Turtles 和 CAPI 集群的故障，您需要

* 访问已配置 kubectl 的终端，以访问 Rancher 管理集群。
* 已安装用于 kubectl 的 Krew 插件管理器。
* 已安装 Helm CLI 工具。

您可以按照https://krew.sigs.k8s.io/docs/user-guide/setup/install/[安装指南] Krew中的说明安装 Krew。

== 如何检查日志

您可以在以下命名空间中查看以下 pod 并检查其日志：
...
| 处理 Rancher 集成并管理 Turtles 的自定义资源

| fleet-addon-system
| caapf-controller-manager
| 为自定义基础设施提供商控制集群 API 提供商框架

| cattle-capi-system
| capi-controller-manager
| 核心群集应用程序接口控制器，用于调节主群集资源
...
| 处理定制提供商控制平面组件的初始化和管理
|===

'*'\--取决于所使用的基础设施提供商 '$'\--取决于所使用的控制平面/引导提供商，如 kubeadm 或 rke2

监控来自 Cluster API 控制平面 pod 的日志的一种方法是使用`kubectl logs` 和标签选择器。
...
[CAUTION]
====
使用`kubectl krew` 安装 stern 插件涉及第三方软件，可能会给生产环境带来安全风险。虽然它是一个有用的日志查看工具，但主要应在开发或测试环境中使用。在敏感或生产集群中安装第三方插件之前，请务必对其进行审查。
====
...
*列出 Rancher 和集群 API 资源*

用户经常询问如何处理由 CAPI 和 Rancher 创建的长长的不同 CRD 列表，以下是一些处理方法。

. 列出所有与 CAPI 和 Rancher 相关的证书。
+
[source,bash]
----
...
----
+
. 了解资源。您可以获得关于它在做什么或配置选项是什么的完整解释。
+
[source,bash]
----
...
*列出现有的 Rancher 和集群 API 对象资源*

最常见的错误是使用 Kubernetes 中的`default` 命名空间进行工作。这使得配置非常混乱，难以管理和排除故障。我们强烈建议在部署和创建 CAPI 下游集群时使用单独的命名空间，避免使用 Kubernetes 系统命名空间，如`default` 和`kube-*` 。

在使用`kubectl` 应用模板时，始终指定`-n <NAMESPACE>` 在已知位置配置资源。您可以使用此命令在*KUBECONFIG*中切换命名空间：
...
有了独立的命名空间，您就可以通过这种方法轻松管理资源。

[CAUTION]
====
使用`kubectl krew` 安装 lineage 插件涉及第三方软件，可能会给生产环境带来安全风险。虽然它是一个有用的日志查看工具，但主要应在开发或测试环境中使用。在敏感或生产集群中安装第三方插件之前，请务必对其进行审查。
====

. 使用`krew` 安装`get-all` 和`lineage` 插件。 `kubectl`
+
[source,bash]
----
...
----
+
. 然后列出命名空间中的所有现有资源，例如`capi-clusters` ，您的下游群集配置就部署在该命名空间中。
+
[source,bash]
----
...
== 如何启用{product_name} 和 CAPI 提供程序的调试模式

* {product_name} - 要启用{product_name} 的调试模式，需要对`cattle-system/rancher-config` configmap 进行如下修补：
+
[source,bash]
----
...
(5 相当于 DEBUG）。
+
这将导致 Turtles pod 重新启动，以便使用新配置。

* 集群 API 提供程序 - 为需要增加日志级别的提供程序编辑 CAPIProvider 资源。更改到您希望的级别：
+
[source,bash]
----
...
https://github.com/crust-gather/crust-gather[crust-gather]是由集群 API 开发人员创建的一个项目，专门用于从 CAPI 环境中收集日志和资源状态。它是一款安全的官方工具，可用于任何类型的环境（生产、测试或开发），为故障排除收集全面的诊断信息。
...
您可以指定一个筛选器列表来收集数据。默认情况下，它会获取整个群集的快照。数据默认存储在 crust-gather 目录中。
Crust-gather 可接受 Rancher 或子群集的预定义过滤器配置。配置示例见下文。

`child-crust-gather.yaml` 文件的 YAML 过滤器配置示例：
...
`rancher-crust-gather.yaml` 文件的 YAML 过滤器配置示例：
...
确保 kubeconfig 指向正确的群集，然后运行以下命令：
...
你可以指定一个包含秘密的文件，或包含秘密字符串的环境变量来排除。
...
有时，基础设施的清理工作可能会出现故障，从而导致资源被占用。在这种情况下，您必须手动删除资源。

请记住，手动移除终结器**需要**基础设施提供商手动清理已调配的资源。
...
要从管理群集卸载 Rancher Turtles 和 CAPI 组件，请依次按照以下步骤操作：

. 首先，删除用 CAPI 创建的所有下游群组。每个群组
+
[source,bash]
----
...
[NOTE]
====
将`capi-clusters` 替换为部署群集的命名空间，将`cluster1` 替换为群集名称。等待每个群集完全删除后再进行下一步。
====
...
. 清理任何遗留的命名空间和资源。卸载后可能会保留以下命名空间：
   * cattle-turtles-system
   * rke2-bootstrap-system
   * rke2-control-plane-system
   * capi-system
   * capz-system（或其他特定于提供商的命名空间，如 capv-system、capa-system 等）
   * capi-clusters（或您部署群集的其他命名空间）
+
要删除这些命名空间：
+
[source,bash]
...
. 最后，删除与 Cluster API 和 Rancher Turtles 相关的 CRD：
+
[source,bash]
----
...
[CAUTION]
====
确保在删除这些 CRD 之前完全删除所有群集，否则可能会留下孤儿云资源。
====
