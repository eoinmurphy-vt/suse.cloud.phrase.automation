...
Le seul flux de travail pris en charge pour enregistrer des clusters CAPI dans Rancher en utilisant Turtles, est d'utiliser l'étiquette `cluster-api.cattle.io/rancher-auto-import`{nbsp}:
...
[NOTE]
====
Un espace de noms (ou cluster) peut être marqué pour l'importation automatique à tout moment{nbsp}: avant ou après la création du cluster.
====
