...
Cette section vous guidera à travers la création de votre premier cluster et son importation dans Rancher Manager. Deux méthodes alternatives pour le provisionnement de clusters sont présentées{nbsp}: **utiliser un flux de travail GitOps avec Fleet** et **appliquer manuellement les manifests via `kubectl`**.
...
En inspectant le contenu de ce dépôt, vous trouverez{nbsp}:
...
. Sélectionnez *livraison continue* dans le menu{nbsp}:
...
. Ajoutez les chemins suivants pour spécifier quels répertoires déployer{nbsp}:
+
...
Pour créer le cluster avec kubectl, exécutez les commandes suivantes{nbsp}:

. Ouvrez un terminal et appliquez le ClusterClass et les modèles associés{nbsp}:
+
[source,bash]
----
...
. Appliquez la configuration du répartiteur de charge{nbsp}:
+
[source,bash]
----
...
. Appliquez la configuration CNI (Calico){nbsp}:
+
[source,bash]
----
...
----
. Regardez les ressources devenir prêtes{nbsp}:
+
[source,bash]
----
...
== Étapes suivantes{nbsp}: Attendez que le cluster apparaisse dans le tableau de bord Rancher
