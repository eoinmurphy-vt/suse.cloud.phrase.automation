...
Les changements se répartissent en trois domaines{nbsp}:
...
*Recommandation*{nbsp}: évaluez d'abord si vous êtes prêt à adopter `v1beta2` et agissez en conséquence. Si vous choisissez de ne pas migrer immédiatement, créez un plan pour passer à l'utilisation de la nouvelle API afin d'éviter des interruptions à l'avenir.

De plus, la communauté en amont fournit un guide de migration officiel{nbsp}:

* https://cluster-api.sigs.k8s.io/developer/providers/migrations/v1.10-to-v1.11[CAPI v1.10 à v1.11]{nbsp}: cela couvre tous les changements de `v1beta1` à `v1beta2` dans le CAPI noyau.
* https://cluster-api.sigs.k8s.io/developer/providers/migrations/v1.11-to-v1.12[CAPI v1.11 à v1.12]{nbsp}: la plupart des changements dans CAPI v1.12 peuvent être considérés comme mineurs.
...
Les champs suivants ont été supprimés de `RKE2ControlPlane.spec` dans `v1beta2`{nbsp}:
...
. Mettez à jour tous les manifests `Cluster`{nbsp}: remplacez `spec.topology.class` et `spec.topology.classNamespace` par `spec.topology.classRef.name` et `spec.topology.classRef.namespace`.
