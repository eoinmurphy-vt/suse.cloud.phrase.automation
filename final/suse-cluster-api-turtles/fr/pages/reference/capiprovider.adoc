...
Pour utiliser la ressource `CAPIProvider`{nbsp}:
...
Voici un exemple de manifeste `CAPIProvider`{nbsp}:
...
Les champs clés dans la spécification `CAPIProvider` sont{nbsp}:
