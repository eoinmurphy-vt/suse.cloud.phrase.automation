...
Pour utiliser la ressource `ClusterctlConfig`{nbsp}:
...
Voici un exemple de manifeste `ClusterctlConfig`{nbsp}:
...
Les champs clés dans la spécification `ClusterctlConfig` sont{nbsp}:
