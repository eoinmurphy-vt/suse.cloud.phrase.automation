...
:description: Implemente grupos de políticas com regras específicas que rejeitam Pods não compatíveis com base em assinaturas de imagem e outros critérios.
...
[TIP]
====
Antes de trabalhar com Grupos de Políticas, consulte axref:explanations/policy-groups.adoc[explicação].
====


Usando o exemplo da xref:../explanations/policy-groups.adoc[explicação sobre Policy Groups], use estes comandos para implementá-lo.
...
Quando a política estiver ativa, a criação de um Pod fora de conformidade será rejeitada.
Para obter mais informações sobre a avaliação das políticas que fazem parte do grupo, aumente o nível de verbosidade de `kubectl`:
...
[NOTE]
====
O resultado da avaliação produzido pelas políticas que fazem parte do grupo fica visível somente com o aumento do nível de verbosidade de `kubectl`.

Um nível de verbosidade de `4` é suficiente para ver o resultado da avaliação das políticas.
====
