...
:description: Escreva a lógica de validação para os recursos do Kubernetes no arquivo src/index.ts, acessando os dados da solicitação de entrada para retornar uma resposta.
:keywords: "{project-name}", kubewarden, kubernetes, escrever políticas, typescript
:doc-type: tutorial
...
A lógica de validação está no arquivo `src/index.ts`.
...
A solicitação de entrada contém um objeto JSON com o recurso do Kubernetes a ser validado.
Você pode acessar esses dados do objeto JSON por meio das funções auxiliares do SDK {short-project-name}.

== A função `validate` 

A política fornecida pelo modelo já tem uma função `validate` em `src/index.ts`.
Você pode usá-lo lá, adicionando sua lógica para rejeitar Pods com nomes de host negados.

Esta é a aparência que a função deve ter depois de concluída:
...
* NOTA 2: Extrair configurações definidas pelo usuário da solicitação de validação (por exemplo, nomes de host negados).
* NOTA 3: Analise o objeto Kubernetes (espera-se que seja um Pod) a partir da carga útil da solicitação.
* NOTA 4: Extraia o campo hostname da seção de especificações do Pod.
...
Essa função extrai o recurso do Kubernetes da solicitação de validação:
...
Essa função lida com o caso em que o objeto do Kubernetes é uma cadeia de caracteres JSON ou um objeto já analisado.

=== getPodHostname

Essa função extrai o nome do host de um recurso do Pod:
...
Essa função extrai o nome do host da especificação do Pod.
...
O plug-in {short-project-name} Javy fornece a função `policyAction()` que indica se a política deve validar um recurso ou validar suas configurações.