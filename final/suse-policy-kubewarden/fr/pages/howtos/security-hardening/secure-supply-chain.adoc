...
Ce qui entraîne une invite pour taper et vérifier un mot de passe{nbsp}:
...
Pour signer une stratégie, vous pouvez utiliser `cosign sign` en passant l'argument de ligne de commande `--key` avec votre fichier de clé privée{nbsp}:
...
Ce qui entraîne une invite pour le mot de passe, pour la clé privée spécifiée{nbsp}:
...
Vous pouvez vérifier la signature dans une stratégie publiée avec `kwctl inspect`. Cela montre les informations sur la stratégie et ses signatures comme indiqué ci-dessous{nbsp}:
...
Vous pouvez vérifier si une stratégie est correctement signée avec `cosign` ou `kwctl`. Ils ont des options de ligne de commande similaires pour vérifier les signatures des stratégies. Pour vérifier le binaire de signature avec une clé, utilisez `kwctl` comme ceci{nbsp}:
...
Par exemple, vous souhaitez exécuter des stratégies signées par l'organisation GitHub {short-project-name}. Alors un exemple de `ConfigMap` pour ce scénario serait{nbsp}:
...
Vous pouvez utiliser `kwctl scaffold verification-config` pour générer un fichier de configuration de vérification par défaut pour le `ConfigMap`{nbsp}:
...
Vous pouvez valider les exigences de signature contenues dans un fichier. Voici un exemple{nbsp}:
...
La configuration ci-dessus contient les deux sections surlignées, `allOf` et `anyOf`{nbsp}:

* `allOf`{nbsp}: la stratégie est fiable uniquement si toutes les exigences de signature ici sont valides.
* `anyOf`{nbsp}: la stratégie est fiable si le critère `minimumMatches` est respecté.
...
Il est possible de vérifier les informations de la signature{nbsp}:

* `issuer`(obligatoire){nbsp}: Cela correspond à l'attribut `Issuer` dans le certificat généré par Fulcio. Cela montre l'OIDC utilisé pour signer la stratégie.
* `subject` : Champ utilisé pour faire correspondre l'attribut `Subject` dans le certificat de Fulcio. Le champ `Subject` (Fulcio) contient l'utilisateur utilisé pour s'authentifier auprès du fournisseur OIDC. Le champ de vérification, `subject`, peut avoir l'un des deux sous-champs{nbsp}:
 ** `equal` : Le `Subject` (Fulcio) du certificat doit être égal à la valeur dans la validation de signature{nbsp};
...
Par exemple, cette configuration signifie que la stratégie doit avoir une signature sans clé d'Alice utilisant l'OIDC de GitHub{nbsp}:
...
* `owner` (obligatoire){nbsp}: ID GitHub de l'utilisateur ou de l'organisation en qui il faut avoir confiance
* `repo` : le nom du dépôt auquel il faut faire confiance

Par exemple, le dernier extrait, utilisant `genericIssuer`, pourrait être réécrit comme suit{nbsp}:
