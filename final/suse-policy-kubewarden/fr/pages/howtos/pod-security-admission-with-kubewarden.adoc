...
La PSA a trois profils (décrits https://kubernetes.io/docs/concepts/security/pod-security-standards/[dans les normes de sécurité des pods]) pour les espaces de noms{nbsp}:
...
Les actions sont{nbsp}: `enforce`, `audit` et `warn`.
Elles peuvent être configurées.

Au moment de la rédaction, avec Kubernetes 1.28, le contrôleur PSA a les limitations suivantes{nbsp}:
...
Lors de la définition de ce conteneur{nbsp}:

* l'attribut `runAsNonRoot` doit être défini sur `true`
* le `runAsUser` ne peut pas être défini sur `0`.

Ainsi, la ressource suivante n'atteindra pas son état souhaité{nbsp}:
...
Vous pouvez corriger cela en supprimant le `runAsUser: 0` de la définition du conteneur{nbsp}:
...
Cela n'est pas autorisé par la directive `runAsNonRoot`{nbsp}:
...
Il est possible d'imposer une plage d'identifiants d'utilisateur, par exemple, 1000--2000 et 4000--5000{nbsp}:
...
Vérifiez que la stratégie est active avant de continuer{nbsp}:
...
Lorsque la stratégie est active, recréez le déploiement{nbsp}:
...
Le conteneur défini à l'intérieur du Pod a une valeur par défaut de `runAsUser`{nbsp}:
...
Cette ressource est rejetée par la stratégie {short-project-name} mentionnée précédemment{nbsp}:
...
Elle est rejetée parce que la valeur `runAsUser` est définie sur `7000`, ce qui est en dehors des plages autorisées par la stratégie{nbsp}:
