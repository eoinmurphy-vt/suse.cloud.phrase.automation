...
Récupérez les éléments suivants de votre instance Sigstore privée{nbsp}:
...
Définissez des variables d'environnement pointant vers les URL de votre service Sigstore privé{nbsp}:
...
Exécutez `cosign` pour générer la racine de confiance{nbsp}:
...
Exécutez `cosign` pour générer la configuration de signature{nbsp}:
