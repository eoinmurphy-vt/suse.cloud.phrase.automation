...
Vous créez ce Secret pour le PolicyServer avec la commande suivante{nbsp}:
...
Une fois que vous avez créé le Secret, il est nécessaire de configurer l'instance du PolicyServer. Définissez le champ `spec.imagePullSecret` avec le nom du Secret qui contient les identifiants{nbsp}:
...
Lorsqu'il est déployé à partir du chart Helm `kubewarden-defaults`, vous pouvez définir la valeur `policyServer.imagePullSecret` sur le nom du Secret. Ensuite, le PolicyServer par défaut créé est capable de télécharger des stratégies depuis votre registre privé également{nbsp}:
