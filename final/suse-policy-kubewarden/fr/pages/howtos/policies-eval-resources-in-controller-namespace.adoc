...
Pour ce faire, mettez à niveau votre installation de {short-project-name} et définissez la valeur `alwaysAcceptAdmissionReviewsOnDeploymentsNamespace` du chart Helm `kubewarden-controller` sur `false`{nbsp}:
...
[NOTE]
.Et les stratégies de l'espace de noms{nbsp}?
====
...
[NOTE]
.Qu'en est-il de mes sélecteurs d'espace de noms personnalisés{nbsp}?
====
