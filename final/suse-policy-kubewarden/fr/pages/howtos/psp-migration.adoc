...
Avec une instance de {short-project-name}, vous pouvez déployer des politiques pour remplacer l'objet `PodSecurityPolicy`. Nous considérons ces règles dans cet exemple{nbsp}:
...
La définition YAML de ce PSP est{nbsp}:
...
Ainsi, dans cet exemple, vous avez besoin d'une politique distincte pour l'application de chacun de{nbsp}:
...
Vous pouvez déployer une politique comme indiqué ci-dessous{nbsp}:
...
Ainsi, maintenant les utilisateurs ne peuvent pas déployer des pods s'exécutant en tant que root{nbsp}:

.Exemple de sortie où runAsNonRoot{nbsp}: faux
[%collapsible]
======
...
.Exemple de sortie où runAsUser{nbsp}: 0
[%collapsible]
======
...
Pour tester la politique, nous pouvons essayer d'exécuter un pod avec la configuration privilégiée activée{nbsp}:
...
Le pod ne devrait pas être en mesure de partager les espaces de noms de l'hôte{nbsp}:
...
Examinons un exemple. Dans un cluster avec le PSP{nbsp}:

* bloquer l'accès aux espaces de noms d'hôte{nbsp}:
...
{short-project-name}Les politiques peuvent être appliquées directement à un cluster en utilisant la commande suivante{nbsp}:
...
Pour stocker les stratégies générées et les visualiser{nbsp}:
