...
Par exemple, voici une stratégie qui cible uniquement les espaces de noms `kube-system` et `my-namespace`{nbsp}:
...
Voici une stratégie qui cible tous les espaces de noms sauf les `kube-system` et `my-namespace`{nbsp}:
...
Vous pouvez obtenir tous les détails d'une demande d'admission rejetée en augmentant le niveau de verbosité de kubectl{nbsp}:
...
Par exemple, supposons que nous souhaitions que les utilisateurs utilisent toutes les classes de priorité définies à l'intérieur du cluster, sauf pour l'une des suivantes{nbsp}: `low-priority`, `med-priority` et `high-priority`.
