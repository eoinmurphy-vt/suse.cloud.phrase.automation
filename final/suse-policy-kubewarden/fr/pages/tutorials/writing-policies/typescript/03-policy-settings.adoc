...
Tout d'abord, définissez la structure qui contient les paramètres de stratégie dans `src/types.ts`{nbsp}:
...
Les stratégies {short-project-name} utilisent deux fonctions qui gèrent les paramètres{nbsp}:

* `validate` : Appelées lors de la validation de l'objet.
* `validateSettings` : Appelées au moment du chargement de la stratégie.

Dans `src/index.ts`, la fonction `validate` ressemble à{nbsp}:
