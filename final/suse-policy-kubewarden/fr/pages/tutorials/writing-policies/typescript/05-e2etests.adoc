...
Rappelez-vous, vous avez besoin de ces outils sur votre machine de développement{nbsp}:

* `bats` : Utilisé pour écrire les tests et automatiser leur exécution.
* `kwctl` >= v1.30{nbsp}: Outil CLI fourni par {project-name} pour exécuter ses stratégies en dehors de Kubernetes, entre autres actions. C'est couvert dans xref:/tutorials/testing-policies/index.adoc[la section des tests de stratégies] de la documentation.

== Écriture de tests

Vous utiliserez `bats` pour écrire et automatiser vos tests. Chaque test a les étapes suivantes{nbsp}:
...
Par exemple, lors du test de la création d'une ressource Pod{nbsp}:
...
Le modèle fournit déjà plusieurs tests de base. Voici à quoi devrait ressembler le fichier `e2e.bats` complet{nbsp}:
...
Voici les tests existants avec des explications{nbsp}:

=== Test 1{nbsp}: Rejeter les noms d'hôtes refusés
...
=== Test 2{nbsp}: Accepter les noms d'hôtes autorisés
...
=== Test 3{nbsp}: Accepter lorsque aucun paramètre n'est fourni
...
=== Test 4{nbsp}: Accepter les pods sans noms d'hôte
...
=== Test 5{nbsp}: Accepter les ressources non-Pod
...
Vous pouvez étendre la couverture de test en ajoutant plus de scénarios de test{nbsp}:

=== Test 6{nbsp}: Validation des paramètres

Vous pouvez également ajouter des tests pour vérifier que la validation des paramètres fonctionne correctement{nbsp}:
...
Vous pouvez exécuter tous les tests de bout en bout en utilisant cette commande{nbsp}:
...
Cela produit une sortie comme{nbsp}:
...
Chaque test utilise `kwctl` pour exécuter la stratégie et vérifie{nbsp}:

* Statut de sortie{nbsp}: La commande `kwctl` doit se terminer avec le statut 0 pour une exécution réussie de la stratégie.
* Champ autorisé{nbsp}: La sortie JSON contient un champ `allowed` indiquant si la demande a été acceptée.
* Contenu du message{nbsp}: Pour les demandes rejetées, la sortie contient un message d'erreur descriptif.
