...
Pour résumer, les paramètres de la stratégie devraient ressembler à ceci{nbsp}:
...
La stratégie rejette la création de ce Pod{nbsp}:
...
Elle rejette également la création de ce Pod{nbsp}:
...
Vous pouvez utiliser les paramètres de la stratégie pour forcer l'utilisation d'une spécification d'étiquette, quel que soit le contenu{nbsp}:
...
Clonez le dépôt localement et définissez la directive `module` dans le fichier `go.mod` pour qu'elle ressemble à{nbsp}:
