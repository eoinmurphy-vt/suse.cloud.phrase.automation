...
Ce job effectue les étapes suivantes{nbsp}:
...
Créer une étiquette qui correspond au modèle `v*` entraîne{nbsp}:
...
Le contenu de la section `jobs.push-to-oci-registry.env` de `ci.yml` devrait ressembler à{nbsp}:
...
La version inclut les ressources suivantes{nbsp}:
