...
Votre logique de validation doit{nbsp}:
...
et vous pouvez obtenir les données de deux manières{nbsp}:
...
Voici comment la fonction devrait être une fois terminée{nbsp}:
...
Le code comporte `NOTE` sections{nbsp}:
...
Vous devez également définir la fonction `validateLabel` dans le fichier `validate.go`{nbsp}:
...
Vous devez remplacer le contenu du fichier d'échafaudage pour correspondre à ceci{nbsp}:
...
Vous commencez par définir un `struct` qui contient les données nécessaires à un cas de test, voir `NOTE 1`{nbsp}:
...
Vous pouvez tester cela avec ces valeurs d'entrée{nbsp}:
...
C'est ici que vous itérez sur les différents cas de test en utilisant le code suivant{nbsp}:
...
. Créez un objet `payload`. Faites cela en utilisant une fonction d'assistance du SDK {short-project-name}{nbsp}: `kubewarden_testing.BuildValidationRequest`.
...
Vous pouvez maintenant exécuter tous les tests unitaires, y compris celui défini dans `settings_test.go`, en utilisant{nbsp}:
...
Comme vous pouvez le voir, tous les tests `Settings` passent, mais il y a un cas de test du `TestValidateLabel` qui ne passe pas{nbsp}:
...
Actuellement, le noyau de votre logique de validation est les lignes suivantes{nbsp}:
...
Ajoutez le nouveau code, juste après la boucle `for`{nbsp}:
...
Exécutez à nouveau les tests unitaires{nbsp}:
