...
Par exemple, vous pouvez rédiger une stratégie qui accepte une demande dans le format suivant{nbsp}:
...
Vous devez déclarer un type `RawValidationRequest` personnalisé contenant le `Request` et le `Settings`, au lieu d'utiliser le type `ValidationRequest` fourni par le `kw_sdk.go`{nbsp}:
...
Ensuite, vous définissez le type `Settings` et la fonction `validateSettings` dans le fichier `settings.go`{nbsp}:
...
Enfin, vous mettez à jour la logique de validation dans `validate.go`{nbsp}:
...
Vous devez mettre à jour le type `Settings` avec les nouveaux champs{nbsp}:
...
Vous devez également mettre à jour la structure `ValidationResponse` et la fonction `MutateRequest` dans `kw_sdk.go` pour supprimer les types spécifiques à Kubernetes et utiliser les types {short-project-name} à la place{nbsp}:
...
Vous pouvez maintenant mettre à jour la fonction `validate` pour transformer la demande si elle n'est pas valide{nbsp}:
