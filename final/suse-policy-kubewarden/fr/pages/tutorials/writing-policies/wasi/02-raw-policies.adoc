...
:description: Rédiger des politiques brutes qui évaluent des documents JSON arbitraires en définissant une logique de validation et de mutation personnalisée pour.
...
Veuillez vous référer à la section xref:tutorials/writing-policies/wasi/01-intro-wasi.adoc[Introduction à WASI] pour une vue d'ensemble du mode d'exécution de WASI.

[NOTE]
====

Vous marquez la politique comme `raw` en utilisant le champ `policyType` dans la configuration `metadata.yml`.
...
A titre d'exemple, vous pouvez écrire une politique qui accepte une requête dans le format suivant :
...
Commencez par échafauder la politique en utilisant lehttps://github.com/kubewarden/go-wasi-policy-template[modèle de politique].

Tout d'abord, vous devez définir les types qui représentent la charge utile de la demande.

Vous devez déclarer un type `RawValidationRequest` personnalisé contenant les types `Request` et `Settings`, au lieu d'utiliser le type `ValidationRequest` fourni par `kw_sdk.go`:
...
Vous définissez ensuite le type `Settings` et la fonction `validateSettings` dans le fichier `settings.go`:
...
Vous pouvez modifier l'exemple précédent pour faire muter la demande au lieu de la rejeter.

Dans ce cas, les paramètres doivent contenir les adresses `defaultUser`, `defaultAction` et `defaultRequest` à utiliser pour modifier la demande si l'utilisateur, l'action ou la ressource n'est pas valide.
...
Vous pouvez maintenant mettre à jour la fonction `validate` pour modifier la demande si elle n'est pas valide :
