...
Le {short-project-name} serveur de stratégie charge les stratégies à partir de ces sources{nbsp}:

* Système de fichiers local
* Serveur HTTP(s)
* Registres conformes à l'OCI{nbsp}:
...
La commande `kwctl annotate` nécessite deux entrées{nbsp}:
...
Par exemple, vous pouvez enregistrer ce fichier sous le nom `metadata.yml` dans le répertoire courant{nbsp}:
...
Maintenant, annotons la stratégie{nbsp}:
...
Vous pouvez pousser une stratégie annotée comme ceci{nbsp}:
