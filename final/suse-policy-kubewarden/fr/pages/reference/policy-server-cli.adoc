...
+
Valeur par défaut{nbsp}: `0.0.0.0`
...
+
Valeur par défaut{nbsp}: `policy-server.pid`
...
+
Valeur par défaut{nbsp}: `text`
+
Valeurs possibles{nbsp}: `text`, `json`, `otlp`
* `--log-level ++<+LOG_LEVEL+>++` — Niveau de journalisation
+
Valeur par défaut{nbsp}: `info`
+
Valeurs possibles{nbsp}: `trace`, `debug`, `info`, `warn`, `error`
...
+
Valeur par défaut{nbsp}: `policies.yml`
