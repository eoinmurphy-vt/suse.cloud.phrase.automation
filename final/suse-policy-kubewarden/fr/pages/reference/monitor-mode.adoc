...
On peut choisir de déployer une stratégie en mode de surveillance. En mode de surveillance{nbsp}:
...
Le `mode` est un attribut inclus dans les ressources `ClusterAdmissionPolicy` et `AdmissionPolicy`. Il y a deux valeurs que l'attribut `mode` peut assumer{nbsp}: `monitor` et `protect`. Le `mode` par défaut est `protect` s'il est omis.

Pour créer une stratégie en `monitor mode`, vous devez inclure l'instruction `mode : monitor` comme partie de la spécification de la ressource. Par exemple, dans la section `spec` (marquée ➀), de ce `ClusterAdmissionPolicy`{nbsp}:
