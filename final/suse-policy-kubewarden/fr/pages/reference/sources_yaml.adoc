...
Si vous omettez l'argument `--sources-path` de la commande `kwctl`, il essaie de charger le fichier `sources.yaml` à partir de ces dossiers{nbsp}:

* Linux{nbsp}: `$HOME/.config/kubewarden/sources.yaml`
* Mac{nbsp}: `$HOME/Library/Application Support/io.kubewarden.kubewarden/sources.yaml`
* Windows{nbsp}: `$HOME\AppData\Roaming\kubewarden\config\sources.yaml`

Sa structure est la suivante{nbsp}:
...
Ce fichier est au format YAML ou JSON. Toutes les clés sont optionnelles, donc ce qui suit est un fichier `sources.yaml` valide{nbsp}:
...
* Hôtes non listés, essayez{nbsp}:
 ** se connecter en utilisant HTTPS, en vérifiant l'identité du serveur

+
Si la connexion échoue, alors l'opération s'arrête. * Les hôtes listés dans `insecure_sources`, essayez dans l'ordre{nbsp}: ** se connecter en utilisant HTTPS en vérifiant l'identité du serveur ** se connecter en utilisant HTTPS, en sautant la vérification de l'hôte ** se connecter en utilisant HTTP
