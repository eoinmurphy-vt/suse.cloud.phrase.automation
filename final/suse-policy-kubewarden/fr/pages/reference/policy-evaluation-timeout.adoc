...
Lors de l'utilisation d'un langage traditionnel https://en.wikipedia.org/wiki/Turing_completeness[complet de Turing], il est possible de rencontrer des problèmes tels que{nbsp}:
...
Par exemple, pour définir un délai d'évaluation plus long pour une stratégie spécifique{nbsp}:
...
Vous pouvez ajuster ce comportement en utilisant ces variables d'environnement{nbsp}:
...
Lors de l'utilisation de la https://doc.crds.dev/github.com/kubewarden/kubewarden-controller/policies.kubewarden.io/PolicyServer/v1@v1.4.2[`PolicyServer`] Définition de Ressource Personnalisée Kubernetes, vous pouvez définir ces variables d'environnement comme suit{nbsp}:
...
En citant la https://kubernetes.io/docs/reference/access-authn-authz/extensible-admission-controllers/#timeouts[documentation officielle de Kubernetes]{nbsp}:
...
Toutes les demandes d'admission Kubernetes faites à un serveur de stratégies sont soumises à deux délais d'attente différents{nbsp}:
