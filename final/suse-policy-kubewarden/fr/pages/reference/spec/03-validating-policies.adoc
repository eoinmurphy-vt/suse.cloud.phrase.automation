...
Le {project-name} serveur de stratégie reçoit{nbsp}:
...
Étant donné le `AdmissionReview` Kubernetes suivant{nbsp}:
...
L'objet `ValidationRequest` ressemblerait à{nbsp}:
...
Le `ValidationResponse` a cette structure{nbsp}:
