...
{short-project-name}的 https://github.com/chimera-kube/policy-server[policy-server]在启动时验证用户提供的所有策略设置。如果至少有一个策略接收到错误的配置参数，policy-server将立即退出并显示错误。
...
作为一个例子，考虑 https://github.com/kubewarden/psp-capabilities[psp-capabilities]策略，其配置格式如下：
