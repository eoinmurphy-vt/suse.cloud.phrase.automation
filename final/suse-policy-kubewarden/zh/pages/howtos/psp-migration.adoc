...
您应该使用`mutation: true`配置策略。这是必需的，因为当用户未定义它们时，策略将添加 https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups[supplementalGroups]。
...
下面的示例显示了添加一个 https://kubernetes.io/docs/concepts/security/pod-security-policy/#users-and-groups[supplemental group]，尽管它并未被我们定义。
...
您需要替换阻止特权容器的旧PSP配置。部署 https://github.com/kubewarden/pod-privileged-policy[pod-privileged policy]是必要的。此策略不需要任何设置。一旦运行，它将阻止特权 Pod。
...
为了完成 PSP 迁移练习，您需要禁用主机名称空间共享。为此，我们将使用 https://github.com/kubewarden/host-namespaces-psp-policy[`host-namespace-psp` policy]。它允许集群管理员单独阻止 IPC、PID 和网络名称空间。它还设置了 Pod 可以在主机 IP 上开放的端口。
...
{short-project-name}团队开发了一个用于PSP迁移的脚本。它使用来自 https://github.com/appvia/psp-migration[AppVia]的迁移工具。AppVia 工具读取 PSP YAML 配置。它会生成相应的策略。它为 {short-project-name} 和其他策略引擎执行此操作。
