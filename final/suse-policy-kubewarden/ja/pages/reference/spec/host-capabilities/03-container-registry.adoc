...
:description: OCI マニフェスト・ダイジェストの計算や OCI オブジェクト・マニフェストのフェッチなど、コンテナ・レジストリの機能を探ります。
...
:sidebar_label: コンテナ登録機能
:current-version: {page-origin-branch}

コンテナレジストリは、多くのタイプのOCIオブジェクトを配布するために使用できる。
よく知られたコンテナ・イメージから一般的なOCIアーティファクトまで。
OCI アーティファクトは、{project-name} ポリシー、Helm チャートなどのオブジェクトを保存するために使用されます。

これらは、コンテナレジストリとやりとりするために、{short-project-name} ホストによって公開される機能である。

== OCIマニフェスト・ダイジェストの取得

このコールバックは、OCI マニフェストのダイジェストを計算します。ダイジェストは、OCIレジストリに格納されているオブジェクトを識別するために使用できる。これは、`tags` がミュータブルであるのとは対照的に、イミュータブルな方法である。

=== キャッシング

ダイジェストの計算には、{short-project-name} ポリシーホストとリモートレジストリ間の一連のネットワーク要求が含まれる。
これらの操作は時間がかかるので、結果は1分間キャッシュされる。

=== 認証

プライベートレジストリとのやりとりでは、{short-project-name} ポリシーホストがリモートレジストリに対して認証を行う必要がある。

ポリシーホストは、リモートレジストリからポリシーをフェッチするために使用されるのと同じ資格情報のセットを使用します。

=== 通信プロトコル

これは、このケイパビリティを公開するために使用されるwaPCプロトコルの説明である：

==== waPC機能 -`v1/manifest_digest` 入力
...
例えば、`busybox:latest` 画像のマニフェスト・ダイジェストを要求する場合、ペイロードは次のようになる：
...
このコールバックは、OCI オブジェクトのマニフェストをフェッチします。利用可能な場合、この情報は、1つまたは複数のプラットフォームの特定の画像マニフェストを識別するために使用することができます。または、画像のための単一の画像マニフェスト。

=== キャッシング

この情報を取得するには、{short-project-name}ポリシーホストとリモートレジストリ間のネットワーク要求が必要です。これらの操作は時間がかかるので、結果は1分間キャッシュされる。

=== 認証

プライベートレジストリとのやりとりでは、{short-project-name} ポリシーホストがリモートレジストリに対して認証を行う必要がある。

ポリシーホストは、リモートレジストリからポリシーをフェッチするために使用されるものと同じ資格情報のセットを使用します。

=== 通信プロトコル

これは、このケイパビリティを公開するために使用されるwaPCプロトコルの説明である：

==== waPC機能 -`v1/oci_manifest` 入力
...
例えば、`ghcr.io/kubewarden/policy-server:v1.10.0` 画像のマニフェストを要求する場合、ペイロードは次のようになる：

* 入力ペイロード： `"ghcr.io/kubewarden/policy-server:v1.10.0"`
* 出力ペイロード：レジストリから得られた成功応答の本体。
これはhttps://github.com/opencontainers/image-spec/blob/main/image-index.md[インデックス画像] OCIまたはhttps://github.com/opencontainers/image-spec/blob/main/manifest.md[画像マニフェスト] OCIである。
詳細はレジストリやイメージによって変わる可能性がある。

== OCIマニフェストおよびコンフィグ

このコールバックは、OCI イメージマニフェストとその設定をフェッチします。この情報を使用して、コンテナ・イメージのマニフェストと、それを実行するためにコンテナ・ランタイムが使用する構成情報を取得することができます。
...
==== waPC機能 -`v1/oci_manifest_config` 入力
...
==== waPC機能 -`v1/oci_manifest_config` 出力
...
例えば、`ghcr.io/kubewarden/policy-server:v1.13.0` イメージのマニフェストとコンフィギュレーションを要求する場合、ペイロードは次のようになる：

* 入力ペイロード： `"ghcr.io/kubewarden/policy-server:v1.13.0"`
* 出力ペイロード：レジストリから得られた成功応答の本体。https://github.com/opencontainers/image-spec/blob/main/manifest.md[OCIイメージ・マニフェスト]、イメージ・ダイジェスト、https://github.com/opencontainers/image-spec/blob/main/config.md[OCIイメージ・コンフィギュレーション]が含まれます。
