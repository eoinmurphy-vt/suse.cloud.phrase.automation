...
{short-project-name}の https://github.com/chimera-kube/policy-server[policy-server]は、起動時にユーザーが提供したすべてのポリシー設定を検証します。ポリシーサーバーは、少なくとも1つのポリシーが誤った設定パラメータを受け取った場合、直ちにエラーで終了します。
...
例として、次の設定形式を持つ https://github.com/kubewarden/psp-capabilities[psp-capabilities]ポリシーを考えてみてください。
