...
Pour exposer SUSE Observability Server en utilisant le contrôleur d'Ingress Traefik avec des certificats auto-signés, vous devez{nbsp}:
...
Créer des secrets TLS Kubernetes à partir de vos fichiers de certificat et de clé privée{nbsp}:
...
Créer un fichier de valeurs (par exemple, `ingress-with-tls-values.yaml`) avec la configuration d'Ingress{nbsp}:
...
Déployez SUSE Observability Server en utilisant la configuration Ingress{nbsp}:
...
. Créez un répertoire de travail `workdir` et copiez le fichier de certificat `site.cert` dans ce répertoire.
. Démarrez le conteneur Docker Java avec le `workdir` monté en tant que volume afin qu'il puisse être accessible{nbsp}:
+
[,bash]
----
...
. Changez de répertoire vers le `workdir` et faites une copie du fichier `cacerts`{nbsp}:
+
[,bash]
----
cd /workdir
...
Pour utiliser un magasin de confiance encodé en base64, exécutez la commande suivante `helm upgrade`{nbsp}:
...
Le certificat peut être téléchargé directement depuis le navigateur Chrome. Les étapes peuvent varier légèrement en fonction de la version que vous utilisez{nbsp}:
...
Configurer des certificats personnalisés via les valeurs du graphique Helm en utilisant l'une de ces deux méthodes{nbsp}:

==== Méthode 1 : Données PEM directes

Intégrez les données du certificat directement dans votre configuration Helm{nbsp}:
...
==== Méthode 2{nbsp}: Référence ConfigMap

Créez un ConfigMap Kubernetes avec votre certificat et référencez-le dans la configuration Helm{nbsp}:
...
Référencez le ConfigMap dans votre configuration Helm{nbsp}:
...
Pour l'approche des données PEM directes, commencez par stocker votre certificat dans une variable shell{nbsp}:
...
Déployez SUSE Observability Agent avec la configuration du certificat{nbsp}:
...
Pour l'approche ConfigMap, créez le ConfigMap contenant votre certificat{nbsp}:
...
Déployez SUSE Observability Agent avec la référence ConfigMap{nbsp}:
...
Configurez des certificats CA personnalisés en utilisant l'une de ces méthodes{nbsp}:

* **Configuration persistante**{nbsp}: Utilisez `sts context save` pour stocker la configuration du certificat pour les commandes futures
* **Utilisation unique**{nbsp}: Ajoutez des indicateurs de certificat aux commandes CLI individuelles si nécessaire

==== Méthode 1 : Chemin du fichier de certificat CA

Spécifiez le chemin vers votre fichier de certificat CA encodé en PEM{nbsp}:
...
==== Méthode 2{nbsp}: Données de certificat CA encodées en Base64

Fournissez les données du certificat CA sous forme de chaîne encodée en base64{nbsp}:
...
Utilisez les indicateurs de certificat avec n'importe quelle commande CLI pour une validation de certificat unique{nbsp}:
...
Les configurations de certificat sont stockées dans{nbsp}: `~/.config/stackstate-cli/config.yaml`

[NOTE]
====
**Important**{nbsp}: L'indicateur `--skip-ssl` désactive toute vérification SSL et ignore les configurations de certificat. Utilisez toujours les options de certificat CA pour des connexions sécurisées avec des certificats personnalisés.
====
...
. Montez le configmap en ajoutant ce qui suit au niveau supérieur de `otel-collector.yaml`{nbsp}:
...
. Mettez à jour la configuration de l'exportateur dans le `otel-collector.yaml` pour utiliser le certificat *ca_file* en ajoutant la clé `tls.ca_file`{nbsp}:
...
**Solution** : Ajoutez votre certificat personnalisé à Rancher avant d'installer l'extension. Suivez la documentation de Rancher{nbsp}: https://ranchermanager.docs.rancher.com/getting-started/installation-and-upgrade/resources/custom-ca-root-certificates[configuration de certificats CA racine personnalisés^].
