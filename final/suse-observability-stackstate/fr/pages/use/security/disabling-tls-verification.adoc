...
Pour désactiver la validation SSL pour votre xref:/use/security/self-signed-certificates.adoc#_suse_observability_agent[SUSE Observability Agent], vous devez configurer la valeur de chaîne Helm comme indiqué ci-dessous{nbsp}:
...
Pour désactiver la validation SSL depuis le xref:/use/security/self-signed-certificates.adoc#_suse_observability_cli[SUSE Observability CLI], vous devez spécifier l'option `--skip-ssl` avec les commandes `sts CLI`. Cela peut être fait en utilisant l'une des deux méthodes ci-dessous{nbsp}:

*Méthode 1*{nbsp}: 

Spécifiez l'option `--skip-ssl` dans le fichier de certificat CA encodé en PEM pour une configuration persistante{nbsp}:
...
Spécifiez l'option `--skip-ssl` dans toute commande CLI utilisée pour une validation de certificat ponctuelle{nbsp}:
