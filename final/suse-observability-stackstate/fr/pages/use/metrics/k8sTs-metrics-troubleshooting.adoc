...
S'il y a des résultats, mais pas pour une valeur spécifique d'une étiquette (par exemple pour l'étiquette `pod_name`), la requête est correcte mais il n'y a pas de séries temporelles pour cette métrique spécifique pour ce composant spécifique. Choses à vérifier dans ce cas :

* Les données sont-elles collectées pour ce composant (soit via l'agent SUSE Observability, soit par d'autres moyens) ?
* Le composant rapporte-t-il même la métrique{nbsp}?

Comment faire cela dépend de la façon dont la collecte de données est configurée.
