...
Lorsque SUSE Observability affiche des données dans un graphique, il est presque toujours nécessaire de modifier la résolution des données stockées pour les adapter à l'espace disponible pour le graphique. Pour obtenir les graphiques les plus représentatifs possibles, suivez ces directives{nbsp}:
...
[CAUTION]
====
Lorsque la liaison de métriques ne spécifie pas d'agrégation, SUSE Observability utilisera automatiquement l'agrégation `last_over_time` pour réduire le nombre de points de données pour un graphique. Voir aussi xref:/use/metrics/k8s-writing-promql-for-charts.adoc#_why[Pourquoi est-ce nécessaire ?] pour une explication.
====
...
Pour résumer, lors de l'exécution d'une requête PromQL pour une plage de temps d'une semaine avec une étape d'une heure, cette requête{nbsp}:

----
container_cpu_usage /1000000000
----

est automatiquement convertie en{nbsp}:

----
last_over_time(container_cpu_usage[1h]) /1000000000
----

Essayez-le vous-même sur le https://play.stackstate.com/#_/metrics?promql=last_over_time%28container_cpu_usage%7Bnamespace%3D%22sock_shop%22%2C%20pod_name%3D~%22carts.%2A%22%7D%5B%24%7B%5F%5Finterval%7D%5D%29%20%2F%201000000000&timeRange=LAST_7_DAYS[SUSE Observability playground].
...
Il existe d'excellents articles de blog sur Internet qui expliquent cela plus en détail{nbsp}:
...
Quelques ressources supplémentaires pour comprendre les requêtes PromQL{nbsp}:
