...
Cela créera un nouveau rôle dans SUSE Observability appelé `grafana` et lui accordera la permission `get-metrics`. Vous pouvez ensuite créer un jeton de service pour ce rôle{nbsp}:
...
Sur la page de configuration des sources de données, entrez les détails de configuration suivants{nbsp}:

* *Nom*{nbsp}: SUSE Observability
* *URL*{nbsp}: `https://<tenant-name>.app.stackstate.io/prometheus`
* *En-têtes HTTP personnalisés*
 ** *En-tête*{nbsp}: `X-API-Key`
 ** *Valeur*{nbsp}: `<service-token>`
...
Pour accorder l'accès à toutes les métriques d'un cluster{nbsp}:
[,sh]
----
...
Pour accorder l'accès à toutes les métriques d'un espace de noms{nbsp}:
[,sh]
----
...
Inspecter toutes les autorisations qui ont été accordées au sujet{nbsp}:
[,sh]
----
