...
Parce que le seuil exact et la gravité peuvent dépendre de l'application, les seuils peuvent être xref:/use/alerting/k8s-override-monitor-arguments.adoc[remplacés via une annotation Kubernetes] sur le service. Par exemple, pour remplacer le seuil déviant préconfiguré et n'avoir qu'un seuil critique à 6 %, ajoutez cette annotation à votre service{nbsp}:
```
...
Pour personnaliser le moniteur, ajoutez une annotation comme celle-ci à votre service et ajustez les valeurs si nécessaire{nbsp}:
...
Le cluster n'a pas de santé propre. Mais un cluster est constitué de quelques composants, dont certains sont critiques pour le fonctionnement normal. Le moniteur agrège les états de ces composants{nbsp}:
...
Pour utiliser ce moniteur efficacement, assurez-vous que certaines ou toutes les vérifications de santé suivantes sont désactivées{nbsp}:
