...
Le mécanisme de déclaration des dérogations se fait via des annotations de ressources kubernetes qui indiquent à quel moniteur et à quel composant elles doivent s'appliquer. Par exemple, nous pouvons remplacer le moniteur `failureState` par le moniteur `Available service endpoints` pour un service spécifique pour lequel nous voulons signaler un état `CRITICAL` en cas d'échec plutôt que l'état par défaut `DEVIATING`.
