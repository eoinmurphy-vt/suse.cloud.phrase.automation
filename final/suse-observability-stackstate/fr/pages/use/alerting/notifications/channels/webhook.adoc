...
Pour configurer un webhook, complétez les champs{nbsp}:
...
Les sections dans la charge utile `open` sont{nbsp}:

. Composant{nbsp}: le composant SUSE Observability auquel la notification s'applique. Cela inclut le nom du composant, l'identifiant, le type et les tags. Il contient également un lien vers l'interface utilisateur de SUSE Observability qui ouvrira le composant au moment du changement d'état de santé.
. Événement{nbsp}: l'événement qui a déclenché cette notification. Il peut être de type `open` ou `close` (voir la section suivante). Un état `open` signifie que le moniteur est toujours dans un état critique (ou déviant) pour le composant spécifié. Un état `close` signifie que le moniteur était ouvert auparavant mais que le problème a été résolu. L'état et l'heure de déclenchement sont inclus. Est également inclus un `title` qui est une brève description du problème fournie par le moniteur, c'est le même titre affiché sur la page des points saillants du composant, cela peut être différent et plus détaillé que le nom du moniteur.
. Moniteur{nbsp}: le moniteur qui a déclenché la notification. À côté du nom du moniteur, des tags et un identifiant, un lien est également inclus. Le lien ouvrira le moniteur dans l'interface utilisateur de SUSE Observability.
. Configuration de la notification{nbsp}: La configuration de la notification pour cette notification. Comprend un nom, un identifiant et un lien. Le lien ouvrira la configuration de la notification dans l'interface utilisateur de SUSE Observability.
. Identifiant de notification{nbsp}: Un identifiant unique pour cette notification. Voir aussi le <<_notification_life_cycle,cycle de vie de la notification>>
...
Notez qu'une notification peut être à la fois ouverte et fermée pour des raisons différentes d'un changement d'état de santé{nbsp}:
...
. Enregistrez ce script Python sous `webhook.py`{nbsp}:
```python
...
. Exécutez le serveur webhook sur un port inutilisé (par exemple 8000){nbsp}: `python3 webhook.py 8000`
