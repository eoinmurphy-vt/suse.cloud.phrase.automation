...
Pour des raisons de simplicité, l'exemple ne traite que des sélections non filtrées, qui afficheront toutes les lignes de journal, de tous les conteneurs, de n'importe quel type d'entrée de journal.  La sélection de `View all` permet d'ouvrir le tiroir à journaux sans aucun filtrage :
