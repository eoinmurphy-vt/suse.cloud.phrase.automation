...
Des détails sur la manière d'ingérer les données de santé peuvent être trouvés sur les pages suivantes{nbsp}:
...
Le cadre de synchronisation de l'état de santé fonctionne comme suit{nbsp}:

* Les données de santé sont envoyées à SUSE Observability et ingérées via l'API Receiver.
* Les éléments de topologie de SUSE Observability liés aux vérifications de santé ingérées sont identifiés et liés en fonction de{nbsp}:
...
Un moniteur externe permet d'attacher les états de santé aux composants et d'afficher un indice de remédiation sur les pages de mise en évidence de SUSE Observability. Cette ressource doit être créée via le xref:/setup/cli/cli-sts.adoc[SUSE Observability CLI] ou dans le cadre d'un stackpack. Voici un exemple de moniteur externe{nbsp}:
...
Chaque charge utile `ExternalMonitor` a les détails suivants{nbsp}:
