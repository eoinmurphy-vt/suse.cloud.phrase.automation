...
Les éléments de topologie utilisent deux types d'identifiants dans SUSE Observability{nbsp}:
...
Les identifiants de portée globale dans SUSE Observability sont un URN globalement unique qui correspond à la convention suivante{nbsp}:
...
Notez que tous les caractères ne sont pas autorisés dans un identifiant URN. Vous pouvez vérifier vos identifiants avec l'expression régulière URN suivante{nbsp}:
...
Les préfixes URN reconnus sont{nbsp}:
...
* Type de composant serveur{nbsp}:
...
* Service découvert avec des traces{nbsp}:
 ** `urn:service:/prod-db`
* Instance de service découverte avec des traces{nbsp}:
...
* Vérifiez la fonction d'état d'exécution d'AWS Event partagée entre les instances de StackPack AWS{nbsp}:
 ** `urn:stackpack:aws:shared:check-function:aws-event-run-state`
* Type de composant cmdb_ci_netgear dans le StackPack ServiceNow{nbsp}:
