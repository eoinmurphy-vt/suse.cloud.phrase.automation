...
* *Système d'exploitation*{nbsp}: Linux ou MacOS
* *Outils installés*{nbsp}:
...
* *Accès Internet*{nbsp}: Nécessaire pour tirer les images Docker de Quay.io et les graphiques Helm de ChartMuseum.

=== Sur l'hôte du réseau privé

* *Accès*{nbsp}: Accès SSH à l'hôte.
* *Outils installés*{nbsp}:
 ** https://www.docker.com/products/docker-desktop/[Docker]
 ** https://helm.sh/docs/intro/install/[Helm CLI]
 ** Scripts pour télécharger les images Docker depuis le registre source (les liens seront fournis plus tard dans ce guide).
...
Exécutez les commandes suivantes sur l'hôte local pour obtenir les images Docker et les graphiques Helm requis{nbsp}:
...
Copiez les fichiers suivants de l'hôte local vers l'hôte dans le réseau privé{nbsp}:
...
* Graphiques Helm téléchargés précédemment{nbsp}:
...
Créez un fichier `values.yaml` avec votre configuration, y compris le registre privé{nbsp}:
...
Déployez en utilisant l'archive du graphique téléchargée{nbsp}:
...
Créez un fichier private-registry.yaml avec le contenu suivant{nbsp}:
...
Si le registre privé nécessite une authentification, incluez le nom d'utilisateur et le mot de passe du secret de pull{nbsp}:
