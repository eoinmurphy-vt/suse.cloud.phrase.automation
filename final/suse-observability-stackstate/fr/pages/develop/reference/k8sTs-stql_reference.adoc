...
Les filtres de composants sont utilisés de deux manières dans STQL{nbsp}:
...
Les opérateurs décrits ci-dessous sont disponibles pour une utilisation dans les requêtes STQL. Notez que les opérateurs booléens seront exécutés dans l'ordre standard{nbsp}: NON, OU, ET.
...
* Les filtres de base utilisent AND/OR d'une manière spécifique{nbsp}:
 ** Tous les éléments dans chaque boîte de filtre de base sont reliés par un *OU*{nbsp}: `layer IN ("Containers", "Services", "Storage")`
 ** Les différentes boîtes de filtre de base sont enchaînées ensemble avec un *ET*{nbsp}: `layer IN ("Containers") AND domain IN ("cluster.test.stackstate.io”)`
 ** La boîte de filtre de base *Inclure des composants* (`name`) est l'exception - celle-ci est reliée aux autres boîtes de filtre avec un OU{nbsp}: `layer IN ("Containers") AND domain IN ("cluster.test.stackstate.io") OR name IN ("cert-manager”)`
 ** Pour être compatible avec le filtrage de base, la fonction *avecVoisinsDe* et le filtre *identifiant* doivent être reliés à d'autres filtres avec un *OU*{nbsp}: `layer in ("Containers") OR identifier IN ("urn:kubernetes:/cluster.test.stackstate.io:kube-system:pod/cert-manager-7749f44bb4-vspjj:container/cert-manager")`
