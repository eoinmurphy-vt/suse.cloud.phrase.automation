...
L'approche générale pour dépanner les problèmes opérationnels de la plateforme SUSE Observability est la suivante{nbsp}:
...
* Inspectez les journaux/métadonnées des pods suspects via{nbsp}:
...
* `Zookeeper` : Zookeeper est utilisé pour la découverte de services, l'orchestration et le basculement. Zookeeper est déployé en utilisant 1 ou plusieurs pods avec le nom{nbsp}:
 ** `suse-observability-zookeeper-<n>`
* `Kafka` : Kafka est utilisé pour le passage de messages entre presque tous les services{nbsp}: Kafka est déployé par les pods suivants{nbsp}:
 ...
 ** `HBase-HA` : Stocke les données de StackGraph, réparties sur plusieurs pods avec différentes responsabilités{nbsp}:
...
* `VictoriaMetrics` : Stocke les données métriques. Est déployé par les pods{nbsp}:
...
* `ClickHouse` : Stocke les données de trace. Déployé par les pod(s) suivants{nbsp}:
 ** `suse-observability-clickhouse-shard0-<n>` : Stockage principal de ClickHouse
* `ElasticSearch` : Stocke les événements et les journaux. Déployé par les pods suivants{nbsp}:
...
* `Receiver` : Le récepteur implémente l'API côté collecte pour l'agent d'observabilité SUSE. Il accepte et autorise les données de télémétrie (journaux, événements, métriques ou topologie) et les transfère au magasin de données correspondant ou à Kafka. Il peut être déployé en mode simple ou en mode partagé{nbsp}:
 ** `Receiver-Split` :
...
* `Correlator` : Corrèle les informations de connexion TCP pour les transformer en topologie. Implémenté par le pod{nbsp}:
 ** `<release-name>-suse-observability-correlate-*`
* `Events2Elasticsearch` : Traite les événements et les stocke dans Elasticsearch{nbsp}: Implémenté par le pod{nbsp}:
 ** `<release-name>-suse-observability-e2es-*`
* `Anomaly Detection` : La plateforme d'observabilité SUSE effectue la détection d'anomalies (désactivée par défaut) sur les métriques, produisant des violations de santé{nbsp}:
...
* `Platform-Distributed` : La plateforme contient les principaux composants de traitement et l'API de service. En mode distribué, les unités fonctionnelles sont séparées. Les pods qui appartiennent à la plateforme{nbsp}:
...
* `Routing` : Accepte les connexions et les dirige vers le bon service backend{nbsp}:
...
* `Backup/Restore` : Exécute périodiquement des tâches pour sauvegarder les différents magasins de données. Dispose d'un pod en cours d'exécution en continu{nbsp}:
