...
L'ancienne méthode d'installation nécessitait deux étapes{nbsp}:
...
La nouvelle méthode utilise un seul fichier de valeurs avec la configuration `global.suseObservability`{nbsp}:
...
Convertissez votre commande `helm template` en une commande `helm upgrade --install` en mappant les paramètres{nbsp}:
...
[NOTE]
====
Avant d'exécuter les commandes helm, assurez-vous d'avoir la dernière version du chart{nbsp}:
...
Si vous préférez utiliser un fichier de valeurs, créez `values.yaml` avec la configuration indiquée dans <<_step_3_create_new_values_file>>, assurez-vous d'avoir le dernier chart avec `helm repo update`, puis exécutez{nbsp}:
...
Lors de l'exécution d'une migration depuis la méthode d'installation héritée, soyez conscient des éléments suivants{nbsp}:
...
Avant de commencer la migration, sauvegardez vos valeurs Helm existantes et toutes personnalisations{nbsp}:
...
Examinez votre configuration existante pour identifier les paramètres suivants{nbsp}:

* *Profil de dimensionnement*{nbsp}: Vérifiez `sizing.profile` dans votre entrée suse-observability-values
* *Clé de licence*{nbsp}: Votre licence {stackstate-product-name}
* *URL de base*{nbsp}: L'URL où {stackstate-product-name} est accessible
* *Mot de passe de l'administrateur*{nbsp}: Le mot de passe de l'administrateur (vous pouvez également utiliser le hachage bcrypt du mot de passe, la valeur pour cela est `adminPasswordBcrypt`)
* *Identifiants du secret de tirage*: Nom d'utilisateur et mot de passe du registre (si utilisé)
* *Paramètres d'affinité personnalisés*{nbsp}: Toute personnalisation d'affinité de nœud ou d'anti-affinité de pod

Vous pouvez extraire certains de ces éléments de votre installation actuelle{nbsp}:
...
Lorsque vous définissez `global.suseObservability.sizing.profile`, le chart configure automatiquement{nbsp}:
...
Préservez uniquement les valeurs qui sont *vraiment personnalisées* - configurations que vous avez ajoutées ou modifiées au-delà de ce que le profil de dimensionnement fournit{nbsp}:
...
Votre fichier de valeurs de migration devrait être *beaucoup plus petit* que la sortie `helm get values`{nbsp}:
...
=== Étape 3{nbsp}: Créer un nouveau fichier de valeurs

Créer un nouveau `values.yaml` avec la structure `global.suseObservability`{nbsp}:
...
=== Étape 4{nbsp}: Référence de mappage de configuration

Utilisez ce tableau pour mapper votre ancienne configuration au nouveau format{nbsp}:
...
Tout d'abord, assurez-vous d'avoir la dernière version du chart{nbsp}:
...
Ensuite, effectuez la mise à niveau{nbsp}:
