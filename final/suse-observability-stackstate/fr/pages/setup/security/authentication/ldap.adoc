...
Suivez les étapes ci-dessous pour configurer SUSE Observability afin de s'authentifier en utilisant LDAP{nbsp}:

. Dans `authentication.yaml` - ajoutez les détails LDAP (voir l'exemple ci-dessus){nbsp}:
 ...
 ** *groupMemberKey* - Le nom de l'attribut qui indique si un utilisateur est membre d'un groupe. Le filtre LDAP construit suit ce modèle{nbsp}: `<groupMemberKey>=<user.dn>,ou=groups,dc=acme,dc=com`. Pour retourner tous les groupes imbriqués, utilisez `groupMemberKey: "member:1.2.840.113556.1.4.1941:"`.
. Dans `authentication.yaml` - associez les rôles des utilisateurs de LDAP aux sujets appropriés de SUSE Observability (voir l'exemple ci-dessus){nbsp}:
...
Lorsque le mot de passe ldap doit provenir d'un secret externe, suivez xref:/setup/security/external-secrets.adoc#_getting_authentication_data_from_an_external_secret[ces étapes] mais remplissez les données suivantes{nbsp}:
