...
Suivez les étapes ci-dessous pour configurer les utilisateurs et appliquer les modifications{nbsp}:

. Dans `authentication.yaml` - ajouter des utilisateurs. La configuration suivante doit être ajoutée pour chaque utilisateur (voir l'exemple ci-dessus){nbsp}:
...
Lorsque les mots de passe des utilisateurs doivent provenir d'un secret externe, suivez xref:/setup/security/external-secrets.adoc#_getting_authentication_data_from_an_external_secret[ces étapes] mais remplissez les données suivantes{nbsp}:
