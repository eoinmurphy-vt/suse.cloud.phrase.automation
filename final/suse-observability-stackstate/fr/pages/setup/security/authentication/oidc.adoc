...
Rancher mettra le client ID et le secret à disposition dans son champ d'état{nbsp}:
[,yaml]
----
apiVersion: management.cattle.io/v3
kind: OIDCClient
...
Utilisez les paramètres suivants pour le client (si nécessaire par le fournisseur OIDC){nbsp}:
...
Pour configurer Rancher en tant que fournisseur OIDC pour SUSE Observability, vous devez ajouter les détails OIDC aux valeurs d'authentification{nbsp}:
[,yaml]
----
stackstate:
  authentication:
    rancher:
      ...
      baseUrl: "<rancher-url>"
----
Vous pouvez remplacer et étendre la configuration OIDC pour Rancher avec les champs suivants{nbsp}:

* **discoveryUri** - URI que vous pouvez utiliser pour découvrir le fournisseur OIDC{nbsp}: Normalement, également documenté ou retourné lors de la création du client dans le fournisseur OIDC{nbsp}:
* **redirectUri** - Optionnel (non dans l'exemple){nbsp}: L'URI où le point de terminaison de rappel de connexion de SUSE Observability est accessible. Rempli par défaut en utilisant le `stackstate.baseUrl`, mais peut être remplacé{nbsp}: Cela doit être une URL entièrement qualifiée qui pointe vers le chemin `/loginCallback`{nbsp}:
* **customParameters** - Carte optionnelle de paires clé/valeur que vous envoyez au fournisseur OIDC en tant que paramètres de requête personnalisés{nbsp}: Certains fournisseurs OIDC nécessitent des paramètres de requête supplémentaires qui ne sont pas envoyés par défaut{nbsp}:

Si vous devez désactiver la vérification TLS en raison d'une configuration n'utilisant pas de certificats SSL vérifiables, vous pouvez désactiver les vérifications SSL avec la configuration de l'application (ne pas utiliser en production){nbsp}:

Pour la configuration **Non-HA**{nbsp}:
[,yaml]
----
stackstate:
  components:
...
----
Pour la configuration **HA (haute disponibilité)**{nbsp}:
[,yaml]
----
stackstate:
  components:
...
Dans les déploiements HA, le composant api gère la configuration différemment{nbsp}: Utilisez ce qui suit{nbsp}: +
=== Kubernetes

Pour configurer SUSE Observability afin d'utiliser un fournisseur d'authentification OIDC sur Kubernetes, vous devez ajouter les détails OIDC et la cartographie des rôles utilisateurs au fichier `authentication.yaml`{nbsp}: Par exemple :
...
Suivez les étapes ci-dessous pour configurer SUSE Observability afin de s'authentifier en utilisant OIDC{nbsp}:

. Dans `authentication.yaml` - ajoutez les détails du fournisseur d'authentification OIDC (voir l'exemple ci-dessus){nbsp}:
 ** *clientId* - L'ID du xref:/setup/security/authentication/oidc.adoc#_configure_the_oidc_provider[client OIDC que vous avez créé pour SUSE Observability]{nbsp}:
 ** *secret* - Le secret pour le xref:/setup/security/authentication/oidc.adoc#_configure_the_oidc_provider[client OIDC que vous avez créé pour SUSE Observability]{nbsp}:
 ** *discoveryUri* - URI qui peut être utilisée pour découvrir le fournisseur OIDC{nbsp}: Normalement également documenté ou retourné lors de la création du client dans le fournisseur OIDC{nbsp}:
 ** *jwsAlgorithm* - La valeur par défaut pour OIDC est `RS256`{nbsp}: Si votre fournisseur OIDC utilise un autre, il peut être défini ici{nbsp}:
 ** *scope* - Doit correspondre, ou être un sous-ensemble, de la portée fournie dans la configuration du fournisseur OIDC{nbsp}: SUSE Observability utilise cela pour demander l'accès à ces parties d'un profil utilisateur dans le fournisseur OIDC{nbsp}:
 ** *redirectUri* - Optionnel (non dans l'exemple){nbsp}: L'URI où le point de terminaison de rappel de connexion de SUSE Observability est accessible. Rempli par défaut en utilisant le `stackstate.baseUrl`, mais peut être remplacé{nbsp}: Cela doit être une URL entièrement qualifiée qui pointe vers le chemin `/loginCallback`{nbsp}:
 ** *customParameters* - Carte optionnelle de paires clé/valeur qui sont envoyées au fournisseur OIDC en tant que paramètres de requête personnalisés{nbsp}: Certains fournisseurs OIDC nécessitent des paramètres de requête supplémentaires qui ne sont pas envoyés par défaut.
 ** *jwtClaims* -
...
. Exécutez une mise à niveau Helm pour appliquer les modifications{nbsp}:
+
[,text]
----
 helm upgrade \
...
Lorsque les secrets OIDC doivent provenir d'un secret externe, suivez xref:/setup/security/external-secrets.adoc#_getting_authentication_data_from_an_external_secret[ces étapes] mais remplissez les données suivantes{nbsp}:
