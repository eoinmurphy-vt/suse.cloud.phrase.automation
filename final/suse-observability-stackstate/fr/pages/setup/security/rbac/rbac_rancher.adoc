...
Accès de base à l'instance:: Accorde des autorisations minimales pour utiliser {stackstate-product-name}. Ce rôle n'a pas besoin d'être explicitement attribué et il n'existe pas de *Modèle de Rôle* pour cela{nbsp}; chaque utilisateur connecté l'a. 
...
Les ressources dans ces rôles correspondent à xref:/setup/security/rbac/rbac_permissions.adoc#_scoped_permissions[autorisations à portée]. Elles sont disponibles dans le groupe API `scope.observability.cattle.io` (avec juste le verbe `get` car ces ressources sont en lecture seule){nbsp}:
...
* Verbe{nbsp}: `get`
* Ressource{nbsp}: `metricbindings` et `monitors`
* ApiGroup{nbsp}: `instance.observability.cattle.io`
