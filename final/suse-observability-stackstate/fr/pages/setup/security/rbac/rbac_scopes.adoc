...
== Comment fonctionnent les portées de topologie{nbsp}?
...
== Pourquoi des portées de topologie{nbsp}?
...
Il est possible de spécifier une portée de topologie sous forme de joker de requête{nbsp}; cependant, cela donne accès à tout et n'est pas recommandé. S'il y a un besoin d'accès sans portée de topologie, il est recommandé d'utiliser l'un des xref:/setup/security/rbac/rbac_permissions.adoc#_predefined_roles[rôles prédéfinis] à la place.
...
=== Cet utilisateur fait partie du groupe Administrateur de SUSE Observability, donc il n'y a pas de portée de topologie{nbsp}:

image::v51_allperm.png[Autorisations de vue complètes]

La requête pour cette vue est la même que pour les autres, mais sans aucun préfixe{nbsp}:
...
=== L'utilisateur ci-dessous se trouve dans un groupe avec le sujet configuré X ayant la portée de topologie suivante{nbsp}:
...
La requête avec le préfixe pour cette vue est{nbsp}:
...
=== Un autre utilisateur qui fait partie d'un groupe avec un sujet configuré Y ayant la portée de topologie suivante{nbsp}:
...
La requête avec le préfixe pour cet utilisateur est alors{nbsp}:
...
Ce qui donne lieu à la vue suivante{nbsp}:
