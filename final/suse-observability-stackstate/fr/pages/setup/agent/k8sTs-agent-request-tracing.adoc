...
== Comment cela fonctionne-t-il{nbsp}?
...
L'activation de l'injection de l'en-tête de trace est un processus en deux étapes{nbsp}:
...
La désactivation de l'injection de l'en-tête de trace peut être effectuée avec le processus inverse{nbsp}:
...
Pour ajouter l'en-tête `X-Request-Id` d'un proxy existant, deux propriétés sont importantes{nbsp}:
...
Pour vous assurer que votre configuration est correcte, validez d'abord que les étapes suivantes ont été suivies{nbsp}:
