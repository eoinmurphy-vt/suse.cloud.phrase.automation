...
Open Telemetry reconnaît 3 signaux de télémétrie{nbsp}:
...
Les attributs de ressource sont souvent attribués automatiquement par les SDK. Cependant, il est recommandé de toujours définir explicitement les attributs `service.name` et `service.namespace`. Le premier est le nom logique du service{nbsp}; si ce n'est pas défini, le SDK attribuera une valeur `unknown_service`, rendant très difficile l'utilisation des données ultérieurement dans SUSE Observability. L'espace de noms est un moyen pratique d'organiser vos services, particulièrement utile si vous avez les mêmes services exécutés à plusieurs endroits.
