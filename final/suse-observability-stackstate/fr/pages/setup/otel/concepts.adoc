...
Les événements de la travée peuvent être utilisés pour représenter un moment où quelque chose d'important s'est produit dans le cadre du fonctionnement de la travée. Par exemple, si le span échoue, il peut y avoir un événement `exception` ou `error` qui capture le message d'erreur, une trace de la pile et le moment exact où l'erreur s'est produite.
