...
Dans Kubernetes, définissez ces variables d'environnement dans le manifeste de votre charge de travail (remplacez `<the-service-name>` par un nom pour votre service d'application){nbsp}:
...
Le nom du service peut également être dérivé des étiquettes Kubernetes qui peuvent déjà être présentes. Par exemple comme ceci{nbsp}:
...
Pour configurer le SDK, définissez ces variables d'environnement pour votre application{nbsp}:
...
Pour résumer, vous pouvez essayer d'utiliser HTTP si gRPC ne fonctionne pas pour vous{nbsp}:
