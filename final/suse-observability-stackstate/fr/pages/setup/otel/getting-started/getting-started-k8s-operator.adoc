...
Voici la configuration que nous allons créer, pour une application qui doit être surveillée{nbsp}:
...
L'opérateur OpenTelemetry offre des fonctionnalités supplémentaires par rapport à la configuration Kubernetes normale{nbsp}:
...
Il existe deux façons de créer un jeton de service{nbsp}:
...
Nous allons installer dans l'espace de noms `open-telemetry` et utiliser le token de service{nbsp}:
...
Créons un fichier `otel-operator.yaml` pour configurer l'opérateur{nbsp}:
...
Installez maintenant le collecteur, en utilisant le fichier de configuration{nbsp}:
...
Appliquez maintenant ce `collector.yaml` dans l'espace de noms `open-telemetry` pour déployer un collecteur{nbsp}:
...
Pour indiquer à l'opérateur d'auto-instrumenter vos pods d'application, nous devons ajouter une annotation au pod{nbsp}:
...
Lorsqu'un pod avec l'une des annotations est créé, l'opérateur modifie le pod via un hook de mutation{nbsp}:
...
Pour que xref:/setup/security/rbac/rbac_rancher.adoc[Rancher RBAC] fonctionne, les données de télémétrie doivent avoir les attributs de ressource suivants présents{nbsp}:
