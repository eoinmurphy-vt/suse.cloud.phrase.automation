...
Vous pouvez ajouter de nouveaux graphiques aux composants, par exemple le service ou l'instance de service, pour votre application, en suivant xref:/setup/custom-integrations/metric-bindings/index.adoc[notre guide.] Il est également possible de créer de xref:use/alerting/k8s-add-monitors-cli.adoc[nouveaux moniteurs] à l'aide des métriques et de configurer des xref:/use/alerting/notifications/configure.adoc[notifications] afin d'être informé lorsque votre application n'est pas disponible ou qu'elle présente des problèmes de performance.
endif::[]
