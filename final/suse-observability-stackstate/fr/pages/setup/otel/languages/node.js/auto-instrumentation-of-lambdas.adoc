...
Avant de commencer, assurez-vous d'avoir ce qui suit{nbsp}:
...
Pour la configuration d'OpenTelemetry décrite dans cette documentation, vous devrez définir les variables d'environnement suivantes{nbsp}:
...
*Un exemple complet{nbsp}: veuillez noter que vous devez entrer vos propres valeurs.*
...
Consultez cette page pour xref:/setup/otel/proxy-collector.adoc[des conseils et des instructions] sur la façon de configurer un collecteur par lots qui agit comme un proxy de sécurité pour SUSE Observability.
Consultez cette page pour xref:/setup/otel/collector.adoc[des instructions] sur la façon de configurer un collecteur qui effectue également un échantillonnage de queue.
Pour plus d'informations sur la configuration des processeurs sur le collecteur OpenTelemetry, consultez la https://github.com/open-telemetry/opentelemetry-collector/blob/main/processor/README.md[documentation officielle].
...
Si l'ajout des couches OTEL Lambda entraîne des fonctions Lambda qui expirent (la vérification des journaux peut indiquer que le collecteur a reçu l'instruction de s'arrêter alors qu'il était encore occupé, par exemple en observant l'entrée de journal suivante{nbsp}:)
...
peu après avoir reçu l'instruction de s'arrêter{nbsp}:
...
Essayez de modifier les propriétés MemorySize et TimeOut des fonctions Lambda qui échouent{nbsp}:
