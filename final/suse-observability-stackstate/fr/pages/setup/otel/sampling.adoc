...
Il existe 2 types principaux d'échantillonnage{nbsp}: l'échantillonnage par tête et l'échantillonnage par queue.
...
Les avantages de l'échantillonnage par tête sont{nbsp}:
...
Pour activer l'échantillonnage par tête, configurez le processeur et incluez-le dans les pipelines. Cet exemple échantillonne 1 trace sur 4 en fonction de l'identifiant de trace{nbsp}:
...
[WARNING]
====
Bien qu'avantageux, l'échantillonnage par queue présente les inconvénients suivants{nbsp}:
