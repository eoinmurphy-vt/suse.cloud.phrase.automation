...
Dans la section de démarrage, la configuration du collecteur n'inclut pas l'échantillonnage. Lors de l'ajout d'un échantillonnage, il convient de veiller à ce que les mesures calculées à partir des traces soient aussi précises que possible. L'échantillonnage par la queue peut en particulier donner lieu à des mesures très asymétriques, car le nombre relatif d'erreurs est généralement beaucoup plus élevé. Pour éviter cela, nous divisons le pipeline de traces en plusieurs parties et nous les connectons à l'aide du connecteur avant. Modifier la configuration pour inclure le connecteur supplémentaire et le processeur d'échantillonnage. Et modifiez les pipelines comme indiqué ici :
