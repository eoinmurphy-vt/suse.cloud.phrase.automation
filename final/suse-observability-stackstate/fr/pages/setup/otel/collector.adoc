...
Le https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/attributesprocessor[processeur d'attributs] peut modifier les attributs des travées, des journaux ou des mesures.
...
Il prend en charge de nombreuses autres transformations, telles que la modification du nom de l'étendue, la conversion des types de métriques ou la modification des événements du journal. Voir le https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/main/processor/transformprocessor[readme de] pour toutes les possibilités. Il utilise l' https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/pkg/ottl/README.md[Open Telemetry Transformation Lanuage (OTTL) de] pour définir les filtres.
...
Pour réduire la taille des requêtes HTTP, il est possible d'ajouter une configuration au processeur `batch` afin de limiter la taille des lots :
