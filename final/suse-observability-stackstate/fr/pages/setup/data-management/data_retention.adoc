...
Par défaut, les données du graphique topologique seront conservées pendant 30 jours. Cela fonctionne de manière à ce que l'état le plus récent du graphique topologique soit toujours conservé{nbsp}; seul l'historique de plus de 30 jours sera supprimé.
Dans certains cas, il peut être utile de conserver des données historiques pendant plus de 30 jours ou de les réduire à moins de 30 jours pour économiser de l'espace disque. La conservation de la topologie peut être configurée via le chart Helm{nbsp}:
...
En cas de problèmes d'espace disque, une ligne de journal - `Not enough replicas was chosen. Reason: {NOT_ENOUGH_STORAGE_SPACE=1` apparaît dans le namenode. Suivez les étapes ci-dessous pour traiter ce scénario{nbsp}:

* Réduisez la période de conservation, préparez l'instance à récupérer immédiatement de l'espace disque et déclenchez une mise à niveau helm{nbsp}:
[,yaml]
----
stackstate:
  topology:
...
* Déclenchez la compaction des données historiques{nbsp}:
[,bash]
----
...
* Suivez les progrès en utilisant{nbsp}:
----
...
SUSE Observability utilise VictoriaMetrics pour stocker les métriques. Il est configuré avec une rétention par défaut de 30 jours. Le chart Helm alloue de l'espace disque et configure la période de rétention pour les 1 ou 2 instances de métriques Victoria comme suit{nbsp}:
...
Pour changer la période de rétention, remplacez les deux `retentionPeriod` clés par la même valeur dans votre fichier custom values.yaml et xref:/setup/data-management/data_retention.adoc#_update_stackstate[mettez à jour SUSE Observability]{nbsp}:

* Les suffixes optionnels suivants sont pris en charge{nbsp}: h (heure), d (jour), w (semaine), y (année). Si aucun suffixe n'est défini, la durée est en mois.
