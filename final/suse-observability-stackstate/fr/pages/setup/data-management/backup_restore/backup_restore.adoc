...
Cet outil de sauvegarde modifie directement les ressources K8s dans votre cluster{nbsp}:
...
Pour restaurer une sauvegarde StackGraph, utilisez l'une des approches suivantes{nbsp}:
...
Lors de l'utilisation de `--background`, la restauration s'exécute de manière asynchrone. Après avoir démarré la restauration, utilisez la commande suivante pour vérifier l'état et finaliser{nbsp}:
...
La commande `check-and-finalize`{nbsp}:
...
Pour lister les sauvegardes des paramètres, exécutez la commande suivante{nbsp}:
...
Pour restaurer une sauvegarde des paramètres, utilisez l'une des approches suivantes{nbsp}:
...
Selon le profil, `nonha` ou `ha`, Victoria Metrics est déployé en différents modes{nbsp}:
...
Pour restaurer une sauvegarde de ClickHouse, utilisez l'une des approches suivantes{nbsp}:
...
Pour restaurer un instantané Elasticsearch, utilisez l'une des approches suivantes{nbsp}:
