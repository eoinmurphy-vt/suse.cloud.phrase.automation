...
Remarque{nbsp}: aucun comportement ni aucune capacité n'ont encore été mis en œuvre.
...
Type de `binding` actuellement pris en charge{nbsp}:

* `ComponentPresentationQueryBinding`{nbsp}: une requête STQL (reportez-vous au xref:/develop/reference/k8sTs-stql_reference.adoc[STQL] pour plus d'informations)
