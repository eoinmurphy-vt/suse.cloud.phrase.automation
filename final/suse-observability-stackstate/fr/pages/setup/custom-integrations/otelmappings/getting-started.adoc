...
Le guide se concentrera sur{nbsp}:
...
Plus précisément, nous voulons modéliser{nbsp}:
...
En plus de l'identité de service, la ressource de trace inclut des attributs au niveau des processus, tels que{nbsp}:
...
== Traces vers la topologie{nbsp}: le modèle mental
...
Pour ce guide, un service est défini comme{nbsp}:
...
Chaque instance de service{nbsp}:
...
Pour ce guide, un processus est défini comme{nbsp}:
...
L'identifiant de relation est construit (automatiquement) à partir du `sourceId` et du `targetId`, et est de la forme{nbsp}: `sourceId-relationId`
...
En utilisant la commande `sts stackpack test-deploy --yes`, vous pouvez{nbsp}:
...
Par exemple, en utilisant le `checkoutservice` de l'application de démonstration OTel, visuellement, la topologie devrait apparaître comme suit{nbsp}:
...
Pour supprimer les mappages créés dans ce guide, utilisez les commandes suivantes{nbsp}:
...
À partir d'ici, vous pouvez{nbsp}:
