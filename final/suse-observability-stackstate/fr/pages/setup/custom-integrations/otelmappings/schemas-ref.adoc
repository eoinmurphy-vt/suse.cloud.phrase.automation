...
Les mappages topologiques parcourent les données OpenTelemetry de manière hiérarchique - `resource → scope → metric/span → datapoint`
...
Un terrain de jeu CEL en ligne tel que https://playcel.undistro.io/ est un outil utile pour effectuer des contrôles rapides de la validité des expressions.
