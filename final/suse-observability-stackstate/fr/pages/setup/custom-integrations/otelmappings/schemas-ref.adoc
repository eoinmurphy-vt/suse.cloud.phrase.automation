...
Chaque mappage de composant{nbsp}:
...
Plusieurs enregistrements de télémétrie peuvent se résoudre au même identifiant de composant{nbsp}; dans ce cas, le composant est fusionné et actualisé.
...
Chaque mappage de relation{nbsp}:
...
Chaque niveau peut définir{nbsp}:

* Une condition{nbsp}: une expression booléenne CEL.
* Une action{nbsp}: que faire si la condition correspond.
...
Voici quelques exemples supplémentaires de déclarations d'entrée valides{nbsp}:
...
Le schéma de mappage des composants distingue entre deux comportements de gestion des erreurs{nbsp}:
...
Deux formes sont prises en charge{nbsp}:
...
- source: "${resource.attributes['service.name']}"
  target: "service.name"
----
Donné{nbsp}: `resource.attributes { 'service.name': 'cart-svc' }`

Résultat{nbsp}: `service.name:cart-svc`

La source d'un mappage direct doit être un{nbsp}:
...
----
Donné{nbsp}: `resource.attributes: { 'telemetry.sdk.language': 'go', 'telemetry.sdk.version': '1.23.1' }`

Résultat{nbsp}: `telemetry.sdk.language:go;telemetry.sdk.version:1.23.1`
...
La source d'un mappage basé sur des expressions régulières doit être{nbsp}:

* expression de mappage

Exemple avec plusieurs groupes de capture{nbsp}:
...
== Explication de l'expression CEL (`<cel-*>`){nbsp}:

* `<cel-string>` - doit renvoyer une chaîne{nbsp}; peut être l'un de{nbsp}:
** littéral de chaîne (par exemple, "bonjour")
** expression de chaîne, enveloppée dans `${...}` (par exemple, "${resource.attributes['service.name']}")
** interpolation de chaîne (par exemple, "urn:opentelemetry:namespace/$\{resource.attributes['namespace']}:service/$\{resource.attributes['service.name']}") - note{nbsp}: pour l'interpolation de chaîne, l'ensemble de l'expression n'est pas enveloppé dans `$\{...}`
* `cel-boolean` - doit retourner un booléen{nbsp}; peut être l'un des suivants{nbsp}:
** littéral booléen (par exemple, "true")
** expression booléenne (par exemple, "'namespace' in resource.attributes") - note{nbsp}: les expressions booléennes ne sont pas enveloppées dans `${...}`
* `cel-map` - doit renvoyer un mappage{nbsp}; peut être l'une des suivantes{nbsp}:
** littéral de mappage (par exemple, "${{'a':" 1, 'b': 'deux'}}")
** expression de mappage (par exemple, "${resource.attributes}")
* `cel-expression` - retourne un type "any", peut être l'un des suivants{nbsp}:
** expression de chaîne
...
Comprendre ce comportement de réécriture est important pour deux raisons{nbsp}:

1. *L'interpolation imbriquée n'est pas supportée*{nbsp}: Puisque les expressions de chaîne sont délimitées par `${...}`, vous ne pouvez pas imbriquer des expressions d'interpolation. Cependant, vous pouvez écrire la concaténation directement en utilisant l'opérateur `+`. Par exemple :
+
[source]
----
${
...
+
Par exemple, si `process.pid` est un entier (par exemple, `1`), cette expression échouera{nbsp}:
+
[source]
----
...
----
+
La forme correcte nécessite de convertir d'abord la valeur dynamique en un type concret, puis de la convertir en chaîne{nbsp}:
+
[source]
----
...
----
+
Le double typage (`string(int(...))`) est nécessaire car{nbsp}:
+
...
+
Pour d'autres types, utilisez des motifs similaires{nbsp}: `string(double(...))` pour les flottants, `string(bool(...))` pour les booléens.

=== Référence de la langue CEL

CEL est un langage d'expression sûr, sans effets secondaires, conçu pour des cas d'utilisation de configuration et de stratégie. Dans le contexte des mappages de topologie OpenTelemetry, CEL est utilisé pour{nbsp}:
