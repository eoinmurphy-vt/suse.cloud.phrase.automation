...
La commande `sts settings` peut lister toutes les liaisons de métriques{nbsp}:
...
Vous pouvez obtenir la définition d'une liaison de métriques existante en utilisant la commande `describe`{nbsp}:
...
Créez un fichier `metric-bindings.yaml` qui ressemble à ceci{nbsp}:
