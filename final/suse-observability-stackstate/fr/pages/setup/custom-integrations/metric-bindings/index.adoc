...
Le fait de le créer dans {stackstate-product-name} et d'afficher le graphique "Replica count" sur un composant de déploiement donne un résultat inattendu. Le graphique montre le nombre de répliques pour tous les déploiements. Logiquement, on ne devrait s'attendre qu'à une seule série temporelle : le nombre de répliques pour ce déploiement spécifique.
...
Pour la première option, un exemple est donné dans la xref:/setup/custom-integrations/metric-bindings/index.adoc#_using_metric_labels_in_aliases[section suivante]. La deuxième option peut être utile pour comparer des mesures connexes. Quelques cas d'utilisation typiques :
