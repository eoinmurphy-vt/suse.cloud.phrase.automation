...
Vérifiez que la requête de périmètre a la syntaxe correcte{nbsp}:

. Ouvrez la vue d'exploration, via Vues dans le menu et le bouton bleu "Explorer" à droite. Ou directement via l'URL{nbsp}: `https://<your-stackstate-instance>/#/views/explore`
...
Soit la métrique n'existe pas, soit elle n'a pas l'une des étiquettes, soit l'étiquette existe mais il n'y a pas de séries temporelles correspondant à la valeur. La méthode la plus rapide pour résoudre cela est de réécrire la requête uniquement avec son nom de métrique et de l'exécuter{nbsp}; s'il y a des résultats, la métrique existe (donc pas de fautes de frappe). Le résultat du tableau peut également être utilisé pour vérifier que toutes les étiquettes utilisées existent. Assurez-vous qu'il n'y a pas de fautes de frappe ici non plus.

S'il y a des résultats, mais juste pas pour une valeur spécifique d'une étiquette (par exemple pour l'étiquette `pod_name`), la requête est correcte mais il n'y a pas de séries temporelles pour cette métrique spécifique pour ce composant spécifique. Choses à vérifier dans ce cas{nbsp}:

* Les données sont-elles collectées pour ce composant (soit via l'agent {stackstate-product-name} ou par d'autres moyens){nbsp}?
* Le composant rapporte-t-il même la métrique{nbsp}?
