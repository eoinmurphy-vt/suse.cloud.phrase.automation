...
= Didacticiel{nbsp}: Développer une intégration personnalisée (StackPack)
:revdate: 2026-01-08
...
- Drapeau de fonctionnalité activé{nbsp}:
+
[source,bash]
----
...
- Créer un nouveau projet StackPack en utilisant le CLI{nbsp}:
+
[source,bash]
----
...
- Pour utiliser un modèle GitHub spécifique (par défaut{nbsp}: https://github.com/StackVista/stackpack-templates[stackvista/stackpack-templates]){nbsp}:
+
[source,bash]
----
...
+
- Pour utiliser un modèle local{nbsp}:
+
[source,bash]
----
...
[NOTE]
====
Le modèle par défaut{nbsp}: https://github.com/StackVista/stackpack-templates[stackvista/stackpack-templates] est un excellent point de départ car il contient des exemples pour la plupart des points d'extension tels que les Moniteurs et les Liaisons de Métriques.
====
...
- Testez rapidement votre StackPack dans un environnement de pré-production (nécessite une instance SUSE Observability en cours d'exécution){nbsp}:
+
[source,bash]
----
...
- Emballer votre StackPack dans un zip distribuable{nbsp}:
+
[source,bash]
----
...
- Pour spécifier le fichier de sortie{nbsp}:
+
[source,bash]
----
...
Avant de téléverser et d'installer votre StackPack, vous pouvez le valider pour vous assurer qu'il est correctement configuré (pour un retour d'information plus rapide). Il existe deux approches selon votre flux de travail{nbsp}:
...
- Validez un répertoire StackPack (automatiquement empaqueté et téléversé){nbsp}:
+
[source,bash]
----
...
- Validez un fichier `.sts` empaqueté{nbsp}:
+
[source,bash]
----
...
Utilisez directement l'image Docker du serveur StackState dans votre pipeline GitLab CI{nbsp}:
...
Si vous exécutez la validation localement ou dans d'autres systèmes CI/CD, utilisez la commande `docker run`{nbsp}:
...
- Validez un répertoire StackPack{nbsp}:
+
[source,bash]
----
docker run --rm --entrypoint="/opt/docker/bin/stack-pack-validator" \
...
- Téléversez l'archive vers {stackstate-product-name}{nbsp}:
+
[source,bash]
----
