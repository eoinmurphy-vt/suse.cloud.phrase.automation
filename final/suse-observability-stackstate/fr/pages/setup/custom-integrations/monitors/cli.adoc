...
Vous pouvez utiliser la {stackstate-product-name} CLI pour inspecter, exécuter et modifier les moniteurs{nbsp}:
...
La commande `sts monitor` peut lister tous les moniteurs{nbsp}:
...
Vous pouvez obtenir la définition d'un moniteur existant en utilisant la commande `describe`{nbsp}:
...
Exécuter un moniteur vous donne un aperçu des états de vérification qui sont produits{nbsp}:
...
Vous pouvez créer un moniteur en appliquant un fichier YAML, par exemple `monitor.yaml`.  Il doit avoir le schéma suivant{nbsp}:
...
Utilisez xref:/setup/cli/cli-sts.adoc[SUSE Observability CLI] pour créer ou mettre à jour le moniteur{nbsp}:
...
Pour modifier un moniteur, éditez l'original du moniteur qui a été appliqué, puis appliquez à nouveau. Ou il existe une commande `sts monitor edit` pour modifier le moniteur configuré directement dans l'instance {stackstate-product-name}{nbsp}:
