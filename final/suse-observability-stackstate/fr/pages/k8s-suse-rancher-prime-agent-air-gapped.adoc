...
Exécutez les commandes suivantes sur l'hôte local pour obtenir les images Docker et les graphiques Helm nécessaires{nbsp}:
...
La commande suivante téléchargera une archive TGZ du graphique depuis le dépôt Helm{nbsp}:
...
Les fichiers suivants doivent être copiés de l'hôte local vers l'hôte dans le réseau privé{nbsp}:
...
[NOTE]
====
_Remarque{nbsp}: si le registre de destination n'utilise pas d'authentification, les variables d'environnement, `DST_REGISTRY_USERNAME` et `DST_REGISTRY_PASSWORD` ne doivent pas être configurées ou doivent être définies sur des valeurs vides._
====
