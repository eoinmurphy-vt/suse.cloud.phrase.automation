...
Este tema es interesante sobre todo para los ingenieros que quieren hacer una integración personalizada con un sistema de vigilancia existente.
Para los monitores fuera de la caja se puede mirar xref:/use/alerting/kubernetes-monitors.adoc[aquí].

== Visión general

La sincronización de salud añade comprobaciones de salud existentes de sistemas de supervisión externos a los elementos de topología de SUSE Observability. Los datos de salud se calculan en el sistema de monitorización externo utilizando sus propios datos y reglas y, a continuación, se sincronizan automáticamente y se adjuntan a los elementos de topología asociados en SUSE Observability.
