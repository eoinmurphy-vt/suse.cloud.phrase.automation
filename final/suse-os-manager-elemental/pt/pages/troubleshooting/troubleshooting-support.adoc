...
O `elemental-support` é um programa utilitário que coleta todas as informações sobre a máquina em que a pilha {elemental-product-name} está sendo executada.

Ao solucionar problemas com qualquer nó do site {elemental-product-name}, é possível gerar um arquivo compactado contendo logs e arquivos de configuração. +
Esteja ciente de que *o arquivo tar.gz gerado pode conter informações confidenciais*, como tokens de acesso ao cluster de gerenciamento e outros.

== Requisitos

Opcionalmente, o utilitário `elemental-support` precisa que o `kubectl` esteja disponível no computador de destino para coletar informações sobre o nó. Isso pode ser muito útil para depurar problemas de atualização do {elemental-product-name} em um nó. +
A configuração do kubectl usada será pesquisada na variável de ambiente `KUBECONFIG` ou, na sua ausência, serão usados os arquivos `/etc/rancher/k3s/k3s.yaml` ou `/etc/rancher/rke2/rke2.yaml`.

== Coleta de informações

Uma vez executado, o `elemental-support` criará o arquivo compactado no mesmo diretório.
