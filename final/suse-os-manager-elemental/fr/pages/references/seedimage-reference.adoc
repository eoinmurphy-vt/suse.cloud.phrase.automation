...
Les fichiers de configuration sont disponibles dans{nbsp}:
...
La ressource `SeedImage` suit le processus de création de l'image de semence à travers deux conditions de statut{nbsp}:

- **Ready**{nbsp}: suit la création de toutes les ressources enfants requises qui effectuent le processus de construction réel.
- **SeedImageReady**  : suit l'état du processus de construction dans les ressources enfants.

Alternativement, il est également possible d'attendre que le pod `SeedImage` soit prêt{nbsp}:
