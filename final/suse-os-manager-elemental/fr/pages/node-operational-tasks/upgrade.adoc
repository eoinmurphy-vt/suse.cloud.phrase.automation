...
Choisissez un nom, sélectionnez les clusters à cibler et choisissez entre les deux façons de mettre à niveau{nbsp}:
...
Enfin, vous pouvez sélectionner le `OS Versions` lorsque vous créez votre `Upgrade Group` selon la capture d'écran suivante{nbsp}: 
...
+
====
Il suffit de spécifier une image OCI dans le champ `Image path` pour mettre à niveau vers{nbsp}: image:upgrade-ui-image-registry.png[Mettre à niveau via le registre d'images]
====
======
