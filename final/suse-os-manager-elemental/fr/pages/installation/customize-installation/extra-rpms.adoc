...
Pour ce cas d'utilisation, les fichiers suivants sont requis{nbsp}:
...
Créons un répertoire `overlay` pour créer l'arborescence de répertoires qui doit être ajoutée à la racine de l'ISO. Dans ce cas, le répertoire `overlay` pourrait contenir{nbsp}:
...
En supposant que nous avons déjà téléchargé une ISO Elemental liée à un enregistrement de machine spécifique avec l'appel `xorriso` suivant, tout le contenu du dossier `overlay` serait inclus dans une nouvelle ISO{nbsp}:
