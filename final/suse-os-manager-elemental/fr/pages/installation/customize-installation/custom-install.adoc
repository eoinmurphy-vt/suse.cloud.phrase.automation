...
Par défaut, les systèmes live Elemental montent la racine ISO à `/run/initramfs/live`, qui est également le chemin par défaut défini pour `config-url` dans `MachineRegistrations`. Voir l'exemple ci-dessous{nbsp}:
...
L'exemple suivant définit une partition supplémentaire lors de l'installation{nbsp}:
...
Pour la rendre disponible au moment de l'installation, cela pourrait être réalisé en ajoutant le fichier supplémentaire dans le cloud-config de la ressource SeedImage, comme décrit dans la section précédente de cette page. Considérons l'exemple{nbsp}:
