...
Des crochets d'installation peuvent être inclus dans la section `SeedImage.spec.cloud-config` avec quelque chose comme{nbsp}:
...
Consid rez l'exemple suivant pour personnaliser le fichier `/etc/fstab` en utilisant la section `cloud-config` d'une ressource MachineRegistration{nbsp}:
