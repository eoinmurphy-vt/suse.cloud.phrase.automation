...
Si un `ManagedOSVersion` ne peut pas être supprimé, vous pouvez découvrir par quelles ressources il est référencé{nbsp}:
...
Il est recommandé de nommer tout canal comme{nbsp}: `{BaseOS}-{BaseOSVersion}-{Flavor}`.
...
Sur l'interface utilisateur de Rancher, cela ressemblera à quelque chose comme l'image suivante{nbsp}:
...
Une bonne pratique est d'utiliser la convention{nbsp}: `\{Flavor}-\{Version}-\{Type}`

Un exemple du format JSON est le suivant{nbsp}:
