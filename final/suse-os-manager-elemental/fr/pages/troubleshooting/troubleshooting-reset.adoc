...
Vous pouvez enquêter sur les raisons pour lesquelles un `MachineInventory` n'a pas encore été supprimé, en l'examinant{nbsp}:
...
Vous pouvez également suivre ses progrès à partir des journaux de la machine{nbsp}: `journalctl -u elemental-system-agent -f`.
...
Le plan créé par le `elemental-operator` devrait contenir les instructions suivantes{nbsp}:
...
Les instructions cloud-init devraient ressembler à ce qui suit{nbsp}:
