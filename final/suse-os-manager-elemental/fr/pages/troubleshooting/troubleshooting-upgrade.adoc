...
Dans cet exemple, nous avons mis à niveau les nœuds du cluster avec la définition suivante de ManagedOSImage{nbsp}:
...
Pour surveiller le bon fonctionnement de ce contrôleur, vous pouvez consulter ses journaux{nbsp}:
...
Si tout est correct, le `system-upgrade-controller` créera un plan de mise à niveau sur le cluster{nbsp}:
...
Vous pouvez surveiller ces tâches avec{nbsp}:
...
Si le travail échoue, vous pouvez vérifier son statut en examinant les journaux{nbsp}:
