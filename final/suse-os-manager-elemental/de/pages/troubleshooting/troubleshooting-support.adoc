...
`elemental-support` ist ein Dienstprogramm, das alle Informationen über den Rechner sammelt, auf dem der {elemental-product-name} Stack läuft.

Bei der Behebung von Problemen mit einem beliebigen {elemental-product-name} Knoten kann eine komprimierte Datei mit Protokollen und Konfigurationsdateien erstellt werden. +
Beachten Sie, dass *die erzeugte tar.gz-Datei sensible Informationen enthalten kann*, z. B. Zugriffstoken für den Verwaltungscluster.

== Anforderungen

Optional muss das Dienstprogramm `elemental-support` auf dem Zielcomputer `kubectl` verfügbar sein, um Knoteninformationen zu sammeln. Dies kann sehr nützlich sein, um {elemental-product-name} Upgrade-Probleme mit einem Knoten zu beheben. +
Die verwendete kubectl-Konfiguration wird aus der Umgebungsvariablen `KUBECONFIG` entnommen, oder in Ermangelung dessen werden stattdessen die Dateien `/etc/rancher/k3s/k3s.yaml` oder `/etc/rancher/rke2/rke2.yaml` verwendet.

== Sammeln von Informationen

Nach der Ausführung erstellt `elemental-support` das komprimierte Archiv in demselben Verzeichnis.
