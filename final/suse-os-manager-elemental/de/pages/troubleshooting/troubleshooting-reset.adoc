= Fehlerbehebung zurücksetzen
:revdate: 2025-07-03
:page-revdate: {revdate}

Jedes `MachineInventory` mit dem Vermerk `elemental.cattle.io/resettable: "true"` löst beim Löschen die Ausführung eines Reset-Plans aus. +
Der `machineinventory.elemental.cattle.io` finalizer wird erst nach erfolgreicher Ausführung des Plans durch den `elemental-system-agent` auf dem Rechner entfernt.

Sie können herausfinden, warum eine `MachineInventory` noch nicht gelöscht wurde, indem Sie sie untersuchen:

[,yaml]
----
apiVersion: elemental.cattle.io/v1beta1
kind: MachineInventory
metadata:
  # deletionTimestamp has been set. This object has been marked for deletion.
  deletionTimestamp: "2023-08-04T08:41:25Z"
  annotations:
    # `resettable` annotation is enabled.
    # This means the machine has to go through reset, before deletion of this object.
    elemental.cattle.io/resettable: "true"
  # `machineinventory.elemental.cattle.io` finalizer is set.
  #  The `elemental-operator` is going to create a reset plan for the machine to execute.
  #  After successful execution of the reset plan, the finalizer is removed and the object will be deleted.
  finalizers:
  - machineinventory.elemental.cattle.io
status:
  conditions:
  # Most recent condition shows that the MachineInventory is waiting for a plan to be applied.
  - lastTransitionTime: "2023-08-04T08:41:25Z"
    message: waiting for plan to be applied
    reason: WaitingForPlan
    status: "False"
    type: Ready
  # The plan to be executed is referenced.
  # Normally it has the same name of the MachineInventory and lives within the same namespace.
  plan:
    checksum: 5aba8b6b3161bc52d8953b2428e54ecda3b59e8e0043b49d761d1e79174eded6
    secretRef:
      name: m-bf1008a1-61d6-4355-b5f5-f7d1c527affe
      namespace: fleet-default
----

Sie können auch den referenzierten Plan `Secret` einsehen. +
Beachten Sie, dass die `elemental-system-agent`, die auf dem Rechner läuft, dieses Geheimnis beobachtet und den Plan ausführen sollte. +
Sie können den Fortschritt auch anhand der Maschinenprotokolle verfolgen: `journalctl -u elemental-system-agent -f`.

[,yaml]
----
apiVersion: v1
kind: Secret
# This is a `elemental.cattle.io/plan` secret plan.
type: elemental.cattle.io/plan
metadata:
  annotations:
    # This is a `reset` plan type.
    elemental.cattle.io/plan.type: reset
  labels:
    elemental.cattle.io/managed: "true"
  name: m-bf1008a1-61d6-4355-b5f5-f7d1c527affe
  namespace: fleet-default
  # It is owned by the `MachineInventory` waiting for deletion.
  ownerReferences:
  - apiVersion: elemental.cattle.io/v1beta1
    controller: true
    kind: MachineInventory
    name: m-bf1008a1-61d6-4355-b5f5-f7d1c527affe
    uid: 5aa3863c-63a5-4cb9-91fd-7a45191d4842
data:
  # The plan has not been applied yet.
  applied-checksum: ""
  # It also hasn't failed.
  failed-checksum: ""
  # The actual plan to be executed, base64 encoded.
  plan: 
----

Der von `elemental-operator` erstellte Plan sollte die folgenden Anweisungen enthalten:

[,json]
----
{
  "files": [
    // A cloud-init config file is created on the default /oem directory.
    // This config will be executed once in recovery mode.
    {
      "content": "",
      "path": "/oem/reset-cloud-config.yaml",
      "permissions": "0600"
    }
  ],
  "instructions": [
    {
      "name": "configure next boot to recovery mode",
      "args": [
        "/oem/grubenv",
        "set",
        "next_entry=recovery"
      ],
      "command": "grub2-editenv"
    },
    {
      "name": "schedule reboot",
      "args": [
        "-r",
        "+1"
      ],
      "command": "shutdown"
    }
  ]
}
----

Wenn der `elemental-system-agent` den Plan erfolgreich ausgeführt hat, wird der `machineinventory.elemental.cattle.io` finalizer auf dem `MachineInventory` entfernt und der `MachineInventory` wird gelöscht. +
Beachten Sie, dass dies kein Hinweis darauf ist, dass das Gerät bereits vollständig zurückgesetzt worden ist. +
Dies ist eine Einschränkung der derzeitigen Implementierung und wird sich mit der Zeit verbessern, so dass es möglich sein wird, den Rücksetzungsstatus vollständig zu verfolgen.

In diesem Stadium erwarten wir jedoch, dass der Host einen Neustart durchführt und im Wiederherstellungsmodus neu startet. +
Sobald der Wiederherstellungsmodus aktiviert ist, sollte `cos-setup-network` die Cloud-init-Konfiguration ausführen, die auf `/oem/reset-cloud-config.yaml` geschrieben wurde. +
Sie können den Status mit `journalctl -u cos-setup-network -f` überwachen.

Die cloud-init-Anweisungen sollten wie folgt aussehen:

[,yaml]
----
name: {elemental-product-name} Reset
stages:
    network.after:
        - if: '[ -f /run/cos/recovery_mode ]'
          name: Runs elemental reset
          commands:
            - systemctl start elemental-register-reset
----

Der `elemental-register` cli wird auf der Website `elemental-operator` als neues Gerät registriert. Dies führt zur Erstellung eines neuen `MachineInventory` Objekts. +
Die Fernkonfiguration `MachineRegistration` wird ebenfalls abgerufen, um die Rücksetzoptionen anzuwenden, z. B. `reset-persistent`, `reset-oem`, oder die Leistungseinstellungen, entweder `reboot` oder `power-off`. +
Nach dem Zurücksetzen sollte der Rechner, je nach Einstellungen, entweder heruntergefahren oder neu gestartet werden und bereit sein, in einen neuen Cluster aufgenommen zu werden.

== Zwangsweise Löschung eines MachineInventory beim Zurücksetzen

Wenn das Gerät nicht in der Lage ist, die Rücksetzanweisungen auszuführen und die entsprechende `MachineInventory` nicht gelöscht wird, gibt es zwei gleichwertige Möglichkeiten, das Problem manuell zu beheben.

* Entfernen Sie den Vermerk `elemental.cattle.io/resettable: "true"` aus der `MachineInventory`.
* Entfernen Sie den `machineinventory.elemental.cattle.io` finalizer von der `MachineInventory`.

Denken Sie daran, sich auch um den Rechner selbst zu kümmern, indem Sie ihn vollständig neu bereitstellen oder in den Wiederherstellungsmodus booten und direkt den Befehl `elemental reset` verwenden.
