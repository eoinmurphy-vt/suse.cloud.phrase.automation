= {elemental-product-name} Ayuda
:revdate: 2025-07-03
:page-revdate: {revdate}

`elemental-support` es un programa utilitario que recoge toda la información sobre la máquina en la que se está ejecutando la pila {elemental-product-name}.

Al solucionar problemas con cualquier nodo de {elemental-product-name}, es posible generar un archivo comprimido que contenga registros y archivos de configuración. +
Tenga en cuenta que *el archivo tar.gz generado puede contener información confidencial*, como tokens de acceso al clúster de gestión y más.

== Requisitos

Opcionalmente, la utilidad `elemental-support` necesita que `kubectl` esté disponible en la máquina de destino, con el fin de recopilar información del nodo. Esto puede ser muy útil para depurar problemas de actualización de {elemental-product-name} con un nodo. +
La configuración kubectl utilizada se buscará en la variable de entorno `KUBECONFIG` o, en su ausencia, se utilizarán los archivos `/etc/rancher/k3s/k3s.yaml` o `/etc/rancher/rke2/rke2.yaml`.

== Recopilación de información

Una vez ejecutado, `elemental-support` creará el archivo comprimido dentro del mismo directorio.
