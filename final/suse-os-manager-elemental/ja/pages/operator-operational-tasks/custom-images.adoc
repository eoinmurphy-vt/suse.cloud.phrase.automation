...
すべての`size`値はメガバイトで表され、`0`の値はディスクの残りを占めます。これは、`persistent`が定義されていない場合の`size`パーティションのデフォルトの動作です。詳細については、完全な https://github.com/rancher/elemental-toolkit/blob/main/config.yaml.example[設定サンプル]を参照してください。
