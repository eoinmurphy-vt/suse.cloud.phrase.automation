...
`elemental-register`和`elemental-system-agent`依赖于 https://documentation.suse.com/cloudnative/rancher-manager/latest/en/installation-and-upgrade/resources/update-rancher-certificate.html[Rancher的证书颁发机构配置]来验证xref:references/machineregistration-reference.adoc#_config_elemental_registration[机器注册] URL，并远程监视计划。
