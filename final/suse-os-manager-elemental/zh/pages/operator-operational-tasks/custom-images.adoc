...
请注意，所有`size`值以兆字节表示，`0`的值将占用剩余的磁盘空间。如果没有为`size`定义，`persistent`分区的默认行为就是这样。有关更多信息，请参见完整的 https://github.com/rancher/elemental-toolkit/blob/main/config.yaml.example[配置示例]。
