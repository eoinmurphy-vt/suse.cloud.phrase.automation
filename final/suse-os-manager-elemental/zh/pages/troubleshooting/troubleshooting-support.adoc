...
`elemental-support` 是一个实用程序，用于收集运行{elemental-product-name} 堆栈的机器的所有信息。

在对任何{elemental-product-name} 节点的问题进行故障排除时，可以生成包含日志和配置文件的压缩文件。 +
请注意，*生成的 tar.gz 文件可能包含敏感信息*，如管理群集的访问令牌等。

== 要求

作为选项，`elemental-support` 实用程序需要`kubectl` 在目标计算机上可用，以便收集节点信息。这对调试{elemental-product-name} 节点的升级问题非常有用。 +
使用的 kubectl 配置将从`KUBECONFIG` 环境变量中查找，如果没有该变量，则使用`/etc/rancher/k3s/k3s.yaml` 或`/etc/rancher/rke2/rke2.yaml` 文件。

== 收集信息

运行后，`elemental-support` 将在同一目录下创建压缩包。
